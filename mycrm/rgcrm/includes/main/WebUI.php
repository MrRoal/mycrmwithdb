...
class Mycrm_WebUI extends Mycrm_EntryPoint {
	 ...
	 * @throws AppException
	 */
	protected function checkLogin (Mycrm_Request $request) {
		if (!$this->hasLogin()) {
            ...
            if($return_params && !$_SESSION['return_params']) {
                ...
                Mycrm_Session::set('return_params', $return_params);
            }
	...
	function getLogin() {
		...
		if (!$user && isset($_SESSION['authenticated_user_id'])) {
			$userid = Mycrm_Session::get('AUTHUSERID', $_SESSION['authenticated_user_id']);
	...
	function process (Mycrm_Request $request) {
		...
		if ($currentUser && $qualifiedModuleName) {
			$moduleLanguageStrings = Mycrm_Language_Handler::getModuleStringsFromFile($currentLanguage,$qualifiedModuleName);
		...
		if ($currentUser) {
			$moduleLanguageStrings = Mycrm_Language_Handler::getModuleStringsFromFile($currentLanguage);
			if(isset($moduleLanguageStrings['languageStrings'])){
		...
		try {
			...
			$handlerClass = Mycrm_Loader::getComponentClassName($componentType, $componentName, $qualifiedModuleName);
            ...
            if ($handler) {
				...
				$skipList = array('Users', 'Home', 'CustomView', 'Import', 'Export', 'Inventory', 'Mycrm','PriceBooks','Migration','Install');
