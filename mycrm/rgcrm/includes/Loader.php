...
class Mycrm_Loader {
	 ...
	 * @throws AppException
	 */
	public static function getComponentClassName($componentType, $componentName, $moduleName='Mycrm') {
		...
		$fallBackModuleDir = $fallBackModuleClassPath = 'Mycrm';
		...
		if(strpos($moduleName, ':') > 0) {
			...
			if($baseModule == 'Settings')  $baseModule = 'Settings:Mycrm';
		...
		$moduleSpecificComponentFilePath = Mycrm_Loader::resolveNameToPath('modules.'.$moduleDir.'.'.$componentTypeDirectory.'.'.$componentName);
		...
		if(!empty($firstFallBackDir) && !empty($firstFallBackClassPath)) {
			$fallBackComponentFilePath = Mycrm_Loader::resolveNameToPath('modules.'.$firstFallBackDir.'.'.$componentTypeDirectory.'.'.$componentName);
		...
		if(!empty($secondFallBackModuleDir) && !empty($secondFallBackModuleClassPath)) {
			$fallBackComponentFilePath = Mycrm_Loader::resolveNameToPath('modules.'.$secondFallBackModuleDir.'.'.$componentTypeDirectory.'.'.$componentName);
		...
		$fallBackComponentFilePath = Mycrm_Loader::resolveNameToPath('modules.'.$fallBackModuleDir.'.'.$componentTypeDirectory.'.'.$componentName);
	...
	public static function autoLoad($className) {
		...
		if($noOfParts > 2) {
            ...
            return Mycrm_Loader::includeOnce($filePath);
		}
		return false;
	}
}
...
function vimport($qualifiedName) {
	return Mycrm_Loader::includeOnce($qualifiedName);
}

function vimport_try($qualifiedName) {
	return Mycrm_Loader::includeOnce($qualifiedName, true);
}

function vimport_path($qualifiedName) {
	return Mycrm_Loader::includePath($qualifiedName);
}

spl_autoload_register('Mycrm_Loader::autoLoad');
