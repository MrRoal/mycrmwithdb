...
class Mycrm_Language_Handler {
     ...
     */
    public static function getModuleStringsFromFile($language, $module='Mycrm'){
        $module = str_replace(':', '.', $module);
        if(empty(self::$languageContainer[$language][$module])){
            $qualifiedName = 'languages.'.$language.'.'.$module;
            $file = Mycrm_Loader::resolveNameToPath($qualifiedName);
    ...
    public static function getLanguageLabel($name) {
        ...
        $languageResult = $db->pquery('SELECT label FROM mycrm_language WHERE prefix = ?', array($name));
...
function vtranslate($key, $moduleName = '') {
    $args = func_get_args();
    $formattedString = call_user_func_array(array('Mycrm_Language_Handler', 'getTranslatedString'), $args);
    array_shift($args);
    array_shift($args);
...
function vJSTranslate($key, $moduleName = '') {
    $args = func_get_args();
    return call_user_func_array(array('Mycrm_Language_Handler', 'getJSTranslatedString'), $args);
}
