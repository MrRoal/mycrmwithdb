 ...
 ********************************************************************************/
/*********************************************************************************
 * $Header: /advent/projects/wesat/mycrm_crm/sugarcrm/data/Tracker.php,v 1.15 2005/04/28 05:44:22 samk Exp $
...
class Tracker {
    ...
    /**
     * Add this new item to the mycrm_tracker mycrm_table.  If there are too many items (global config for now)
...
//No genius required. Just add an if case and change the query so that it puts the tracker entry whenever you touch on the DetailView of the required entity
	 ...
	 #if condition added to skip mycrm_faq in last viewed history
     ...
     * return - return the array of result set rows from the query.  All of the mycrm_table mycrm_fields are included
 * Portions created by SugarCRM are Copyright (C) SugarCRM, Inc.
 * All Rights Reserved.
 * Contributor(s): ______________________________________..
     */
...
//        $query = "SELECT * from $this->table_name WHERE user_id='$user_id' ORDER BY id DESC";
	$query = "SELECT * from $this->table_name inner join mycrm_crmentity on mycrm_crmentity.crmid=mycrm_tracker.item_id WHERE user_id=? and mycrm_crmentity.deleted=0 ORDER BY id DESC";
