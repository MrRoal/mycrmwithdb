 ...
 ********************************************************************************/
/*********************************************************************************
 * $Header: /advent/projects/wesat/mycrm_crm/mycrmcrm/data/CRMEntity.php,v 1.16 2005/04/29 04:21:31 mickie Exp $
...
class CRMEntity {
	...
	static function isBulkSaveMode() {
		...
		if (isset($MYCRM_BULK_SAVE_MODE) && $MYCRM_BULK_SAVE_MODE) {
			return true;
		}
		return false;
	}
	...
	function uploadAndSaveFile($id, $module, $file_details) {
		...
		$current_id = $adb->getUniqueID("mycrm_crmentity");
		...
		if ($save_file == 'true' && $upload_status == 'true') {
			//This is only to update the attached filename in the mycrm_notes mycrm_table for the Notes module
			if ($module == 'Contacts' || $module == 'Products') {
				$sql1 = "insert into mycrm_crmentity (crmid,smcreatorid,smownerid,setype,description,createdtime,modifiedtime) values(?, ?, ?, ?, ?, ?, ?)";
			...
			$sql2 = "insert into mycrm_attachments(attachmentsid, name, description, type, path) values(?, ?, ?, ?, ?)";
			$params2 = array($current_id, $filename, $this->column_fields['description'], $filetype, $upload_file_path);
			$result = $adb->pquery($sql2, $params2);

			if ($_REQUEST['mode'] == 'edit') {
				if ($id != '' && vtlib_purify($_REQUEST['fileid']) != '') {
					$delquery = 'delete from mycrm_seattachmentsrel where crmid = ? and attachmentsid = ?';
			...
			if ($module == 'Contacts') {
				$att_sql = "select mycrm_seattachmentsrel.attachmentsid  from mycrm_seattachmentsrel inner join mycrm_crmentity on mycrm_crmentity.crmid=mycrm_seattachmentsrel.attachmentsid where mycrm_crmentity.setype='Contacts Image' and mycrm_seattachmentsrel.crmid=?";
				...
				if ($attachmentsid != '') {
					...
					$crm_delquery = "delete from mycrm_crmentity where crmid=?";
					$adb->pquery($crm_delquery, array($attachmentsid));
					$sql5 = 'insert into mycrm_seattachmentsrel values(?,?)';
					$adb->pquery($sql5, array($id, $current_id));
				} else {
					$sql3 = 'insert into mycrm_seattachmentsrel values(?,?)';
	...
	/** Function to insert values in the mycrm_crmentity for the specified module
	...
	function insertIntoCrmEntity($module, $fileid = '') {
		...
		if ($this->mode == 'edit') {
			...
			if ($is_admin == true || $profileGlobalPermission[1] == 0 || $profileGlobalPermission[2] == 0) {
				$sql = "update mycrm_crmentity set smownerid=?,modifiedby=?,description=?, modifiedtime=? where crmid=?";
				$params = array($ownerid, $current_user->id, $description_val, $adb->formatDate($date_var, true), $this->id);
			} else {
				$profileList = getCurrentUserProfileList();
				$perm_qry = "SELECT columnname FROM mycrm_field INNER JOIN mycrm_profile2field ON mycrm_profile2field.fieldid = mycrm_field.fieldid INNER JOIN mycrm_def_org_field ON mycrm_def_org_field.fieldid = mycrm_field.fieldid WHERE mycrm_field.tabid = ? AND mycrm_profile2field.visible = 0 AND mycrm_profile2field.readonly = 0 AND mycrm_profile2field.profileid IN (" . generateQuestionMarks($profileList) . ") AND mycrm_def_org_field.visible = 0 and mycrm_field.tablename='mycrm_crmentity' and mycrm_field.displaytype in (1,3) and mycrm_field.presence in (0,2);";
				...
				if (is_array($columname) && in_array("description", $columname)) {
					...
					$sql = "update mycrm_crmentity set smownerid=?,modifiedby=?, modifiedtime=? where crmid=?";
			...
			$description_val = from_html($this->column_fields['description'], ($insertion_mode == 'edit') ? true : false);
			$sql = "insert into mycrm_crmentity (crmid,smcreatorid,smownerid,setype,description,modifiedby,createdtime,modifiedtime) values(?,?,?,?,?,?,?,?)";
	...
	function insertIntoEntityTable($table_name, $module, $fileid = '') {
		...
		$log->info("function insertIntoEntityTable " . $module . ' mycrm_table name ' . $table_name);
		global $adb;
		$insertion_mode = $this->mode;

		//Checkin whether an entry is already is present in the mycrm_table to update
		...
		if ($insertion_mode == 'edit') {
			...
			require('user_privileges/user_privileges_' . $current_user->id . '.php');
			if ($is_admin == true || $profileGlobalPermission[1] == 0 || $profileGlobalPermission[2] == 0) {
				$sql = "select * from mycrm_field where tabid in (" . generateQuestionMarks($tabid) . ") and tablename=? and displaytype in (1,3) and presence in (0,2) group by columnname";
				...
				if (count($profileList) > 0) {
					$sql = "SELECT *
			  			FROM mycrm_field
			  			INNER JOIN mycrm_profile2field
			  			ON mycrm_profile2field.fieldid = mycrm_field.fieldid
			  			INNER JOIN mycrm_def_org_field
			  			ON mycrm_def_org_field.fieldid = mycrm_field.fieldid
			  			WHERE mycrm_field.tabid = ?
			  			AND mycrm_profile2field.visible = 0 AND mycrm_profile2field.readonly = 0
			  			AND mycrm_profile2field.profileid IN (" . generateQuestionMarks($profileList) . ")
			  			AND mycrm_def_org_field.visible = 0 and mycrm_field.tablename=? and mycrm_field.displaytype in (1,3) and mycrm_field.presence in (0,2) group by columnname";
			...
			if ($table_index_column == 'id' && $table_name == 'mycrm_users') {
				$currentuser_id = $adb->getUniqueID("mycrm_users");
				$this->id = $currentuser_id;
			}
			$column = array($table_index_column);
			$value = array($this->id);
			$sql = "select * from mycrm_field where tabid=? and tablename=? and displaytype in (1,3,4) and mycrm_field.presence in (0,2)";
			$params = array($tabid, $table_name);
		}
		...
		for ($i = 0; $i < $noofrows; $i++) {
			...
			if (isset($this->column_fields[$fieldname])) {
				if ($uitype == 56) {
					...
					if ($this->column_fields[$fieldname] == null) {
						$fileQuery = $adb->pquery("SELECT filename from mycrm_notes WHERE notesid = ?", array($this->id));
						$fldvalue = null;
						if (isset($fileQuery)) {
					...
					if (empty($fldvalue)) {
						$query = "SELECT email1 FROM mycrm_users WHERE id = ?";
			...
			if ($insertion_mode == 'edit') {
				if ($table_name != 'mycrm_ticketcomments' && $uitype != 4) {
		...
		if ($insertion_mode == 'edit') {
			if ($module == 'Potentials') {
				$dbquery = 'select sales_stage from mycrm_potential where potentialid = ?';
				...
				if ($sales_stage != $_REQUEST['sales_stage'] && $_REQUEST['sales_stage'] != '') {
					...
					$sql = "insert into mycrm_potstagehistory values(?,?,?,?,?,?,?,?)";
	 ...
	 * The function will get the attachmentsid for the given entityid from mycrm_seattachmentsrel table and get the attachmentsname from mycrm_attachments table
	...
	function getOldFileName($notesid) {
		...
		global $adb;
		$query1 = "select * from mycrm_seattachmentsrel where crmid=?";
		$result = $adb->pquery($query1, array($notesid));
		$noofrows = $adb->num_rows($result);
		if ($noofrows != 0)
			$attachmentid = $adb->query_result($result, 0, 'attachmentsid');
		if ($attachmentid != '') {
			$query2 = "select * from mycrm_attachments where attachmentsid=?";
	...
	function retrieve_entity_info($record, $module) {
		...
		if (isset($this->multirow_tables)) {
			...
			$multirow_tables = array(
				'mycrm_campaignrelstatus',
				'mycrm_attachments',
				//'mycrm_inventoryproductrel',
				//'mycrm_cntactivityrel',
				'mycrm_email_track'
			);
		}
		...
		if ($cachedModuleFields === false) {
			...
			$sql0 = "SELECT fieldname, fieldid, fieldlabel, columnname, tablename, uitype, typeofdata,presence FROM mycrm_field WHERE tabid=?";
		...
		if ($cachedModuleFields) {
			...
			foreach ($cachedModuleFields as $fieldinfo) {
				...
				// Added to avoid picking shipping tax fields for Inventory modules, the shipping tax detail are stored in mycrm_inventoryshippingrel
				// table, but in mycrm_field table we have set tablename as mycrm_inventoryproductrel.
			...
			$column_clause .= 'mycrm_crmentity.deleted';

			if (isset($required_tables['mycrm_crmentity'])) {
				$from_clause  = ' mycrm_crmentity';
				unset($required_tables['mycrm_crmentity']);
				foreach ($required_tables as $tablename => $tableindex) {
					...
					$from_clause .= sprintf(' %s %s ON %s.%s=%s.%s', $join_type,
						$tablename, $tablename, $tableindex, 'mycrm_crmentity', 'crmid');
				}
			}

			$where_clause .= ' mycrm_crmentity.crmid=?';
			$params[] = $record;
	...
	function save($module_name, $fileid = '') {
		...
		$em->triggerEvent("mycrm.entity.beforesave.modifiable", $entityData);
		$em->triggerEvent("mycrm.entity.beforesave", $entityData);
		$em->triggerEvent("mycrm.entity.beforesave.final", $entityData);
		}
		...
		if($em) {
		//Event triggering code
		$em->triggerEvent("mycrm.entity.aftersave", $entityData);
		$em->triggerEvent("mycrm.entity.aftersave.final", $entityData);
		//Event triggering code ends
	}
...
//Added by Richie
	...
	function mark_deleted($id) {
		global $current_user;
		$date_var = date("Y-m-d H:i:s");
		$query = "UPDATE mycrm_crmentity set deleted=1,modifiedtime=?,modifiedby=? where crmid=?";
	...
	// the keys are user defined, and don't directly map to the bean's mycrm_fields
	// the value is the method name within that bean that will do extra
	// processing for that mycrm_field. example: 'full_name'=>'get_names_from_full_name'
	...
	/**
	 * Function to check if the custom mycrm_field mycrm_table exists
	...
	/**
	 * function to construct the query to fetch the custom mycrm_fields
	 * return the query to fetch the custom mycrm_fields
	 */
	function constructCustomQueryAddendum($tablename, $module) {
		global $adb;
		$tabid = getTabid($module);
		$sql1 = "select columnname,fieldlabel from mycrm_field where generatedtype=2 and tabid=? and mycrm_field.presence in (0,2)";
	...
	function track_view($user_id, $current_module, $id = '') {
		$this->log->debug("About to call mycrm_tracker (user_id, module_name, item_id)($user_id, $current_module, $this->id)");
	...
	function initRequiredFields($module) {
		...
		$tabid = getTabId($module);
		$sql = "select * from mycrm_field where tabid= ? and typeofdata like '%M%' and uitype not in ('53','70') and mycrm_field.presence in (0,2)";
	...
	function trash($module, $id) {
		...
		if(!self::isBulkSaveMode()) {
            ...
            $em->triggerEvent("mycrm.entity.beforedelete", $entityData);
        }
		...
		$sql_recentviewed = 'DELETE FROM mycrm_tracker WHERE user_id = ? AND item_id = ?';
		$this->db->pquery($sql_recentviewed, array($current_user->id, $id));

		if($em){
            $em->triggerEvent("mycrm.entity.afterdelete", $entityData);
        }
	}
	...
	function unlinkDependencies($module, $id) {
		...
		$fieldRes = $this->db->pquery('SELECT tabid, tablename, columnname FROM mycrm_field WHERE fieldid IN (
			SELECT fieldid FROM mycrm_fieldmodulerel WHERE relmodule=?)', array($module));
		...
		for ($i = 0; $i < $numOfFields; $i++) {
			...
			if ($tableName == 'mycrm_inventoryproductrel') {
	...
	function unlinkRelationship($id, $return_module, $return_id) {
		...
		$query = 'DELETE FROM mycrm_crmentityrel WHERE (crmid=? AND relmodule=? AND relcrmid=?) OR (relcrmid=? AND module=? AND crmid=?)';
		$params = array($id, $return_module, $return_id, $id, $return_module, $return_id);
		$this->db->pquery($query, $params);

		$fieldRes = $this->db->pquery('SELECT tabid, tablename, columnname FROM mycrm_field WHERE fieldid IN (
			SELECT fieldid FROM mycrm_fieldmodulerel WHERE module=? AND relmodule=?)', array($currentModule, $return_module));
		$numOfFields = $this->db->num_rows($fieldRes);
		for ($i = 0; $i < $numOfFields; $i++) {
			$tabId = $this->db->query_result($fieldRes, $i, 'tabid');
			$tableName = $this->db->query_result($fieldRes, $i, 'tablename');
	...
	function restore($module, $id) {
		...
		$date_var = date("Y-m-d H:i:s");
		$query = 'UPDATE mycrm_crmentity SET deleted=0,modifiedtime=?,modifiedby=? WHERE crmid = ?';
		...
		$entityData = VTEntityData::fromCRMEntity($this);
		//Event triggering code
		$em->triggerEvent("mycrm.entity.afterrestore", $entityData);
		//Event triggering code ends
	...
	function restoreRelatedRecords($module, $record) {
		...
		$result = $this->db->pquery('SELECT * FROM mycrm_relatedlists_rb WHERE entityid = ?', array($record));
		...
		for ($i = 0; $i < $numRows; $i++) {
			...
			if (strtoupper($action) == RB_RECORD_UPDATED) {
				...
				if ($rel_table == 'mycrm_crmentity' && $rel_column == 'deleted') {
				...
				if ($rel_table == 'mycrm_seproductrel') {
		...
		$this->db->pquery('DELETE FROM mycrm_relatedlists_rb WHERE entityid = ?', array($record));
	}
	...
	function initSortByField($module) {
		...
		$sql = "SELECT columnname FROM mycrm_field " .
				" WHERE (fieldname not like '%\_id' OR fieldname in ('assigned_user_id'))" .
				" AND tabid in (" . generateQuestionMarks($tabid) . ") and mycrm_field.presence in (0,2)";
		$params = array($tabid);
	...
	function setModuleSeqNumber($mode, $module, $req_str = '', $req_no = '') {
		...
		if ($mode == "configure" && $req_no != '') {
			$check = $adb->pquery("select cur_id from mycrm_modentity_num where semodule=? and prefix = ?", array($module, $req_str));
			if ($adb->num_rows($check) == 0) {
				$numid = $adb->getUniqueId("mycrm_modentity_num");
				$active = $adb->pquery("select num_id from mycrm_modentity_num where semodule=? and active=1", array($module));
				$adb->pquery("UPDATE mycrm_modentity_num SET active=0 where num_id=?", array($adb->query_result($active, 0, 'num_id')));

				$adb->pquery("INSERT into mycrm_modentity_num values(?,?,?,?,?,?)", array($numid, $module, $req_str, $req_no, $req_no, 1));
				return true;
				...
				if ($req_no < $num_check) {
					...
					$adb->pquery("UPDATE mycrm_modentity_num SET active=0 where active=1 and semodule=?", array($module));
					$adb->pquery("UPDATE mycrm_modentity_num SET cur_id=?, active = 1 where prefix=? and semodule=?", array($req_no, $req_str, $module));
					return true;
				}
			}
		} else if ($mode == "increment") {
			//when we save new invoice we will increment the invoice id and write
			$check = $adb->pquery("select cur_id,prefix from mycrm_modentity_num where semodule=? and active = 1", array($module));
			...
			$req_no.= $temp . ($curid + 1);
			$adb->pquery("UPDATE mycrm_modentity_num SET cur_id=? where cur_id=? and active=1 AND semodule=?", array($req_no, $curid, $module));
	...
	function isModuleSequenceConfigured($module) {
		...
		$result = $adb->pquery('SELECT 1 FROM mycrm_modentity_num WHERE semodule = ? AND active = 1', array($module));
	...
	function updateMissingSeqNumber($module) {
		...
		$tabid = getTabid($module);
		$fieldinfo = $adb->pquery("SELECT * FROM mycrm_field WHERE tabid = ? AND uitype = 4", Array($tabid));
			...
			if ($fld_table == $this->table_name) {
				...
				if ($records && $adb->num_rows($records)) {
					...
					if ($old_cur_id != $cur_id) {
						$adb->pquery("UPDATE mycrm_modentity_num set cur_id=? where semodule=? and active=1", Array($cur_id, $module));
					}
				}
			} else {
	...
	function get_attachments($id, $cur_tab_id, $rel_tab_id, $actions = false) {
		...
		$userNameSql = getSqlForNameInDisplayFormat(array('first_name'=>'mycrm_users.first_name',
														'last_name' => 'mycrm_users.last_name'), 'Users');
		$query = "select case when (mycrm_users.user_name not like '') then $userNameSql else mycrm_groups.groupname end as user_name," .
				...
				mycrm_seattachmentsrel.attachmentsid attachmentsid, mycrm_crmentity.smownerid smownerid, mycrm_notes.notesid crmid,
				mycrm_notes.notecontent description,mycrm_notes.*
				from mycrm_notes
	...
	function isViewed($crmid = false) {
		...
		if ($crmid) {
			global $adb;
			$result = $adb->pquery("SELECT viewedtime,modifiedtime,smcreatorid,smownerid,modifiedby FROM mycrm_crmentity WHERE crmid=?", Array($crmid));
	...
	function markAsViewed($userid) {
		global $adb;
		$adb->pquery("UPDATE mycrm_crmentity set viewedtime=? WHERE crmid=? AND smownerid=?", Array(date('Y-m-d H:i:s', time()), $this->id, $userid));
	}
	...
	function save_related_module($module, $crmid, $with_module, $with_crmid) {
		...
		foreach ($with_crmid as $relcrmid) {
			...
			if ($with_module == 'Documents') {
				$checkpresence = $adb->pquery("SELECT crmid FROM mycrm_senotesrel WHERE crmid = ? AND notesid = ?", Array($crmid, $relcrmid));
				// Relation already exists? No need to add again
				if ($checkpresence && $adb->num_rows($checkpresence))
					continue;

				$adb->pquery("INSERT INTO mycrm_senotesrel(crmid, notesid) VALUES(?,?)", array($crmid, $relcrmid));
			} else {
				$checkpresence = $adb->pquery("SELECT crmid FROM mycrm_crmentityrel WHERE
					crmid = ? AND module = ? AND relcrmid = ? AND relmodule = ?", Array($crmid, $module, $relcrmid, $with_module));
				// Relation already exists? No need to add again
				if ($checkpresence && $adb->num_rows($checkpresence))
					continue;

				$adb->pquery("INSERT INTO mycrm_crmentityrel(crmid, module, relcrmid, relmodule) VALUES(?,?,?,?)", Array($crmid, $module, $relcrmid, $with_module));
			}
		}
	}
	...
	function delete_related_module($module, $crmid, $with_module, $with_crmid) {
		...
		foreach ($with_crmid as $relcrmid) {
			...
			if ($with_module == 'Documents') {
				$adb->pquery("DELETE FROM mycrm_senotesrel WHERE crmid=? AND notesid=?", Array($crmid, $relcrmid));
			} else {
				$adb->pquery("DELETE FROM mycrm_crmentityrel WHERE (crmid=? AND module=? AND relcrmid=? AND relmodule=?) OR (relcrmid=? AND relmodule=? AND crmid=? AND module=?)",
	...
	function get_related_list($id, $cur_tab_id, $rel_tab_id, $actions = false) {
		...
		$query .= " INNER JOIN mycrm_crmentityrel ON (mycrm_crmentityrel.relcrmid = mycrm_crmentity.crmid OR mycrm_crmentityrel.crmid = mycrm_crmentity.crmid)";
		$query .= $more_relation;
		...
		$query .= " WHERE mycrm_crmentity.deleted = 0 AND (mycrm_crmentityrel.crmid = $id OR mycrm_crmentityrel.relcrmid = $id)";
	...
	function get_dependents_list($id, $cur_tab_id, $rel_tab_id, $actions = false) {
		...
		if ($numOfFields > 0) {
			...
			$query .= " WHERE mycrm_crmentity.deleted = 0 AND $this->table_name.$this->table_index = $id";
	...
	function transferRelatedRecords($module, $transferEntityIds, $entityId) {
		...
		foreach ($transferEntityIds as $transferId) {
			...
			$relatedRecords = $adb->pquery("SELECT relcrmid, relmodule FROM mycrm_crmentityrel WHERE crmid=? AND module=?" .
					" AND relcrmid NOT IN (SELECT relcrmid FROM mycrm_crmentityrel WHERE crmid=? AND module=?)", array($transferId, $module, $entityId, $module));
			...
			for ($i = 0; $i < $numOfRecords; $i++) {
				...
				$adb->pquery("UPDATE mycrm_crmentityrel SET crmid=? WHERE relcrmid=? AND relmodule=? AND crmid=? AND module=?", array($entityId, $relcrmid, $relmodule, $transferId, $module));
			}
			...
			$parentRecords = $adb->pquery("SELECT crmid, module FROM mycrm_crmentityrel WHERE relcrmid=? AND relmodule=?" .
					" AND crmid NOT IN (SELECT crmid FROM mycrm_crmentityrel WHERE relcrmid=? AND relmodule=?)", array($transferId, $module, $entityId, $module));
			...
			for ($i = 0; $i < $numOfRecords; $i++) {
				...
				$adb->pquery("UPDATE mycrm_crmentityrel SET relcrmid=? WHERE crmid=? AND module=? AND relcrmid=? AND relmodule=?", array($entityId, $parcrmid, $parmodule, $transferId, $module));
			}
			$adb->pquery("UPDATE mycrm_modcomments SET related_to = ? WHERE related_to = ?", array($entityId, $transferId));
		}
	...
	function generateReportsQuery($module, $queryPlanner) {
		...
		$fields_query = $adb->pquery("SELECT mycrm_field.fieldname,mycrm_field.tablename,mycrm_field.fieldid from mycrm_field INNER JOIN mycrm_tab on mycrm_tab.name = ? WHERE mycrm_tab.tabid=mycrm_field.tabid AND mycrm_field.uitype IN (10) and mycrm_field.presence in (0,2)", array($module));
		...
		if ($adb->num_rows($fields_query) > 0) {
			for ($i = 0; $i < $adb->num_rows($fields_query); $i++) {
				...
				$ui10_modules_query = $adb->pquery("SELECT relmodule FROM mycrm_fieldmodulerel WHERE fieldid=?", array($field_id));

				if ($adb->num_rows($ui10_modules_query) > 0) {

					// Capture the forward table dependencies due to dynamic related-field
					$crmentityRelModuleFieldTable = "mycrm_crmentityRel$module$field_id";
					...
					if ($queryPlanner->requireTable($crmentityRelModuleFieldTable, $matrix)) {
						$relquery.= " left join mycrm_crmentity as $crmentityRelModuleFieldTable on $crmentityRelModuleFieldTable.crmid = $tab_name.$field_name and mycrm_crmentityRel$module$field_id.deleted=0";
					}
		...
		$query = "from $moduletable inner join mycrm_crmentity on mycrm_crmentity.crmid=$moduletable.$moduleindex";
	...
	function generateReportsSecQuery($module, $secmodule,$queryPlanner) {
		...
		$fields_query = $adb->pquery("SELECT mycrm_field.fieldname,mycrm_field.tablename,mycrm_field.fieldid from mycrm_field INNER JOIN mycrm_tab on mycrm_tab.name = ? WHERE mycrm_tab.tabid=mycrm_field.tabid AND mycrm_field.uitype IN (10) and mycrm_field.presence in (0,2)", array($secmodule));
		...
		if ($adb->num_rows($fields_query) > 0) {
			for ($i = 0; $i < $adb->num_rows($fields_query); $i++) {
				...
				if ($adb->num_rows($ui10_modules_query) > 0) {
					// Capture the forward table dependencies due to dynamic related-field
					$crmentityRelSecModuleTable = "mycrm_crmentityRel$secmodule$field_id";
					...
					if ($queryPlanner->requireTable($crmentityRelSecModuleTable, $matrix)) {
						$relquery .= " left join mycrm_crmentity as $crmentityRelSecModuleTable on $crmentityRelSecModuleTable.crmid = $tab_name.$field_name and $crmentityRelSecModuleTable.deleted=0";
					}
					for ($j = 0; $j < $adb->num_rows($ui10_modules_query); $j++) {
						$rel_mod = $adb->query_result($ui10_modules_query, $j, 'relmodule');
						$rel_obj = CRMEntity::getInstance($rel_mod);
		...
		$matrix->setDependency("mycrm_crmentity$secmodule", array("mycrm_groups$secmodule", "mycrm_users$secmodule", "mycrm_lastModifiedBy$secmodule"));
		$matrix->addDependency($tablename, "mycrm_crmentity$secmodule");
		...
		if ($queryPlanner->requireTable("mycrm_crmentity$secmodule", $matrix)) {
			$query .= " left join mycrm_crmentity as mycrm_crmentity$secmodule on mycrm_crmentity$secmodule.crmid = $tablename.$tableindex AND mycrm_crmentity$secmodule.deleted=0";
		}

		// Add the pre-joined custom table query
		$query .= " ".$cfquery;

		if ($queryPlanner->requireTable("mycrm_groups$secmodule")) {
			$query .= " left join mycrm_groups as mycrm_groups" . $secmodule . " on mycrm_groups" . $secmodule . ".groupid = mycrm_crmentity$secmodule.smownerid";
		}
		if ($queryPlanner->requireTable("mycrm_users$secmodule")) {
			$query .= " left join mycrm_users as mycrm_users" . $secmodule . " on mycrm_users" . $secmodule . ".id = mycrm_crmentity$secmodule.smownerid";
		}
		if ($queryPlanner->requireTable("mycrm_lastModifiedBy$secmodule")) {
			$query .= " left join mycrm_users as mycrm_lastModifiedBy" . $secmodule . " on mycrm_lastModifiedBy" . $secmodule . ".id = mycrm_crmentity" . $secmodule . ".modifiedby";
		}
	...
	function getListViewSecurityParameter($module) {
		...
		if($is_admin==false && $profileGlobalPermission[1] == 1 && $profileGlobalPermission[2] == 1
			...
			$sec_query .= " and (mycrm_crmentity.smownerid in($current_user->id) or mycrm_crmentity.smownerid
					in (select mycrm_user2role.userid from mycrm_user2role
							...
							where mycrm_role.parentrole like '" . $current_user_parent_role_seq . "::%') or mycrm_crmentity.smownerid
					in(select shareduserid from mycrm_tmp_read_user_sharing_per
						where userid=" . $current_user->id . " and tabid=" . $tabid . ") or (";
			if (sizeof($current_user_groups) > 0) {
				$sec_query .= " mycrm_groups.groupid in (" . implode(",", $current_user_groups) . ") or ";
			}
			$sec_query .= " mycrm_groups.groupid in(select mycrm_tmp_read_group_sharing_per.sharedgroupid
						from mycrm_tmp_read_group_sharing_per where userid=" . $current_user->id . " and tabid=" . $tabid . "))) ";
		}
		return $sec_query;
	}
	...
	function getRelationQuery($module, $secmodule, $table_name, $column_name, $queryPlanner) {
		...
		$secQuery = "select $table_name.* from $table_name inner join mycrm_crmentity on " .
				"mycrm_crmentity.crmid=$table_name.$column_name and mycrm_crmentity.deleted=0";

		$secQueryTempTableQuery = $queryPlanner->registerTempTable($secQuery, array($column_name, $fields[1], $prifieldname));

		$query = '';
		if ($pritablename == 'mycrm_crmentityrel') {
			$condition = "($table_name.$column_name={$tmpname}.{$secfieldname} " .
					"OR $table_name.$column_name={$tmpname}.{$prifieldname})";
			$query = " left join mycrm_crmentityrel as $tmpname ON ($condvalue={$tmpname}.{$secfieldname} " .
			...
			$condition = "$table_name.$column_name=$tmpname.$secfieldname";
            if($pritablename == 'mycrm_seactivityrel') {
                ...
                $query = " left join $pritablename as $tmpname ON ($sectablename.$sectableindex=$tmpname.$prifieldname
                    AND $tmpname.activityid IN (SELECT crmid FROM mycrm_crmentity WHERE setype='$tmpModule' AND deleted = 0))";
            } else if($pritablename == 'mycrm_senotesrel') {
                    $query = " left join $pritablename as $tmpname ON ($sectablename.$sectableindex=$tmpname.$prifieldname
                    AND $tmpname.notesid IN (SELECT crmid FROM mycrm_crmentity WHERE setype='Documents' AND deleted = 0))";
            } else if($pritablename == 'mycrm_inventoryproductrel' && ($module =="Products" || $module =="Services") && ($secmodule == "Invoice" || $secmodule == "SalesOrder" || $secmodule == "PurchaseOrder" || $secmodule == "Quotes")) {
                /** In mycrm_inventoryproductrel table, we'll have same product related to quotes/invoice/salesorder/purchaseorder
                ...
                $query = " left join $pritablename as $tmpname ON ($sectablename.$sectableindex=$tmpname.$prifieldname AND $tmpname.id in 
                        (select crmid from mycrm_crmentity where setype='$secmodule' and deleted=0))";
            } else {
                $query = " left join $pritablename as $tmpname ON ($sectablename.$sectableindex=$tmpname.$prifieldname)";
            }
            ...
            // To get all the Contacts from mycrm_contpotentialrel table
            $condition .= " OR $table_name.potentialid = mycrm_contpotentialrel.potentialid";
            $query .= " left join mycrm_contpotentialrel on  mycrm_contpotentialrel.contactid = mycrm_contactdetails.contactid";
        }else if($module == "Potentials" && $secmodule == "Contacts"){
            // To get all the Potentials from mycrm_contpotentialrel table
            $condition .= " OR $table_name.contactid = mycrm_contpotentialrel.contactid";
            $query .= " left join mycrm_contpotentialrel on mycrm_potential.potentialid = mycrm_contpotentialrel.potentialid";
        }
	...
	function add_related_to($module, $fieldname) {
		...
		$query = "SELECT mycrm_crmentity.deleted, $focus1->table_name.*
					FROM $focus1->table_name
					INNER JOIN mycrm_crmentity ON mycrm_crmentity.crmid=$focus1->table_name.$focus1->table_index
						where $entityName=? and mycrm_crmentity.deleted=0";
	...
	function filterInactiveFields($module) {
		...
		if ($cachedModuleFields) {
			foreach ($cachedModuleFields as $fieldinfo) {
				...
				$tableName = str_replace("mycrm_", "", $fieldinfo['tablename']);
	...
	function buildSearchQueryForFieldTypes($uitypes, $value=false) {
		...
		$query .=" INNER JOIN mycrm_crmentity ON $this->table_name.$this->table_index = mycrm_crmentity.crmid AND deleted = 0 ";
		...
		if($this->table_name == 'mycrm_leaddetails') {
	...
	function getNonAdminUserAccessQuery($user, $parentRole, $userGroups) {
		$query = "(SELECT $user->id as id) UNION (SELECT mycrm_user2role.userid AS userid FROM " .
				"mycrm_user2role INNER JOIN mycrm_users ON mycrm_users.id=mycrm_user2role.userid " .
				"INNER JOIN mycrm_role ON mycrm_role.roleid=mycrm_user2role.roleid WHERE " .
				"mycrm_role.parentrole like '$parentRole::%')";
		if (count($userGroups) > 0) {
			$query .= " UNION (SELECT groupid FROM mycrm_groups where" .
	...
	function getNonAdminModuleAccessQuery($module, $user) {
		...
		if (!empty($sharingRuleInfo) && (count($sharingRuleInfo['ROLE']) > 0 ||
			...
			$query = " (SELECT shareduserid FROM mycrm_tmp_read_user_sharing_per " .
					"WHERE userid=$user->id AND tabid=$tabId) UNION (SELECT " .
					"mycrm_tmp_read_group_sharing_per.sharedgroupid FROM " .
					"mycrm_tmp_read_group_sharing_per WHERE userid=$user->id AND tabid=$tabId)";
		}
		return $query;
	}
            ...
            // for secondary module we should join the records even if record is not there(primary module without related record)
                if($scope == ''){
                    $query = " INNER JOIN $tableName $tableName$scope ON $tableName$scope.id = " .
                            "mycrm_crmentity$scope.smownerid ";
                }else{
                    $query = " INNER JOIN $tableName $tableName$scope ON $tableName$scope.id = " .
                            "mycrm_crmentity$scope.smownerid OR mycrm_crmentity$scope.smownerid IS NULL";
                }
            }
		return $query;
	}
	...
	function setRelationTables($secmodule) {
		$rel_tables = array(
			"Documents" => array("mycrm_senotesrel" => array("crmid", "notesid"),
	...
	function trackLinkedInfo($module, $crmid, $with_module, $with_crmid) {
		...
		$adb->pquery('UPDATE mycrm_crmentity SET modifiedtime = ?, modifiedby = ? WHERE crmid = ?', array($currentTime, $current_user->id, $crmid));
