...
class freetag {
	...
	/**
	 * @access private
	 * @param string The prefix of freetag database mycrm_tables.
	 */
	var $_table_prefix = 'mycrm_';
	/**
	 * @access private
	...
	/**
	 * @access private
	 * @param string Whether to prevent multiple mycrm_users from tagging the same object. By default, set to block (ala Upcoming.org)
	 */
	 ...
	 * - mycrm_table_prefix: If you wish to create multiple Freetag databases on the same database, you can put a prefix in front of the mycrm_table names and pass separate prefixes to the constructor. [default: '']
	 ...
	 * - block_multiuser_tag_on_object: Set to 0 in order to allow individual mycrm_users to all tag the same object with the same tag. Default is 1 to only allow one occurence of a tag per object. [default: 1]
...
/*
	 ...
	 * tagArray parameter. Use this to provide tag combo services to your mycrm_users.
	 *
	...
	function safe_tag($tagger_id, $object_id, $tag, $module) {
		...
		// Dynamically switch between allowing duplication between mycrm_users on the constructor param 'block_multiuser_tag_on_object'.
		...
		if(!$rs->EOF) {
			...
			// Add new tag! 
			$tag_id = $adb->getUniqueId('mycrm_freetags');
	 ...
	 * @param string The raw string form of the tag to delete. See above for mycrm_notes.
	 *
	 * @return string Returns the tag in normalized form.
	 */ 
	 ...
	 * Confusing? Think of it like e-commerce's "Other mycrm_users who bought this also bought," 
