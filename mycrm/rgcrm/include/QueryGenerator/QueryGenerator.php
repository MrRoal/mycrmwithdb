...
class QueryGenerator {
	...
	public function initForCustomViewById($viewId) {
		...
		foreach ($this->customViewColumnList as $customViewColumnInfo) {
			...
			if(empty($details[2]) && $details[1] == 'crmid' && $details[0] == 'mycrm_crmentity') {
				$name = 'id';
			...
			if(count($filtercolumns) > 0) {
				...
				foreach ($filtercolumns as $index=>$filter) {
                    ...
                    if($nameComponents[2] == 'due_date' && $nameComponents[3] == 'Events_End_Date_&_Time')
                        $nameComponents[4] = 'DT';
					if(empty($nameComponents[2]) && $nameComponents[1] == 'crmid' && $nameComponents[0] == 'mycrm_crmentity') {
		...
		foreach ($this->fields as $field) {
			...
			if($this->meta->getEntityName() == 'Calendar' && ($field == 'date_start' || $field == 'due_date' || $field == 'taskstatus' || $field == 'eventstatus')) {
				if($field=='date_start') {
					...
					$sql = "CASE WHEN (mycrm_activity.status not like '') THEN mycrm_activity.status ELSE mycrm_activity.eventstatus END AS ";
					if ( $field == 'taskstatus') {
						$sql .= "status";
					} else {
						$sql .= $field;
	...
	public function getFromClause() {
			...
			if($field->getFieldDataType() == 'reference') {
				...
				foreach($moduleList as $module) {
					if($module == 'Users' && $baseModule != 'Users') {
						$tableJoinCondition[$fieldName]['mycrm_users'.$fieldName] = $field->getTableName().
								".".$field->getColumnName()." = mycrm_users".$fieldName.".id";
						$tableJoinCondition[$fieldName]['mycrm_groups'.$fieldName] = $field->getTableName().
								".".$field->getColumnName()." = mycrm_groups".$fieldName.".groupid";
						$tableJoinMapping['mycrm_users'.$fieldName] = 'LEFT JOIN mycrm_users AS';
						$tableJoinMapping['mycrm_groups'.$fieldName] = 'LEFT JOIN mycrm_groups AS';
						$i++;
					}
				}
			} elseif($field->getFieldDataType() == 'owner') {
				$tableList['mycrm_users'] = 'mycrm_users';
				$tableList['mycrm_groups'] = 'mycrm_groups';
				$tableJoinMapping['mycrm_users'] = 'LEFT JOIN';
				$tableJoinMapping['mycrm_groups'] = 'LEFT JOIN';
		...
		foreach ($this->whereFields as $fieldName) {
			...
			if($field->getFieldDataType() == 'reference') {
				...
				// This is special condition as the data is not stored in the base table, 
                                ...
                                if ($fieldName == 'parent_id' && $field->getTableName() == 'mycrm_seactivityrel') {
                                    $tableJoinMapping[$field->getTableName()] = 'LEFT JOIN';
                                } else if ($fieldName == 'contact_id' && $field->getTableName() == 'mycrm_cntactivityrel') {
                                    $tableJoinMapping[$field->getTableName()] = "LEFT JOIN";
                                } else {
                                    $tableJoinMapping[$field->getTableName()] = 'INNER JOIN';
                                }
		...
		if( $this->meta->getTabName() == 'Documents') {
			$tableJoinCondition['folderid'] = array(
				'mycrm_attachmentsfolderfolderid'=>"$baseTable.folderid = mycrm_attachmentsfolderfolderid.folderid"
			);
			$tableJoinMapping['mycrm_attachmentsfolderfolderid'] = 'INNER JOIN mycrm_attachmentsfolder';
		}
			...
			foreach ($this->referenceModuleField as $index=>$conditionInfo) {
				...
				if(!in_array($tableName, $referenceFieldTableList)) {
					if($referenceFieldObject->getFieldName() == 'parent_id' && ($this->getModule() == 'Calendar' || $this->getModule() == 'Events')) {
						$sql .= ' LEFT JOIN mycrm_seactivityrel ON mycrm_seactivityrel.activityid = mycrm_activity.activityid ';
					}
					...
					if($referenceFieldObject->getFieldName() == 'contact_id' && ($this->getModule() == 'Calendar' || $this->getModule() == 'Events')) {
						$sql .= ' LEFT JOIN mycrm_cntactivityrel ON mycrm_cntactivityrel.activityid = mycrm_activity.activityid ';
					}
        ...
        foreach ($this->conditionals as $index=>$conditionInfo) {
			...
			foreach ($valueSqlList as $valueSql) {
				if (in_array($fieldName, $this->referenceFieldList)) {
                    ...
                    if($fieldName == 'created_user_id'){
                        $concatSql = getSqlForNameInDisplayFormat(array('first_name'=>"mycrm_users$fieldName.first_name",'last_name'=>"mycrm_users$fieldName.last_name"), 'Users');
                        $fieldSql .= "$fieldGlue (trim($concatSql) $valueSql)";
                    }else{
						$concatSql = getSqlForNameInDisplayFormat(array('first_name'=>"mycrm_users.first_name",'last_name'=>"mycrm_users.last_name"), 'Users');
						$fieldSql .= "$fieldGlue (trim($concatSql) $valueSql or "."mycrm_groups.groupname $valueSql)";
                    }
					...
					if($fieldName == 'date_start') {
						$dateFieldColumnName = 'mycrm_activity.date_start';
						$timeFieldColumnName = 'mycrm_activity.time_start';
					} else {
						$dateFieldColumnName = 'mycrm_activity.due_date';
						$timeFieldColumnName = 'mycrm_activity.time_end';
					}
					if($operator == 'bw') {
			...
			foreach ($advfilterlist as $groupindex=>$groupcolumns) {
				...
				if(count($filtercolumns) > 0) {
					...
					foreach ($filtercolumns as $index=>$filter) {
						...
						if(empty($name[2]) && $name[1] == 'crmid' && $name[0] == 'mycrm_crmentity') {
							$name = $this->getSQLColumn('id');
						} else {
							$name = $name[2];
						}
