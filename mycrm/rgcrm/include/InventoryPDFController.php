...
include_once 'vtlib/Mycrm/PDF/models/Model.php';
include_once 'vtlib/Mycrm/PDF/inventory/HeaderViewer.php';
include_once 'vtlib/Mycrm/PDF/inventory/FooterViewer.php';
include_once 'vtlib/Mycrm/PDF/inventory/ContentViewer.php';
include_once 'vtlib/Mycrm/PDF/inventory/ContentViewer2.php';
include_once 'vtlib/Mycrm/PDF/viewers/PagerViewer.php';
include_once 'vtlib/Mycrm/PDF/PDFGenerator.php';
include_once 'data/CRMEntity.php';

class Mycrm_InventoryPDFController {
	...
	function getPDFGenerator() {
		return new Mycrm_PDF_Generator();
	}

	function getContentViewer() {
		if($this->focusColumnValue('hdnTaxType') == "individual") {
			$contentViewer = new Mycrm_PDF_InventoryContentViewer();
		} else {
			$contentViewer = new Mycrm_PDF_InventoryTaxGroupContentViewer();
		}
	...
	function getHeaderViewer() {
		$headerViewer = new Mycrm_PDF_InventoryHeaderViewer();
		$headerViewer->setModel($this->buildHeaderModel());
		return $headerViewer;
	}

	function getFooterViewer() {
		$footerViewer = new Mycrm_PDF_InventoryFooterViewer();
	...
	function getPagerViewer() {
		$pagerViewer = new Mycrm_PDF_PagerViewer();
	...
	function buildContentModels() {
		...
		foreach($associated_products as $productLineItem) {
			...
			$contentModel = new Mycrm_PDF_Model();
	...
	function buildContentLabelModel() {
		$labelModel = new Mycrm_PDF_Model();
	...
	function buildSummaryModel() {
		...
		$summaryModel = new Mycrm_PDF_Model();
	...
	function buildHeaderModel() {
		$headerModel = new Mycrm_PDF_Model();
	...
	function buildHeaderModelColumnLeft() {
		...
		// Company information
		$result = $adb->pquery("SELECT * FROM mycrm_organizationdetails", array());
	...
	function buildFooterModel() {
		$footerModel = new Mycrm_PDF_Model();
		$footerModel->set(Mycrm_PDF_InventoryFooterViewer::$DESCRIPTION_DATA_KEY, from_html($this->focusColumnValue('description')));
		$footerModel->set(Mycrm_PDF_InventoryFooterViewer::$TERMSANDCONDITION_DATA_KEY, from_html($this->focusColumnValue('terms_conditions')));
		return $footerModel;
	}

	function buildFooterLabelModel() {
		$labelModel = new Mycrm_PDF_Model();
		$labelModel->set(Mycrm_PDF_InventoryFooterViewer::$DESCRIPTION_LABEL_KEY, getTranslatedString('Description',$this->moduleName));
		$labelModel->set(Mycrm_PDF_InventoryFooterViewer::$TERMSANDCONDITION_LABEL_KEY, getTranslatedString('Terms & Conditions',$this->moduleName));
		return $labelModel;
	}
	...
	function buildWatermarkModel() {
		$watermarkModel = new Mycrm_PDF_Model();
	...
	function buildCurrencySymbol() {
		...
		if(!empty($currencyId)) {
			$result = $adb->pquery("SELECT currency_symbol FROM mycrm_currency_info WHERE id=?", array($currencyId));
