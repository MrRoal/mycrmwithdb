...
class MycrmCRMObject{
	
	private $moduleName ;
	private $moduleId ;
	private $instance ;
	
	function MycrmCRMObject($moduleCredential, $isId=false){
	...
	private function getObjectTypeId($objectName){
		...
		if($tid === false) {
			...
			$sql = "select * from mycrm_tab where name=?;";
	...
	function exists($id){
		...
		$exists = false;
		$sql = "select * from mycrm_crmentity where crmid=? and deleted=0";
