...
class MycrmCRMActorMeta extends EntityMeta {
	protected $pearDB;
	protected static $fieldTypeMapping = array();
	protected static $referenceTypeMapping = array();
	
	function MycrmCRMActorMeta($tableName,$webserviceObject,$adb,$user){
	...
	protected function getReferenceList($dbField, $tableName){
		...
		if(!isset(MycrmCRMActorMeta::$fieldTypeMapping[$tableName][$dbField->name])){
			$this->getFieldType($dbField, $tableName);
		}
		$fieldTypeData = MycrmCRMActorMeta::$fieldTypeMapping[$tableName][$dbField->name];
		
		if (empty(MycrmCRMActorMeta::$referenceTypeMapping)) {
			$sql = "SELECT * FROM mycrm_ws_entity_referencetype";
			...
			for ($index =0, $count = $this->pearDB->num_rows($result); $index < $count; ++$index) {
				...
				MycrmCRMActorMeta::$referenceTypeMapping[$row['fieldtypeid']][] = $row['type'];
			}
		}
		
		$referenceTypes = array();
		if (isset(MycrmCRMActorMeta::$referenceTypeMapping[$fieldTypeData['fieldtypeid']])) {
			$referenceTypes = MycrmCRMActorMeta::$referenceTypeMapping[$fieldTypeData['fieldtypeid']];
		}
	...
	protected function getFieldType($dbField,$tableName){
		...
		if(isset(MycrmCRMActorMeta::$fieldTypeMapping[$tableName][$dbField->name])){
			if(MycrmCRMActorMeta::$fieldTypeMapping[$tableName][$dbField->name] === 'null'){
				return null;
			}
			$row = MycrmCRMActorMeta::$fieldTypeMapping[$tableName][$dbField->name];
			return $row['fieldtype'];
		}
		
		if (empty(MycrmCRMActorMeta::$fieldTypeMapping)) {
			// Optimization to avoid repeated initialization
			$sql = "select * from mycrm_ws_entity_fieldtype";
			$result = $this->pearDB->pquery($sql,array());
			$rowCount = $this->pearDB->num_rows($result);
			while ($rowCount) {
				$row = $this->pearDB->query_result_rowdata($result,$rowCount-1);
				MycrmCRMActorMeta::$fieldTypeMapping[$row['table_name']][$row['field_name']] = $row;
				--$rowCount;
			}
		}
		
		if (!isset(MycrmCRMActorMeta::$fieldTypeMapping[$tableName][$dbField->name])) {
			MycrmCRMActorMeta::$fieldTypeMapping[$tableName][$dbField->name] = 'null';
			return null;
		}
		return MycrmCRMActorMeta::$fieldTypeMapping[$tableName][$dbField->name]['fieldtype'];
	}
	...
	public function getNameFields(){
		$query = "select name_fields from mycrm_ws_entity_name where entity_id = ?";
	...
	public function getEntityDeletedQuery() {
		if($this->getEntityName() == 'Currency'){
			return 'mycrm_currency_info.deleted=0';
		}
