...
function vtws_convertlead($entityvalues, $user) {
	...
	$leadObject = MycrmWebserviceObject::fromName($adb, 'Leads');
	...
	$sql = "select converted from mycrm_leaddetails where converted = 1 and leadid=?";
	...
	foreach ($availableModules as $entityName) {
		if ($entityvalues['entities'][$entityName]['create']) {
			...
			$entityObject = MycrmWebserviceObject::fromName($adb, $entityvalue['name']);
			...
			try {
				...
				if ($entityvalue['name'] == 'Accounts') {
					$sql = "SELECT mycrm_account.accountid FROM mycrm_account,mycrm_crmentity WHERE mycrm_crmentity.crmid=mycrm_account.accountid AND mycrm_account.accountname=? AND mycrm_crmentity.deleted=0";
...
function vtws_updateConvertLeadStatus($entityIds, $leadId, $user) {
	...
	if ($entityIds['Accounts'] != '' || $entityIds['Contacts'] != '') {
		$sql = "UPDATE mycrm_leaddetails SET converted = 1 where leadid=?";
		$result = $adb->pquery($sql, array($leadIdComponents[1]));
		if ($result === false) {
		...
		$sql = "DELETE FROM mycrm_campaignleadrel WHERE leadid=?";
		$adb->pquery($sql, array($leadIdComponents[1]));

		$sql = "DELETE FROM mycrm_tracker WHERE item_id=?";
		$adb->pquery($sql, array($leadIdComponents[1]));

		//update the modifiedtime and modified by information for the record
		$leadModifiedTime = $adb->formatDate(date('Y-m-d H:i:s'), true);
		$crmentityUpdateSql = "UPDATE mycrm_crmentity SET modifiedtime=?, modifiedby=? WHERE crmid=?";
		$adb->pquery($crmentityUpdateSql, array($leadModifiedTime, $user->id, $leadIdComponents[1]));
                ...
                $em->triggerEvent("mycrm.entity.leadconverted", $entityData);
	}
