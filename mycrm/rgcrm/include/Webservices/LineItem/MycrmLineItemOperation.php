...
require_once "include/Webservices/LineItem/MycrmInventoryOperation.php";
...
/**
 * Description of MycrmLineItemOperation
 */
class MycrmLineItemOperation  extends MycrmActorOperation {
	...
	public function __construct($webserviceObject,$user,$adb,$log) {
		...
		$this->meta = new MycrmLineItemMeta($this->entityTableName,$webserviceObject,$adb,$user);
	...
	private function updateTaxes($createdElement){
		if(count($this->taxList) > 0 ) {
			...
			$id = $id[1];
			$sql = 'UPDATE mycrm_inventoryproductrel set ';
	...
	function getProductPrice($productId){
		...
		$sql = "select unit_price from mycrm_products where productid=?";
	...
	public function getCollectiveTaxList(){
		$db = PearDatabase::getInstance();
		$sql = 'select * from mycrm_inventorytaxinfo where deleted=0';
		$params = array();
		$result = null;
		$transactionSuccessful = vtws_runQueryAsTransaction($sql,$params,$result);
		if(!$transactionSuccessful){
	...
	private function getProductTaxList($productId){
		$db = PearDatabase::getInstance();
		$sql = 'select * from mycrm_producttaxrel inner join mycrm_inventorytaxinfo on
			mycrm_producttaxrel.taxid=mycrm_inventorytaxinfo.taxid and deleted=0
			where productid=?';
		$params = array($productId);
		$result = null;
		$transactionSuccessful = vtws_runQueryAsTransaction($sql,$params,$result);
