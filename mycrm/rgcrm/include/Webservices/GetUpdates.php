...
require_once 'include/Webservices/DescribeObject.php';
	...
	function vtws_sync($mtime,$elementType,$syncType,$user){
        ...
        if ($userAndGroupSync) {
        $groupresult = $adb->pquery("select groupid from mycrm_users2group where userid=?", array($user->id));
		...
		foreach($accessableModules as $elementType){
			...
			$fromClause.= ' ) mycrm_ws_sync ON (mycrm_crmentity.crmid = mycrm_ws_sync.crmid)';
	...
	function vtws_getEmailFromClause(){
		$q = "FROM mycrm_activity
				...
				LEFT JOIN mycrm_seattachmentsrel ON mycrm_activity.activityid = mycrm_seattachmentsrel.crmid
				LEFT JOIN mycrm_attachments ON mycrm_seattachmentsrel.attachmentsid = mycrm_attachments.attachmentsid
				LEFT JOIN mycrm_email_track ON mycrm_activity.activityid = mycrm_email_track.mailid
				INNER JOIN mycrm_emaildetails ON mycrm_activity.activityid = mycrm_emaildetails.emailid
				LEFT JOIN mycrm_users mycrm_users2 ON mycrm_emaildetails.idlists = mycrm_users2.id
				LEFT JOIN mycrm_groups mycrm_groups2 ON mycrm_emaildetails.idlists = mycrm_groups2.groupid";
		return $q;
	}

	function getSyncQueryBaseTable($elementType){
		if($elementType!="Calendar" && $elementType!="Events"){
			return "mycrm_crmentity";
		}
		else{
			$activityCondition = getCalendarTypeCondition($elementType);
			$query = "mycrm_crmentity INNER JOIN mycrm_activity ON (mycrm_crmentity.crmid = mycrm_activity.activityid and $activityCondition)";
			return $query;
		}
	}
	...
	function getCalendarTypeCondition($elementType){
		if($elementType == "Events")
			$activityCondition = "mycrm_activity.activitytype !='Task' and mycrm_activity.activitytype !='Emails'";
		else
			$activityCondition = "mycrm_activity.activitytype ='Task'";
