...
class MycrmModuleOperation extends WebserviceEntityOperation {
	protected $tabId;
	protected $isEntity = true;
	protected $partialDescribeFields = null;
	
	public function MycrmModuleOperation($webserviceObject,$user,$adb,$log){
	...
	protected function getMetaInstance(){
		if(empty(WebserviceEntityOperation::$metaCache[$this->webserviceObject->getEntityName()][$this->user->id])){
			WebserviceEntityOperation::$metaCache[$this->webserviceObject->getEntityName()][$this->user->id]  = new MycrmCRMObjectMeta($this->webserviceObject,$this->user);
		}
		return WebserviceEntityOperation::$metaCache[$this->webserviceObject->getEntityName()][$this->user->id];
	}
	
	public function create($elementType,$element){
		$crmObject = new MycrmCRMObject($elementType, false);
	...
	public function retrieve($id){
		...
		$crmObject = new MycrmCRMObject($this->tabId, true);
    ...
    public function relatedIds($id, $relatedModule, $relatedLabel, $relatedHandler=null) {
		...
		$sourceRecordModel = Mycrm_Record_Model::getInstanceById($ids[1], $sourceModule);
		$targetModel       = Mycrm_RelationListView_Model::getInstance($sourceRecordModel, $relatedModule, $relatedLabel);
        $sql = $targetModel->getRelationQuery();

        $relatedWebserviceObject = MycrmWebserviceObject::fromName($adb,$relatedModule);
        ...
        $sql = sprintf("SELECT DISTINCT concat('%sx',mycrm_crmentity.crmid) as wsid FROM %s", $relatedModuleWSId, $sqlFromPart);
