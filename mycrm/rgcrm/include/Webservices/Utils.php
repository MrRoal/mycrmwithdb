...
/**
 * get current mycrm version from the database.
 */
function vtws_getMycrmVersion(){
	global $adb;
	$query = 'select * from mycrm_version';
...
function vtws_getWebserviceGroupFromGroups($groups){
	global $adb;
	$webserviceObject = MycrmWebserviceObject::fromName($adb,'Groups');
...
function getEmailFieldId($meta, $entityId){
	...
	$query="SELECT fieldid,fieldlabel,columnname FROM mycrm_field WHERE tabid=?
...
function vtws_getModuleNameList(){
	...
	$sql = "select name from mycrm_tab where isentitytype=1 and name not in ('Rss','Webmails',".
...
function vtws_getWebserviceEntities(){
	...
	$sql = "select name,id,ismodule from mycrm_ws_entity";
	$res = $adb->pquery($sql, array());
	$moduleArray = Array();
	$entityArray = Array();
	while($row = $adb->fetchByAssoc($res)){
...
/**
 *
 * @param MycrmWebserviceObject $webserviceObject
 * @return CRMEntity
 */
...
function vtws_isRecordOwnerUser($ownerId){
	...
	if (!array_key_exists($ownerId, $cache)) {
		$result = $adb->pquery("select first_name from mycrm_users where id = ?",array($ownerId));
		$rowCount = $adb->num_rows($result);
...
function vtws_isRecordOwnerGroup($ownerId){
	...
	static $cache = array();
	if (!array_key_exists($ownerId, $cache)) {
		$result = $adb->pquery("select groupname from mycrm_groups where groupid = ?",array($ownerId));
		$rowCount = $adb->num_rows($result);
...
function vtws_getCalendarEntityType($id){
	...
	$sql = "select activitytype from mycrm_activity where activityid=?";
...
function vtws_getWebserviceEntityId($entityName, $id){
	global $adb;
	$webserviceObject = MycrmWebserviceObject::fromName($adb,$entityName);
...
function vtws_addModuleTypeWebserviceEntity($moduleName,$filePath,$className){
	global $adb;
	$checkres = $adb->pquery('SELECT id FROM mycrm_ws_entity WHERE name=? AND handler_path=? AND handler_class=?',
		array($moduleName, $filePath, $className));
	if($checkres && $adb->num_rows($checkres) == 0) {
		$isModule=1;
		$entityId = $adb->getUniqueID("mycrm_ws_entity");
		$adb->pquery('insert into mycrm_ws_entity(id,name,handler_path,handler_class,ismodule) values (?,?,?,?,?)',
...
function vtws_deleteWebserviceEntity($moduleName) {
	global $adb;
	$adb->pquery('DELETE FROM mycrm_ws_entity WHERE name=?',array($moduleName));
}

function vtws_addDefaultActorTypeEntity($actorName,$actorNameDetails,$withName = true){
	$actorHandler = array('file'=>'include/Webservices/MycrmActorOperation.php',
		'class'=>'MycrmActorOperation');
	if($withName == true){
...
function vtws_addActorTypeName($entityId,$fieldNames,$indexColumn,$tableName){
	global $adb;
	$adb->pquery('insert into mycrm_ws_entity_name(entity_id,name_fields,index_field,table_name) values (?,?,?,?)',
...
function vtws_getName($id,$user){
	...
	$webserviceObject = MycrmWebserviceObject::fromId($adb,$id);
...
function vtws_addWebserviceOperation($name,$handlerFilePath,$handlerMethodName,$requestType,$preLogin = 0){
	global $adb;
	$createOperationQuery = "insert into mycrm_ws_operation(operationid,name,handler_path,handler_method,type,prelogin)
		values (?,?,?,?,?,?);";
	...
	$operationId = $adb->getUniqueID("mycrm_ws_operation");
...
function vtws_addWebserviceOperationParam($operationId,$paramName,$paramType,$sequence){
	...
	$createOperationParamsQuery = "insert into mycrm_ws_operation_parameters(operationid,name,type,sequence)
		values (?,?,?,?);";
...
function vtws_getModuleHandlerFromName($name,$user){
	global $adb, $log;
	$webserviceObject = MycrmWebserviceObject::fromName($adb,$name);
	$handlerPath = $webserviceObject->getHandlerPath();
	$handlerClass = $webserviceObject->getHandlerClass();
...
function vtws_getActorEntityName ($name, $idList) {
	...
	$entity = MycrmWebserviceObject::fromName($db, $name);
...
function vtws_getActorEntityNameById ($entityId, $idList) {
	...
	$nameList = array();
	$webserviceObject = MycrmWebserviceObject::fromId($db, $entityId);
	$query = "select * from mycrm_ws_entity_name where entity_id = ?";
...
function vtws_getConvertLeadFieldMapping(){
	global $adb;
	$sql = "select * from mycrm_convertleadmapping";
...
function vtws_getRelatedNotesAttachments($id,$relatedId) {
	...
	$sql = "select * from mycrm_senotesrel where crmid=?";
	$result = $adb->pquery($sql, array($id));
	if($result === false){
		return false;
	}
	...
	$sql="insert into mycrm_senotesrel(crmid,notesid) values (?,?)";
	for($i=0; $i<$rowCount;++$i ) {
	...
	$sql = "select * from mycrm_seattachmentsrel where crmid=?";
	$result = $adb->pquery($sql, array($id));
	if($result === false){
		return false;
	}
	...
	$sql = "insert into mycrm_seattachmentsrel(crmid,attachmentsid) values (?,?)";
	for($i=0;$i<$rowCount;++$i) {
...
function vtws_saveLeadRelatedProducts($leadId, $relatedId, $setype) {
	...
	$result = $adb->pquery("select * from mycrm_seproductsrel where crmid=?", array($leadId));
	if($result === false){
		return false;
	}
	$rowCount = $adb->num_rows($result);
	for($i = 0; $i < $rowCount; ++$i) {
		...
		$resultNew = $adb->pquery("insert into mycrm_seproductsrel values(?,?,?)", array($relatedId, $productId, $setype));
		if($resultNew === false){
			return false;
		}
	}
...
function vtws_saveLeadRelations($leadId, $relatedId, $setype) {
	...
	$result = $adb->pquery("select * from mycrm_crmentityrel where crmid=?", array($leadId));
	if($result === false){
		return false;
	}
	$rowCount = $adb->num_rows($result);
	for($i = 0; $i < $rowCount; ++$i) {
		...
		$adb->pquery("insert into mycrm_crmentityrel values(?,?,?,?)",
		array($relatedId, $setype, $recordId, $recordModule));
		if($resultNew === false){
			return false;
		}
	}
	$result = $adb->pquery("select * from mycrm_crmentityrel where relcrmid=?", array($leadId));
	if($result === false){
		return false;
	}
	$rowCount = $adb->num_rows($result);
...
function vtws_getRelatedActivities($leadId,$accountId,$contactId,$relatedId) {
	...
	global $adb;
	$sql = "select * from mycrm_seactivityrel where crmid=?";
	...
	for($i=0;$i<$rowCount;++$i) {
		...
		$sql ="select setype from mycrm_crmentity where crmid=?";
		...
		if($type != "Emails") {
				if(!empty($accountId)){
					$sql = "insert into mycrm_seactivityrel(crmid,activityid) values (?,?)";
				...
				if(!empty($contactId)){
					$sql="insert into mycrm_cntactivityrel(contactid,activityid) values (?,?)";
...
function vtws_saveLeadRelatedCampaigns($leadId, $relatedId, $seType) {
	...
	$result = $adb->pquery("select * from mycrm_campaignleadrel where leadid=?", array($leadId));
	if($result === false){
		return false;
	}
	$rowCount = $adb->num_rows($result);
	for($i = 0; $i < $rowCount; ++$i) {
		...
		if($seType == 'Accounts') {
			$resultNew = $adb->pquery("insert into mycrm_campaignaccountrel (campaignid, accountid) values(?,?)",
				array($campaignId, $relatedId));
		} elseif ($seType == 'Contacts') {
			$resultNew = $adb->pquery("insert into mycrm_campaigncontrel (campaignid, contactid) values(?,?)",
				array($campaignId, $relatedId));
		}
		if($resultNew === false){
			return false;
...
function vtws_transferOwnership($ownerId, $newOwnerId, $delete=true) {
	$db = PearDatabase::getInstance();
	//Updating the smcreatorid,smownerid, modifiedby in mycrm_crmentity
	$sql = "UPDATE mycrm_crmentity SET smcreatorid=? WHERE smcreatorid=? AND setype<>?";
	$db->pquery($sql, array($newOwnerId, $ownerId,'ModComments'));

	$sql = "UPDATE mycrm_crmentity SET smownerid=? WHERE smownerid=? AND setype<>?";
	$db->pquery($sql, array($newOwnerId, $ownerId, 'ModComments'));

	$sql = "update mycrm_crmentity set modifiedby=? where modifiedby=?";
	$db->pquery($sql, array($newOwnerId, $ownerId));

	//deleting from mycrm_tracker
	if ($delete) {
		$sql = "delete from mycrm_tracker where user_id=?";
		$db->pquery($sql, array($ownerId));
	}

	//updating the mycrm_import_maps
	$sql ="update mycrm_import_maps set assigned_user_id=? where assigned_user_id=?";
	$db->pquery($sql, array($newOwnerId, $ownerId));

	if(Mycrm_Utils::CheckTable('mycrm_customerportal_prefs')) {
		$query = 'UPDATE mycrm_customerportal_prefs SET prefvalue = ? WHERE prefkey = ? AND prefvalue = ?';
	...
	//delete from mycrm_homestuff
	if ($delete) {
		$sql = "delete from mycrm_homestuff where userid=?";
		$db->pquery($sql, array($ownerId));
	}

	//delete from mycrm_users to mycrm_role mycrm_table
	if ($delete) {
		$sql = "delete from mycrm_users2group where userid=?";
		$db->pquery($sql, array($ownerId));
	}

	$sql = "select tabid,fieldname,tablename,columnname from mycrm_field left join ".
	"mycrm_fieldmodulerel on mycrm_field.fieldid=mycrm_fieldmodulerel.fieldid where uitype ".
...
function vtws_updateWebformsRoundrobinUsersLists($ownerId, $newOwnerId){
    $db = PearDatabase::getInstance();
    $sql = 'SELECT id,roundrobin_userid FROM mycrm_webforms;';
    $result = $db->pquery($sql, array());
        ...
        if(is_array($usersList)){
            if(($key = array_search($ownerId, $usersList)) !== false){
                ...
                if(count($usersList) == 0){
                    $db->pquery('UPDATE mycrm_webforms SET roundrobin_userid = ?,roundrobin = ? where id =?',array("--None--",0,$webformId));
                }
                else{
                    $usersList = json_encode($usersList);
                    $db->pquery('UPDATE mycrm_webforms SET roundrobin_userid = ? where id =?',array($usersList,$webformId));
                }
            }
        }
    }
...
function vtws_transferOwnershipForWorkflowTasks($ownerModel, $newOwnerModel) {
	...
	$query = "SELECT task,task_id,workflow_id FROM com_mycrm_workflowtasks where task LIKE '%".$nameSearchValue."%' OR task LIKE '%".$idSearchValue.
	...
	for ($i = 0; $i < $num_rows; $i++) {
		...
		require_once 'modules/com_mycrm_workflow/tasks/'.$className.'.inc';
		...
		if(array_key_exists("field_value_mapping",$unserializeTask)) {
			...
			if (!empty($fieldMapping)) {
				...
				$query = 'UPDATE com_mycrm_workflowtasks SET task=? where workflow_id=? AND task_id=?';
