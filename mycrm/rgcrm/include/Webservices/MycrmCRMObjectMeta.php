...
class MycrmCRMObjectMeta extends EntityMeta {
	...
	function MycrmCRMObjectMeta($webserviceObject,$user){
		...
		if(in_array('mycrm_crmentity',$instance->tab_name)){
			$this->defaultTableList = array('mycrm_crmentity');
		}else{
	...
	private function computeAccess(){
		...
		if($is_admin == true || $profileGlobalPermission[1] == 0 || $profileGlobalPermission[2] == 0){
			...
			$sql = "select * from mycrm_profile2globalpermissions where profileid in (".generateQuestionMarks($profileList).");";
			...
			$sql = 'select * from mycrm_profile2tab where profileid in ('.generateQuestionMarks($profileList).') and tabid = ?;';
			...
			$sql = "select * from mycrm_profile2standardpermissions where profileid in (".generateQuestionMarks($profileList).") and tabid=?";
			$result = $adb->pquery($sql,array($profileList,$this->getTabId()));
	...
	function hasAssignPrivilege($webserviceId){
		...
		$webserviceObject = MycrmWebserviceObject::fromId($adb,$ownerTypeId);
	...
	private function retrieveMetaForBlock($block){
		...
		require('user_privileges/user_privileges_'.$this->user->id.'.php');
		if($is_admin == true || $profileGlobalPermission[1] == 0 || $profileGlobalPermission[2] ==0){
			$sql = "select *, '0' as readonly from mycrm_field where tabid =? and block in (".generateQuestionMarks($block).") and displaytype in (1,2,3,4,5)";
			$params = array($tabid, $block);	
		}else{
			$profileList = getCurrentUserProfileList();
			
			if (count($profileList) > 0) {
				$sql = "SELECT mycrm_field.*, mycrm_profile2field.readonly
						FROM mycrm_field
						...
						WHERE mycrm_field.tabid =? AND mycrm_profile2field.visible = 0 
						AND mycrm_profile2field.profileid IN (". generateQuestionMarks($profileList) .")
						AND mycrm_def_org_field.visible = 0 and mycrm_field.block in (".generateQuestionMarks($block).") and mycrm_field.displaytype in (1,2,3,4,5) and mycrm_field.presence in (0,2) group by columnname";
				...
				$sql = "SELECT mycrm_field.*, mycrm_profile2field.readonly
						...
						AND mycrm_profile2field.visible = 0 
						AND mycrm_def_org_field.visible = 0 and mycrm_field.block in (".generateQuestionMarks($block).") and mycrm_field.displaytype in (1,2,3,4,5) and mycrm_field.presence in (0,2) group by columnname";
				$params = array($tabid, $block);
			}
		}
	...
	function getObjectEntityName($webserviceId){
		...
		if($this->objectName == 'Users'){
			$sql = "select user_name from mycrm_users where id=? and deleted=0";
			...
			$sql = "select setype from mycrm_crmentity where crmid=? and deleted=0";
			$result = $adb->pquery($sql , array($id));
			if($result != null && isset($result)){
				if($adb->num_rows($result)>0){
	...
	function exists($recordId){
		...
		if($this->objectName == 'Users'){
			if (array_key_exists($recordId, $user_exists_cache)) {
				...
				$sql = "select 1 from mycrm_users where id=? and deleted=0 and status='Active'";
			}
			
		}else{
			$sql = "select 1 from mycrm_crmentity where crmid=? and deleted=0 and setype='".
				$this->getTabName()."'";
		}
