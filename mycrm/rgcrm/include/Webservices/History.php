...
function vtws_history($element, $user) {
	...
	$sql = 'SELECT mycrm_modtracker_basic.* FROM mycrm_modtracker_basic
		INNER JOIN mycrm_crmentity ON mycrm_modtracker_basic.crmid = mycrm_crmentity.crmid
		AND mycrm_crmentity.deleted = 0';

	if ($mode == 'Private') {
		$sql .= ' WHERE mycrm_modtracker_basic.whodid = ?';
		$params[] = $user->getId();
	} else if ($mode == 'All') {
		if ($acrossAllModule) {
			// TODO collate only active (or enabled) modules for tracking.
		} else if($moduleName) {
            $sql .= ' WHERE mycrm_modtracker_basic.module = ?';
            $params[] = $moduleName;
        } else {
            $sql .= ' WHERE mycrm_modtracker_basic.crmid = ?';
	...
	$sql .= sprintf(' ORDER BY mycrm_modtracker_basic.id DESC LIMIT %s,%s', $start, $MAXLIMIT);
	...
	if (!empty($orderedIds)) {
		$sql = 'SELECT mycrm_modtracker_detail.* FROM mycrm_modtracker_detail';
		$sql .= ' WHERE mycrm_modtracker_detail.id IN (' . generateQuestionMarks($orderedIds) . ')';
