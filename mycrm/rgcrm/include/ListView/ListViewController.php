			...
			foreach ($listViewFields as $fieldName) {
				...
				if($module == 'Documents' && $fieldName == 'filename') {
                    ...
                    if($module == 'Calendar' && ($fieldName == 'taskstatus' || $fieldName == 'eventstatus' || $fieldName == 'activitytype')) {
                        $value = Mycrm_Language_Handler::getTranslatedString($value,$module);
						...
						$value = "<font color='red'>". Mycrm_Language_Handler::getTranslatedString('LBL_NOT_ACCESSIBLE',
								$module)."</font>";
					} else {
						$value =  Mycrm_Language_Handler::getTranslatedString($value,$module);
						$value = textlength_check($value);
					...
					if($value != '' && $value != '0000-00-00') {
						...
						if($fieldDataType == 'datetime') {
							$value = Mycrm_Datetime_UIType::getDateTimeValue($value);
					...
					if(!empty($value)){
						...
						if($userModel->get('hour_format') == '12'){
							$value = Mycrm_Time_UIType::getTimeValueInAMorPM($value);
						}
					}
...
//							$value = CurrencyField::appendCurrencySymbol($currencyValue, $currencySymbol);
					...
					if($current_user->internal_mailer == 1){
						...
						$value = "<a class='emailField' onclick=\"Mycrm_Helper_Js.getInternalMailer($recordId,".
                    ...
                    if($outgoingCallPermission && !empty($value)) {
                        ...
                        $value = '<a class="phoneField" data-value="'.$phoneNumber.'" record="'.$recordId.'" onclick="Mycrm_PBXManager_Js.registerPBXOutboundCall(\''.$phoneNumber.'\', '.$recordId.')">'.textlength_check($value).'</a>';
                    }else {
                        $value = textlength_check($value);
                    }
					...
					$result1 = $this->db->pquery("SELECT access_count FROM mycrm_email_track WHERE ".
