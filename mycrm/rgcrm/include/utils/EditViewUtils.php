 ...
 ********************************************************************************/
/*********************************************************************************
 * $Header: /advent/projects/wesat/mycrm_crm/sugarcrm/include/utils/EditViewUtils.php,v 1.188 2005/04/29 05:5 * 4:39 rank Exp
...
/** This function returns the mycrm_invoice object populated with the details from sales order object.
...
/** This function returns the detailed list of mycrm_products associated to a given entity or a record.
...
function getAssociatedProducts($module,$focus,$seid='')
{
	...
	{
		$query="SELECT
					case when mycrm_products.productid != '' then mycrm_products.productname else mycrm_service.servicename end as productname,
 		            case when mycrm_products.productid != '' then mycrm_products.product_no else mycrm_service.service_no end as productcode,
					case when mycrm_products.productid != '' then mycrm_products.unit_price else mycrm_service.unit_price end as unit_price,
 		            case when mycrm_products.productid != '' then mycrm_products.qtyinstock else 'NA' end as qtyinstock,
 		            case when mycrm_products.productid != '' then 'Products' else 'Services' end as entitytype,
 		                        mycrm_inventoryproductrel.listprice,
 		                        mycrm_inventoryproductrel.description AS product_description,
 		                        mycrm_inventoryproductrel.*,mycrm_crmentity.deleted
 	                            FROM mycrm_inventoryproductrel
								LEFT JOIN mycrm_crmentity ON mycrm_crmentity.crmid=mycrm_inventoryproductrel.productid
 		                        LEFT JOIN mycrm_products
 		                                ON mycrm_products.productid=mycrm_inventoryproductrel.productid
 		                        LEFT JOIN mycrm_service
	...
	elseif($module == 'Potentials')
	{
		$query="SELECT
 		                        ...
 		                        mycrm_products.productcode,
 		                        mycrm_products.unit_price,
 		                        mycrm_products.qtyinstock,
 		                        mycrm_seproductsrel.*,mycrm_crmentity.deleted,
 		                        mycrm_crmentity.description AS product_description
 		                        FROM mycrm_products
 		                        INNER JOIN mycrm_crmentity
 		                                ON mycrm_crmentity.crmid=mycrm_products.productid
 		                        INNER JOIN mycrm_seproductsrel
 		                                ON mycrm_seproductsrel.productid=mycrm_products.productid
 		                        WHERE mycrm_seproductsrel.crmid=?";
			$params = array($seid);
	}
	elseif($module == 'Products')
	{
		$query="SELECT
 		                        mycrm_products.productid,
 		                        mycrm_products.productcode,
 		                        mycrm_products.productname,
 		                        mycrm_products.unit_price,
 		                        mycrm_products.qtyinstock,mycrm_crmentity.deleted,
 		                        mycrm_crmentity.description AS product_description,
	...
	{
		$query="SELECT
 		                        mycrm_service.serviceid AS productid,
 		                        'NA' AS productcode,
 		                        mycrm_service.servicename AS productname,
 		                        mycrm_service.unit_price AS unit_price,
 		                        'NA' AS qtyinstock,mycrm_crmentity.deleted,
 		                        mycrm_crmentity.description AS product_description,
 		                       	'Services' AS entitytype
 								FROM mycrm_service
 		                        INNER JOIN mycrm_crmentity
 		                                ON mycrm_crmentity.crmid=mycrm_service.serviceid
 		                        WHERE mycrm_crmentity.deleted=0
 		                                AND serviceid=?";
			$params = array($seid);
	}
	...
	for($i=1;$i<=$num_rows;$i++)
	{
		...
		if(empty($focus->mode) && $seid!=''){
			$sub_prod_query = $adb->pquery("SELECT crmid as prod_id from mycrm_seproductsrel WHERE productid=? AND setype='Products'",array($seid));
		} else {
			$sub_prod_query = $adb->pquery("SELECT productid as prod_id from mycrm_inventorysubproductrel WHERE id=? AND sequence_no=?",array($focus->id,$i));
		}
		$subprodid_str='';
		$subprodname_str='';
	...
	//if taxtype is group then the tax should be same for all products in mycrm_inventoryproductrel table
...
/** This function returns the data type of the mycrm_fields, with mycrm_field label, which is used for javascript validation.
* Param $validationData - array of mycrm_fieldnames with datatype
* Return type array
*/
