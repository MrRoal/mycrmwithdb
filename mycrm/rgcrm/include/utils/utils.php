...
require_once 'vtlib/Mycrm/Functions.php';
require_once 'vtlib/Mycrm/Deprecated.php';
...
function get_user_array($add_blank=true, $status="Active", $assigned_user="",$private="",$module=false)
{
	...
	if($user_array == null)
	{
		...
		$temp_result = Array();
		// Including deleted mycrm_users for now.
		if (empty($status)) {
				$query = "SELECT id, user_name from mycrm_users";
				$params = array();
		}
		else {
				if($private == 'private')
				{
					...
					$query = "select id as id,user_name as user_name,first_name,last_name from mycrm_users where id=? and status='Active' union select mycrm_user2role.userid as id,mycrm_users.user_name as user_name ,
							  mycrm_users.first_name as first_name ,mycrm_users.last_name as last_name
							  from mycrm_user2role inner join mycrm_users on mycrm_users.id=mycrm_user2role.userid inner join mycrm_role on mycrm_role.roleid=mycrm_user2role.roleid where mycrm_role.parentrole like ? and status='Active' union
							  select shareduserid as id,mycrm_users.user_name as user_name ,
							  mycrm_users.first_name as first_name ,mycrm_users.last_name as last_name  from mycrm_tmp_write_user_sharing_per inner join mycrm_users on mycrm_users.id=mycrm_tmp_write_user_sharing_per.shareduserid where status='Active' and mycrm_tmp_write_user_sharing_per.userid=? and mycrm_tmp_write_user_sharing_per.tabid=?";
					$params = array($current_user->id, $current_user_parent_role_seq."::%", $current_user->id, getTabid($module));
				}
				else
				{
					$log->debug("Sharing is Public. All mycrm_users should be listed");
					$query = "SELECT id, user_name,first_name,last_name from mycrm_users WHERE status=?";
					$params = array($status);
				}
		}
...
function get_group_array($add_blank=true, $status="Active", $assigned_user="",$private="",$module = false)
{
	...
	if($group_array == null)
	{
		...
		// Including deleted mycrm_users for now.
		$log->debug("Sharing is Public. All mycrm_users should be listed");
		$query = "SELECT groupid, groupname from mycrm_groups";
		$params = array();
		...
		if($private == 'private'){
			...
			if(count($current_user_groups) != 0) {
				$query .= " OR mycrm_groups.groupid in (".generateQuestionMarks($current_user_groups).")";
				array_push($params, $current_user_groups);
			}
			$log->debug("Sharing is Private. Only the current user should be listed");
			$query .= " union select mycrm_group2role.groupid as groupid,mycrm_groups.groupname as groupname from mycrm_group2role inner join mycrm_groups on mycrm_groups.groupid=mycrm_group2role.groupid inner join mycrm_role on mycrm_role.roleid=mycrm_group2role.roleid where mycrm_role.parentrole like ?";
			array_push($params, $current_user_parent_role_seq."::%");

			if(count($current_user_groups) != 0) {
				$query .= " union select mycrm_groups.groupid as groupid,mycrm_groups.groupname as groupname from mycrm_groups inner join mycrm_group2rs on mycrm_groups.groupid=mycrm_group2rs.groupid where mycrm_group2rs.roleandsubid in (".generateQuestionMarks($parent_roles).")";
				array_push($params, $parent_roles);
			}

			$query .= " union select sharedgroupid as groupid,mycrm_groups.groupname as groupname from mycrm_tmp_write_group_sharing_per inner join mycrm_groups on mycrm_groups.groupid=mycrm_tmp_write_group_sharing_per.sharedgroupid where mycrm_tmp_write_group_sharing_per.userid=?";
			array_push($params, $current_user->id);

			$query .= " and mycrm_tmp_write_group_sharing_per.tabid=?";
...
function return_app_list_strings_language($language) {
	return Mycrm_Deprecated::return_app_list_strings_language($language);
}
...
function return_module_language($language, $module) {
	return Mycrm_Deprecated::getModuleTranslationStrings($language, $module);
}
...
function return_specified_module_language($language, $module) {
	return Mycrm_Deprecated::return_app_list_strings_language($language, $module);
}
 ...
 */
function get_themes() {
	return Mycrm_Theme::getAllSkins();
   }
...
{
        $startstring=$string;
	$cachedresult=Mycrm_Cache::get('to_html',$startstring);
	if($cachedresult){
		return $cachedresult;
	}
	...
	if(is_string($string))
	{
		...
		// In mycrm5 ajax request are treated specially and the data is encoded
		if ($doconvert == true)
		{
			if($inUTF8)
	...
	Mycrm_Cache::set('to_html', $startstring, $string);
	return $string;
}
...
function getTabname($tabid)
{
	...
	if (!isset($cache[$tabid])) {
		$sql = "select tablabel from mycrm_tab where tabid=?";
...
{
	return Mycrm_Functions::getModuleName($tabid);
}
...
function getColumnFields($module)
{
	...
	if($cachedModuleFields === false) {
		...
		FROM mycrm_field WHERE tabid in (" . generateQuestionMarks($tabid) . ")";
...
function getUserEmail($userid)
{
	...
	$log->info("in getUserEmail ".$userid);
        ...
        if($userid != '')
        {
                $sql = "select email1 from mycrm_users where id=?";
...
function getUserId_Ol($username)
{
	...
	global $adb;
	$sql = "select id from mycrm_users where user_name=?";
...
function getActionid($action)
{
	...
	{
		$query="select * from mycrm_actionmapping where actionname=?";
...
function getActionname($actionid)
{
	...
	else
	{
		...
		$query="select * from mycrm_actionmapping where actionid=? and securitycheck=0";
...
function getRecordOwnerId($record)
{
	...
	if($ownerId === false) {
		$query="select smownerid from mycrm_crmentity where crmid = ?";
	...
	if($ownerId)
	{
		...
		if($count === false) {
			$sql_result = $adb->pquery("select count(*) as count from mycrm_users where id = ?",array($ownerId));
...
function insertProfile2field($profileid)
{
	...
	$fld_result = $adb->pquery("select * from mycrm_field where generatedtype=1 and displaytype in (1,2,3) and mycrm_field.presence in (0,2) and tabid != 29", array());
    ...
    for($i=0; $i<$num_rows; $i++) {
         ...
         $adb->pquery("insert into mycrm_profile2field values (?,?,?,?,?)", $params);
	}
...
function insert_def_org_field()
{
	...
	$fld_result = $adb->pquery("select * from mycrm_field where generatedtype=1 and displaytype in (1,2,3) and mycrm_field.presence in (0,2) and tabid != 29", array());
        ...
        for($i=0; $i<$num_rows; $i++)
        {
                 ...
                 $adb->pquery("insert into mycrm_def_org_field values (?,?,?,?)", $params);
	}
...
function updateProductQty($product_id, $upd_qty)
{
	...
	global $adb;
	$query= "update mycrm_products set qtyinstock=? where productid=?";
...
function addToProductStock($productId,$qty)
{
	...
	$sql = "UPDATE mycrm_products set qtyinstock=? where productid=?";
...
function addToProductDemand($productId,$qty)
{
	...
	$updQty=$qtyInStck + $qty;
	$sql = "UPDATE mycrm_products set qtyindemand=? where productid=?";
	$adb->pquery($sql, array($updQty, $productId));
...
function getProductQtyInStock($product_id)
{
	...
	global $adb;
        $query1 = "select qtyinstock from mycrm_products where productid=?";
...
function getProductQtyInDemand($product_id)
{
	...
	global $adb;
        $query1 = "select qtyindemand from mycrm_products where productid=?";
        $result = $adb->pquery($query1, array($product_id));
...
/**     Function to get the mycrm_table name from 'field' mycrm_table for the input mycrm_field based on the module
 *      @param  : string $module - current module value
 *      @param  : string $fieldname - mycrm_fieldname to which we want the mycrm_tablename
 *      @return : string $tablename - mycrm_tablename in which $fieldname is a column, which is retrieved from 'field' mycrm_table per $module basis
  */
function getTableNameForField($module,$fieldname)
{
	...
	$sql = "select tablename from mycrm_field where tabid in (". generateQuestionMarks($tabid) .") and mycrm_field.presence in (0,2) and columnname like ?";
...
function getPotentialsRelatedAccounts($record_id)
{
	...
	global $adb;
	$query="select related_to from mycrm_potential where potentialid=?";
...
function getEmailsRelatedAccounts($record_id)
{
	...
	global $adb;
	$query = "select mycrm_seactivityrel.crmid from mycrm_seactivityrel inner join mycrm_crmentity on mycrm_crmentity.crmid=mycrm_seactivityrel.crmid where mycrm_crmentity.setype='Accounts' and activityid=?";
	$result = $adb->pquery($query, array($record_id));
...
function getEmailsRelatedLeads($record_id)
{
	...
	global $adb;
	$query = "select mycrm_seactivityrel.crmid from mycrm_seactivityrel inner join mycrm_crmentity on mycrm_crmentity.crmid=mycrm_seactivityrel.crmid where mycrm_crmentity.setype='Leads' and activityid=?";
	$result = $adb->pquery($query, array($record_id));
...
function getHelpDeskRelatedAccounts($record_id)
{
	...
	global $adb;
        $query="select parent_id from mycrm_troubletickets inner join mycrm_crmentity on mycrm_crmentity.crmid=mycrm_troubletickets.parent_id where ticketid=? and mycrm_crmentity.setype='Accounts'";
        $result=$adb->pquery($query, array($record_id));
...
function getQuotesRelatedAccounts($record_id)
{
	...
	global $adb;
        $query="select accountid from mycrm_quotes where quoteid=?";
        $result=$adb->pquery($query, array($record_id));
...
function getQuotesRelatedPotentials($record_id)
{
	...
	global $adb;
        $query="select potentialid from mycrm_quotes where quoteid=?";
        $result=$adb->pquery($query, array($record_id));
...
function getSalesOrderRelatedAccounts($record_id)
{
	...
	global $adb;
        $query="select accountid from mycrm_salesorder where salesorderid=?";
        $result=$adb->pquery($query, array($record_id));
        $accountid=$adb->query_result($result,0,'accountid');
...
function getSalesOrderRelatedPotentials($record_id)
{
	...
	global $adb;
        $query="select potentialid from mycrm_salesorder where salesorderid=?";
        $result=$adb->pquery($query, array($record_id));
        $potid=$adb->query_result($result,0,'potentialid');
...
function getSalesOrderRelatedQuotes($record_id)
{
	...
	global $adb;
        $query="select quoteid from mycrm_salesorder where salesorderid=?";
        $result=$adb->pquery($query, array($record_id));
...
function getInvoiceRelatedAccounts($record_id)
{
	...
	global $adb;
        $query="select accountid from mycrm_invoice where invoiceid=?";
        $result=$adb->pquery($query, array($record_id));
        $accountid=$adb->query_result($result,0,'accountid');
...
function getInvoiceRelatedSalesOrder($record_id)
{
	...
	global $adb;
        $query="select salesorderid from mycrm_invoice where invoiceid=?";
        $result=$adb->pquery($query, array($record_id));
...
function getAccessPickListValues($module)
{
	...
	$id = getTabid($module);
	$query = "select fieldname,columnname,fieldid,fieldlabel,tabid,uitype from mycrm_field where tabid = ? and uitype in ('15','33','55') and mycrm_field.presence in (0,2)";
	...
	for($i=0;$i < $adb->num_rows($result);$i++)
{
		...
		if (count($roleids) > 1)
	{
			$mulsel="select distinct $fieldname from mycrm_$fieldname inner join mycrm_role2picklist on mycrm_role2picklist.picklistvalueid = mycrm_$fieldname.picklist_valueid where roleid in (\"". implode($roleids,"\",\"") ."\") and picklistid in (select picklistid from mycrm_$fieldname) order by sortid asc";
	}
	else
	{
			$mulsel="select distinct $fieldname from mycrm_$fieldname inner join mycrm_role2picklist on mycrm_role2picklist.picklistvalueid = mycrm_$fieldname.picklist_valueid where roleid ='".$roleid."' and picklistid in (select picklistid from mycrm_$fieldname) order by sortid asc";
	}
...
function transferUserCurrency($old_cur, $new_cur) {
	...
	$sql = "update mycrm_users set currency_id=? where currency_id=?";
...
function transferProductCurrency($old_cur, $new_cur) {
	...
	$prod_res = $adb->pquery("select productid from mycrm_products where currency_id = ?", array($old_cur));
	...
	if(count($prod_ids) > 0) {
		...
		for($i=0;$i<count($prod_ids);$i++) {
			...
			$query = "update mycrm_products set currency_id=?, unit_price=? where productid=?";
...
function transferPriceBookCurrency($old_cur, $new_cur) {
	...
	$pb_res = $adb->pquery("select pricebookid from mycrm_pricebook where currency_id = ?", array($old_cur));
...
/**
 * this function searches for a given number in mycrm and returns the callerInfo in an array format
...
function get_use_asterisk($id){
	...
	$sql = "select * from mycrm_asteriskextensions where userid = ?";
...
function addToCallHistory($userExtension, $callfrom, $callto, $status, $adb, $useCallerInfo){
	$sql = "select * from mycrm_asteriskextensions where asterisk_extension=?";
	$result = $adb->pquery($sql,array($userExtension));
	$userID = $adb->query_result($result, 0, "userid");
	if(empty($userID)) {
		// we have observed call to extension not configured in Mycrm will returns NULL
		return;
	}
	if(empty($callfrom)){
		$callfrom = "Unknown";
	...
	$crmID = $adb->getUniqueID('mycrm_crmentity');
	$timeOfCall = date('Y-m-d H:i:s');

	$query = "INSERT INTO mycrm_crmentity (crmid,smcreatorid,smownerid,modifiedby,setype,description,createdtime,
	...
	$sql = "insert into mycrm_pbxmanager (pbxmanagerid,callfrom,callto,timeofcall,status)values (?,?,?,?,?)";
...
function getRelationTables($module,$secmodule){
	...
	$ui10_query = $adb->pquery("SELECT mycrm_field.tabid AS tabid,mycrm_field.tablename AS tablename, mycrm_field.columnname AS columnname FROM mycrm_field INNER JOIN mycrm_fieldmodulerel ON mycrm_fieldmodulerel.fieldid = mycrm_field.fieldid WHERE (mycrm_fieldmodulerel.module=? AND mycrm_fieldmodulerel.relmodule=?) OR (mycrm_fieldmodulerel.module=? AND mycrm_fieldmodulerel.relmodule=?)",array($module,$secmodule,$secmodule,$module));
	...
	if(is_array($reltables) && !empty($reltables)){
		...
		$rel_array = array("mycrm_crmentityrel"=>array("crmid","relcrmid"),"".$primary_obj->table_name."" => "".$primary_obj->table_index."");
	}
	return $rel_array;
}
...
function installVtlibModule($packagename, $packagepath, $customized=false) {
	global $log, $Mycrm_Utils_Log, $_installOrUpdateVtlibModule;
	...
	require_once('vtlib/Mycrm/Package.php');
	require_once('vtlib/Mycrm/Module.php');
	$Mycrm_Utils_Log = defined('INSTALLATION_MODE_DEBUG')? INSTALLATION_MODE_DEBUG : true;
	...
	if($package->isLanguageType()) {
		require_once('vtlib/Mycrm/Language.php');
		$languagePack = new Mycrm_Language();
	...
	} else if(Mycrm_Module::getInstance($module)) {
...
function com_vtGetModules($adb) {
	$sql="select distinct mycrm_field.tabid, name
		from mycrm_field
		inner join mycrm_tab
			on mycrm_field.tabid=mycrm_tab.tabid
		where mycrm_field.tabid not in(9,10,16,15,29) and mycrm_tab.presence = 0 and mycrm_tab.isentitytype=1";
...
function isRecordExists($recordId) {
	global $adb;
	$query = "SELECT crmid FROM mycrm_crmentity where crmid=? AND deleted=0";
...
function getTabInfo($tabId) {
	...
	$tabInfoResult = $adb->pquery('SELECT prefname, prefvalue FROM mycrm_tab_info WHERE tabid=?', array($tabId));
	$tabInfo = array();
...
function getBlockName($blockid) {
	...
	if(!empty($blockid) && $blockname === false){
		$block_res = $adb->pquery('SELECT blocklabel FROM mycrm_blocks WHERE blockid = ?',array($blockid));
...
function getActivityRelatedContacts($activityId) {
	...
	$query = 'SELECT * FROM mycrm_cntactivityrel WHERE activityid=?';
...
function isLeadConverted($leadId) {
	...
	$query = 'SELECT converted FROM mycrm_leaddetails WHERE converted = 1 AND leadid=?';
	$params = array($leadId);
...
function getSelectAllQuery($input,$module) {
	...
	if($module == "Calendar") {
		...
		if($module == 'Documents') {
			...
			$query .= " AND mycrm_notes.folderid in (".$folderid.")";
		}
	}
...
function getCampaignAccountIds($id) {
	global $adb;
	$sql="SELECT mycrm_account.accountid as id FROM mycrm_account
		INNER JOIN mycrm_campaignaccountrel ON mycrm_campaignaccountrel.accountid = mycrm_account.accountid
		LEFT JOIN mycrm_crmentity ON mycrm_crmentity.crmid = mycrm_account.accountid
		WHERE mycrm_campaignaccountrel.campaignid = ? AND mycrm_crmentity.deleted=0";
	$result = $adb->pquery($sql, array($id));
	return $result;
}
...
function getCampaignContactIds($id) {
	global $adb;
	$sql="SELECT mycrm_contactdetails.contactid as id FROM mycrm_contactdetails
		INNER JOIN mycrm_campaigncontrel ON mycrm_campaigncontrel.contactid = mycrm_contactdetails.contactid
		LEFT JOIN mycrm_crmentity ON mycrm_crmentity.crmid = mycrm_contactdetails.contactid
		WHERE mycrm_campaigncontrel.campaignid = ? AND mycrm_crmentity.deleted=0";
	$result = $adb->pquery($sql, array($id));
	return $result;
}
...
function getCampaignLeadIds($id) {
	global $adb;
	$sql="SELECT mycrm_leaddetails.leadid as id FROM mycrm_leaddetails
		INNER JOIN mycrm_campaignleadrel ON mycrm_campaignleadrel.leadid = mycrm_leaddetails.leadid
		LEFT JOIN mycrm_crmentity ON mycrm_crmentity.crmid = mycrm_leaddetails.leadid
		WHERE mycrm_campaignleadrel.campaignid = ? AND mycrm_crmentity.deleted=0";
	$result = $adb->pquery($sql, array($id));
	return $result;
}
