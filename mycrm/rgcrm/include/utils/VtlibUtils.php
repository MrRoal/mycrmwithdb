...
if(!isset($__cache_mycrm_imagepath)) {
    $__cache_mycrm_imagepath = Array();
}
function mycrm_imageurl($imagename, $themename) {
	global $__cache_mycrm_imagepath;
	if($__cache_mycrm_imagepath[$imagename]) {
        $imagepath = $__cache_mycrm_imagepath[$imagename];
    } else {
		$imagepath = false;
        ...
        $__cache_mycrm_imagepath[$imagename] = $imagepath;
    }
	return $imagepath;
}
...
function vtlib_getModuleNameById($tabid) {
	global $adb;
	$sqlresult = $adb->pquery("SELECT name FROM mycrm_tab WHERE tabid = ?",array($tabid));
...
function vtlib_prefetchModuleActiveInfo($force = true) {
	...
	if($tabrows === false || $force) {
		global $adb;
		$tabres = $adb->query("SELECT * FROM mycrm_tab");
		$tabrows = array();
		if($tabres) {
...
function vtlib_isModuleActive($module) {
	...
	$active = false;
	//Fix for http://trac.mycrm.com/cgi-bin/trac.cgi/ticket/7991
...
function vtlib_RecreateUserPrivilegeFiles() {
	global $adb;
	$userres = $adb->query('SELECT id FROM mycrm_users WHERE deleted = 0');
...
function vtlib_moduleAlwaysActive() {
	$modules = Array (
		...
		'Utilities', 'uploads', 'Import', 'System', 'com_mycrm_workflow', 'PickList'
	);
	return $modules;
}
...
function vtlib_toggleModuleAccess($module, $enable_disable) {
	...
	if($enable_disable === true) {
		$enable_disable = 0;
		$event_type = Mycrm_Module::EVENT_MODULE_ENABLED;
	} else if($enable_disable === false) {
		$enable_disable = 1;
		$event_type = Mycrm_Module::EVENT_MODULE_DISABLED;
	}

	$adb->pquery("UPDATE mycrm_tab set presence = ? WHERE name = ?", array($enable_disable,$module));
	...
	if(version_compare($mycrm_current_version, '5.0.4', '>')) {
		vtlib_RecreateUserPrivilegeFiles();
	}

	Mycrm_Module::fireEvent($module, $event_type);
}
...
function vtlib_getToggleModuleInfo() {
	...
	$sqlresult = $adb->query("SELECT name, presence, customized, isentitytype FROM mycrm_tab WHERE name NOT IN ('Users','Home') AND presence IN (0,1) ORDER BY name");
...
function vtlib_getToggleLanguageInfo() {
	...
	$langinfo = Array();
	$sqlresult = $adb->query("SELECT * FROM mycrm_language");
	if($sqlresult) {
...
function vtlib_toggleLanguageAccess($langprefix, $enable_disable) {
	...
	$adb->pquery('UPDATE mycrm_language set active = ? WHERE prefix = ?', Array($enable_disable, $langprefix));
...
function vtlib_getFieldHelpInfo($module) {
	global $adb;
	$fieldhelpinfo = Array();
	if(in_array('helpinfo', $adb->getColumnNames('mycrm_field'))) {
		$result = $adb->pquery('SELECT fieldname,helpinfo FROM mycrm_field WHERE tabid=?', Array(getTabid($module)));
		...
		Array(
			...
			Array(
				...
				'related_tables' => Array(
					'mycrm_accountbillads' => Array ('accountaddressid', 'mycrm_account', 'accountid'),
					'mycrm_accountshipads' => Array ('accountaddressid', 'mycrm_account', 'accountid'),
					'mycrm_accountscf' => Array ('accountid', 'mycrm_account', 'accountid'),
				),
			...
			Array(
				'IsCustomModule'=>false,
				'table_name'  => 'mycrm_contactdetails',
				'table_index' => 'contactid',
				'related_tables'=> Array( 
					'mycrm_account' => Array ('accountid' ),
					//REVIEW: Added these tables for displaying the data into relatedlist (based on configurable fields)
					'mycrm_contactaddress' => Array('contactaddressid', 'mycrm_contactdetails', 'contactid'),
					'mycrm_contactsubdetails' => Array('contactsubscriptionid', 'mycrm_contactdetails', 'contactid'),
					'mycrm_customerdetails' => Array('customerid', 'mycrm_contactdetails', 'contactid'),
					'mycrm_contactscf' => Array('contactid', 'mycrm_contactdetails', 'contactid')
					),
			...
			Array(
				'IsCustomModule'=>false,
				'table_name'  => 'mycrm_leaddetails',
				'table_index' => 'leadid',
				'related_tables' => Array (
					'mycrm_leadsubdetails' => Array ( 'leadsubscriptionid', 'mycrm_leaddetails', 'leadid' ),
					'mycrm_leadaddress'    => Array ( 'leadaddressid', 'mycrm_leaddetails', 'leadid' ),
					'mycrm_leadscf'    => Array ( 'leadid', 'mycrm_leaddetails', 'leadid' ),
				),
				'popup_fields'=> Array ('lastname'),
			),
			'Campaigns' =>
			...
			Array(
				'IsCustomModule'=>false,
				'table_name' => 'mycrm_potential',
				'table_index'=> 'potentialid',
				// NOTE: UIType 10 is being used instead of direct relationship from 5.1.0
				//'related_tables' => Array ('mycrm_account' => Array('accountid')),
				'popup_fields'=> Array('potentialname'),
				'related_tables' => Array (
					'mycrm_potentialscf'    => Array ( 'potentialid', 'mycrm_potential', 'potentialid' ),
				),
			),
			'Quotes' =>
			Array(
				'IsCustomModule'=>false,
				'table_name' => 'mycrm_quotes',
				'table_index'=> 'quoteid',
				'related_tables' => Array ('mycrm_account' => Array('accountid')),
			...
			Array(
				'IsCustomModule'=>false,
				'table_name' => 'mycrm_purchaseorder',
			...
			Array(
				'IsCustomModule'=>false,
				'table_name' => 'mycrm_invoice',
				'table_index'=> 'invoiceid',
				'popup_fields'=> Array('subject'),
                'related_tables'=> Array( 
					'mycrm_invoicecf' => Array('invoiceid', 'mycrm_invoice', 'invoiceid')
					),
			),
			'HelpDesk'=>
			Array(
				'IsCustomModule'=>false,
				'table_name' => 'mycrm_troubletickets',
				'table_index'=> 'ticketid',
				'related_tables'=> Array ('mycrm_ticketcf' => Array('ticketid')),
				'popup_fields'=> Array('ticket_title')
			),
			'Faq'=>
			Array(
				'IsCustomModule'=>false,
				'table_name' => 'mycrm_faq',
				'table_index'=> 'id',
			),
			'Documents'=>
			Array(
				'IsCustomModule'=>false,
				'table_name' => 'mycrm_notes',
				'table_index'=> 'notesid',
			),
			'Products'=>
			Array(
				'IsCustomModule'=>false,
				'table_name' => 'mycrm_products',
			...
			Array(
				'IsCustomModule'=>false,
				'table_name' => 'mycrm_pricebook',
				'table_index'=> 'pricebookid',
			),
			'Vendors'=>
			Array(
				'IsCustomModule'=>false,
				'table_name' => 'mycrm_vendor',
			...
			Array(
				'IsCustomModule'=>false,
				'table_name' => 'mycrm_project',
				'table_index'=> 'projectid',
				'related_tables'=> Array( 
					'mycrm_projectcf' => Array('projectid', 'mycrm_project', 'projectid')
					),
			),
            'ProjectMilestone' =>
            Array(
                'IsCustomModule'=>false,
				'table_name' => 'mycrm_projectmilestone',
				'table_index'=> 'projectmilestoneid',
				'related_tables'=> Array( 
					'mycrm_projectmilestonecf' => Array('projectmilestoneid', 'mycrm_projectmilestone', 'projectmilestoneid')
					),
            ),
            'ProjectTask' => 
            Array(
                'IsCustomModule'=>false,
				'table_name' => 'mycrm_projecttask',
				'table_index'=> 'projecttaskid',
				'related_tables'=> Array( 
					'mycrm_projecttaskcf' => Array('projecttaskid', 'mycrm_projecttask', 'projecttaskid')
					),
            ),
			'Services' => 
			Array(
				'IsCustomModule'=>false,
				'table_name' => 'mycrm_service',
				'table_index'=> 'serviceid',
				'related_tables'=> Array( 
					'mycrm_servicecf' => Array('serviceid')
					),
			),
			'ServiceContracts' => 
			Array(
				'IsCustomModule'=>false,
				'table_name' => 'mycrm_servicecontracts',
				'table_index'=> 'servicecontractsid',
				'related_tables'=> Array( 
					'mycrm_servicecontractscf' => Array('servicecontractsid')
					),
			),
            'Assets' => 
			Array(
				'IsCustomModule'=>false,
				'table_name' => 'mycrm_assets',
				'table_index'=> 'assetsid',
				'related_tables'=> Array( 
					'mycrm_assetscf' => Array('assetsid')
					),
			)
		);
...
function vtlib_getPicklistValues_AccessibleToAll($field_columnname) {
	...
	$tablename = "mycrm_$columnname";

	// Gather all the roles (except H1 which is organization role)
	$roleres = $adb->query("SELECT roleid FROM mycrm_role WHERE roleid != 'H1'");
	...
	$picklistres = $adb->query(
		...
		INNER JOIN mycrm_role2picklist ON $tablename.picklist_valueid=mycrm_role2picklist.picklistvalueid
		WHERE roleid != 'H1'");
...
function vtlib_isDirWriteable($dirpath) {
	if(is_dir($dirpath)) {
		do {
			$tmpfile = 'mycrm' . time() . '-' . rand(1,1000) . '.tmp';
 ...
 * @param Mycrm_Link $widgetLinkInfo
...
function vtlib_module_icon($modulename){
	...
	return "modules/Mycrm/Mycrm.png";
}

function vtlib_mime_content_type($filename) {
	return Mycrm_Functions::mime_content_type($filename);
}
