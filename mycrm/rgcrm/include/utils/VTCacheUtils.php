...
class VTCacheUtils {
	...
	static function updateFieldInfo($tabid, $fieldname, $fieldid, $fieldlabel,
        ...
        Mycrm_Cache::set('fieldInfo', $tabid, self::$_fieldinfo_cache[$tabid]);
	}
	static function lookupFieldInfo($tabid, $fieldname) {
        $fieldInfo = Mycrm_Cache::get('fieldInfo', $tabid);
        ...
        $field = Mycrm_Cache::get('field-'.$tabid,$fieldname);
        if($field){
            $cacheField = array(
                'tabid' => $tabid,
                'fieldid' => $field->getId(),
	...
	static function lookupFieldInfo_Module($module, $presencein = array('0', '2')) {
        ...
        $fieldInfo = Mycrm_Cache::get('ModuleFields',$tabid);
        if($fieldInfo){
	...
	static function updateEntityNameInfo($module, $data) {
        ...
        Mycrm_Cache::set('EntityInfo', $module, self::$_module_entityname_cache[$module]);
	}
	static function lookupEntityNameInfo($module) {
        $entityNames = Mycrm_Cache::get('EntityInfo', $module);
        if($entityNames){
            return $entityNames;
