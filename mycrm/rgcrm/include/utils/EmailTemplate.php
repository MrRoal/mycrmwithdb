...
class EmailTemplate {
	...
	public function process($params) {
		...
		if (count($variableList) > 0) {
			...
			if (count($tableList) > 0 && count($columnList) > 0) {
				...
				if($module == 'Leads' && !in_array('mycrm_leaddetails', $tableList)){
					$sql .=' INNER JOIN mycrm_leaddetails ON mycrm_leaddetails.leadid = mycrm_crmentity.crmid';
				}
				...
				foreach ($it as $row) {
					foreach ($fieldList as $field) {
						     $moduleModel = Mycrm_Module_Model::getInstance($module); 
 	                         $fieldModel = Mycrm_Field_Model::getInstance($field, $moduleModel); 
	...
	public function isActive($field, $mod) {
		global $adb;
		$tabid = getTabid($mod);
		$query = 'select * from mycrm_field where fieldname = ?  and tabid = ? and presence in (0,2)';
