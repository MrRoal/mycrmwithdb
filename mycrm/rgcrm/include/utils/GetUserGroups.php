...
class GetUserGroups { 
	...
	/** to get all the parent mycrm_groups of the specified group
	...
	function getAllUserGroups($userid)
	{
		...
		$query="select * from mycrm_users2group where userid=?";
		...
		//Retreiving from the mycrm_user2role
		$query="select * from mycrm_group2role where roleid=?";
		...
		$query="select * from mycrm_group2rs where roleandsubid in (". generateQuestionMarks($parentRolelist) .")";
