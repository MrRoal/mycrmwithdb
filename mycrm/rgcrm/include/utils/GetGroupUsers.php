...
/** Class to retreive all the mycrm_users present in a group 
 *
 */
...
class GetGroupUsers { 
	...
	/** to get all the mycrm_users and mycrm_groups of the specified group
	 * @params $groupId --> Group Id :: Type Integer
         * @returns the mycrm_users present in the group in the variable $parent_groups of the class
         * @returns the sub mycrm_groups present in the group in the variable $group_subgroups of the class
         */
	function getAllUsersInGroup($groupid)
	{
		...
		$query="select * from mycrm_users2group where groupid=?";
		...
		//Retreiving from the mycrm_group2role
		$query="select * from mycrm_group2role where groupid=?";
                $result = $adb->pquery($query, array($groupid));
                $num_rows=$adb->num_rows($result);
                for($i=0;$i<$num_rows;$i++)
                {
		...
		//Retreiving from the mycrm_group2rs
		$query="select * from mycrm_group2rs where groupid=?";
                $result = $adb->pquery($query, array($groupid));
                $num_rows=$adb->num_rows($result);
                for($i=0;$i<$num_rows;$i++)
                {
		...
		$query="select * from mycrm_group2grouprel where groupid=?";
                $result = $adb->pquery($query, array($groupid));
                $num_rows=$adb->num_rows($result);
                for($i=0;$i<$num_rows;$i++)
                {
