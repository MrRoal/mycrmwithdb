...
function is_admin($user) {
	return Mycrm_Functions::userIsAdministrator($user);
}

function parse_calendardate($local_format) {
	return Mycrm_Functions::currentUserJSDateFormat($local_format);
}

function from_html($string, $encode = true) {
	return Mycrm_Functions::fromHTML($string, $encode);
}

function fck_from_html($string) {
	return Mycrm_Functions::fromHTML_FCK($string);
}

function popup_from_html($string, $encode = true) {
	return Mycrm_Functions::fromHTML_Popup($string, $encode);
}

function fetchCurrency($id) {
	return Mycrm_Functions::userCurrencyId($id);
}

function getCurrencyName($currencyid, $show_symbol = true) {
	return Mycrm_Functions::getCurrencyName($currencyid, $show_symbol);
}

function getTabid($module) {
	return Mycrm_Functions::getModuleId($module);
	}

function getFieldid($tabid, $fieldname, $onlyactive = true) {
	return Mycrm_Functions::getModuleFieldId($tabid, $fieldname, $onlyactive);
}

function getTabOwnedBy($module) {
	return Mycrm_Functions::getModuleOwner($module);
}

function getSalesEntityType($crmid) {
	return Mycrm_Functions::getCRMRecordType($crmid);
}

function getAccountName($account_id) {
	return Mycrm_Functions::getCRMRecordLabel($account_id);
}

function getProductName($product_id) {
	return Mycrm_Functions::getCRMRecordLabel($product_id);
}

function getPotentialName($potential_id) {
	return Mycrm_Functions::getCRMRecordLabel($potential_id);
}

function getContactName($contact_id) {
	return Mycrm_Functions::getCRMRecordLabel($contact_id);
}

function getFullNameFromQResult($result, $row_count, $module) {
	return Mycrm_Deprecated::getFullNameFromQResult($result, $row_count, $module);
}

function getFullNameFromArray($module, $fieldValues) {
	return Mycrm_Deprecated::getFullNameFromArray($module, $fieldValues);
}

function getCampaignName($campaign_id) {
	return Mycrm_Functions::getCRMRecordLabel($campaign_id);
}

function getVendorName($vendor_id) {
	return Mycrm_Functions::getCRMRecordLabel($vendor_id);
}

function getQuoteName($quote_id) {
	return Mycrm_Functions::getCRMRecordLabel($quote_id);
}

function getPriceBookName($pricebookid) {
	return Mycrm_Functions::getCRMRecordLabel($pricebookid);
}

function getSoName($so_id) {
	return Mycrm_Functions::getCRMRecordLabel($so_id);
}

function getGroupName($groupid) {
	return Mycrm_Functions::getGroupName($groupid);
}

function getUserName($userid) {
	return Mycrm_Functions::getUserName($userid);
}

function getUserFullName($userid) {
	return Mycrm_Functions::getUserRecordLabel($userid);
}

function getParentName($parent_id) {
	return Mycrm_Functions::getCRMRecordLabel($parent_id);
	}

function getValidDisplayDate($cur_date_val) {
	return Mycrm_Functions::currentUserDisplayDate($cur_date_val);
}

function getNewDisplayDate() {
	return Mycrm_Functions::currentUserDisplayDateNew();
}

/** This function returns the conversion rate and mycrm_currency symbol
 * in array format for a given id.
 * param $id - mycrm_currency id.
 */
function getCurrencySymbolandCRate($id) {
	return Mycrm_Functions::getCurrencySymbolandRate($id);
}
...
function getTermsandConditions() {
	return Mycrm_Functions::getInventoryTermsAndCondition();
}
 ...
 */
function br2nl($str) {
	return Mycrm_Functions::br2nl($str);
}
...
function getBlockId($tabid, $label) {
	return Mycrm_Deprecated::getBlockId($tabid, $label);
}
...
function getParentTabFromModule($module) {
	return Mycrm_Deprecated::getParentTabFromModule($module);
}

/**
 * This function is used to get the Parent Tab name for a given module.
 * Takes no parameter but gets the mycrm_parenttab value from form request
 * This returns value string type
 */
function getParentTab() {
	return Mycrm_Deprecated::getParentTab();
}
...
function setObjectValuesFromRequest($focus) {
	return Mycrm_Deprecated::copyValuesFromRequest($focus);
}

function create_tab_data_file() {
	return Mycrm_Deprecated::createModuleMetaFile();
}

/**
 * Function to write the mycrm_parenttabid and name to a flat file parent_tabdata.txt so that the data
 * is obtained from the file instead of repeated queries
 * returns null
 */
function create_parenttab_data_file() {
	return Mycrm_Deprecated::createModuleGroupMetaFile();
}

function getEntityName($module, $ids_list, $compute=true) {
	if ($compute) {
		return Mycrm_Functions::computeCRMRecordLabels($module, $ids_list);
	} else {
		return Mycrm_Functions::getCRMRecordLabels($module, $ids_list);
	}
}
...
function decideFilePath() {
	return Mycrm_Functions::initStorageFileDirectory();
}

/**
 * 	This function is used to check whether the attached file is a image file or not
 * 	@param string $file_details  - mycrm_files array which contains all the uploaded file details
...
function validateImageFile($file_details) {
	return Mycrm_Functions::validateImage($file_details);
}
...
function getTemplateDetails($templateid) {
	return Mycrm_Deprecated::getTemplateDetails($templateid);
}
...
function getMergedDescription($description, $id, $parent_type) {
	return Mycrm_Functions::getMergedDescription($description, $id, $parent_type);
}
...
function getSingleFieldValue($tablename, $fieldname, $idname, $id) {
	return Mycrm_Functions::getSingleFieldValue($tablename, $fieldname, $idname, $id);
}
...
function get_announcements() {
	return Mycrm_Deprecated::getAnnouncements();
}
...
function getrecurringObjValue() {
	return Mycrm_Functions::getRecurringObjValue();
}

function getTranslatedString($str, $module = 'Mycrm') {
	return Mycrm_Functions::getTranslatedString($str, $module);
}
...
function getTranslatedCurrencyString($str) {
	return Mycrm_Deprecated::getTranslatedCurrencyString($str);
	}

function getTicketComments($ticketid) {
	return Mycrm_Functions::getTicketComments($ticketid);
}

function makeRandomPassword() {
	return Mycrm_Functions::generateRandomPassword();
}
...
function getCvIdOfAll($module) {
	return Mycrm_Deprecated::getIdOfCustomViewByNameAll($module);
}
...
function getTagCloudView($id = "") {
	return Mycrm_Functions::getTagCloudView($id);
}
...
function SaveTagCloudView($id = "") {
	return Mycrm_Deprecated::SaveTagCloudView($id);
}
 ...
 * *	        -------------------+------------+----------------------+------------------+
 * *		Potential Name     | I~O        | mycrm_quotes        | potentialid      |
 * *
 * *     Then put an entry in $new_field_details  like this:
 * *
 * *				"mycrm_quotes:potentialid"=>"V",
 * *
...
function ChangeTypeOfData_Filter($table_name, $column_name, $type_of_data) {
	return Mycrm_Functions::transformFieldTypeOfData($table_name, $column_name, $type_of_data);
}
...
function clear_smarty_cache($path = null) {
	Mycrm_Deprecated::clearSmartyCompiledFiles($path);
}
...
function get_smarty_compiled_file($template_file, $path = null) {
	return Mycrm_Deprecated::getSmartyCompiledTemplateFile($template_file, $path);
}

/** Function to carry out all the necessary actions after migration */
function perform_post_migration_activities() {
	Mycrm_Deprecated::postApplicationMigrationTasks();
}
...
function getPickListValues($tablename, $roleid) {
	return Mycrm_Functions::getPickListValuesFromTableForRole($tablename, $roleid);
}
...
function checkFileAccessForInclusion($filepath) {
	Mycrm_Deprecated::checkFileAccessForInclusion($filepath);
}

/** Function to check the file deletion within the deletable (safe) directories*/
function checkFileAccessForDeletion($filepath) {
	Mycrm_Deprecated::checkFileAccessForDeletion($filepath);
}

/** Function to check the file access is made within web root directory. */
function checkFileAccess($filepath) {
	Mycrm_Deprecated::checkFileAccess($filepath);
}

/**
 * function to return whether the file access is made within mycrm root directory
 * and it exists.
 * @global String $root_directory mycrm root directory as given in config.inc.php file.
 * @param String $filepath relative path to the file which need to be verified
 * @return Boolean true if file is a valid file within mycrm root directory, false otherwise.
 */
function isFileAccessible($filepath) {
	return Mycrm_Deprecated::isFileAccessible($filepath);
}
...
function getActivityType($id) {
	return Mycrm_Functions::getActivityType($id);
}

/** Function to get owner name either user or group */
function getOwnerName($id) {
	return Mycrm_Functions::getOwnerRecordLabel($id);
}

/** Function to get owner name either user or group */
function getOwnerNameList($idList) {
	return Mycrm_Functions::getOwnerRecordLabels($idList);
}
...
function getSettingsBlockId($label) {
	return Mycrm_Deprecated::getSettingsBlockId($label);
}
...
function getEntityField($module) {
	return Mycrm_Functions::getEntityModuleSQLColumnString($module);
}

/**
 * this function returns the entity information for a given module; for e.g. for Contacts module
 * it returns the information of tablename, modulename, fieldsname and id gets from mycrm_entityname
 * @param string $module - the module name
 * @return array $data - the entity information for the module
 */
function getEntityFieldNames($module) {
	return Mycrm_Functions::getEntityModuleInfoFieldsFormatted($module);
}
...
function getEntityFieldNameDisplay($module, $fieldsName, $fieldValues) {
	return Mycrm_Deprecated::getCurrentUserEntityFieldNameDisplay($module, $fieldsName, $fieldValues);
}

// mycrm cache utility
require_once('include/utils/VTCacheUtils.php');

// vtlib customization: Extended mycrm CRM utlitiy functions
require_once('include/utils/VtlibUtils.php');

// END

function vt_suppressHTMLTags($string) {
	return Mycrm_Functions::suppressHTMLTags($string);
}

function getSqlForNameInDisplayFormat($input, $module, $glue = ' ') {
	return Mycrm_Deprecated::getSqlForNameInDisplayFormat($input, $module, $glue);
}

function getModuleSequenceNumber($module, $recordId) {
	return Mycrm_Deprecated::getModuleSequenceNumber($module, $recordId);
	}

function getInvoiceStatus($invoiceId) {
	return Mycrm_Functions::getInvoiceStatus($invoiceId);
}

function decimalFormat($value){
	return Mycrm_Functions::formatDecimal($value);
}

function updateRecordLabel($module,$recordId){
	return Mycrm_Functions::updateCRMRecordLabel($module, $recordId);
}


function get_group_options() {
return Mycrm_Functions::get_group_options();
}
?>
