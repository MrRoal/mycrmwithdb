 ...
 * Param $ext_prod_arr - existing mycrm_products
...
function sendPrdStckMail($product_id,$upd_qty,$prod_name,$qtyinstk,$qty,$module)
{
	...
	{	
		...
		$query = "select * from mycrm_inventorynotification where notificationname=?";
...
function getPrdReOrderLevel($product_id)
{
	...
	global $adb;
	$query1 = "SELECT reorderlevel FROM mycrm_products WHERE productid = ?";
	$result=$adb->pquery($query1, array($product_id));
 ...
 *	return int   $taxid - taxid corresponding to the Tax type from mycrm_inventorytaxinfo mycrm_table
 */
function getTaxId($type)
{
	...
	$res = $adb->pquery("SELECT taxid FROM mycrm_inventorytaxinfo WHERE taxname=?", array($type));
 ...
 *	@param string $type       - tax type (VAT or Sales or Service)
 *	return int $taxpercentage - taxpercentage corresponding to the Tax type from mycrm_inventorytaxinfo mycrm_table
 */
function getTaxPercentage($type)
{
	...
	$res = $adb->pquery("SELECT percentage FROM mycrm_inventorytaxinfo WHERE taxname = ?", array($type));
...
function getProductTaxPercentage($type,$productid,$default='')
{
	...
	$res = $adb->pquery("SELECT taxpercentage
			FROM mycrm_inventorytaxinfo
			INNER JOIN mycrm_producttaxrel
				ON mycrm_inventorytaxinfo.taxid = mycrm_producttaxrel.taxid
			WHERE mycrm_producttaxrel.productid = ?
			AND mycrm_inventorytaxinfo.taxname = ?", array($productid, $type));
...
function addInventoryHistory($module, $id, $relatedname, $total, $history_fldval)
{
	...
	$history_table_array = Array(
					"PurchaseOrder"=>"mycrm_postatushistory",
					"SalesOrder"=>"mycrm_sostatushistory",
					"Quotes"=>"mycrm_quotestagehistory",
					"Invoice"=>"mycrm_invoicestatushistory"
				    );
...
function getAllTaxes($available='all', $sh='',$mode='',$id='')
{
	...
	$taxtypes = Array();
	if($sh != '' && $sh == 'sh') {
		$tablename = 'mycrm_shippingtaxinfo';
		$value_table='mycrm_inventoryshippingrel';
		if($mode == 'edit' && id != '') {
		...
		$tablename = 'mycrm_inventorytaxinfo';
		$value_table='mycrm_inventoryproductrel';
		if($mode == 'edit' && $id != '' ) {
...
function getTaxDetailsForProduct($productid, $available='all')
{
	...
	if($productid != '')
	{
		...
		if($available != 'all' && $available == 'available')
		{
			$where = ' and mycrm_inventorytaxinfo.deleted=0';
		}
		if($available != 'all' && $available == 'available_associated')
		{
			$query = "SELECT mycrm_producttaxrel.*, mycrm_inventorytaxinfo.* FROM mycrm_inventorytaxinfo left JOIN mycrm_producttaxrel ON mycrm_inventorytaxinfo.taxid = mycrm_producttaxrel.taxid WHERE mycrm_producttaxrel.productid = ? or mycrm_inventorytaxinfo.deleted=0 GROUP BY mycrm_inventorytaxinfo.taxid";
		}
		else
		{
			$query = "SELECT mycrm_producttaxrel.*, mycrm_inventorytaxinfo.* FROM mycrm_inventorytaxinfo INNER JOIN mycrm_producttaxrel ON mycrm_inventorytaxinfo.taxid = mycrm_producttaxrel.taxid WHERE mycrm_producttaxrel.productid = ? $where";
		}
...
function deleteInventoryProductDetails($focus)
{
	...
	$product_info = $adb->pquery("SELECT productid, quantity, sequence_no, incrementondel from mycrm_inventoryproductrel WHERE id=?",array($focus->id));
	...
	for($index = 0;$index <$numrows;$index++){
		...
		if($incrementondel){
			...
			$sub_prod_query = $adb->pquery("SELECT productid from mycrm_inventorysubproductrel WHERE id=? AND sequence_no=?",array($focus->id,$sequence_no));
    ...
    $adb->pquery("delete from mycrm_inventoryproductrel where id=?", array($focus->id));
    $adb->pquery("delete from mycrm_inventorysubproductrel where id=?", array($focus->id));
    $adb->pquery("delete from mycrm_inventoryshippingrel where id=?", array($focus->id));
...
function updateInventoryProductRel($entity) {
	...
	if($statusChanged) {
		if($recordDetails[$statusFieldName] == $statusFieldValue) {
			$adb->pquery("UPDATE mycrm_inventoryproductrel SET incrementondel=0 WHERE id=?",array($entity_id));
	...
	if($updateInventoryProductRel_deduct_stock) {
		$adb->pquery("UPDATE mycrm_inventoryproductrel SET incrementondel=1 WHERE id=?",array($entity_id));

		$product_info = $adb->pquery("SELECT productid,sequence_no, quantity from mycrm_inventoryproductrel WHERE id=?",array($entity_id));
		$numrows = $adb->num_rows($product_info);
		for($index = 0;$index <$numrows;$index++) {
			$productid = $adb->query_result($product_info,$index,'productid');
			$qty = $adb->query_result($product_info,$index,'quantity');
			...
			$sub_prod_query = $adb->pquery("SELECT productid from mycrm_inventorysubproductrel WHERE id=? AND sequence_no=?",array($entity_id,$sequence_no));
			if($adb->num_rows($sub_prod_query)>0) {
				for($j=0;$j<$adb->num_rows($sub_prod_query);$j++) {
					$sub_prod_id = $adb->query_result($sub_prod_query,$j,"productid");
...
function saveInventoryProductDetails(&$focus, $module, $update_prod_stock='false', $updateDemand='')
{
	...
	for($i=1; $i<=$tot_no_prod; $i++)
	{
		...
		/*else{
			$desc_duery = "select mycrm_crmentity.description AS product_description from mycrm_crmentity where mycrm_crmentity.crmid=?";
		...
		$query ="insert into mycrm_inventoryproductrel(id, productid, sequence_no, quantity, listprice, comment, description) values(?,?,?,?,?,?,?)";
		...
		if (!empty($sub_prod_str)) {
			...
			for($j=0;$j<count($sub_prod);$j++){
				$query ="insert into mycrm_inventorysubproductrel(id, sequence_no, productid) values(?,?,?)";
		...
		$updatequery = "update mycrm_inventoryproductrel set ";
		$updateparams = array();
	...
	//to save the S&H tax details in mycrm_inventoryshippingrel table
	...
	$sh_query = "insert into mycrm_inventoryshippingrel($sh_query_fields) values($sh_query_values)";
...
function getInventoryTaxType($module, $id)
{
	...
	$inv_table_array = Array('PurchaseOrder'=>'mycrm_purchaseorder','SalesOrder'=>'mycrm_salesorder','Quotes'=>'mycrm_quotes','Invoice'=>'mycrm_invoice');
...
function getInventoryCurrencyInfo($module, $id)
{
	...
	$res = $adb->pquery("select currency_id, $inventory_table.conversion_rate as conv_rate, mycrm_currency_info.* from $inventory_table
						inner join mycrm_currency_info on $inventory_table.currency_id = mycrm_currency_info.id
...
function getInventoryProductTaxValue($id, $productid, $taxname)
{
	...
	$res = $adb->pquery("select $taxname from mycrm_inventoryproductrel where id = ? and productid = ?", array($id, $productid));
...
function getInventorySHTaxPercent($id, $taxname)
{
	...
	$res = $adb->pquery("select $taxname from mycrm_inventoryshippingrel where id= ?", array($id));
...
function getAllCurrencies($available='available') {
	...
	$sql = "select * from mycrm_currency_info";
	if ($available != 'all') {
...
function getPriceDetailsForProduct($productid, $unit_price, $available='available', $itemtype='Products')
{
	...
	if($productid != '')
	{
		...
		if ($available == 'available_associated') {
			$query = "select mycrm_currency_info.*, mycrm_productcurrencyrel.converted_price, mycrm_productcurrencyrel.actual_price
					from mycrm_currency_info
					inner join mycrm_productcurrencyrel on mycrm_currency_info.id = mycrm_productcurrencyrel.currencyid
					where mycrm_currency_info.currency_status = 'Active' and mycrm_currency_info.deleted=0
					and mycrm_productcurrencyrel.productid = ? and mycrm_currency_info.id != ?";
			$params = array($productid, $product_currency_id);
		} else { // Edit View
			$query = "select mycrm_currency_info.*, mycrm_productcurrencyrel.converted_price, mycrm_productcurrencyrel.actual_price
					from mycrm_currency_info
					left join mycrm_productcurrencyrel
					on mycrm_currency_info.id = mycrm_productcurrencyrel.currencyid and mycrm_productcurrencyrel.productid = ?
					where mycrm_currency_info.currency_status = 'Active' and mycrm_currency_info.deleted=0";
			$params = array($productid);
		}
	...
	else
	{
		if($available == 'available') { // Create View
			...
			$query = "select mycrm_currency_info.* from mycrm_currency_info
					where mycrm_currency_info.currency_status = 'Active' and mycrm_currency_info.deleted=0";
			$params = array();
...
function getProductBaseCurrency($productid,$module='Products') {
	global $adb, $log;
	if ($module == 'Services') {
		$sql = "select currency_id from mycrm_service where serviceid=?";
	} else {
		$sql = "select currency_id from mycrm_products where productid=?";
	}
	$params = array($productid);
...
function getBaseConversionRateForProduct($productid, $mode='edit', $module='Products') {
	...
	if ($mode == 'edit') {
		if ($module == 'Services') {
			$sql = "select conversion_rate from mycrm_service inner join mycrm_currency_info
					on mycrm_service.currency_id = mycrm_currency_info.id where mycrm_service.serviceid=?";
		} else {
			$sql = "select conversion_rate from mycrm_products inner join mycrm_currency_info
					on mycrm_products.currency_id = mycrm_currency_info.id where mycrm_products.productid=?";
		}
		$params = array($productid);
	} else {
		$sql = "select conversion_rate from mycrm_currency_info where id=?";
...
function getPricesForProducts($currencyid, $product_ids, $module='Products') {
	...
	if (count($product_ids) > 0) {
		if ($module == 'Services') {
			$query = "SELECT mycrm_currency_info.id, mycrm_currency_info.conversion_rate, " .
					"mycrm_service.serviceid AS productid, mycrm_service.unit_price, " .
					"mycrm_productcurrencyrel.actual_price " .
					"FROM (mycrm_currency_info, mycrm_service) " .
					"left join mycrm_productcurrencyrel on mycrm_service.serviceid = mycrm_productcurrencyrel.productid " .
					"and mycrm_currency_info.id = mycrm_productcurrencyrel.currencyid " .
					"where mycrm_service.serviceid in (". generateQuestionMarks($product_ids) .") and mycrm_currency_info.id = ?";
		} else {
			$query = "SELECT mycrm_currency_info.id, mycrm_currency_info.conversion_rate, " .
					"mycrm_products.productid, mycrm_products.unit_price, " .
					"mycrm_productcurrencyrel.actual_price " .
					"FROM (mycrm_currency_info, mycrm_products) " .
					"left join mycrm_productcurrencyrel on mycrm_products.productid = mycrm_productcurrencyrel.productid " .
					"and mycrm_currency_info.id = mycrm_productcurrencyrel.currencyid " .
					"where mycrm_products.productid in (". generateQuestionMarks($product_ids) .") and mycrm_currency_info.id = ?";
		}
...
function getPriceBookCurrency($pricebook_id) {
	global $adb;
	$result = $adb->pquery("select currency_id from mycrm_pricebook where pricebookid=?", array($pricebook_id));
...
function deductProductsFromStock($recordId) {
	global $adb;
	$adb->pquery("UPDATE mycrm_inventoryproductrel SET incrementondel=1 WHERE id=?",array($recordId));

	$product_info = $adb->pquery("SELECT productid,sequence_no, quantity from mycrm_inventoryproductrel WHERE id=?",array($recordId));
	$numrows = $adb->num_rows($product_info);
	for($index = 0;$index <$numrows;$index++) {
		$productid = $adb->query_result($product_info,$index,'productid');
		$qty = $adb->query_result($product_info,$index,'quantity');
		...
		$upd_qty = $qtyinstk-$qty;
		updateProductQty($productid, $upd_qty);
		$sub_prod_query = $adb->pquery("SELECT productid from mycrm_inventorysubproductrel WHERE id=? AND sequence_no=?",array($recordId,$sequence_no));
		if($adb->num_rows($sub_prod_query)>0) {
			for($j=0;$j<$adb->num_rows($sub_prod_query);$j++) {
				$sub_prod_id = $adb->query_result($sub_prod_query,$j,"productid");
				$sqtyinstk= getPrdQtyInStck($sub_prod_id);
		...
		for ($j = 0; $j < $count; ++$j) {
			...
			if ($subjectRow['productid'] == '' || $subjectRow['quantity'] == '' || $subjectRow['listprice'] == '') {
				...
				foreach ($fieldMapping as $fieldName => $index) {
					if($moduleFields[$fieldName]->getTableName() == 'mycrm_inventoryproductrel') {
...
function importRecord($obj, $inventoryFieldData, $lineItemDetails) {
	...
	$webserviceObject = MycrmWebserviceObject::fromName($adb, $moduleName);
	$inventoryOperation = new MycrmInventoryOperation($webserviceObject, $obj->user, $adb, $log);
...
function getInventoryFieldsForExport($tableName) {
	...
	$sql .= 'mycrm_currency_info.currency_name as "Currency" ';
...
function getCurrencyId($fieldValue) {
	...
	$sql = 'SELECT id FROM mycrm_currency_info WHERE currency_name = ? AND deleted = 0';
...
function getLineItemFields(){
	...
	$sql = 'SELECT DISTINCT columnname FROM mycrm_field WHERE tablename=?';
	$result = $adb->pquery($sql, array('mycrm_inventoryproductrel'));
