...
include_once 'vtlib/Mycrm/Functions.php';
...
$NAMESPACE = 'http://www.mycrm.com/products/crm';
 ...
 */
class Mycrm_Soap_CustomerPortal {
...
function _getTicketModComments($ticketId) {
	global $adb;
	$sql = "SELECT * FROM mycrm_modcomments
			INNER JOIN mycrm_crmentity ON mycrm_modcomments.modcommentsid = mycrm_crmentity.crmid AND deleted = 0
	...
	for($i=0; $i<$rows; $i++) {
		...
		if(!empty($customer)) {
			$emailResult = $adb->pquery('SELECT * FROM mycrm_portalinfo WHERE id = ?', array($customer));
...
function get_combo_values($input_array)
{
	...
	$output = Array();
	$sql = "select  productid, productname from mycrm_products inner join mycrm_crmentity on mycrm_crmentity.crmid=mycrm_products.productid where mycrm_crmentity.deleted=0";
	...
	$roleres = $adb->pquery("SELECT roleid from mycrm_user2role where userid = ?",array($userid));
	$RowCount = $adb->num_rows($roleres);
	if($RowCount > 0){
		$admin_role = $adb->query_result($roleres,0,'roleid');
	}
	$result1 = $adb->pquery("select mycrm_ticketpriorities.ticketpriorities from mycrm_ticketpriorities inner join mycrm_role2picklist on mycrm_role2picklist.picklistvalueid = mycrm_ticketpriorities.picklist_valueid and mycrm_role2picklist.roleid='$admin_role' order by sortorderid", array());
	for($i=0;$i<$adb->num_rows($result1);$i++)
	{
		$output['ticketpriorities']['ticketpriorities'][$i] = $adb->query_result($result1,$i,"ticketpriorities");
	}

	$result2 = $adb->pquery("select mycrm_ticketseverities.ticketseverities from mycrm_ticketseverities inner join mycrm_role2picklist on mycrm_role2picklist.picklistvalueid = mycrm_ticketseverities.picklist_valueid and mycrm_role2picklist.roleid='$admin_role' order by sortorderid", array());
	for($i=0;$i<$adb->num_rows($result2);$i++)
	{
		$output['ticketseverities']['ticketseverities'][$i] = $adb->query_result($result2,$i,"ticketseverities");
	}

	$result3 = $adb->pquery("select mycrm_ticketcategories.ticketcategories from mycrm_ticketcategories inner join mycrm_role2picklist on mycrm_role2picklist.picklistvalueid = mycrm_ticketcategories.picklist_valueid and mycrm_role2picklist.roleid='$admin_role' order by sortorderid", array());
	...
	if(!vtlib_isModuleActive('ServiceContracts')) {
		...
		$servicequery = "SELECT mycrm_servicecontracts.servicecontractsid,mycrm_servicecontracts.subject
							FROM mycrm_servicecontracts
							INNER JOIN mycrm_crmentity on mycrm_crmentity.crmid=mycrm_servicecontracts.servicecontractsid
									AND mycrm_crmentity.deleted = 0
							WHERE mycrm_servicecontracts.sc_related_to = ?";
		$params = array($id);
		$showAll = show_all('HelpDesk');
		if($showAll == 'true') {
			$servicequery .= ' OR mycrm_servicecontracts.sc_related_to = (SELECT accountid FROM mycrm_contactdetails WHERE contactid=? AND accountid <> 0)
								OR mycrm_servicecontracts.sc_related_to IN
											(SELECT contactid FROM mycrm_contactdetails WHERE accountid =
													(SELECT accountid FROM mycrm_contactdetails WHERE contactid=? AND accountid <> 0))
							';
...
function get_KBase_details($input_array)
{
	...
	$category_query = "select mycrm_faqcategories.faqcategories from mycrm_faqcategories inner join mycrm_role2picklist on mycrm_role2picklist.picklistvalueid = mycrm_faqcategories.picklist_valueid and mycrm_role2picklist.roleid='$admin_role'";
	...
	if($check == true) {
		$product_query = "select productid, productname from mycrm_products inner join mycrm_crmentity on mycrm_crmentity.crmid=mycrm_products.productid where mycrm_crmentity.deleted=0";
	...
	$faq_query = "select mycrm_faq.*, mycrm_crmentity.createdtime, mycrm_crmentity.modifiedtime from mycrm_faq " .
		"inner join mycrm_crmentity on mycrm_crmentity.crmid=mycrm_faq.id " .
		"where mycrm_crmentity.deleted=0 and mycrm_faq.status='Published' order by mycrm_crmentity.modifiedtime DESC";
	...
	for($k=0;$k<$faq_noofrows;$k++)
	{
		...
		$faq_comment_query = "select * from mycrm_faqcomments where faqid=? order by createdtime DESC";
...
function save_faq_comment($input_array)
{
	...
	if(trim($comment) != '')
	{
		$faq_query = "insert into mycrm_faqcomments values(?,?,?,?)";
...
function get_tickets_list($input_array) {
	...
	{
		$contactquery = "SELECT contactid, accountid FROM mycrm_contactdetails " .
			" INNER JOIN mycrm_crmentity ON mycrm_crmentity.crmid = mycrm_contactdetails.contactid" .
			" AND mycrm_crmentity.deleted = 0 " .
			" WHERE (accountid = (SELECT accountid FROM mycrm_contactdetails WHERE contactid = ?)  AND accountid != 0) OR contactid = ?";
	...
	$query = "SELECT mycrm_troubletickets.*, mycrm_crmentity.smownerid,mycrm_crmentity.createdtime, mycrm_crmentity.modifiedtime, '' AS setype
		FROM mycrm_troubletickets
		INNER JOIN mycrm_crmentity ON mycrm_crmentity.crmid = mycrm_troubletickets.ticketid AND mycrm_crmentity.deleted = 0
		WHERE (mycrm_troubletickets.contact_id IN (". generateQuestionMarks($entity_ids_list) .")";
	if($acc_id) {
		$query .= " OR mycrm_troubletickets.parent_id = $acc_id) ";
	} else {
		$query .= ')';
	}
...
function create_ticket($input_array)
{
	...
	$accountResult = $adb->pquery('SELECT accountid FROM mycrm_contactdetails WHERE contactid = ?', array($parent_id));
	$accountId = $adb->query_result($accountResult, 0, 'accountid');
	if(!empty($accountId)) $ticket->column_fields['parent_id'] = $accountId;

	$ticket->save("HelpDesk");

	$ticketresult = $adb->pquery("select mycrm_troubletickets.ticketid from mycrm_troubletickets
		inner join mycrm_crmentity on mycrm_crmentity.crmid = mycrm_troubletickets.ticketid inner join mycrm_ticketcf on mycrm_ticketcf.ticketid = mycrm_troubletickets.ticketid
		where mycrm_crmentity.deleted=0 and mycrm_troubletickets.ticketid = ?", array($ticket->id));
	...
	if($servicecontractid != ''){
		$res = $adb->pquery("insert into mycrm_crmentityrel values(?,?,?,?)",
...
function authenticate_user($username,$password,$version,$login = 'true')
{
	...
	$sql = "select id, user_name, user_password,last_login_time, support_start_date, support_end_date, cryptmode
				from mycrm_portalinfo
					inner join mycrm_customerdetails on mycrm_portalinfo.id=mycrm_customerdetails.customerid
					inner join mycrm_crmentity on mycrm_crmentity.crmid=mycrm_portalinfo.id
				where mycrm_crmentity.deleted=0 and user_name=?
					and isactive=1 and mycrm_customerdetails.portal=1
					and mycrm_customerdetails.support_start_date <= ? and mycrm_customerdetails.support_end_date >= ?";
	...
	for ($i = 0; $i < $num_rows; ++$i) {
		...
		if (Mycrm_Functions::compareEncryptedPassword($password, $adb->query_result($result, $i, 'user_password'), $adb->query_result($result, $i, 'cryptmode'))) {
			break;
		} else {
			$customerid = null;
		}
	...
	if($login != 'false')
	{
		...
		$sql="insert into mycrm_soapservice values(?,?,?)";
...
function change_password($input_array)
{
	...
	$sql = "update mycrm_portalinfo set user_password=?, cryptmode=? where id=? and user_name=?";
	$result = $adb->pquery($sql, array(Mycrm_Functions::generateEncryptedPassword($password), 'CRYPT', $id, $username));
...
function update_login_details($input_array)
{
	...
	if($flag == 'login')
	{
		$sql = "update mycrm_portalinfo set login_time=? where id=?";
		$result = $adb->pquery($sql, array($current_time, $id));
	}
	elseif($flag == 'logout')
	{
		$sql = "update mycrm_portalinfo set logout_time=?, last_login_time=login_time where id=?";
		$result = $adb->pquery($sql, array($current_time, $id));
	}
...
function send_mail_for_password($mailid)
{
	...
	$sql = "select * from mycrm_portalinfo  where user_name = ? ";
	...
	$fromquery = "select mycrm_users.user_name, mycrm_users.email1 from mycrm_users inner join mycrm_crmentity on mycrm_users.id = mycrm_crmentity.smownerid inner join mycrm_contactdetails on mycrm_contactdetails.contactid=mycrm_crmentity.crmid where mycrm_contactdetails.email =?";
	...
	$mailserverresult = $adb->pquery("select * from mycrm_systems where server_type=?", array('email'));
...
function get_ticket_creator($input_array)
{
	...
	$res = $adb->pquery("select smcreatorid from mycrm_crmentity where crmid=?", array($ticketid));
...
function get_picklists($input_array)
{
	...
	$res = $adb->pquery("select mycrm_". $picklist_name.".* from mycrm_". $picklist_name." inner join mycrm_role2picklist on mycrm_role2picklist.picklistvalueid = mycrm_". $picklist_name.".picklist_valueid and mycrm_role2picklist.roleid='$admin_role'", array());
...
function get_ticket_attachments($input_array)
{
	...
	$query = "select mycrm_troubletickets.ticketid, mycrm_attachments.*,mycrm_notes.filename,mycrm_notes.filelocationtype from mycrm_troubletickets " .
		"left join mycrm_senotesrel on mycrm_senotesrel.crmid=mycrm_troubletickets.ticketid " .
		"left join mycrm_notes on mycrm_notes.notesid=mycrm_senotesrel.notesid " .
		...
		"left join mycrm_attachments on mycrm_attachments.attachmentsid = mycrm_seattachmentsrel.attachmentsid " .
		"and mycrm_crmentity.deleted = 0 where mycrm_troubletickets.ticketid =?";
...
function get_filecontent($input_array)
{
	...
	$query = 'SELECT mycrm_attachments.path FROM mycrm_attachments
	INNER JOIN mycrm_seattachmentsrel ON mycrm_seattachmentsrel.attachmentsid = mycrm_attachments.attachmentsid
	INNER JOIN mycrm_notes ON mycrm_notes.notesid = mycrm_seattachmentsrel.crmid
	INNER JOIN mycrm_senotesrel ON mycrm_senotesrel.notesid = mycrm_notes.notesid
	INNER JOIN mycrm_troubletickets ON mycrm_troubletickets.ticketid = mycrm_senotesrel.crmid
	WHERE mycrm_troubletickets.ticketid = ? AND mycrm_attachments.name = ? AND mycrm_attachments.attachmentsid = ?';
...
function add_ticket_attachment($input_array)
{
	...
	$attachmentid = $adb->getUniqueID("mycrm_crmentity");
	...
	$crmquery = "insert into mycrm_crmentity (crmid,setype,description,createdtime) values(?,?,?,?)";
	$crmresult = $adb->pquery($crmquery, array($attachmentid, 'HelpDesk Attachment', $description, $date_var));

	$attachmentquery = "insert into mycrm_attachments(attachmentsid,name,description,type,path) values(?,?,?,?,?)";
	$attachmentreulst = $adb->pquery($attachmentquery, array($attachmentid, $filename, $description, $filetype, $upload_filepath));

	$relatedquery = "insert into mycrm_seattachmentsrel values(?,?)";
	...
	$related_doc = 'insert into mycrm_seattachmentsrel values (?,?)';
	$res = $adb->pquery($related_doc,array($focus->id,$attachmentid));

	$tic_doc = 'insert into mycrm_senotesrel values(?,?)';
...
function getServerSessionId($id)
{
	...
	$sessionid = Mycrm_Soap_CustomerPortal::lookupSessionId($id);
	if($sessionid === false) {
		$query = "select * from mycrm_soapservice where type='customer' and id=?";
		$result = $adb->pquery($query, array($id));
		if($adb->num_rows($result) > 0) {
			$sessionid = $adb->query_result($result,0,'sessionid');
			Mycrm_Soap_CustomerPortal::updateSessionId($id, $sessionid);
		}
	}
	return $sessionid;
}
...
function unsetServerSessionId($id)
{
	...
	$id = (int) $id;
	Mycrm_Soap_CustomerPortal::updateSessionId($id, false);

	$adb->pquery("delete from mycrm_soapservice where type='customer' and id=?", array($id));
...
function get_account_name($accountid)
{
	...
	$res = $adb->pquery("select accountname from mycrm_account where accountid=?", array($accountid));
...
function get_contact_name($contactid)
{
	...
	if($contactid != '')
	{
		$sql = "select firstname,lastname from mycrm_contactdetails where contactid=?";
...
function get_check_account_id($id)
{
	...
	$res = $adb->pquery("select accountid from mycrm_contactdetails where contactid=?", array($id));
...
function get_vendor_name($vendorid)
{
	...
	$res = $adb->pquery("select vendorname from mycrm_vendor where vendorid=?", array($vendorid));
...
function get_list_values($id,$module,$sessionid,$only_mine='true')
{
	...
	if($module == 'Quotes')
	{
		$query = "select distinct mycrm_quotes.*,mycrm_crmentity.smownerid,
		case when mycrm_quotes.contactid is not null then mycrm_quotes.contactid else mycrm_quotes.accountid end as entityid,
		case when mycrm_quotes.contactid is not null then 'Contacts' else 'Accounts' end as setype,
		mycrm_potential.potentialname,mycrm_account.accountid
		from mycrm_quotes left join mycrm_crmentity on mycrm_crmentity.crmid=mycrm_quotes.quoteid
		...
		where mycrm_crmentity.deleted=0 and (mycrm_quotes.accountid in  (". generateQuestionMarks($entity_ids_list) .") or contactid in (". generateQuestionMarks($entity_ids_list) ."))";
	...
	{
		$query ="select distinct mycrm_invoice.*,mycrm_crmentity.smownerid,
		case when mycrm_invoice.contactid !=0 then mycrm_invoice.contactid else mycrm_invoice.accountid end as entityid,
		case when mycrm_invoice.contactid !=0 then 'Contacts' else 'Accounts' end as setype
		from mycrm_invoice
		left join mycrm_crmentity on mycrm_crmentity.crmid=mycrm_invoice.invoiceid
		where mycrm_crmentity.deleted=0 and (accountid in (". generateQuestionMarks($entity_ids_list) .") or contactid in  (". generateQuestionMarks($entity_ids_list) ."))";
		$params = array($entity_ids_list,$entity_ids_list);
		$fields_list['Related To'] = 'entityid';
	}
	else if ($module == 'Documents')
	{
		$query ="select mycrm_notes.*, mycrm_crmentity.*, mycrm_senotesrel.crmid as entityid, '' as setype,mycrm_attachmentsfolder.foldername from mycrm_notes " .
		"inner join mycrm_crmentity on mycrm_crmentity.crmid = mycrm_notes.notesid " .
		"left join mycrm_senotesrel on mycrm_senotesrel.notesid=mycrm_notes.notesid " .
		"LEFT JOIN mycrm_attachmentsfolder ON mycrm_attachmentsfolder.folderid = mycrm_notes.folderid " .
		"where mycrm_crmentity.deleted = 0 and  mycrm_senotesrel.crmid in (".generateQuestionMarks($entity_ids_list).")";
		$params = array($entity_ids_list);
		$fields_list['Related To'] = 'entityid';
	}else if ($module == 'Contacts'){
		$query = "select mycrm_contactdetails.*,mycrm_crmentity.smownerid from mycrm_contactdetails
		 inner join mycrm_crmentity on mycrm_crmentity.crmid=mycrm_contactdetails.contactid
		 where mycrm_crmentity.deleted = 0 and contactid IN (".generateQuestionMarks($entity_ids_list).")";
		$params = array($entity_ids_list);
	}else if ($module == 'Assets') {
		$accountRes = $adb->pquery("SELECT accountid FROM mycrm_contactdetails
						INNER JOIN mycrm_crmentity ON mycrm_contactdetails.contactid = mycrm_crmentity.crmid
		...
		$query = "select mycrm_assets.*, mycrm_assets.account as entityid , mycrm_crmentity.smownerid from mycrm_assets
						inner join mycrm_crmentity on mycrm_assets.assetsid = mycrm_crmentity.crmid
						left join mycrm_account on mycrm_account.accountid = mycrm_assets.account
						left join mycrm_products on mycrm_products.productid = mycrm_assets.product
						where mycrm_crmentity.deleted = 0 and account = ?";
		$params = array($accountid);
		$fields_list['Related To'] = 'entityid';
		}
	}else if ($module == 'Project') {
		$query = "SELECT mycrm_project.*, mycrm_crmentity.smownerid
					FROM mycrm_project
					INNER JOIN mycrm_crmentity ON mycrm_crmentity.crmid = mycrm_project.projectid
					WHERE mycrm_crmentity.deleted = 0 AND mycrm_project.linktoaccountscontacts IN (".generateQuestionMarks($entity_ids_list).")";
		$params = array($entity_ids_list);
	...
	for( $j= 0;$j < $noofdata; $j++)
	{
		...
		foreach($fields_list as $fieldlabel =>$fieldname ) {
			...
			if($fieldname == 'product' && $module == 'Assets'){
				...
				$fres = $adb->pquery('select mycrm_products.productname from mycrm_products where productid=?',array($crmid));
...
function get_filecontent_detail($id,$folderid,$module,$customerid,$sessionid)
{
	...
	if($module == 'Documents')
	{
		$query="SELECT filetype FROM mycrm_notes WHERE notesid =?";
		$res = $adb->pquery($query, array($id));
		$filetype = $adb->query_result($res, 0, "filetype");
		updateDownloadCount($id);

		$fileidQuery = 'select attachmentsid from mycrm_seattachmentsrel where crmid = ?';
		$fileres = $adb->pquery($fileidQuery,array($id));
		$fileid = $adb->query_result($fileres,0,'attachmentsid');

		$filepathQuery = 'select path,name from mycrm_attachments where attachmentsid = ?';
	...
	{
		$query ='select mycrm_attachments.*,mycrm_seattachmentsrel.* from mycrm_attachments inner join mycrm_seattachmentsrel on mycrm_seattachmentsrel.attachmentsid=mycrm_attachments.attachmentsid where mycrm_seattachmentsrel.crmid =?';
...
function updateDownloadCount($id){
	...
	$updateDownloadCount = "UPDATE mycrm_notes SET filedownloadcount = filedownloadcount+1 WHERE notesid = ?";
...
function get_salesorder_name($id)
{
	...
	$res = $adb->pquery(" select subject from mycrm_salesorder where salesorderid=?", array($id));
...
function get_invoice_detail($id,$module,$customerid,$sessionid)
{
	...
	$fieldquery = "SELECT fieldname, columnname, fieldlabel,block,uitype FROM mycrm_field WHERE tabid = ? AND displaytype in (1,2,4) ORDER BY block,sequence";
	$fieldres = $adb->pquery($fieldquery,array(getTabid($module)));
	$nooffields = $adb->num_rows($fieldres);
	$query = "select mycrm_invoice.*,mycrm_crmentity.* ,mycrm_invoicebillads.*,mycrm_invoiceshipads.*,
		mycrm_invoicecf.* from mycrm_invoice
		inner join mycrm_crmentity on mycrm_crmentity.crmid = mycrm_invoice.invoiceid
		LEFT JOIN mycrm_invoicebillads ON mycrm_invoice.invoiceid = mycrm_invoicebillads.invoicebilladdressid
		LEFT JOIN mycrm_invoiceshipads ON mycrm_invoice.invoiceid = mycrm_invoiceshipads.invoiceshipaddressid
		INNER JOIN mycrm_invoicecf ON mycrm_invoice.invoiceid = mycrm_invoicecf.invoiceid
		where mycrm_invoice.invoiceid=?";
	$res = $adb->pquery($query, array($id));
	...
	for($i=0;$i<$nooffields;$i++)
	{
		...
		$blocknameQuery = "select blocklabel from mycrm_blocks where blockid = ?";
...
function get_product_list_values($id,$modulename,$sessionid,$only_mine='true')
{
	...
	$query = array();
	$params = array();
	$query[] = "SELECT mycrm_products.*,mycrm_seproductsrel.crmid as entityid, mycrm_seproductsrel.setype FROM mycrm_products
		INNER JOIN mycrm_crmentity on mycrm_products.productid = mycrm_crmentity.crmid
		LEFT JOIN mycrm_seproductsrel on mycrm_seproductsrel.productid = mycrm_products.productid
		WHERE mycrm_seproductsrel.crmid in (". generateQuestionMarks($entity_ids_list).") and mycrm_crmentity.deleted = 0 ";
	$params[] = array($entity_ids_list);

	$checkQuotes = checkModuleActive('Quotes');
	if($checkQuotes == true){
		$query[] = "select distinct mycrm_products.*,
			case when mycrm_quotes.contactid is not null then mycrm_quotes.contactid else mycrm_quotes.accountid end as entityid,
			case when mycrm_quotes.contactid is not null then 'Contacts' else 'Accounts' end as setype
			from mycrm_quotes INNER join mycrm_crmentity on mycrm_crmentity.crmid=mycrm_quotes.quoteid
			...
			where mycrm_inventoryproductrel.productid = mycrm_products.productid AND mycrm_crmentity.deleted=0 and (accountid in  (". generateQuestionMarks($entity_ids_list) .") or contactid in (". generateQuestionMarks($entity_ids_list) ."))";
		$params[] = array($entity_ids_list,$entity_ids_list);
	}
...
function get_details($id,$module,$customerid,$sessionid)
{
	...
	if($module == 'Quotes'){
		$query =  "SELECT
			mycrm_quotes.*,mycrm_crmentity.*,mycrm_quotesbillads.*,mycrm_quotesshipads.*,
			mycrm_quotescf.* FROM mycrm_quotes
			...
			WHERE mycrm_quotes.quoteid=(". generateQuestionMarks($id) .") AND mycrm_crmentity.deleted = 0";

	}
	else if($module == 'Documents'){
		$query =  "SELECT
			mycrm_notes.*,mycrm_crmentity.*,mycrm_attachmentsfolder.foldername,mycrm_notescf.*
			FROM mycrm_notes
			INNER JOIN mycrm_crmentity ON mycrm_crmentity.crmid = mycrm_notes.notesid
			...
			WHERE mycrm_notes.notesid=(". generateQuestionMarks($id) .") AND mycrm_crmentity.deleted=0";
	}
	else if($module == 'HelpDesk'){
		$query ="SELECT
			mycrm_troubletickets.*,mycrm_crmentity.smownerid,mycrm_crmentity.createdtime,mycrm_crmentity.modifiedtime,
			mycrm_ticketcf.*,mycrm_crmentity.description  FROM mycrm_troubletickets
			...
			WHERE (mycrm_troubletickets.ticketid=(". generateQuestionMarks($id) .") AND mycrm_crmentity.deleted = 0)";
	}
	else if($module == 'Services'){
		$query ="SELECT mycrm_service.*,mycrm_crmentity.*,mycrm_servicecf.*  FROM mycrm_service
			INNER JOIN mycrm_crmentity
				ON mycrm_crmentity.crmid = mycrm_service.serviceid AND mycrm_crmentity.deleted = 0
			LEFT JOIN mycrm_servicecf
				ON mycrm_service.serviceid = mycrm_servicecf.serviceid
			WHERE mycrm_service.serviceid= (". generateQuestionMarks($id) .")";
	}
	else if($module == 'Contacts'){
		$query = "SELECT mycrm_contactdetails.*,mycrm_contactaddress.*,mycrm_contactsubdetails.*,mycrm_contactscf.*" .
			" ,mycrm_crmentity.*,mycrm_customerdetails.*
			...
			WHERE mycrm_contactdetails.contactid = (". generateQuestionMarks($id) .") AND mycrm_crmentity.deleted = 0";
	}
	else if($module == 'Accounts'){
		$query = "SELECT mycrm_account.*,mycrm_accountbillads.*,mycrm_accountshipads.*,mycrm_accountscf.*,
			mycrm_crmentity.* FROM mycrm_account
			INNER JOIN mycrm_crmentity
		...
		" WHERE mycrm_account.accountid = (". generateQuestionMarks($id) .") AND mycrm_crmentity.deleted = 0";
	}
	else if ($module == 'Products'){
		$query = "SELECT mycrm_products.*,mycrm_productcf.*,mycrm_crmentity.* " .
		"FROM mycrm_products " .
		"INNER JOIN mycrm_crmentity " .
			"ON mycrm_crmentity.crmid = mycrm_products.productid " .
		"LEFT JOIN mycrm_productcf " .
			"ON mycrm_productcf.productid = mycrm_products.productid " .
		"LEFT JOIN mycrm_vendor
			ON mycrm_vendor.vendorid = mycrm_products.vendor_id " .
		"WHERE mycrm_products.productid = (". generateQuestionMarks($id) .") AND mycrm_crmentity.deleted = 0";
	} else if($module == 'Assets') {
		$query = "SELECT mycrm_assets.*, mycrm_assetscf.*, mycrm_crmentity.*
		FROM mycrm_assets
		INNER JOIN mycrm_crmentity
		ON mycrm_assets.assetsid = mycrm_crmentity.crmid
		INNER JOIN mycrm_assetscf
		ON mycrm_assetscf.assetsid = mycrm_assets.assetsid
		WHERE mycrm_crmentity.deleted = 0 AND mycrm_assets.assetsid = (". generateQuestionMarks($id) .")";
	} else if ($module == 'Project') {
		$query = "SELECT mycrm_project.*, mycrm_projectcf.*, mycrm_crmentity.*
					FROM mycrm_project
					INNER JOIN mycrm_crmentity ON mycrm_crmentity.crmid = mycrm_project.projectid
					LEFT JOIN mycrm_projectcf ON mycrm_projectcf.projectid = mycrm_project.projectid
					WHERE mycrm_project.projectid = ? AND mycrm_crmentity.deleted = 0";
	}

	$params = array($id);
	$res = $adb->pquery($query,$params);

	$fieldquery = "SELECT fieldname,columnname,fieldlabel,blocklabel,uitype FROM mycrm_field
		INNER JOIN  mycrm_blocks on mycrm_blocks.blockid=mycrm_field.block WHERE mycrm_field.tabid = ? AND displaytype in (1,2,4)
		ORDER BY mycrm_field.block,mycrm_field.sequence";
	...
	for($i=0;$i<$nooffields;$i++)
	{
		...
		if($module == 'Assets' ){
			if($fieldname == 'account'){
				...
				$accountres = $adb->pquery("select mycrm_account.accountname from mycrm_account where accountid=?",array($accountid));
				$accountname = $adb->query_result($accountres,0,'accountname');
				$fieldvalue = $accountname;
			}
			if($fieldname == 'product'){
				$productid = $adb->query_result($res,0,'product');
				$productres = $adb->pquery("select mycrm_products.productname from mycrm_products where productid=?",array($productid));
			...
			if($fieldname == 'invoiceid'){
				...
				$invoiceres = $adb->pquery("select mycrm_invoice.subject from mycrm_invoice where invoiceid=?",array($invoiceid));
...
function check_permission($customerid, $module, $entityid) {
	...
	$faqquery = "select id from mycrm_faq";
	...
	switch($module) {
		case 'Products'	: 	$query = "SELECT mycrm_seproductsrel.productid FROM mycrm_seproductsrel
								INNER JOIN mycrm_crmentity
								ON mycrm_seproductsrel.productid=mycrm_crmentity.crmid
								WHERE mycrm_seproductsrel.crmid IN (". generateQuestionMarks($allowed_contacts_and_accounts).")
									AND mycrm_crmentity.deleted=0
									AND mycrm_seproductsrel.productid = ?";
							$res = $adb->pquery($query, array($allowed_contacts_and_accounts, $entityid));
							if ($adb->num_rows($res) > 0) {
								return true;
							}
							$query = "SELECT mycrm_inventoryproductrel.productid, mycrm_inventoryproductrel.id
													FROM mycrm_inventoryproductrel
													INNER JOIN mycrm_crmentity
													ON mycrm_inventoryproductrel.productid=mycrm_crmentity.crmid
													LEFT JOIN mycrm_quotes
													ON mycrm_inventoryproductrel.id = mycrm_quotes.quoteid
													WHERE mycrm_crmentity.deleted=0
														AND (mycrm_quotes.contactid IN (". generateQuestionMarks($allowed_contacts_and_accounts).") or mycrm_quotes.accountid IN (".generateQuestionMarks($allowed_contacts_and_accounts)."))
														AND mycrm_inventoryproductrel.productid = ?";
							...
							$query = "SELECT mycrm_inventoryproductrel.productid, mycrm_inventoryproductrel.id
													...
													INNER JOIN mycrm_crmentity
													ON mycrm_inventoryproductrel.productid=mycrm_crmentity.crmid
													LEFT JOIN mycrm_invoice
													ON mycrm_inventoryproductrel.id = mycrm_invoice.invoiceid
													WHERE mycrm_crmentity.deleted=0
														AND (mycrm_invoice.contactid IN (". generateQuestionMarks($allowed_contacts_and_accounts).") or mycrm_invoice.accountid IN (".generateQuestionMarks($allowed_contacts_and_accounts)."))
														AND mycrm_inventoryproductrel.productid = ?";
							$res = $adb->pquery($query, array($allowed_contacts_and_accounts, $allowed_contacts_and_accounts, $entityid));
							if ($adb->num_rows($res) > 0) {
								return true;
		...
		case 'Quotes'	:	$query = "SELECT mycrm_quotes.quoteid
								FROM mycrm_quotes
								INNER JOIN mycrm_crmentity
								ON mycrm_quotes.quoteid=mycrm_crmentity.crmid
								WHERE mycrm_crmentity.deleted=0
									AND (mycrm_quotes.contactid IN (". generateQuestionMarks($allowed_contacts_and_accounts).") or mycrm_quotes.accountid IN (".generateQuestionMarks($allowed_contacts_and_accounts)."))
									AND mycrm_quotes.quoteid = ?";
							$res = $adb->pquery($query, array($allowed_contacts_and_accounts, $allowed_contacts_and_accounts, $entityid));
							if ($adb->num_rows($res) > 0) {
								return true;
							}
		...
		case 'Invoice'	:	$query = "SELECT mycrm_invoice.invoiceid
								FROM mycrm_invoice
								INNER JOIN mycrm_crmentity
								ON mycrm_invoice.invoiceid=mycrm_crmentity.crmid
								WHERE mycrm_crmentity.deleted=0
									AND (mycrm_invoice.contactid IN (". generateQuestionMarks($allowed_contacts_and_accounts).") or mycrm_invoice.accountid IN (".generateQuestionMarks($allowed_contacts_and_accounts)."))
									AND mycrm_invoice.invoiceid = ?";
							$res = $adb->pquery($query, array($allowed_contacts_and_accounts, $allowed_contacts_and_accounts, $entityid));
							if ($adb->num_rows($res) > 0) {
								return true;
							}
		...
		case 'Documents'	: 	$query = "SELECT mycrm_senotesrel.notesid FROM mycrm_senotesrel
									INNER JOIN mycrm_crmentity ON mycrm_crmentity.crmid = mycrm_senotesrel.notesid AND mycrm_crmentity.deleted = 0
									WHERE mycrm_senotesrel.crmid IN (". generateQuestionMarks($allowed_contacts_and_accounts) .")
									AND mycrm_senotesrel.notesid = ?";
								$res = $adb->pquery($query, array($allowed_contacts_and_accounts, $entityid));
								if ($adb->num_rows($res) > 0) {
									return true;
								}
								if(checkModuleActive('Project')) {
									$query = "SELECT mycrm_senotesrel.notesid FROM mycrm_senotesrel
										INNER JOIN mycrm_project ON mycrm_project.projectid = mycrm_senotesrel.crmid
										WHERE mycrm_project.linktoaccountscontacts IN (". generateQuestionMarks($allowed_contacts_and_accounts) .")
										AND mycrm_senotesrel.notesid = ?";
									$res = $adb->pquery($query, array($allowed_contacts_and_accounts, $entityid));
									if ($adb->num_rows($res) > 0) {
										return true;
								...
								$query = "SELECT mycrm_senotesrel.notesid FROM mycrm_senotesrel
															INNER JOIN mycrm_crmentity ON mycrm_crmentity.crmid = mycrm_senotesrel.notesid AND mycrm_crmentity.deleted = 0
															WHERE mycrm_senotesrel.crmid IN (". generateQuestionMarks($faq_id) .")
															AND mycrm_senotesrel.notesid = ?";
		...
		case 'HelpDesk'	:	if($acc_id) $accCondition = "OR mycrm_troubletickets.parent_id = $acc_id";
							$query = "SELECT mycrm_troubletickets.ticketid FROM mycrm_troubletickets
									INNER JOIN mycrm_crmentity ON mycrm_crmentity.crmid = mycrm_troubletickets.ticketid AND mycrm_crmentity.deleted = 0
									WHERE (mycrm_troubletickets.contact_id IN (". generateQuestionMarks($allowed_contacts_and_accounts) .") $accCondition )
									AND mycrm_troubletickets.ticketid = ?";
							$res = $adb->pquery($query, array($allowed_contacts_and_accounts, $entityid));
							if ($adb->num_rows($res) > 0) {
								return true;
							}
							...
							$query = "SELECT mycrm_troubletickets.ticketid FROM mycrm_troubletickets
									...
									WHERE mycrm_crmentity.deleted = 0 AND
											(mycrm_crmentityrel.crmid IN
												(SELECT projectid FROM mycrm_project WHERE linktoaccountscontacts
													IN (". generateQuestionMarks($allowed_contacts_and_accounts) .") AND mycrm_crmentityrel.relcrmid = $entityid)
											OR mycrm_crmentityrel.relcrmid IN
												(SELECT projectid FROM mycrm_project WHERE linktoaccountscontacts
													IN (". generateQuestionMarks($allowed_contacts_and_accounts) .") AND mycrm_crmentityrel.crmid = $entityid)
										AND mycrm_troubletickets.ticketid = ?)";
		...
		case 'Services'	:	$query = "SELECT mycrm_service.serviceid FROM mycrm_service
									INNER JOIN mycrm_crmentity ON mycrm_crmentity.crmid = mycrm_service.serviceid AND mycrm_crmentity.deleted = 0
									LEFT JOIN mycrm_crmentityrel ON (mycrm_crmentityrel.relcrmid=mycrm_service.serviceid OR mycrm_crmentityrel.crmid=mycrm_service.serviceid)
									WHERE (mycrm_crmentityrel.crmid IN (". generateQuestionMarks($allowed_contacts_and_accounts) .")  OR " .
		 							"(mycrm_crmentityrel.relcrmid IN (".generateQuestionMarks($allowed_contacts_and_accounts).") AND mycrm_crmentityrel.module = 'Services'))
									AND mycrm_service.serviceid = ?";
							$res = $adb->pquery($query, array($allowed_contacts_and_accounts,$allowed_contacts_and_accounts, $entityid));
							if ($adb->num_rows($res) > 0) {
								return true;
							}
		...
		case 'Accounts' : 	$query = "SELECT mycrm_account.accountid FROM mycrm_account " .
									"INNER JOIN mycrm_crmentity ON mycrm_crmentity.crmid = mycrm_account.accountid " .
									"INNER JOIN mycrm_contactdetails ON mycrm_contactdetails.accountid = mycrm_account.accountid " .
									"WHERE mycrm_crmentity.deleted = 0 and mycrm_contactdetails.contactid = ? and mycrm_contactdetails.accountid = ?";
		...
		case 'Assets' : $query = "SELECT mycrm_assets.assetname FROM mycrm_assets
								INNER JOIN mycrm_crmentity ON  mycrm_assets.assetsid = mycrm_crmentity.crmid
								WHERE mycrm_crmentity.deleted = 0 and mycrm_assets.account = ? ";
						$accountid = '';
						$accountRes = $adb->pquery("SELECT accountid FROM mycrm_contactdetails
								INNER JOIN mycrm_crmentity ON mycrm_contactdetails.contactid = mycrm_crmentity.crmid
		...
		case 'Project'	:	$query = "SELECT mycrm_project.projectid FROM mycrm_project
									INNER JOIN mycrm_crmentity ON mycrm_crmentity.crmid = mycrm_project.projectid AND mycrm_crmentity.deleted = 0
									WHERE mycrm_project.linktoaccountscontacts IN (". generateQuestionMarks($allowed_contacts_and_accounts) .")
									AND mycrm_project.projectid = ?";
							$res = $adb->pquery($query, array($allowed_contacts_and_accounts, $entityid));
							if ($adb->num_rows($res) > 0) {
								return true;
							}
...
function get_documents($id,$module,$customerid,$sessionid)
{
	...
	$query ="select mycrm_notes.title,'Documents' ActivityType, mycrm_notes.filename,
		crm2.createdtime,mycrm_notes.notesid,mycrm_notes.folderid,
		mycrm_notes.notecontent description, mycrm_users.user_name, mycrm_notes.filelocationtype
		from mycrm_notes
		LEFT join mycrm_senotesrel on mycrm_senotesrel.notesid= mycrm_notes.notesid
		INNER join mycrm_crmentity on mycrm_crmentity.crmid= mycrm_senotesrel.crmid
		LEFT join mycrm_crmentity crm2 on crm2.crmid=mycrm_notes.notesid and crm2.deleted=0
		LEFT JOIN mycrm_groups
		ON mycrm_groups.groupid = mycrm_crmentity.smownerid
		LEFT join mycrm_users on crm2.smownerid= mycrm_users.id
		where mycrm_crmentity.crmid=?";
	$res = $adb->pquery($query,array($id));
	$noofdata = $adb->num_rows($res);
	for( $j= 0;$j < $noofdata; $j++)
	{
...
function get_project_components($id,$module,$customerid,$sessionid) {
	...
	if ($module == 'ProjectTask') {
		$query ="SELECT mycrm_projecttask.*, mycrm_crmentity.smownerid
				FROM mycrm_projecttask
				INNER JOIN mycrm_project ON mycrm_project.projectid = mycrm_projecttask.projectid AND mycrm_project.projectid = ?
				INNER JOIN mycrm_crmentity ON mycrm_crmentity.crmid = mycrm_projecttask.projecttaskid AND mycrm_crmentity.deleted = 0";
	} elseif ($module == 'ProjectMilestone') {
		$query ="SELECT mycrm_projectmilestone.*, mycrm_crmentity.smownerid
				FROM mycrm_projectmilestone
				INNER JOIN mycrm_project ON mycrm_project.projectid = mycrm_projectmilestone.projectid AND mycrm_project.projectid = ?
				INNER JOIN mycrm_crmentity ON mycrm_crmentity.crmid = mycrm_projectmilestone.projectmilestoneid AND mycrm_crmentity.deleted = 0";
	}
...
function get_project_tickets($id,$module,$customerid,$sessionid) {
	...
	$query = "SELECT mycrm_troubletickets.*, mycrm_crmentity.smownerid FROM mycrm_troubletickets
		INNER JOIN mycrm_crmentity ON mycrm_crmentity.crmid = mycrm_troubletickets.ticketid
		INNER JOIN mycrm_crmentityrel ON (mycrm_crmentityrel.relcrmid = mycrm_crmentity.crmid OR mycrm_crmentityrel.crmid = mycrm_crmentity.crmid)
		WHERE mycrm_crmentity.deleted = 0 AND (mycrm_crmentityrel.crmid = ? OR mycrm_crmentityrel.relcrmid = ?)";
...
function get_service_list_values($id,$modulename,$sessionid,$only_mine='true')
{
	...
	$query[] = "select mycrm_service.*," .
		"case when mycrm_crmentityrel.crmid != mycrm_service.serviceid then mycrm_crmentityrel.crmid else mycrm_crmentityrel.relcrmid end as entityid, " .
		 "'' as setype from mycrm_service " .
		 "inner join mycrm_crmentity on mycrm_crmentity.crmid=mycrm_service.serviceid " .
		 "left join mycrm_crmentityrel on (mycrm_crmentityrel.relcrmid=mycrm_service.serviceid or mycrm_crmentityrel.crmid=mycrm_service.serviceid) " .
		 "where mycrm_crmentity.deleted = 0 and " .
		 "( mycrm_crmentityrel.crmid in (".generateQuestionMarks($entity_ids_list).") OR " .
		 "(mycrm_crmentityrel.relcrmid in (".generateQuestionMarks($entity_ids_list).") AND mycrm_crmentityrel.module = 'Services')" .
		 ")";
	...
	$checkQuotes = checkModuleActive('Quotes');
	if($checkQuotes == true){
		$query[] = "select distinct mycrm_service.*,
			case when mycrm_quotes.contactid is not null then mycrm_quotes.contactid else mycrm_quotes.accountid end as entityid,
			case when mycrm_quotes.contactid is not null then 'Contacts' else 'Accounts' end as setype
			from mycrm_quotes INNER join mycrm_crmentity on mycrm_crmentity.crmid=mycrm_quotes.quoteid
			left join mycrm_inventoryproductrel on mycrm_inventoryproductrel.id=mycrm_quotes.quoteid
			left join mycrm_service on mycrm_service.serviceid = mycrm_inventoryproductrel.productid
			where mycrm_inventoryproductrel.productid = mycrm_service.serviceid AND mycrm_crmentity.deleted=0 and (accountid in  (". generateQuestionMarks($entity_ids_list) .") or contactid in (". generateQuestionMarks($entity_ids_list) ."))";
		$params[] = array($entity_ids_list,$entity_ids_list);
	}
	$checkInvoices = checkModuleActive('Invoice');
	if($checkInvoices == true){
		...
		for( $j= 0;$j < $noofdata[$k]; $j++)
		{
			...
			foreach($fields_list as $fieldlabel=> $fieldname) {
				...
				if($fieldname == 'entityid') {
					...
					$module = $adb->query_result($res[$k],$j,'setype');
					if($module == ''){
						$module = $adb->query_result($adb->pquery("SELECT setype FROM mycrm_crmentity WHERE crmid = ?", array($crmid)),0,'setype');
					}
					if ($crmid != '' && $module != '') {
						$fieldvalues = getEntityName($module, array($crmid));
						if($module == 'Contacts')
...
function get_modules()
{
	...
	$modules = Mycrm_Soap_CustomerPortal::lookupAllowedModules();
	if($modules === false) {
		$modules = array();

		$query = $adb->pquery("SELECT mycrm_customerportal_tabs.* FROM mycrm_customerportal_tabs
			INNER JOIN mycrm_tab ON mycrm_tab.tabid = mycrm_customerportal_tabs.tabid
			WHERE mycrm_tab.presence = 0 AND mycrm_customerportal_tabs.visible = 1", array());
		...
		Mycrm_Soap_CustomerPortal::updateAllowedModules($modules);
	}
...
function show_all($module){
	...
	$query = $adb->pquery("SELECT prefvalue from mycrm_customerportal_prefs where tabid = ?", array($tabid));
	$norows = $adb->num_rows($query);
	if($norows > 0){
...
function getRelatedServiceContracts($crmid){
	...
	$query = "SELECT * FROM mycrm_servicecontracts " .
	"INNER JOIN mycrm_crmentity ON mycrm_crmentity.crmid = mycrm_servicecontracts.servicecontractsid AND mycrm_crmentity.deleted = 0 " .
	"LEFT JOIN mycrm_crmentityrel ON mycrm_crmentityrel.crmid = mycrm_servicecontracts.servicecontractsid " .
	"WHERE (mycrm_crmentityrel.relcrmid = ? and mycrm_crmentityrel.module= 'ServiceContracts')";
...
function getPortalUserid() {
	...
	$userid = Mycrm_Soap_CustomerPortal::lookupPrefValue('userid');
	if($userid === false) {
		$res = $adb->pquery("SELECT prefvalue FROM mycrm_customerportal_prefs WHERE prefkey = 'userid' AND tabid = 0", array());
		$norows = $adb->num_rows($res);
		if($norows > 0) {
			$userid = $adb->query_result($res,0,'prefvalue');
			// Update the cache information now.
			Mycrm_Soap_CustomerPortal::updatePrefValue('userid', $userid);
		}
	}
	return $userid;
...
function getDefaultAssigneeId() {
	...
	// Look the value from cache first
	$defaultassignee = Mycrm_Soap_CustomerPortal::lookupPrefValue('defaultassignee');
	if($defaultassignee === false) {
		$res = $adb->pquery("SELECT prefvalue FROM mycrm_customerportal_prefs WHERE prefkey = 'defaultassignee' AND tabid = 0", array());
		$norows = $adb->num_rows($res);
		if($norows > 0) {
			$defaultassignee = $adb->query_result($res,0,'prefvalue');
			// Update the cache information now.
			Mycrm_Soap_CustomerPortal::updatePrefValue('defaultassignee', $defaultassignee);
		}
	}
