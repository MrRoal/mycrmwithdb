...
$server->configureWSDL('mycrmsoap');
...
function track_email($user_name,$password,$contact_ids, $date_sent, $email_subject, $email_body)
{
	if(authentication($user_name,$password))
	{
		...
		$email->save("Emails");
		$query = "select fieldid from mycrm_field where fieldname = 'email' and tabid = 4 and mycrm_field.presence in (0,2)";
		...
		$sql = "select email from mycrm_contactdetails inner join mycrm_crmentity on mycrm_crmentity.crmid = mycrm_contactdetails.contactid where mycrm_crmentity.deleted =0 and mycrm_contactdetails.contactid=?";
		...
		if(isset($camodulerow))
		{
			$emailid = $camodulerow["email"];
		    	...
		    	$user_emailid = str_replace('>','&gt;',$user_emailid);
			$query = 'insert into mycrm_emaildetails values (?,?,?,?,?,?,?,?)';
...
function retrieve_account_id($account_name,$user_id)
{
	...
	$query = "select mycrm_account.accountname accountname,mycrm_account.accountid accountid from mycrm_account inner join mycrm_crmentity on mycrm_crmentity.crmid=mycrm_account.accountid where mycrm_crmentity.deleted=0 and mycrm_account.accountname=?";
...
function AddContact($user_name,$first_name, $last_name, $email_address ,$account_name , $salutation , $title, $phone_mobile, $reports_to,$primary_address_street,$primary_address_city,$primary_address_state,$primary_address_postalcode,$primary_address_country,$alt_address_city,$alt_address_street,$alt_address_state,$alt_address_postalcode,$alt_address_country,$office_phone="",$home_phone="",$fax="",$department="",$password,$description="")
{
	if(authentication($user_name,$password))
	{
		...
		if($is_admin == true || $profileGlobalPermission[1] == 0 || $profileGlobalPermission[2] == 0) {
	    	$sql1 = "select fieldname,columnname from mycrm_field where tabid=4 and block <> 75 and block <> 6 and block <> 5 and mycrm_field.presence in (0,2)";
			$params1 = array();
	  	} else {
	    	$profileList = getCurrentUserProfileList();
	    	$sql1 = "select fieldname,columnname from mycrm_field inner join mycrm_profile2field on mycrm_profile2field.fieldid=mycrm_field.fieldid inner join mycrm_def_org_field on mycrm_def_org_field.fieldid=mycrm_field.fieldid where mycrm_field.tabid=4 and mycrm_field.block <> 75 and mycrm_field.block <> 6 and mycrm_field.block <> 5 and mycrm_field.displaytype in (1,2,4) and mycrm_profile2field.visible=0 and mycrm_def_org_field.visible=0 and mycrm_field.presence in (0,2)";
			$params1 = array();
...
function AddLead($user_name, $first_name, $last_name, $email_address ,$account_name , $salutation , $title, $phone_mobile, $reports_to ,$primary_address_street , $website ,$primary_address_city,$primary_address_state,$primary_address_postalcode,$primary_address_country,$alt_address_city,$alt_address_street,$alt_address_state,$alt_address_postalcode,$alt_address_country,$office_phone="",$home_phone="",$fax="",$department="",$password,$description="")
{
	if(authentication($user_name,$password))
	{
		...
		if($is_admin == true || $profileGlobalPermission[1] == 0 || $profileGlobalPermission[2] == 0) {
	    	$sql1 = "select fieldname,columnname from mycrm_field where tabid=7 and block <> 14 and mycrm_field.presence in (0,2)";
			$params1 = array();
	  	} else {
	    	$profileList = getCurrentUserProfileList();
	    	$sql1 = "select fieldname,columnname from mycrm_field inner join mycrm_profile2field on mycrm_profile2field.fieldid=mycrm_field.fieldid inner join mycrm_def_org_field on mycrm_def_org_field.fieldid=mycrm_field.fieldid where mycrm_field.tabid=7 and mycrm_field.block <> 14 and mycrm_field.displaytype in (1,2,4) and mycrm_profile2field.visible=0 and mycrm_def_org_field.visible=0 and mycrm_field.presence in (0,2)";
	  		$params1 = array();
			if (count($profileList) > 0) {
				$sql1 .= " and mycrm_profile2field.profileid in (". generateQuestionMarks($profileList) .")";
				array_push($params1, $profileList);
...
function create_session($user_name, $password,$version)
{
  ...
  include('mycrmversion.php');
	...
	if($password != "" && $user_name != '')
	{
		...
		{
			$query = "select id from mycrm_users where user_name=? and user_password=?";
