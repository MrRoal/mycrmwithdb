...
function LogintoMycrmCRM($user_name,$password,$version)
{
	global $log,$adb;
	require_once('modules/Users/Users.php');
	include('mycrmversion.php');
	if($version != $mycrm_current_version)
	{
...
function create_account($username,$sessionid,$accountname,$email,$phone,$primary_address_street,$primary_address_city,$primary_address_state,$primary_address_postalcode,$primary_address_country)
{
	...
	if(isPermitted("Accounts","EditView") == "yes")
	{
		$query = "SELECT accountname FROM mycrm_account,mycrm_crmentity WHERE accountname =? and mycrm_account.accountid = mycrm_crmentity.crmid and mycrm_crmentity.deleted != 1";
...
function GetPicklistValues($username,$sessionid,$tablename)
{
	...
	{
		$query = "select " . $adb->sql_escape_string($tablename) . " from mycrm_". $adb->sql_escape_string($tablename);		
	...
	{
		$query = "select " .$adb->sql_escape_string($tablename) . " from mycrm_". $adb->sql_escape_string($tablename) ." inner join mycrm_role2picklist on mycrm_role2picklist.picklistvalueid = mycrm_". $adb->sql_escape_string($tablename) .".picklist_valueid where roleid=? and picklistid in (select picklistid from mycrm_". $adb->sql_escape_string($tablename)." ) order by sortid";	
...
function unsetServerSessionId($id)
{
	...
	$adb->query("delete from mycrm_soapservice where type='FireFox' and id=$id");
...
function getServerSessionId($id)
{
	...
	$query = "select * from mycrm_soapservice where type='FireFox' and id={$id}";
