 ...
 * All Rights Reserved.
 ************************************************************************************/
include_once('vtlib/Mycrm/PackageExport.php');
include_once('vtlib/Mycrm/Unzip.php');
include_once('vtlib/Mycrm/Module.php');
include_once('vtlib/Mycrm/Event.php');
include_once('vtlib/Mycrm/Cron.php');

/**
 * Provides API to import module into mycrm CRM
 * @package vtlib
 */
class Mycrm_PackageImport extends Mycrm_PackageExport {
	...
	function checkZip($zipfile) {
		...
		$language_modulename = null;
			...
			if(count($matches)) {
				...
				if($isModuleBundle === 'true' && (!empty($this->_modulexml)) &&
						...
						(!empty($this->_modulexml->dependencies->mycrm_version))) {
		...
		if(!empty($this->_modulexml) &&
			!empty($this->_modulexml->dependencies) &&
			!empty($this->_modulexml->dependencies->mycrm_version)) {
				$mycrmVersion = (string)$this->_modulexml->dependencies->mycrm_version;
				if(version_compare($mycrmVersion, '6.0.0rc', '>=') === true) {
					$mycrmversion_found = true;
				}
		}

		$validzip = false;
		if($manifestxml_found && $languagefile_found && $mycrmversion_found)
			$validzip = true;

                if($manifestxml_found && $layoutfile_found && $mycrmversion_found)
			$validzip = true;
                
                if($manifestxml_found && $extensionfile_found && $mycrmversion_found)
			$validzip = true;
	...
	function initImport($zipfile, $overwrite=true) {
		...
		if($module != null) {
			...
			$unzip = new Mycrm_Unzip($zipfile, $overwrite);
	 ...
	 * @access private
	 */
	function getDependentMycrmVersion() {
		return $this->_modulexml->dependencies->mycrm_version;
	}
	 ...
	 * @access private
	 */
	function getDependentMaxMycrmVersion() {
		return $this->_modulexml->dependencies->mycrm_max_version;
	}
	...
	function import_Module() {
		...
		$mycrmMinVersion = $this->_modulexml->dependencies->mycrm_version;
		$mycrmMaxVersion = $this->_modulexml->dependencies->mycrm_max_version;

		$moduleInstance = new Mycrm_Module();
		...
		$moduleInstance->minversion = (!$mycrmMinVersion)? false : $mycrmMinVersion;
		$moduleInstance->maxversion = (!$mycrmMaxVersion)?  false : $mycrmMaxVersion;
		$moduleInstance->save();
		...
		Mycrm_Module::fireEvent($moduleInstance->name,
			Mycrm_Module::EVENT_MODULE_POSTINSTALL);
	...
	function import_Tables($modulenode) {
		...
		foreach($modulenode->tables->table as $tablenode) {
			...
			if(Mycrm_Utils::IsCreateSql($tablesql)) {
				if(!Mycrm_Utils::checkTable($tablename)) {
					self::log("SQL: $tablesql ... ", false);
					Mycrm_Utils::ExecuteQuery($tablesql);
					self::log("DONE");
				}
			} else {
				if(Mycrm_Utils::IsDestructiveSql($tablesql)) {
	...
	function import_CustomView($modulenode, $moduleInstance, $customviewnode) {
		...
		$filterInstance = new Mycrm_Filter();
	...
	function import_Event($modulenode, $moduleInstance, $eventnode) {
		...
		Mycrm_Event::register($moduleInstance,
	...
	function import_Relatedlist($modulenode, $moduleInstance, $relatedlistnode) {
		$relModuleInstance = Mycrm_Module::getInstance($relatedlistnode->relatedmodule);
	...
	function import_CronTasks($modulenode){
		...
		foreach ($modulenode->crons->cron as $cronTask){
			if(empty($cronTask->status)){
                            $cronTask->status = Mycrm_Cron::$STATUS_DISABLED;
                        } else {
                            $cronTask->status = Mycrm_Cron::$STATUS_ENABLED;
                        } 
			if((empty($cronTask->sequence))){
				$cronTask->sequence=Mycrm_Cron::nextSequence();
			}
			Mycrm_Cron::register("$cronTask->name","$cronTask->handler", "$cronTask->frequency", "$modulenode->name","$cronTask->status","$cronTask->sequence","$cronTask->description");
		}
	}
}
?>
