...
/**
 * Provides basic API to work with mycrm CRM Fields
 * @package vtlib
 */
class Mycrm_FieldBasic {
     ...
     * @param Array 
     * @param Mycrm_Module Instance of module to which this field belongs
     * @param Mycrm_Block Instance of block to which this field belongs
     * @access private
     */
    function initialize($valuemap, $moduleInstance = false, $blockInstance = false) {
        ...
        $this->block = $blockInstance ? $blockInstance : Mycrm_Block::getInstance($valuemap['block'], $moduleInstance);
    }
...
// Add helpinfo column to the mycrm_field table
        if (empty(self::$__cacheSchemaChanges['mycrm_field.helpinfo'])) {
            Mycrm_Utils::AddColumn('mycrm_field', 'helpinfo', ' TEXT');
            self::$__cacheSchemaChanges['mycrm_field.helpinfo'] = true;
        }
        if (empty(self::$__cacheSchemaChanges['mycrm_field.summaryfield'])) {
            Mycrm_Utils::AddColumn('mycrm_field', 'summaryfield', ' INT(10) NOT NULL DEFAULT 0');
            self::$__cacheSchemaChanges['mycrm_field.summaryfield'] = 0;
        }
    }
    ...
    function __getUniqueId() {
        global $adb;
        return $adb->getUniqueID('mycrm_field');
    }
    ...
    function __getNextSequence() {
        global $adb;
        $result = $adb->pquery("SELECT MAX(sequence) AS max_seq FROM mycrm_field WHERE tabid=? AND block=?", Array($this->getModuleId(), $this->getBlockId()));
        $maxseq = 0;
    ...
    function __getNextQuickCreateSequence() {
        global $adb;
        $result = $adb->pquery("SELECT MAX(quickcreatesequence) AS max_quickcreateseq FROM mycrm_field WHERE tabid=?", Array($this->getModuleId()));
     ...
     * @param Mycrm_Block Instance of the block to use
     * @access private
     */
...
// Initialize other variables which are not done
        ...
        $adb->pquery("INSERT INTO mycrm_field (tabid, fieldid, columnname, tablename, generatedtype,
...
// Set the field status for mass-edit (if set)
        $adb->pquery('UPDATE mycrm_field SET masseditable=? WHERE fieldid=?', Array($this->masseditable, $this->id));

        Mycrm_Profile::initForField($this);

        if (!empty($this->columntype)) {
            Mycrm_Utils::AddColumn($this->table, $this->column, $this->columntype);
        }
    ...
    function __delete() {
        ...
        Mycrm_Profile::deleteForField($this);

        $adb->pquery("DELETE FROM mycrm_field WHERE fieldid=?", Array($this->id));
     ...
     * @param Mycrm_Block Instance of block to which this field should be added.
     */
...
// Make sure to initialize the core tables first
        ...
        global $adb;
        $adb->pquery('UPDATE mycrm_field SET helpinfo=? WHERE fieldid=?', Array($helptext, $this->id));
    ...
    function setMassEditable($value) {
        global $adb;
        $adb->pquery('UPDATE mycrm_field SET masseditable=? WHERE fieldid=?', Array($value, $this->id));
    ...
    function setSummaryField($value) { 
        global $adb; 
        $adb->pquery('UPDATE mycrm_field SET summaryfield=? WHERE fieldid=?', Array($value, $this->id)); 
