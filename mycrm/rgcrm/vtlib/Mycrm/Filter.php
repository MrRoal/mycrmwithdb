...
/**
 * Provides API to work with mycrm CRM Custom View (Filter)
 * @package vtlib
 */
class Mycrm_Filter {
	...
	function __getUniqueId() {
		global $adb;
		return $adb->getUniqueID('mycrm_customview');
	}

	/**
	 * Initialize this filter instance
	 * @param Mycrm_Module Instance of the module to which this filter is associated.
	 * @access private
	 */
	function initialize($valuemap, $moduleInstance=false) {
		$this->id = $valuemap[cvid];
		$this->name= $valuemap[viewname];
		$this->module=$moduleInstance? $moduleInstance: Mycrm_Module::getInstance($valuemap[tabid]);
	}

	/**
	 * Create this instance
	 * @param Mycrm_Module Instance of the module to which this filter should be associated with
	 * @access private
	 */
	function __create($moduleInstance) {
		...
		$adb->pquery("INSERT INTO mycrm_customview(cvid,viewname,setdefault,setmetrics,entitytype) VALUES(?,?,?,?,?)", 
		...
		if(!$this->status) {
			...
			$adb->pquery("UPDATE mycrm_customview SET status=? WHERE cvid=?", Array($this->status, $this->id));
	...
	function __delete() {
		global $adb;
		$adb->pquery("DELETE FROM mycrm_cvadvfilter WHERE cvid=?", Array($this->id));
		$adb->pquery("DELETE FROM mycrm_cvcolumnlist WHERE cvid=?", Array($this->id));
		$adb->pquery("DELETE FROM mycrm_customview WHERE cvid=?", Array($this->id));
	}
	...
	function addField($fieldInstance, $index=0) {
		...
		$adb->pquery("UPDATE mycrm_cvcolumnlist SET columnindex=columnindex+1 WHERE cvid=? AND columnindex>=? ORDER BY columnindex DESC", 
			Array($this->id, $index));
		$adb->pquery("INSERT INTO mycrm_cvcolumnlist(cvid,columnindex,columnname) VALUES(?,?,?)", Array($this->id, $index, $cvcolvalue));
	...
	function addRule($fieldInstance, $comparator, $comparevalue, $index=0, $group=1, $condition='and') {
		...
		$adb->pquery("UPDATE mycrm_cvadvfilter set columnindex=columnindex+1 WHERE cvid=? AND columnindex>=? ORDER BY columnindex DESC",
			Array($this->id, $index));		
		$adb->pquery("INSERT INTO mycrm_cvadvfilter(cvid, columnindex, columnname, comparator, value, groupid, column_condition) VALUES(?,?,?,?,?,?,?)",
			Array($this->id, $index, $cvcolvalue, $comparator, $comparevalue, $group, $condition));

		Mycrm_Utils::Log("Adding Condition " . self::translateComparator($comparator,true) ." on $fieldInstance->name of $this->name filter ... DONE");
	 ...
	 * @access private
	 * @internal Used from Mycrm_PackageExport also
	 */
	 ...
	 * @param Mycrm_Module Instance of the module to use when filtername is used
	 */
	static function getInstance($value, $moduleInstance=false) {
		...
		if(Mycrm_Utils::isNumber($value)) {
			$query = "SELECT * FROM mycrm_customview WHERE cvid=?";
			$queryParams = Array($value);
		} else {
			$query = "SELECT * FROM mycrm_customview WHERE viewname=? AND entitytype=?";
	...
	static function getAllForModule($moduleInstance) {
		...
		$query = "SELECT * FROM mycrm_customview WHERE entitytype=?";
	...
	static function deleteForModule($moduleInstance) {
		...
		$cvidres = $adb->pquery("SELECT cvid FROM mycrm_customview WHERE entitytype=?", Array($moduleInstance->name));
		if($adb->num_rows($cvidres)) {
			...
			if(!empty($cvids)) {
				$adb->pquery("DELETE FROM mycrm_cvadvfilter WHERE cvid  IN (" . implode(',', $cvids) . ")", array());
				$adb->pquery("DELETE FROM mycrm_cvcolumnlist WHERE cvid IN (" . implode(',', $cvids) . ")", array());
				$adb->pquery("DELETE FROM mycrm_customview WHERE cvid   IN (" . implode(',', $cvids) . ")", array());
			}
		}
	}
}
