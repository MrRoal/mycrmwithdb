 ...
 * All Rights Reserved.
 ************************************************************************************/
include_once('vtlib/Mycrm/PackageImport.php');

/**
 * Provides API to update module into mycrm CRM
 * @package vtlib
 */
class Mycrm_PackageUpdate extends Mycrm_PackageImport {
     ...
     * @param Mycrm_Module Instance of the module to update
                ...
                foreach ($installSequenceArray as $sequence) {
                    foreach ($buildModuleArray as $moduleInfo) {
                        if($moduleInfo['install_sequence'] == $sequence) {
                            $moduleInstance = Mycrm_Module::getInstance($moduleInfo['name']);
    ...
    function update_Module($moduleInstance) {
        ...
        Mycrm_Module::fireEvent($moduleInstance->name,
                Mycrm_Module::EVENT_MODULE_PREUPDATE);
        ...
        Mycrm_Module::fireEvent($moduleInstance->name,
                Mycrm_Module::EVENT_MODULE_POSTUPDATE);
    }
                ...
                if(!empty($migrationnode->tables) && !empty($migrationnode->tables->table)) {
                    foreach($migrationnode->tables->table as $tablenode) {
                        ...
                        if(Mycrm_Utils::IsDestructiveSql($tablesql)) {
                            ...
                            Mycrm_Utils::ExecuteQuery($tablesql, true);
                            self::log("DONE");
                        }
                    }
                }
    ...
    function update_Blocks($modulenode, $moduleInstance) {
        ...
        foreach($modulenode->blocks->block as $blocknode) {
            $blockInstance = Mycrm_Block::getInstance((string)$blocknode->label, $moduleInstance);
            if(!$blockInstance) {
    ...
    function update_Fields($blocknode, $blockInstance, $moduleInstance) {
        ...
        foreach($blocknode->fields->field as $fieldnode) {
            $fieldInstance = Mycrm_Field::getInstance((string)$fieldnode->fieldname, $moduleInstance);
            if(!$fieldInstance) {
    ...
    function update_CustomViews($modulenode, $moduleInstance) {
        ...
        foreach($modulenode->customviews->customview as $customviewnode) {
            $filterInstance = Mycrm_Filter::getInstance($customviewnode->viewname, $moduleInstance);
            if(!$filterInstance) {
    ...
    function update_Event($modulenode, $moduleInstance, $eventnode) {
        //Mycrm_Event::register($moduleInstance, $eventnode->eventname, $eventnode->classname, $eventnode->filename);
    ...
    function update_Relatedlist($modulenode, $moduleInstance, $relatedlistnode) {
        $relModuleInstance = Mycrm_Module::getInstance((string)$relatedlistnode->relatedmodule);
	...
	function update_CronTasks($modulenode) {
		...
		$cronTasks = Mycrm_Cron::listAllInstancesByModule($modulenode->name);
		foreach ($modulenode->crons->cron as $importCronTask) {
			foreach($cronTasks as $cronTask) {
				if($cronTask->getName() == $importCronTask->name && $importCronTask->handler == $cronTask->getHandlerFile()) {
					Mycrm_Cron::deregister($importCronTask->name);
				}
			}
			...
			if((empty($importCronTask->sequence))){
				$importCronTask->sequence=Mycrm_Cron::nextSequence();
			}
			Mycrm_Cron::register("$importCronTask->name","$importCronTask->handler", "$importCronTask->frequency", "$modulenode->name","$importCronTask->status","$importCronTask->sequence","$importCronTask->description");
		}
	}
}
?>
