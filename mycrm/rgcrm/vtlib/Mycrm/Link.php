...
include_once('vtlib/Mycrm/Utils/StringTemplate.php');
include_once 'vtlib/Mycrm/LinkData.php';
...
class Mycrm_Link {
	...
	static function __getUniqueId() {
		global $adb;
		return $adb->getUniqueID('mycrm_links');
	}
	...
	static function __initSchema() {
		/* mycrm_links is already core product table */
		/*if(empty(self::$__cacheSchemaChanges['mycrm_links'])) {
			if(!Mycrm_Utils::CheckTable('mycrm_links')) {
				Mycrm_Utils::CreateTable(
					...
					true);
				Mycrm_Utils::ExecuteQuery(
					'CREATE INDEX link_tabidtype_idx on mycrm_links(tabid,linktype)');
			}
			self::$__cacheSchemaChanges['mycrm_links'] = true;
		}*/
	}
	...
	static function addLink($tabid, $type, $label, $url, $iconpath='',$sequence=0, $handlerInfo=null) {
		global $adb;
		self::__initSchema();
		$checkres = $adb->pquery('SELECT linkid FROM mycrm_links WHERE tabid=? AND linktype=? AND linkurl=? AND linkicon=? AND linklabel=?',
			Array($tabid, $type, $url, $iconpath, $label));
		if(!$adb->num_rows($checkres)) {
			$uniqueid = self::__getUniqueId();
			$sql = 'INSERT INTO mycrm_links (linkid,tabid,linktype,linklabel,linkurl,linkicon,'.
			'sequence';
	...
	static function deleteLink($tabid, $type, $label, $url=false) {
		...
		self::__initSchema();
		if($url) {
			$adb->pquery('DELETE FROM mycrm_links WHERE tabid=? AND linktype=? AND linklabel=? AND linkurl=?',
				Array($tabid, $type, $label, $url));
			self::log("Deleting Link ($type - $label - $url) ... DONE");
		} else {
			$adb->pquery('DELETE FROM mycrm_links WHERE tabid=? AND linktype=? AND linklabel=?',
	...
	static function deleteAll($tabid) {
		global $adb;
		self::__initSchema();
		$adb->pquery('DELETE FROM mycrm_links WHERE tabid=?', Array($tabid));
	...
	static function getAllByType($tabid, $type=false, $parameters=false) {
		...
		if($type) {
			...
			if(is_array($type)) { 
				...
				if($tabid === self::IGNORE_MODULE) {
					$sql = 'SELECT * FROM mycrm_links WHERE linktype IN ('.
						Mycrm_Utils::implodestr('?', count($type), ',') .') ';
					$params = $type;
					...
					if(count($permittedTabIdList) > 0 && $current_user->is_admin !== 'on') {
						...
						$sql .= ' and tabid IN ('.
							Mycrm_Utils::implodestr('?', count($permittedTabIdList), ',').')';
						$params[] = $permittedTabIdList;
					}
					$result = $adb->pquery($sql, Array($adb->flatten_array($params)));
				} else {
					$result = $adb->pquery('SELECT * FROM mycrm_links WHERE (tabid=? OR tabid=0) AND linktype IN ('.
						Mycrm_Utils::implodestr('?', count($type), ',') .')',
				...
				if($tabid === self::IGNORE_MODULE) {
					$result = $adb->pquery('SELECT * FROM mycrm_links WHERE linktype=?', Array($type));
				} else {
					$result = $adb->pquery('SELECT * FROM mycrm_links WHERE (tabid=? OR tabid=0) AND linktype=?', Array($tabid, $type));				
				}
			}
		} else {
			$result = $adb->pquery('SELECT * FROM mycrm_links WHERE tabid=?', Array($tabid));
		}

		$strtemplate = new Mycrm_StringTemplate();
		if($parameters) {
		...
		while($row = $adb->fetch_array($result)){
			...
			if(!empty($row['handler_path']) && isFileAccessible($row['handler_path'])) {
				...
				$linkData = new Mycrm_LinkData($instance, $current_user);
