...
include_once('vtlib/Mycrm/Zip.php');
include_once('vtlib/Mycrm/Cron.php');
/**
 * Provides API to package mycrm CRM module and associated files.
 * @package vtlib
 */
class Mycrm_PackageExport {
	...
	function __initExport($module, $moduleInstance) {
		if($moduleInstance->isentitytype) {
			...
			Mycrm_Utils::checkFileAccessForInclusion("modules/$module/$module.php");
		}
	...
	/**
	 * Export mycrm dependencies
	 * @access private
	 */
	function export_Dependencies($moduleInstance) {
		global $mycrm_current_version, $adb;
		$moduleid = $moduleInstance->id;

		$sqlresult = $adb->pquery("SELECT * FROM mycrm_tab_info WHERE tabid = ?", array($moduleid));
		$mycrmMinVersion = $mycrm_current_version;
		$mycrmMaxVersion = false;
		...
		for($i=0; $i<$noOfPreferences; ++$i) {
			...
			if($prefName == 'mycrm_min_version') {
				$mycrmMinVersion = $prefValue;
			}
			if($prefName == 'mycrm_max_version') {
				$mycrmMaxVersion = $prefValue;
			}

		}

		$this->openNode('dependencies');
		$this->outputNode($mycrmMinVersion, 'mycrm_version');
		if($mycrmMaxVersion !== false)	$this->outputNode($mycrmMaxVersion, 'mycrm_max_version');
	...
	function export_Module($moduleInstance) {
		...
		$sqlresult = $adb->pquery("SELECT * FROM mycrm_parenttabrel WHERE tabid = ?", array($moduleid));
		$parenttabid = $adb->query_result($sqlresult, 0, 'parenttabid');
		$menu = Mycrm_Menu::getInstance($parenttabid);
		$parent_name = $menu->label;

		$sqlresult = $adb->pquery("SELECT * FROM mycrm_tab WHERE tabid = ?", array($moduleid));
	...
	function export_Tables($moduleInstance) {
		...
		if($moduleInstance->isentitytype) {
			...
			foreach($tables as $table) {
				...
				$this->outputNode('<![CDATA['.Mycrm_Utils::CreateTableSql($table).']]>', 'sql');
	...
	function export_Blocks($moduleInstance) {
		global $adb;
		$sqlresult = $adb->pquery("SELECT * FROM mycrm_blocks WHERE tabid = ?", Array($moduleInstance->id));
	...
	function export_Fields($moduleInstance, $blockid) {
		...
		$fieldresult = $adb->pquery("SELECT * FROM mycrm_field WHERE tabid=? AND block=?", Array($moduleInstance->id, $blockid));
		$fieldcount = $adb->num_rows($fieldresult);

		if(empty($fieldcount)) return;

		$entityresult = $adb->pquery("SELECT * FROM mycrm_entityname WHERE tabid=?", Array($moduleInstance->id));
		...
		for($index = 0; $index < $fieldcount; ++$index) {
			...
			if($uitype == '10') {
				$relatedmodres = $adb->pquery("SELECT * FROM mycrm_fieldmodulerel WHERE fieldid=?", Array($fieldid));
	...
	function export_CustomViews($moduleInstance) {
		...
		$customviewres = $adb->pquery("SELECT * FROM mycrm_customview WHERE entitytype = ?", Array($moduleInstance->name));
		...
		for($cvindex = 0; $cvindex < $customviewcount; ++$cvindex) {
			...
			$cvcolumnres = $adb->pquery("SELECT * FROM mycrm_cvcolumnlist WHERE cvid=?", array($cvid));
			...
			for($index = 0; $index < $cvcolumncount; ++$index) {
				...
				$cvcolumnruleres = $adb->pquery("SELECT * FROM mycrm_cvadvfilter WHERE cvid=? AND columnname=?",
				...
				if($cvcolumnrulecount) {
					...
					for($rindex = 0; $rindex < $cvcolumnrulecount; ++$rindex) {
						...
						$cvcolumnrulecomp  = Mycrm_Filter::translateComparator($cvcolumnrulecomp, true);
	...
	function export_SharingAccess($moduleInstance) {
		...
		$deforgshare = $adb->pquery("SELECT * FROM mycrm_def_org_share WHERE tabid=?", Array($moduleInstance->id));
	...
	function export_Events($moduleInstance) {
		$events = Mycrm_Event::getAll($moduleInstance);
		if(!$events) return;
	...
	function export_Actions($moduleInstance) {
		...
		global $adb;
		$result = $adb->pquery('SELECT distinct(actionname) FROM mycrm_profile2utility, mycrm_actionmapping
			WHERE mycrm_profile2utility.activityid=mycrm_actionmapping.actionid and tabid=?', Array($moduleInstance->id));
	...
	function export_RelatedLists($moduleInstance) {
		...
		global $adb;
		$result = $adb->pquery("SELECT * FROM mycrm_relatedlists WHERE tabid = ?", Array($moduleInstance->id));
		if($adb->num_rows($result)) {
			...
			for($index = 0; $index < $adb->num_rows($result); ++$index) {
				...
				$relModuleInstance = Mycrm_Module::getInstance($row['related_tabid']);
	...
	function export_CronTasks($moduleInstance){
        $cronTasks = Mycrm_Cron::listAllInstancesByModule($moduleInstance->name);
        $this->openNode('crons');
