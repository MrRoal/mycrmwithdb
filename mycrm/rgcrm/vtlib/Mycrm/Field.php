...
include_once('vtlib/Mycrm/FieldBasic.php');
require_once 'includes/runtime/Cache.php';

/**
 * Provides APIs to control mycrm CRM Field
 * @package vtlib
 */
class Mycrm_Field extends Mycrm_FieldBasic {
	...
	function __getPicklistUniqueId() {
		global $adb;
		return $adb->getUniqueID('mycrm_picklist');
	}
	...
	function setPicklistValues($values) {
		...
		$picklist_table = 'mycrm_'.$this->name;
		$picklist_idcol = $this->name.'id';
		if(!Mycrm_Utils::CheckTable($picklist_table)) {
			...
			$adb->pquery("INSERT INTO mycrm_picklist (picklistid,name) VALUES(?,?)",Array($new_picklistid, $this->name));
			self::log("Creating table $picklist_table ... DONE");
		} else {
                        $picklistResult = $adb->pquery("SELECT picklistid FROM mycrm_picklist WHERE name=?", Array($this->name));
		...
		foreach($values as $value) {
			...
			$adb->pquery("INSERT INTO mycrm_role2picklist(roleid, picklistvalueid, picklistid, sortid) SELECT roleid,
				$new_picklistvalueid, $new_picklistid, $sortid FROM mycrm_role", array());
		}
	}
	 ...
	 *
	 * @internal Creates table mycrm_fieldmodulerel if it does not exists
	 */
	function setRelatedModules($moduleNames) {

		// We need to create core table to capture the relation between the field and modules.
		if(!Mycrm_Utils::CheckTable('mycrm_fieldmodulerel')) {
			Mycrm_Utils::CreateTable(
				'mycrm_fieldmodulerel',
		...
		foreach($moduleNames as $relmodule) {
			$checkres = $adb->pquery('SELECT * FROM mycrm_fieldmodulerel WHERE fieldid=? AND module=? AND relmodule=?',
				Array($this->id, $this->getModuleName(), $relmodule));

			// If relation already exist continue
			if($adb->num_rows($checkres)) continue;

			$adb->pquery('INSERT INTO mycrm_fieldmodulerel(fieldid, module, relmodule) VALUES(?,?,?)',
				Array($this->id, $this->getModuleName(), $relmodule));
	...
	function unsetRelatedModules($moduleNames) {
		global $adb;
		foreach($moduleNames as $relmodule) {
			$adb->pquery('DELETE FROM mycrm_fieldmodulerel WHERE fieldid=? AND module=? AND relmodule = ?',
				Array($this->id, $this->getModuleName(), $relmodule));

			Mycrm_Utils::Log("Unsetting $this->name relation with $relmodule ... DONE");
		}
		return true;
	}

	/**
	 * Get Mycrm_Field instance by fieldid or fieldname
	 * @param mixed fieldid or fieldname
	 * @param Mycrm_Module Instance of the module if fieldname is used
	 */
	static function getInstance($value, $moduleInstance=false) {
		global $adb;
		$instance = false;
		$data = Mycrm_Functions::getModuleFieldInfo($moduleInstance->id, $value);
		if ($data) {
            $instance = new self();
	...
	/**
	 * Get Mycrm_Field instances related to block
	 * @param Mycrm_Block Instnace of block to use
	 * @param Mycrm_Module Instance of module to which block is associated
	 */
	 static function getAllForBlock($blockInstance, $moduleInstance=false) {
		...
		if($cache->getBlockFields($blockInstance->id,$moduleInstance->id)){
			...
			$queryParams = false;
			if($moduleInstance) {
				$query = "SELECT * FROM mycrm_field WHERE block=? AND tabid=? ORDER BY sequence";
				$queryParams = Array($blockInstance->id, $moduleInstance->id);
			} else {
				$query = "SELECT * FROM mycrm_field WHERE block=? ORDER BY sequence";
	...
	/**
	 * Get Mycrm_Field instances related to module
	...
	static function getAllForModule($moduleInstance) {
		...
		$query = "SELECT * FROM mycrm_field left join mycrm_blocks on mycrm_field.block=mycrm_blocks.blockid WHERE mycrm_field.tabid=? ORDER BY mycrm_blocks.sequence,mycrm_field.sequence";
	...
	static function deleteForModule($moduleInstance) {
		global $adb;
		$adb->pquery("DELETE FROM mycrm_field WHERE tabid=?", Array($moduleInstance->id));
