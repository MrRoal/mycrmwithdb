...
class Mycrm_Functions {
	...
	static function getTranslatedString($str, $module = '') {
		return Mycrm_Language_Handler::getTranslatedString($str, $module);
	}
	...
	static function userCurrencyId($userid) {
		...
		if (!isset(self::$userIdCurrencyIdCache[$userid])) {
			$result = $adb->pquery('SELECT id,currency_id FROM mycrm_users', array());
	...
	protected static function getCurrencyInfo($currencyid) {
		...
		if (!isset(self::$currencyInfoCache[$currencyid])) {
			$result = $adb->pquery('SELECT * FROM mycrm_currency_info', array());
			while ($row = $adb->fetch_array($result)) {
	...
	static function getCurrencyName($currencyid, $show_symbol = true) {
		...
		if ($show_symbol) {
			return sprintf("%s : %s", Mycrm_Deprecated::getTranslatedCurrencyString($currencyInfo['currency_name']), $currencyInfo['currency_symbol']);
		}
	...
	protected static function getBasicModuleInfo($mixed) {
		...
		if ($reload) {
			global $adb;
			$result = $adb->pquery('SELECT tabid, name, ownedby FROM mycrm_tab', array());
			while ($row = $adb->fetch_array($result)) {
	...
	static function getModuleData($mixed) {
		...
		if ($reload) {
			global $adb;
			$result = $adb->pquery('SELECT * FROM mycrm_tab', array());
			while ($row = $adb->fetch_array($result)) {
				self::$moduleIdNameCache[$row['tabid']] = $row;
				self::$moduleNameIdCache[$row['name']]  = $row;
	...
	static function getEntityModuleInfo($mixed) {
		...
		if ($name && !isset(self::$moduleEntityCache[$name])) {
			global $adb;
			$result = $adb->pquery('SELECT fieldname,modulename,tablename,entityidfield,entityidcolumn from mycrm_entityname', array());
			while ($row = $adb->fetch_array($result)) {
	...
	protected static function getCRMRecordMetadata($mixedid) {
		...
		if ($missing) {
			$sql = sprintf("SELECT crmid, setype, label FROM mycrm_crmentity WHERE %s", implode(' OR ', array_fill(0, count($missing), 'crmid=?')));
	...
	static function updateCRMRecordLabel($module, $id, $label) {
		...
		if ($labelInfo) {
			...
			$adb->pquery('UPDATE mycrm_crmentity SET label=? WHERE crmid=?', array($label, $id));
	...
	static function computeCRMRecordLabels($module, $ids) {
		...
		if ($module) {
			...
			if ($ids) {
				...
				if ($module == 'Groups') {
					$metainfo = array('tablename' => 'mycrm_groups','entityidfield' => 'groupid','fieldname' => 'groupname');
				} else if ($module == 'DocumentFolders') { 
                                        $metainfo = array('tablename' => 'mycrm_attachmentsfolder','entityidfield' => 'folderid','fieldname' => 'foldername'); 
                                } else {
	...
	static function getGroupName($id) {
		...
		if (!self::$groupIdNameCache[$id]) {
			$result = $adb->pquery('SELECT groupid, groupname FROM mycrm_groups');
			while ($row = $adb->fetch_array($result)) {
	...
	static function getUserName($id) {
		...
		if (!self::$userIdNameCache[$id]) {
			$result = $adb->pquery('SELECT id, user_name FROM mycrm_users');
			while ($row = $adb->fetch_array($result)) {
	...
	static function getModuleFieldInfos($mixed) {
		...
		$module = $moduleInfo['name'];
                ...
                $no_of_fields = $adb->pquery('SELECT COUNT(fieldname) FROM mycrm_field WHERE tabid=?',array(self::getModuleId($module)));
		...
		if ($module && (!isset(self::$moduleFieldInfoByNameCache[$module]) || ((int)$fields_count != (int)$cached_fields_count))) {
			$result =
				($module == 'Calendar')?
				$adb->pquery('SELECT * FROM mycrm_field WHERE tabid=? OR tabid=?', array(9, 16)) :
				$adb->pquery('SELECT * FROM mycrm_field WHERE tabid=?', array(self::getModuleId($module)));
	...
	static function getModuleFieldInfoWithId($fieldid) {
		global $adb;
		$result = $adb->pquery('SELECT * FROM mycrm_field WHERE fieldid=?', array($fieldid));
	...
	static function getInventoryTermsAndCondition() {
		global $adb;
		$sql = "select tandc from mycrm_inventory_tandc";
	...
	static function getTicketComments($ticketid) {
		...
		$commentlist = '';
		$sql = "SELECT commentcontent FROM mycrm_modcomments WHERE related_to = ?";
	...
	static function getTagCloudView($id = "") {
		...
		if ($id == '') {
			...
			$query = "select visible from mycrm_homestuff where userid=? and stufftype='Tag Cloud'";
	...
	static function transformFieldTypeOfData($table_name, $column_name, $type_of_data) {
		...
		static $new_field_details = Array(
			...
			"mycrm_contactdetails:accountid" => "V",
			"mycrm_contactsubdetails:birthday" => "D",
			"mycrm_contactdetails:email" => "V",
			"mycrm_contactdetails:secondaryemail" => "V",
			//Potential Related Fields
			"mycrm_potential:campaignid" => "V",
			//Account Related Fields
			"mycrm_account:parentid" => "V",
			"mycrm_account:email1" => "V",
			"mycrm_account:email2" => "V",
			//Lead Related Fields
			"mycrm_leaddetails:email" => "V",
			"mycrm_leaddetails:secondaryemail" => "V",
			//Documents Related Fields
			"mycrm_senotesrel:crmid" => "V",
			//Calendar Related Fields
			"mycrm_seactivityrel:crmid" => "V",
			"mycrm_seactivityrel:contactid" => "V",
			"mycrm_recurringevents:recurringtype" => "V",
			//HelpDesk Related Fields
			"mycrm_troubletickets:parent_id" => "V",
			"mycrm_troubletickets:product_id" => "V",
			//Product Related Fields
			"mycrm_products:discontinued" => "C",
			"mycrm_products:vendor_id" => "V",
			"mycrm_products:parentid" => "V",
			//Faq Related Fields
			"mycrm_faq:product_id" => "V",
			//Vendor Related Fields
			"mycrm_vendor:email" => "V",
			//Quotes Related Fields
			"mycrm_quotes:potentialid" => "V",
			"mycrm_quotes:inventorymanager" => "V",
			"mycrm_quotes:accountid" => "V",
			//Purchase Order Related Fields
			"mycrm_purchaseorder:vendorid" => "V",
			"mycrm_purchaseorder:contactid" => "V",
			//SalesOrder Related Fields
			"mycrm_salesorder:potentialid" => "V",
			"mycrm_salesorder:quoteid" => "V",
			"mycrm_salesorder:contactid" => "V",
			"mycrm_salesorder:accountid" => "V",
			//Invoice Related Fields
			"mycrm_invoice:salesorderid" => "V",
			"mycrm_invoice:contactid" => "V",
			"mycrm_invoice:accountid" => "V",
			//Campaign Related Fields
			"mycrm_campaign:product_id" => "V",
			//Related List Entries(For Report Module)
			"mycrm_activityproductrel:activityid" => "V",
			"mycrm_activityproductrel:productid" => "V",
			"mycrm_campaigncontrel:campaignid" => "V",
			"mycrm_campaigncontrel:contactid" => "V",
			"mycrm_campaignleadrel:campaignid" => "V",
			"mycrm_campaignleadrel:leadid" => "V",
			"mycrm_cntactivityrel:contactid" => "V",
			"mycrm_cntactivityrel:activityid" => "V",
			"mycrm_contpotentialrel:contactid" => "V",
			"mycrm_contpotentialrel:potentialid" => "V",
			"mycrm_pricebookproductrel:pricebookid" => "V",
			"mycrm_pricebookproductrel:productid" => "V",
			"mycrm_seactivityrel:crmid" => "V",
			"mycrm_seactivityrel:activityid" => "V",
			"mycrm_senotesrel:crmid" => "V",
			"mycrm_senotesrel:notesid" => "V",
			"mycrm_seproductsrel:crmid" => "V",
			"mycrm_seproductsrel:productid" => "V",
			"mycrm_seticketsrel:crmid" => "V",
			"mycrm_seticketsrel:ticketid" => "V",
			"mycrm_vendorcontactrel:vendorid" => "V",
			"mycrm_vendorcontactrel:contactid" => "V",
			"mycrm_pricebook:currency_id" => "V",
		);
	...
	static function getPickListValuesFromTableForRole($tablename, $roleid) {
		global $adb;
		$query = "select $tablename from mycrm_$tablename inner join mycrm_role2picklist on mycrm_role2picklist.picklistvalueid = mycrm_$tablename.picklist_valueid where roleid=? and picklistid in (select picklistid from mycrm_picklist) order by sortid";
	...
	static function getActivityType($id) {
		global $adb;
		$query = "select activitytype from mycrm_activity where activityid=?";
		$res = $adb->pquery($query, array($id));
	...
	static function getInvoiceStatus($id) {
		global $adb;
		$result = $adb->pquery("SELECT invoicestatus FROM mycrm_invoice where invoiceid=?", array($id));
    ...
    */
    static function getUnitPrice($productid, $module='Products') {
        ...
        if($module == 'Services') {
            $query = "select unit_price from mycrm_service where serviceid=?";
        } else {
            $query = "select unit_price from mycrm_products where productid=?";
        }
