...
include_once 'vtlib/Mycrm/Package.php';
include_once 'includes/main/WebUI.php';

include_once 'include/Webservices/Utils.php';

class Mycrm_Tools_Console_Controller {
	const PROMPT_ANY      = 1;
	...
	protected function welcome() {
		if ($this->interactive) {
			echo "Welcome to Mycrm CRM Creator.\n";
	...
	protected function handleCreateModule() {
		$controller = new Mycrm_Tools_Console_ModuleController();
		$controller->setArguments($this->arguments, $this->interactive)->handle();
	}

	protected function handleCreateLanguage() {
		$controller = new Mycrm_Tools_Console_LanguageController();
		$controller->setArguments($this->arguments, $this->interactive)->handle();
	}

	protected function handleCreateTestLanguage() {
		$controller = new Mycrm_Tools_Console_TestLanguageController();
		$controller->setArguments($this->arguments, $this->interactive)->handle();
	}

	protected function handleCreateLayout() {
		$controller = new Mycrm_Tools_Console_LayoutController();
		$controller->setArguments($this->arguments, $this->interactive)->handle();
	}
	
	protected function handleImportModule() {
		$controller = new Mycrm_Tools_Console_ImportController();
		$controller->setArguments($this->arguments, $this->interactive)->handle();
	}
	
	protected function handleUpdateModule() {
		$controller = new Mycrm_Tools_Console_UpdateController();
		$controller->setArguments($this->arguments, $this->interactive)->handle();
	}
	
	protected function handleRemoveModule() {
		$controller = new Mycrm_Tools_Console_RemoveController();
		$controller->setArguments($this->arguments, $this->interactive)->handle();
	}
...
class Mycrm_Tools_Console_ModuleController extends Mycrm_Tools_Console_Controller {
	...
	public function find($name) {
		return Mycrm_Module::getInstance($name);
	}
	...
	protected function create($moduleInformation) {
		...
		/** Common fields that should be in every module, linked to mycrm CRM core table */
		$field2 = new Mycrm_Field();
		$field2->name = 'assigned_user_id';
		$field2->label = 'Assigned To';
		$field2->table = 'mycrm_crmentity';
	...
	protected function createFiles(Mycrm_Module $module, Mycrm_Field $entityField) {
...
class Mycrm_Tools_Console_LayoutController extends Mycrm_Tools_Console_Controller {
	...
	protected function create($layoutInformation) {
		...
		foreach ($files as $file) {
			$tplfolder = $layoutdir . "modules/Mycrm";
...
class Mycrm_Tools_Console_LanguageController extends Mycrm_Tools_Console_Controller {
	...
	protected function deploy($languageInformation) {
		...
		$check = $db->pquery('SELECT 1 FROM mycrm_language WHERE prefix=?', $languageInformation['prefix']);
		if ($check && $db->num_rows($check)) {
			;
		} else {
			$db->pquery('INSERT INTO mycrm_language (id,name,prefix,label,lastupdated,isdefault,active) VALUES(?,?,?,?,?,?,?)',
					array($db->getUniqueId('mycrm_language'), $languageInformation['label'], $languageInformation['prefix'],
							$languageInformation['label'], date('Y-m-d H:i:s'), 0, 1));
		}
	}
}

class Mycrm_Tools_Console_TestLanguageController extends Mycrm_Tools_Console_LanguageController {
...
class Mycrm_Tools_Console_ImportController extends Mycrm_Tools_Console_Controller {
...
class Mycrm_Tools_Console_UpdateController extends Mycrm_Tools_Console_Controller {
...
class Mycrm_Tools_Console_RemoveController extends Mycrm_Tools_Console_Controller {
...
if (php_sapi_name() == 'cli') {
	Mycrm_Tools_Console_Controller::run();
} else {
