...
require_once 'modules/com_mycrm_workflow/VTEventHandler.inc';
...
function CustomerCommentFromPortal($entityData) {
	...
	if($moduleName == 'HelpDesk' && !empty($customerWSId)) {
		...
		$result = $adb->pquery("SELECT email FROM mycrm_contactdetails WHERE contactid=?", array($customerId[0]));
...
function TicketOwnerComments($entityData) {
	...
	if($moduleName == 'HelpDesk') {
		...
		if(!empty($contactId)) {
			$result = $adb->pquery('SELECT email, emailoptout FROM mycrm_contactdetails WHERE contactid=?',
										array($contactId));
