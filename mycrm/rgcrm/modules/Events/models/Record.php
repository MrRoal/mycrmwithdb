...
class Events_Record_Model extends Calendar_Record_Model {
    ...
    public function getRelatedContactInfo() {
        ...
        foreach($contactIdList as $contactId) {
            $relatedContactInfo[] = array('name' => Mycrm_Util_Helper::getRecordName($contactId) ,'id' => $contactId);
        }
     ...
     public function getInvities() {
         $adb = PearDatabase::getInstance();
         $sql = "select mycrm_invitees.* from mycrm_invitees where activityid=?";
     ...
     public function getInviteUserMailData() {
            ...
            $cont_qry = "select * from mycrm_cntactivityrel where activityid=?";
            ...
            foreach($cont_id as $key=>$id) {
                if($id != '') {
                    $contact_name = Mycrm_Util_Helper::getRecordName($id);
			...
			$parentName = '';
			if($parentId != '') {
				$parentName = Mycrm_Util_Helper::getRecordName($parentId);
			}
