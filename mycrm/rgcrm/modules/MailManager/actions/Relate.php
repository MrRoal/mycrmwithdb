...
class MailManager_Relate_Action extends Mycrm_MailScannerAction {
	...
	public function __CreateNewEmail($mailrecord, $module, $linkfocus) {
		...
		$recordModel = Mycrm_Record_Model::getCleanInstance('Emails');
	...
	public function __SaveAttachements($mailrecord, $basemodule, $recordModel) {
		...
		foreach($mailrecord->_attachments as $filename=>$filecontent) {
			...
			if($issaved) {
				...
				$attachRes = $db->pquery("SELECT * FROM mycrm_attachments WHERE attachmentsid = ?", array($attachid));
	...
	public static function getSalesEntityInfo($crmid) {
		$db = PearDatabase::getInstance();
		$result = $db->pquery("SELECT setype FROM mycrm_crmentity WHERE crmid=? AND deleted=0", array($crmid));
	...
	public static function ws_modulename($modulewsid) {
		$db = PearDatabase::getInstance();
		$result = $db->pquery("SELECT name FROM mycrm_ws_entity WHERE id=?", array($modulewsid));
