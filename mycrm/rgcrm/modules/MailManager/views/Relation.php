...
class MailManager_Relation_View extends MailManager_Abstract_View {
	...
	public function process(Mycrm_Request $request) {
		...
		$response = new Mycrm_Response(true);
      ...
      if ($folder=="Sent"){
        $contacts=$request->get('_msendto');
			...
			if (!class_exists($quickCreateviewClassName)) {
				$quickCreateviewClassName = 'Mycrm_QuickCreateAjax_View';
			}
			...
			$recordModel = Mycrm_Record_Model::getCleanInstance($linkModule);
			...
			switch ($linkModule) {
				case 'Calendar' :   $activityType = $recordModel->get('activitytype');
									...
									$startTime = Mycrm_Time_UIType::getTimeValueWithSeconds($request->get('time_start'));
									$startDateTime = Mycrm_Datetime_UIType::getDBDateTimeValue($request->get('date_start')." ".$startTime);
									...
									$endDate = Mycrm_Date_UIType::getDBInsertedValue($request->get('due_date'));
									if ($activityType != 'Task') {
										$endTime = Mycrm_Time_UIType::getTimeValueWithSeconds($request->get('time_end'));
										$endDateTime = Mycrm_Datetime_UIType::getDBDateTimeValue($request->get('due_date')." ".$endTime);
	...
	public function setParentForHelpDesk($parent, $from) {
		...
		if(empty($parent)) {
			if(!empty($from)) {
				$parentInfo = MailManager::lookupMailInMycrm($from[0], $currentUserModel);
