...
class MailManager {
	...
	static function updateMailAssociation($mailuid, $emailid, $crmid) {
		global $adb;
		$adb->pquery("INSERT INTO mycrm_mailmanager_mailrel (mailuid, emailid, crmid) VALUES (?,?,?)", array($mailuid, $emailid, $crmid));
	}

	static function lookupMailInMycrm($searchTerm, $user) {
		...
		foreach($referenceModules as $referenceModule) {
			...
			if(!empty($searchFieldList) && !empty($referenceModuleEmailFields)) {
                ...
                if($referenceModule == 'Users' && !is_admin($user)){
                    ...
                    global $adb;
                    $where .= " AND mycrm_users.status='Active'";
                    $query = "select $searchFieldListString,id from mycrm_users $where";
	...
	static function lookupMailAssociation($mailuid) {
		...
		$result = $adb->pquery(
			"SELECT mycrm_mailmanager_mailrel.* FROM mycrm_mailmanager_mailrel INNER JOIN
			mycrm_crmentity ON mycrm_crmentity.crmid=mycrm_mailmanager_mailrel.crmid AND mycrm_crmentity.deleted=0
			AND mycrm_mailmanager_mailrel.mailuid=? LIMIT 1", array(decode_html($mailuid)));
