...
function Contacts_sendCustomerPortalLoginDetails($entityData){
	...
	if ($entityData->get('portal') == 'on' || $entityData->get('portal') == '1') {
		$sql = "SELECT id, user_name, user_password, isactive FROM mycrm_portalinfo WHERE id=?";
		...
		if($adb->num_rows($result) == 0){
			...
			if($email == $dbusername && $isactive == 1 && !$entityData->isNew()){
				...
				$sql = "UPDATE mycrm_portalinfo SET user_name=?, isactive=? WHERE id=?";
		...
		$enc_password = Mycrm_Functions::generateEncryptedPassword($password);
		if ($insert == true) {
			$sql = "INSERT INTO mycrm_portalinfo(id,user_name,user_password,cryptmode,type,isactive) VALUES(?,?,?,?,?,?)";
			$params = array($entityId, $email, $enc_password, 'CRYPT', 'C', 1);
			$adb->pquery($sql, $params);
		}
		if ($update == true && $portalChanged == true) {
			$sql = "UPDATE mycrm_portalinfo SET user_password=?, cryptmode=? WHERE id=?";
		...
		$sql = "UPDATE mycrm_portalinfo SET user_name=?,isactive=0 WHERE id=?";
