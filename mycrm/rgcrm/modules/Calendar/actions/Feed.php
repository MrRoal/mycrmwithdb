...
class Calendar_Feed_Action extends Mycrm_BasicAjax_Action {
	...
	protected function pullEvents($start, $end, &$result, $userid = false,$color = null,$textColor = 'white') {
		...
		$moduleModel = Mycrm_Module_Model::getInstance('Events');
		if($userid){
			$focus = new Users();
			$focus->id = $userid;
		...
		$query.= " AND mycrm_activity.activitytype NOT IN ('Emails','Task') AND ";
        $hideCompleted = $currentUser->get('hidecompletedevents');
        if($hideCompleted)
            $query.= "mycrm_activity.eventstatus != 'HELD' AND ";
        ...
        $query.= " AND mycrm_crmentity.smownerid IN (".  generateQuestionMarks($params).")";
	...
	protected function pullTasks($start, $end, &$result, $color = null,$textColor = 'white') {
		...
		$moduleModel = Mycrm_Module_Model::getInstance('Calendar');
		...
		$query.= " AND mycrm_activity.activitytype = 'Task' AND ";
        $currentUser = Users_Record_Model::getCurrentUserModel();
        $hideCompleted = $currentUser->get('hidecompletedevents');
        if($hideCompleted)
            $query.= "mycrm_activity.status != 'Completed' AND ";
		$query.= " ((date_start >= ? AND due_date < ?) OR ( due_date >= ?))";
                $params = array($start,$end,$start);
        $params = array_merge($params, $userAndGroupIds);
		$query.= " AND mycrm_crmentity.smownerid IN (".generateQuestionMarks($userAndGroupIds).")";
	...
	protected  function pullContactsByBirthday($start, $end, &$result, $color = null,$textColor = 'white') {
		...
		$query = "SELECT firstname,lastname,birthday,crmid FROM mycrm_contactdetails";
		$query.= " INNER JOIN mycrm_contactsubdetails ON mycrm_contactdetails.contactid = mycrm_contactsubdetails.contactsubscriptionid";
		$query.= " INNER JOIN mycrm_crmentity ON mycrm_contactdetails.contactid = mycrm_crmentity.crmid";
		$query.= " WHERE mycrm_crmentity.deleted=0 AND smownerid IN (".  generateQuestionMarks($userAndGroupIds) .") AND";
	...
	protected function pullProjects($start, $end, &$result, $color = null,$textColor = 'white') {
		...
		$query = "SELECT projectname, startdate, targetenddate, crmid FROM mycrm_project";
		$query.= " INNER JOIN mycrm_crmentity ON mycrm_project.projectid = mycrm_crmentity.crmid";
		$query.= " WHERE mycrm_crmentity.deleted=0 AND smownerid IN (". generateQuestionMarks($userAndGroupIds) .") AND ";
	...
	protected function pullProjectTasks($start, $end, &$result, $color = null,$textColor = 'white') {
		...
		$query = "SELECT projecttaskname, startdate, enddate, crmid FROM mycrm_projecttask";
		$query.= " INNER JOIN mycrm_crmentity ON mycrm_projecttask.projecttaskid = mycrm_crmentity.crmid";
		$query.= " WHERE mycrm_crmentity.deleted=0 AND ";
