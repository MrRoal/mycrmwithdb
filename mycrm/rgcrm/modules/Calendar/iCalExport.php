...
//$ical_query = "SELECT * FROM mycrm_activity WHERE (( STATUS != 'Completed' AND STATUS != 'Deferred' ) OR STATUS IS NULL)
//               AND (( eventstatus != 'Held' AND eventstatus != 'Not Held' ) OR eventstatus IS NULL )";

global $current_user,$adb,$default_timezone;
$filename = $_REQUEST['filename'];
$ical_query = "select mycrm_activity.*,mycrm_crmentity.description,mycrm_activity_reminder.reminder_time from mycrm_activity inner join mycrm_crmentity on mycrm_activity.activityid = mycrm_crmentity.crmid " .
	...
	" where mycrm_crmentity.deleted = 0 and mycrm_crmentity.smownerid = " . $current_user->id . 
