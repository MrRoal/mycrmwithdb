...
class Calendar_Import_View extends Mycrm_Import_View {
	 ...
	 */
	public function import(Mycrm_Request $request) {
	 ...
	 * @param Mycrm_Request $request
	 */
	public function importResult(Mycrm_Request $request) {
		...
		if (Import_Utils_Helper::validateFileUpload($request)) {
			...
			$calendarModel = Mycrm_Module_Model::getInstance($moduleName);
	 ...
	 * @param Mycrm_Request $request
	 */
	public function undoImport(Mycrm_Request $request) {
		$currentUserModel = Users_Record_Model::getCurrentUserModel();
		$moduleName = $request->getModule();
