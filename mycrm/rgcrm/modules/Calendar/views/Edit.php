...
Class Calendar_Edit_View extends Mycrm_Edit_View {
	...
	function Events($request, $moduleName) {
		...
		$eventModule = Mycrm_Module_Model::getInstance($moduleName);
		...
		foreach($requestFieldList as $fieldName=>$fieldValue){
            ...
            if (empty($record) && ($fieldName == 'date_start' || $fieldName == 'due_date') && !empty($fieldValue)) { 
                if($fieldName == 'date_start'){
                    $startTime = Mycrm_Time_UIType::getTimeValueWithSeconds($requestFieldList['time_start']);
                    $startDateTime = Mycrm_Datetime_UIType::getDBDateTimeValue($fieldValue." ".$startTime);
                    list($startDate, $startTime) = explode(' ', $startDateTime);
                    $fieldValue = Mycrm_Date_UIType::getDisplayDateValue($startDate);
                }else{
                    $endTime = Mycrm_Time_UIType::getTimeValueWithSeconds($requestFieldList['time_end']);
                    $endDateTime = Mycrm_Datetime_UIType::getDBDateTimeValue($fieldValue." ".$endTime);
                    list($endDate, $endTime) = explode(' ', $endDateTime);
                    $fieldValue = Mycrm_Date_UIType::getDisplayDateValue($endDate);
                }
            }
		...
		$recordStructureInstance = Mycrm_RecordStructure_Model::getInstanceFromRecordModel($recordModel,
									Mycrm_RecordStructure_Model::RECORD_STRUCTURE_MODE_EDIT);
		...
		$viewer->assign('TOMORROWDATE', Mycrm_Date_UIType::getDisplayDateValue(date('Y-m-d', time()+86400)));
		...
		if(!empty($requestContactIdValue)) {
			$existingRelatedContacts[] = array('name' => Mycrm_Util_Helper::getRecordName($requestContactIdValue) ,'id' => $requestContactIdValue);
		}
