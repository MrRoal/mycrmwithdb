...
// functions added for group calendar	-Jaguar
	...
	function status_availability($owner,$userid,$activity_id,$avail_date,$activity_start_time,$activity_end_time)	
	{
		...
		if( $owner != $userid)
		{
			...
			$usr_query="select activityid,mycrm_activity.date_start,mycrm_activity.due_date, mycrm_activity.time_start,mycrm_activity.duration_hours,mycrm_activity.duration_minutes,mycrm_crmentity.smownerid from mycrm_activity,mycrm_crmentity where mycrm_crmentity.crmid=mycrm_activity.activityid and ('".$avail_date."' like date_start) and mycrm_crmentity.smownerid=? and mycrm_activity.activityid !=?  and mycrm_crmentity.deleted=0";
		}
		else
		{
			$usr_query="select activityid,mycrm_activity.date_start,mycrm_activity.due_date, mycrm_activity.time_start,mycrm_activity.duration_hours,mycrm_activity.duration_minutes,mycrm_crmentity.smownerid from mycrm_activity,mycrm_crmentity where mycrm_crmentity.crmid=mycrm_activity.activityid and ('".$avail_date."' like date_start) and mycrm_crmentity.smownerid=? and mycrm_activity.activityid !=? and mycrm_crmentity.deleted=0";
		...
		if($avail_flag!="true")
		{
			$recur_query="SELECT mycrm_activity.activityid, mycrm_activity.time_start, mycrm_activity.duration_hours, mycrm_activity.duration_minutes , mycrm_crmentity.smownerid, mycrm_recurringevents.recurringid, mycrm_recurringevents.recurringdate as date_start from mycrm_activity inner join mycrm_crmentity on mycrm_activity.activityid = mycrm_crmentity.crmid inner join mycrm_recurringevents on mycrm_activity.activityid=mycrm_recurringevents.activityid where ('".$avail_date."' like mycrm_recurringevents.recurringdate) and mycrm_crmentity.smownerid=? and mycrm_activity.activityid !=? and mycrm_crmentity.deleted=0";
	 	...
	 	if($avail_flag == "true")
                {
                        $availability=' <IMG SRC="' . mycrm_imageurl('busy.gif', $theme). '">';
                }
                else
                {
                        $availability=' <IMG SRC="' . mycrm_imageurl('free.gif', $theme). '">';
                }
