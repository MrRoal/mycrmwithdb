...
class Calendar_Record_Model extends Mycrm_Record_Model {
...
/**
	...
	function save() {
		...
		$_REQUEST['time_start'] = Mycrm_Time_UIType::getTimeValueWithSeconds($_REQUEST['time_start']);
		$_REQUEST['time_end'] = Mycrm_Time_UIType::getTimeValueWithSeconds($_REQUEST['time_end']);
		parent::save();
	}
	...
	public function getRecurringObject() {
		...
		$query = 'SELECT mycrm_recurringevents.*, mycrm_activity.date_start, mycrm_activity.time_start, mycrm_activity.due_date, mycrm_activity.time_end FROM mycrm_recurringevents
					INNER JOIN mycrm_activity ON mycrm_activity.activityid = mycrm_recurringevents.activityid
					WHERE mycrm_recurringevents.activityid = ?';
	...
	public function updateReminderStatus($status=1) {
		$db = PearDatabase::getInstance();
		$db->pquery("UPDATE mycrm_activity_reminder_popup set status = ? where recordid = ?", array($status, $this->getId()));
