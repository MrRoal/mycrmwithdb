...
class Appointment
{
	...
	function readAppointment($userid, &$from_datetime, &$to_datetime, $view)
	{
		...
		$and = "AND (
					(
						...
						AND mycrm_recurringevents.activityid is NULL
					)
				OR (
						(CAST(CONCAT(mycrm_recurringevents.recurringdate,' ',time_start) AS DATETIME) >= ?
							AND CAST(CONCAT(mycrm_recurringevents.recurringdate,' ',time_start) AS DATETIME) <= ?)
						OR	(CAST(CONCAT(due_date,' ',time_end) AS DATETIME) >= ? AND CAST(CONCAT(due_date,' ',time_end) AS DATETIME) <= ?)
						OR	(CAST(CONCAT(mycrm_recurringevents.recurringdate,' ',time_start) AS DATETIME) <= ?
		...
		$userNameSql = getSqlForNameInDisplayFormat(array('first_name'=>
        	...
        	$q= "select mycrm_activity.*, mycrm_crmentity.*,
					...
					FROM mycrm_activity
						...
						left join mycrm_recurringevents on mycrm_activity.activityid=mycrm_recurringevents.activityid
					...
					WHERE mycrm_crmentity.deleted = 0 and mycrm_activity.activitytype not in ('Emails','Task') $and ";
        ...
        $q .= " AND mycrm_recurringevents.activityid is NULL ";
        $q .= " group by mycrm_activity.activityid ORDER by mycrm_activity.date_start,mycrm_activity.time_start";
		...
		$q = "SELECT mycrm_activity.*, mycrm_crmentity.*, case when (mycrm_users.user_name not like '') then $userNameSql else mycrm_groups.groupname end as user_name , mycrm_recurringevents.recurringid, mycrm_recurringevents.recurringdate as date_start ,mycrm_recurringevents.recurringtype,mycrm_groups.groupname from mycrm_activity inner join mycrm_crmentity on mycrm_activity.activityid = mycrm_crmentity.crmid inner join mycrm_recurringevents on mycrm_activity.activityid=mycrm_recurringevents.activityid left join mycrm_groups on mycrm_groups.groupid = mycrm_crmentity.smownerid LEFT JOIN mycrm_users ON mycrm_users.id = mycrm_crmentity.smownerid";
		$q .= getNonAdminAccessControlQuery('Calendar',$current_user);
        $q.=" where mycrm_crmentity.deleted = 0 and mycrm_activity.activitytype not in ('Emails','Task') AND (cast(concat(recurringdate, ' ', time_start) as datetime) between ? and ?) ";
        ...
        $q .= " ORDER by mycrm_recurringevents.recurringid";
		$r = $adb->pquery($q, $params);
        $n = $adb->getRowCount($r);
        $a = 0;
		while ( $a < $n )
	...
	/** To read and set the events value in Appointment Obj
          * @param $act_array -- The mycrm_activity array :: Type Array
          * @param $view -- The calendar view :: Type String
         */
	function readResult($act_array, $view)
	{
		...
		if(!is_admin($current_user))
		{
			if($act_array["smownerid"]!=0 && $act_array["smownerid"] != $current_user->id && $act_array["visibility"] == "Public"){
				$que = "select * from mycrm_sharedcalendar where sharedid=? and userid=?";
...
/** To two array values
  * @param $a -- The mycrm_activity array :: Type Array
  * @param $b -- The mycrm_activity array :: Type Array
		...
		if($is_Admin == 'off' && $fldname != '')
			{
				...
				$sql="select * from mycrm_$fldname where $fldname=?";
				...
				if ($picklistvalueid != null) {
					$pick_query="select * from mycrm_role2picklist where picklistvalueid=$picklistvalueid and roleid in (". generateQuestionMarks($roleids) .")";
