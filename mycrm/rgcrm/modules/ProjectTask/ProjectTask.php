...
class ProjectTask extends CRMEntity {
    ...
    var $table_name = 'mycrm_projecttask';
     ...
     */
    var $customFieldTable = Array('mycrm_projecttaskcf', 'projecttaskid');

    /**
     * Mandatory for Saving, Include tables related to this module.
     */
    var $tab_name = Array('mycrm_crmentity', 'mycrm_projecttask', 'mycrm_projecttaskcf');
    ...
    var $tab_name_index = Array(
        ...
        'mycrm_projecttask'   => 'projecttaskid',
        'mycrm_projecttaskcf' => 'projecttaskid');
   ...
   function save_module($module) {
    ...
    function vtlib_handler($modulename, $event_type) {
        ...
        if($event_type == 'module.postinstall') {
			$projectTaskResult = $adb->pquery('SELECT tabid FROM mycrm_tab WHERE name=?', array('ProjectTask'));
			...
			if(getTabid('CustomerPortal')) {
				$checkAlreadyExists = $adb->pquery('SELECT 1 FROM mycrm_customerportal_tabs WHERE tabid=?', array($projecttaskTabid));
				if($checkAlreadyExists && $adb->num_rows($checkAlreadyExists) < 1) {
					...
					$adb->query("INSERT INTO mycrm_customerportal_tabs(tabid,visible,sequence) VALUES ($projecttaskTabid,1,$nextSequence)");
					$adb->query("INSERT INTO mycrm_customerportal_prefs(tabid,prefkey,prefvalue) VALUES ($projecttaskTabid,'showrelatedinfo',1)");
				}
			}
			...
			if (!($adb->num_rows($result))) {
				...
				$adb->pquery("INSERT INTO mycrm_modentity_num values(?,?,?,?,?,?)", array($adb->getUniqueId("mycrm_modentity_num"), $modulename, 'PT', 1, 1, 1));
			}
