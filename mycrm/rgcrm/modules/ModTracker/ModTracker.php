...
class ModTracker {
	...
	*/	
	function vtlib_handler($moduleName, $eventType) {
		...
		$modtrackerModule = Mycrm_Module::getInstance($currentModule);
        $otherModuleNames = $this->getModTrackerEnabledModules();
 		...
 		if($eventType == 'module.postinstall') {
			...
			$seq_res = $adb->pquery("SELECT max(sequence) AS max_seq FROM mycrm_settings_field WHERE blockid = ?", array($blockid));
			if ($adb->num_rows($seq_res) > 0) {
				$cur_seq = $adb->query_result($seq_res, 0, 'max_seq');
				if ($cur_seq != null)	$seq = $cur_seq + 1;
			}

			$adb->pquery('INSERT INTO mycrm_settings_field(fieldid, blockid, name, iconpath, description, linkto, sequence)
    ...
    */
    function getModTrackerEnabledModules() {
        global $adb;
        $moduleResult = $adb->pquery('SELECT * FROM mycrm_modtracker_tabs', array());
	...
	static function disableTrackingForModule($tabid){
		...
		if(!self::isModulePresent($tabid)){
				$res=$adb->pquery("INSERT INTO mycrm_modtracker_tabs VALUES(?,?)",array($tabid,0));
				self::updateCache($tabid,0);
		} else{
				$updatevisibility = $adb->pquery("UPDATE mycrm_modtracker_tabs SET visible = 0 WHERE tabid = ?", array($tabid));
				self::updateCache($tabid,0);
		}
		if(self::isModtrackerLinkPresent($tabid)) {
			$moduleInstance=Mycrm_Module::getInstance($tabid);
	...
	static function enableTrackingForModule($tabid){
		global $adb;
		if(!self::isModulePresent($tabid)){
			$res=$adb->pquery("INSERT INTO mycrm_modtracker_tabs VALUES(?,?)",array($tabid,1));
			self::updateCache($tabid,1);
		} else{
			$updatevisibility = $adb->pquery("UPDATE mycrm_modtracker_tabs SET visible = 1 WHERE tabid = ?", array($tabid));
			self::updateCache($tabid,1);
		}
		if(!self::isModTrackerLinkPresent($tabid)){
			$moduleInstance=Mycrm_Module::getInstance($tabid);
	...
	static function isTrackingEnabledForModule($modulename){
		...
		if(!self::getVisibilityForModule($tabid) || self::getVisibilityForModule($tabid) !== 0) {
			$query = $adb->pquery("SELECT * FROM mycrm_modtracker_tabs WHERE mycrm_modtracker_tabs.visible = 1
								   AND mycrm_modtracker_tabs.tabid=?", array($tabid));
	...
	static function isModulePresent($tabid){
		...
		if(!self::checkModuleInModTrackerCache($tabid)){
			$query=$adb->pquery("SELECT * FROM mycrm_modtracker_tabs WHERE tabid = ?",array($tabid));
			$rows = $adb->num_rows($query);
			if($rows){
	...
	static function isModtrackerLinkPresent($tabid){
		global $adb;
		$query1=$adb->pquery("SELECT * FROM mycrm_links WHERE linktype='DETAILVIEWBASIC' AND
    ...
    function getChangedRecords($uniqueId, $mtime, $limit = 100) {
        ...
        $query = "SELECT id, module, modifiedtime, mycrm_crmentity.crmid, smownerid, mycrm_modtracker_basic.status
                FROM mycrm_modtracker_basic
                INNER JOIN mycrm_crmentity ON mycrm_modtracker_basic.crmid = mycrm_crmentity.crmid
                    AND mycrm_modtracker_basic.changedon = mycrm_crmentity.modifiedtime
        ...
        $moreQuery = "SELECT * FROM mycrm_modtracker_basic WHERE id > ? AND changedon >= ? AND module
    ...
    static function getRecordFieldChanges($crmid, $time, $decodeHTML = false) {
        ...
        $fieldResult = $adb->pquery('SELECT * FROM mycrm_modtracker_detail
                        INNER JOIN mycrm_modtracker_basic ON mycrm_modtracker_basic.id = mycrm_modtracker_detail.id
    ...
    static function trackRelation($sourceModule, $sourceId, $targetModule, $targetId, $type) {
        ...
        $adb->pquery('INSERT INTO mycrm_modtracker_basic(id, crmid, module, whodid, changedon, status) VALUES(?,?,?,?,?,?)',
                array($id , $sourceId, $sourceModule, $current_user->id, $currentTime, $type));

        $adb->pquery('INSERT INTO mycrm_modtracker_relations(id, targetmodule, targetid, changedon)
