...
<?php
	...
	function checkModuleWorkflow($modulename) {
		...
		$query = "SELECT name FROM mycrm_tab WHERE name not in (" . generateQuestionMarks($modules_not_supported) . ") AND isentitytype=1 AND presence = 0 AND tabid = ?";
	...
	function vtGetModules($adb) {
		...
		$sql = "select distinct mycrm_field.tabid, name
			...
			where mycrm_tab.name not in(" . generateQuestionMarks($modules_not_supported) . ") and mycrm_tab.isentitytype=1 and mycrm_tab.presence in (0,2) ";
