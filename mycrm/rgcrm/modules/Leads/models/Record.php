...
class Leads_Record_Model extends Mycrm_Record_Model {
	...
	function getAccountFieldsForLeadConvert() {
		...
		if ($moduleModel->isActive()) {
            ...
            foreach($complusoryFields as $complusoryField) {
                $fieldModel = Mycrm_Field_Model::getInstance($complusoryField, $moduleModel);
	...
	function getConvertLeadMappedField($fieldName, $moduleName) {
		...
		if (!$mappingFields) {
			...
			$result = $db->pquery('SELECT * FROM mycrm_convertleadmapping', array());
			$numOfRows = $db->num_rows($result);

			$accountInstance = Mycrm_Module_Model::getInstance('Accounts');
			$accountFieldInstances = $accountInstance->getFieldsById();

			$contactInstance = Mycrm_Module_Model::getInstance('Contacts');
			$contactFieldInstances = $contactInstance->getFieldsById();

			$potentialInstance = Mycrm_Module_Model::getInstance('Potentials');
			$potentialFieldInstances = $potentialInstance->getFieldsById();

			$leadInstance = Mycrm_Module_Model::getInstance('Leads');
