...
class Emails_Record_Model extends Mycrm_Record_Model {
	...
	public function getDetailViewUrl() {
		...
		return 'Javascript:Mycrm_Index_Js.showEmailPreview("'.$this->getId().'","'.$parentId.'")';
	}
	...
	public function send() {
		...
		foreach($toEmailInfo as $id => $emails) {
			...
			if ($parentModule) {
			$currentLanguage = Mycrm_Language_Handler::getLanguage();
			$moduleLanguageStrings = Mycrm_Language_Handler::getModuleStringsFromFile($currentLanguage,$parentModule);
	...
	function getFromEmailAddress() {
		...
		$fromEmail = false;
		$result = $db->pquery('SELECT from_email_field FROM mycrm_systems WHERE server_type=?', array('email'));
	...
	function getAttachmentDetails() {
		...
		$attachmentRes = $db->pquery("SELECT * FROM mycrm_attachments
						INNER JOIN mycrm_seattachmentsrel ON mycrm_attachments.attachmentsid = mycrm_seattachmentsrel.attachmentsid
						WHERE mycrm_seattachmentsrel.crmid = ?", array($this->getId()));
	...
	public function getRelatedDocuments() {
		...
		$documentRes = $db->pquery("SELECT * FROM mycrm_senotesrel
						INNER JOIN mycrm_crmentity ON mycrm_senotesrel.notesid = mycrm_crmentity.crmid AND mycrm_senotesrel.crmid = ?
						INNER JOIN mycrm_notes ON mycrm_notes.notesid = mycrm_senotesrel.notesid
						INNER JOIN mycrm_seattachmentsrel ON mycrm_seattachmentsrel.crmid = mycrm_notes.notesid
						INNER JOIN mycrm_attachments ON mycrm_attachments.attachmentsid = mycrm_seattachmentsrel.attachmentsid
						WHERE mycrm_crmentity.deleted = 0", array($this->getId()));
	...
	public function saveDocumentDetails() {
		...
		for ($i=0; $i<$count; $i++) {
			$db->pquery("INSERT INTO mycrm_senotesrel(crmid, notesid) VALUES(?, ?)", array($record, $documentIds[$i]));
		}
	}
    ...
    public function deleteDocumentLink($idList = array()){
        $db = PearDatabase::getInstance();
        $query =  'DELETE FROM mycrm_senotesrel where crmid=?';
    ...
    public function deleteAttachment($emailAttachmentDetails = array()) {
        ...
        $db->pquery('UPDATE mycrm_crmentity SET deleted=0 WHERE crmid IN('.generateQuestionMarks($attachmentIdList).')',$attachmentIdList);
        $db->pquery('DELETE FROM mycrm_attachments WHERE attachmentsid IN('.generateQuestionMarks($attachmentIdList).')',$attachmentIdList);
        $db->pquery('DELETE FROM mycrm_seattachmentsrel WHERE crmid=? and attachmentsid IN('.generateQuestionMarks($attachmentIdList).')',
	...
	public function checkUploadSize($documentIds = false) {
		...
		if (!empty ($documentIds)) {
			$count = count($documentIds);
			for ($i=0; $i<$count; $i++) {
				$documentRecordModel = Mycrm_Record_Model::getInstanceById($documentIds[$i], 'Documents');
	...
	public function updateTrackDetails($parentId) {
		...
		$db->pquery("INSERT INTO mycrm_email_access(crmid, mailid, accessdate, accesstime) VALUES(?, ?, ?, ?)", array($parentId, $recordId, date('Y-m-d'), date('Y-m-d H:i:s')));

		$result = $db->pquery("SELECT 1 FROM mycrm_email_track WHERE crmid = ? AND mailid = ?", array($parentId, $recordId));
		if ($db->num_rows($result)>0) {
			$db->pquery("UPDATE mycrm_email_track SET access_count = access_count+1 WHERE crmid = ? AND mailid = ?", array($parentId, $recordId));
		} else {
			$db->pquery("INSERT INTO mycrm_email_track(crmid, mailid, access_count) values(?, ?, ?)", array($parentId, $recordId, 1));
		}
	}
	...
	public function getAccessCountValue($parentId) {
		...
		$result = $db->pquery("SELECT access_count FROM mycrm_email_track WHERE crmid = ? AND mailid = ?", array($parentId, $this->getId()));
	...
	public function isSentMail(){
		if(!array_key_exists('email_flag', $this->getData())){
			$db = PearDatabase::getInstance();
			$query = 'SELECT email_flag FROM mycrm_emaildetails WHERE emailid=?';
	...
	function getEntityType($id) {
		...
		if (!empty($id)) {
			...
			if(!in_array($relatedModule, $emailRelatedModules)){
				$sql = 'SELECT id FROM mycrm_users WHERE id=?';
				$result = $db->pquery($sql, array($id));
				if($db->num_rows($result) > 0){
					$relatedModule = 'Users';
				}
