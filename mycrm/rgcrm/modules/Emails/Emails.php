 ...
 ********************************************************************************/
/*********************************************************************************
 * $Header: /advent/projects/wesat/mycrm_crm/sugarcrm/modules/Emails/Emails.php,v 1.41 2005/04/28 08:11:21 rank Exp $
...
class Emails extends CRMEntity {
	...
	var $rel_users_table = "mycrm_salesmanactivityrel";
	var $rel_contacts_table = "mycrm_cntactivityrel";
	var $rel_serel_table = "mycrm_seactivityrel";
	var $tab_name = Array('mycrm_crmentity', 'mycrm_activity', 'mycrm_emaildetails');
	var $tab_name_index = Array('mycrm_crmentity' => 'crmid', 'mycrm_activity' => 'activityid',
		'mycrm_seactivityrel' => 'activityid', 'mycrm_cntactivityrel' => 'activityid', 'mycrm_email_track' => 'mailid', 'mycrm_emaildetails' => 'emailid');
	...
	function save_module($module) {
		...
		//modified by Richie as raju's implementation broke the feature for addition of webmail to mycrm_crmentity.need to be more careful in future while integrating code
		if ($_REQUEST['module'] == "Emails" && $_REQUEST['smodule'] != 'webmails' && (!$this->plugin_save)) {
			...
			if ($_REQUEST['module'] != 'Emails' && $_REQUEST['module'] != 'Webmails') {
				if (!$parentid) {
					$parentid = $adb->getUniqueID('mycrm_seactivityrel');
				}
				$mysql = 'insert into mycrm_seactivityrel values(?,?)';
				...
				for ($i = 0; $i < (count($myids) - 1); $i++) {
					...
					$mycrmid = $realid[0];
					//added to handle the relationship of emails with mycrm_users
					if ($realid[1] == -1) {
						$del_q = 'delete from mycrm_salesmanactivityrel where smid=? and activityid=?';
						$adb->pquery($del_q, array($mycrmid, $actid));
						$mysql = 'insert into mycrm_salesmanactivityrel values(?,?)';
					} else {
						$del_q = 'delete from mycrm_seactivityrel where crmid=? and activityid=?';
						$adb->pquery($del_q, array($mycrmid, $actid));
						$mysql = 'insert into mycrm_seactivityrel values(?,?)';
					}
			...
			if (isset($this->column_fields['parent_id']) && $this->column_fields['parent_id'] != '') {
				$adb->pquery("DELETE FROM mycrm_seactivityrel WHERE crmid = ? AND activityid = ? ",
						array($this->column_fields['parent_id'], $this->id));
				//$this->insertIntoEntityTable('mycrm_seactivityrel', $module);
				$sql = 'insert into mycrm_seactivityrel values(?,?)';
	...
	function insertIntoAttachment($id, $module) {
		...
		if ($module == 'Emails' && isset($_REQUEST['att_id_list']) && $_REQUEST['att_id_list'] != '') {
			...
			if ($id_cnt != 0) {
				for ($i = 0; $i < $id_cnt; $i++) {
					$sql_rel = 'insert into mycrm_seattachmentsrel values(?,?)';
	...
	function get_contacts($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = 'select mycrm_contactdetails.accountid, mycrm_contactdetails.contactid, mycrm_contactdetails.firstname,mycrm_contactdetails.lastname, mycrm_contactdetails.department, mycrm_contactdetails.title, mycrm_contactdetails.email, mycrm_contactdetails.phone, mycrm_contactdetails.emailoptout, mycrm_crmentity.crmid, mycrm_crmentity.smownerid, mycrm_crmentity.modifiedtime from mycrm_contactdetails inner join mycrm_cntactivityrel on mycrm_cntactivityrel.contactid=mycrm_contactdetails.contactid inner join mycrm_crmentity on mycrm_crmentity.crmid = mycrm_contactdetails.contactid left join mycrm_groups on mycrm_groups.groupid=mycrm_crmentity.smownerid where mycrm_cntactivityrel.activityid=' . $adb->quote($id) . ' and mycrm_crmentity.deleted=0';
	 ...
	 * Portions created by mycrmCRM are Copyright (C) mycrmCRM.
	 * All Rights Reserved..
	...
	/** Returns a list of the associated mycrm_users
	 * Portions created by SugarCRM are Copyright (C) SugarCRM, Inc..
	 * All Rights Reserved..
	 * Contributor(s): ______________________________________..
	 */
	function get_users($id) {
		...
		$query = 'SELECT mycrm_users.id, mycrm_users.first_name,mycrm_users.last_name, mycrm_users.user_name, mycrm_users.email1, mycrm_users.email2, mycrm_users.secondaryemail , mycrm_users.phone_home, mycrm_users.phone_work, mycrm_users.phone_mobile, mycrm_users.phone_other, mycrm_users.phone_fax from mycrm_users inner join mycrm_salesmanactivityrel on mycrm_salesmanactivityrel.smid=mycrm_users.id and mycrm_salesmanactivityrel.activityid=?';
	...
	function create_export_query(&$order_by, &$where) {
		...
		$query = "SELECT $fields_list FROM mycrm_activity
			...
			LEFT JOIN mycrm_seattachmentsrel
				ON mycrm_activity.activityid=mycrm_seattachmentsrel.crmid
		...
		$query .= "WHERE mycrm_activity.activitytype='Emails' AND mycrm_crmentity.deleted=0 ";
	...
	function unlinkRelationship($id, $return_module, $return_id) {
		...
		$sql = 'DELETE FROM mycrm_seactivityrel WHERE activityid=? AND crmid = ?';
		...
		$this->db->pquery('UPDATE mycrm_crmentity SET modifiedtime = ? WHERE crmid = ?', array(date('y-m-d H:i:d'), $id));
	}
	...
	*/
	function setRelationTables($secmodule) {
		$rel_tables = array (
				...
				"Vendors" => array("mycrm_seactivityrel" => array("activityid", "crmid"), "mycrm_activity" => "activityid"),
				"Contacts" => array("mycrm_seactivityrel" => array("activityid", "crmid"), "mycrm_activity" => "activityid"),
	...
	*/
	function generateReportsSecQuery($module, $secmodule, $queryPlanner){
		...
		$matrix->setDependency("mycrm_crmentityEmails",array("mycrm_groupsEmails","mycrm_usersEmails","mycrm_lastModifiedByEmails"));
		$matrix->setDependency("mycrm_activity",array("mycrm_crmentityEmails","mycrm_email_track"));
		...
		if ($queryPlanner->requireTable("mycrm_crmentityEmails")){
		    $query .= " LEFT JOIN mycrm_crmentity AS mycrm_crmentityEmails ON mycrm_crmentityEmails.crmid=mycrm_activity.activityid and mycrm_crmentityEmails.deleted = 0";
		}
		if ($queryPlanner->requireTable("mycrm_groupsEmails")){
		    $query .= " LEFT JOIN mycrm_groups AS mycrm_groupsEmails ON mycrm_groupsEmails.groupid = mycrm_crmentityEmails.smownerid";
		}
		if ($queryPlanner->requireTable("mycrm_usersEmails")){
		    $query .= " LEFT JOIN mycrm_users AS mycrm_usersEmails ON mycrm_usersEmails.id = mycrm_crmentityEmails.smownerid";
		}
		if ($queryPlanner->requireTable("mycrm_lastModifiedByEmails")){
		    $query .= " LEFT JOIN mycrm_users AS mycrm_lastModifiedByEmails ON mycrm_lastModifiedByEmails.id = mycrm_crmentityEmails.modifiedby and mycrm_seactivityreltmpEmails.activityid = mycrm_activity.activityid";
		}
        if ($queryPlanner->requireTable("mycrm_createdbyEmails")){
			$query .= " left join mycrm_users as mycrm_createdbyEmails on mycrm_createdbyEmails.id = mycrm_crmentityEmails.smcreatorid and mycrm_seactivityreltmpEmails.activityid = mycrm_activity.activityid";
		}
		if ($queryPlanner->requireTable("mycrm_email_track")){
		    $query .= " LEFT JOIN mycrm_email_track ON mycrm_email_track.mailid = mycrm_activity.activityid and mycrm_email_track.crmid = ".$focus->table_name.".".$focus->table_index;
		}
		return $query;
	}

	/*
	 * Function to store the email access count value of emails in 'mycrm_email_track' table
	 * @param - $mailid
	 */
	function setEmailAccessCountValue($mailid) {
		global $adb;
		$successIds = array();
		$result = $adb->pquery('SELECT idlists FROM mycrm_emaildetails WHERE emailid=?', array($mailid));
		...
		for ($i=0; $i<count($successIds); $i++) {
			$adb->pquery("INSERT INTO mycrm_email_track(crmid, mailid,  access_count) VALUES(?,?,?)", array($successIds[$i], $mailid, 0));
		}
	}
...
function pdfAttach($obj, $module, $file_name, $id) {
	...
	if ($status) {
		$query1 = "insert into mycrm_crmentity (crmid,smcreatorid,smownerid,setype,description,createdtime,modifiedtime) values(?,?,?,?,?,?,?)";
		...
		$query2 = "insert into mycrm_attachments(attachmentsid, name, description, type, path) values(?,?,?,?,?)";
		$params2 = array($current_id, $file_name, $obj->column_fields['description'], 'pdf', $upload_file_path);
		$result = $adb->pquery($query2, $params2);

		$query3 = 'insert into mycrm_seattachmentsrel values(?,?)';
