...
/**   Function used to send email
  ...
  *   $from_email	-- currently loggedin mycrm_users's email id. you can give as '' if you are not in HelpDesk module
  ...
  *   $attachment	-- whether we want to attach the currently selected file or all mycrm_files.[values = current,all] - optional
  *   $emailid		-- id of the email object which will be used to get the mycrm_attachments
  */
function send_mail($module,$to_email,$from_name,$from_email,$subject,$contents,$cc='',$bcc='',$attachment='',$emailid='',$logo='', $useGivenFromEmailAddress=false)
{
	...
	//Get the email id of assigned_to user -- pass the value and name, name must be "user_name" or "id"(field names of mycrm_users mycrm_table)
    ...
    if($cachedFromEmail === null) {
        $query = "select from_email_field from mycrm_systems where server_type=?";
        $params = array('email');
...
/**	Function to get the user Email id based on column name and column value
  *	$name -- column name of the mycrm_users mycrm_table
  *	$val  -- column value
  */
function getUserEmailId($name,$val)
{
	...
	if($val != '')
	{
		...
		$sql = "SELECT email1, email2, secondaryemail  from mycrm_users WHERE status='Active' AND ". $adb->sql_escape_string($name)." = ?";
...
function addSignature($contents, $fromname)
{
    ...
    if($sign === null) {
        $result = $adb->pquery("select signature, first_name, last_name from mycrm_users where user_name=?", array($fromname));
...
/**	Function to set all the Mailer properties
  ...
  *	$attachment	-- whether we want to attach the currently selected file or all mycrm_files.
  				[values = current,all] - optional
  *	$emailid	-- id of the email object which will be used to get the mycrm_attachments - optional
  */
function setMailerProperties($mail,$subject,$contents,$from_email,$from_name,$to_email,$attachment='',$emailid='',$module='',$logo='')
{
    ...
    if(empty($userFullName)) {
        $rs = $adb->pquery("select first_name,last_name from mycrm_users where user_name=?", array($from_name));
	...
	//This will add all the mycrm_files which are related to this record or email
...
function setMailServerProperties($mail)
{
	...
	$res = $adb->pquery("select * from mycrm_systems where server_type=?", array('email'));
...
/**     Function to add all the mycrm_files as attachment with the mail object
  *     $mail -- reference of the mail object
  *     $record -- email id ie., record id which is used to get the all mycrm_attachments from database
  */
function addAllAttachments($mail,$record)
{
	...
	//Retrieve the mycrm_files from database where avoid the file which has been currently selected
	$sql = "select mycrm_attachments.* from mycrm_attachments inner join mycrm_seattachmentsrel on mycrm_attachments.attachmentsid = mycrm_seattachmentsrel.attachmentsid inner join mycrm_crmentity on mycrm_crmentity.crmid = mycrm_attachments.attachmentsid where mycrm_crmentity.deleted=0 and mycrm_seattachmentsrel.crmid=?";
...
function MailSend($mail)
{
	...
	{
		 $log->info("Mail has been sent from the mycrmCRM system : Status : '".$mail->ErrorInfo."'");
		return 1;
	}
}
...
/**	Function to get the Parent email id from HelpDesk to send the details about the ticket via email
  ...
  *	$parentid -- id of the parent ie., contact or mycrm_account
  */
function getParentMailId($parentmodule,$parentid)
{
	...
	$adb->println("Inside the function getParentMailId. \n parent module and id => ".$parentmodule."&".$parentid);
        ...
        {
                $tablename = 'mycrm_contactdetails';
                $idname = 'contactid';
		$first_email = 'email';
...
function parseEmailErrorString($mail_error_str)
{
	...
	foreach($mail_status as $key => $val)
	{
		...
		if($status_str[1] != 1 && $status_str[1] != '')
		{
			...
			{
				$adb->println("first elseif part - status will be 0 which is the case of assigned to mycrm_users's email is empty.");
...
function getDefaultAssigneeEmailIds($groupId) {
	...
	if($groupId != '') {
		...
		$result = $adb->pquery('SELECT email1,email2,secondaryemail FROM mycrm_users WHERE mycrm_users.id IN
											('.  generateQuestionMarks($userGroups->group_users).') AND mycrm_users.status= ?',
