...
class Emails_MassSaveAjax_View extends Mycrm_Footer_View {
	 ...
	 */
	public function massSave(Mycrm_Request $request) {
        ...
        if(!empty($parentEmailId) && !empty ($recordId)) {
            $parentEmailModel = Mycrm_Record_Model::getInstanceById($parentEmailId);
		...
		if ($recordModel->checkUploadSize($documentIds)) {
			...
			if($flag == 'SENT') {
				...
				if ($status === true) {
					// This is needed to set mycrm_email_track table as it is used in email reporting
