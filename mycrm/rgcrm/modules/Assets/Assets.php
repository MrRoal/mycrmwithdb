...
class Assets extends CRMEntity {
	 ...
	 */
	var $customFieldTable = Array('mycrm_assetscf', 'assetsid');

	/**
	 * Mandatory for Saving, Include tables related to this module.
	 */
	var $tab_name = Array('mycrm_crmentity','mycrm_assets','mycrm_assetscf');
	...
	var $tab_name_index = Array(
		...
		'mycrm_assets'=>'assetsid',
		'mycrm_assetscf'=>'assetsid');
	...
	*/
	function vtlib_handler($moduleName, $eventType) {
		...
		global $adb;
 		...
 		if($eventType == 'module.postinstall') {
			...
			$AssetsModule  = Mycrm_Module::getInstance('Assets');
			Mycrm_Access::setDefaultSharing($AssetsModule);

			//Showing Assets module in the related modules in the More Information Tab
			$assetInstance = Mycrm_Module::getInstance('Assets');
			$assetLabel = 'Assets';
			...
			$productInstance = Mycrm_Module::getInstance('Products');
			...
			if (!($adb->num_rows($result))) {
				...
				$adb->pquery("INSERT INTO mycrm_modentity_num values(?,?,?,?,?,?)", array($adb->getUniqueId("mycrm_modentity_num"), $moduleName, 'ASSET', 1, 1, 1));
			}
	...
	function addModuleToCustomerPortal() {
		...
		$assetsResult = $adb->pquery('SELECT tabid FROM mycrm_tab WHERE name=?', array('Assets'));
		$assetsTabId = $adb->query_result($assetsResult, 0, 'tabid');
		if(getTabid('CustomerPortal') && $assetsTabId) {
			$checkAlreadyExists = $adb->pquery('SELECT 1 FROM mycrm_customerportal_tabs WHERE tabid=?', array($assetsTabId));
			if($checkAlreadyExists && $adb->num_rows($checkAlreadyExists) < 1) {
				...
				$adb->query("INSERT INTO mycrm_customerportal_tabs(tabid,visible,sequence) VALUES ($assetsTabId,1,$nextSequence)");
			}
			$checkAlreadyExists = $adb->pquery('SELECT 1 FROM mycrm_customerportal_prefs WHERE tabid=?', array($assetsTabId));
			if($checkAlreadyExists && $adb->num_rows($checkAlreadyExists) < 1) {
				$adb->query("INSERT INTO mycrm_customerportal_prefs(tabid,prefkey,prefvalue) VALUES ($assetsTabId,'showrelatedinfo',1)");
			}
		}
	}
