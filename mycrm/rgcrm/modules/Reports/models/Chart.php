...
class Reports_Chart_Model extends Mycrm_Base_Model {

	public static function getInstanceById($reportModel) {
		$self = new self();
		$db = PearDatabase::getInstance();
		$result = $db->pquery('SELECT * FROM mycrm_reporttype WHERE reportid = ?', array($reportModel->getId()));
...
abstract class Base_Chart extends Mycrm_Base_Model{
	...
	function getReportColumnSQL($selectedfields) {
		...
		// Fix for http://code.mycrm.com/mycrm/mycrmcrm/issues/4
	...
	function generateLink($field, $value) {
		...
		if(($dataFieldInfo[4] == 'D' || $dataFieldInfo[4] == 'DT') && !empty($dataFieldInfo[5])) {
			...
			$value = Mycrm_Date_UIType::getDisplayDateTimeValue($value);
		}
			...
			if($legendField) {
				...
				if($fieldDataType == 'picklist') {
					...
					$label =  implode(',', $labelList);
						...
						$label = Mycrm_Date_UIType::getDisplayDateValue($row[strtolower($legendField->get('reportlabel'))]);
				} else if($fieldDataType == 'datetime') {
						$label = Mycrm_Date_UIType::getDisplayDateTimeValue($row[strtolower($legendField->get('reportlabel'))]);
				} else {
			...
			if($groupByColumnsByFieldModel) {
				foreach($groupByColumnsByFieldModel as $gFieldModel) {
					...
					if($fieldDataType == 'picklist') {
						...
						$label = Mycrm_Date_UIType::getDisplayDateValue($row[strtolower($gFieldModel->get('reportlabel'))]);
					} else if($fieldDataType == 'datetime') {
						$label = Mycrm_Date_UIType::getDisplayDateTimeValue($row[strtolower($gFieldModel->get('reportlabel'))]);
					} else {
