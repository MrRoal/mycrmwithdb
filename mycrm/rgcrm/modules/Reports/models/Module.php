...
class Reports_Module_Model extends Mycrm_Module_Model {
	...
	function deleteRecord($reportModel) {
		...
		if($currentUser->isAdminUser() || in_array($owner, $subOrdinates) || $owner == $currentUser->getId()) {
			...
			$db->pquery('DELETE FROM mycrm_selectquery WHERE queryid = ?', array($reportId));

			$db->pquery('DELETE FROM mycrm_report WHERE reportid = ?', array($reportId));

			$db->pquery('DELETE FROM mycrm_schedulereports WHERE reportid = ?', array($reportId));

            $db->pquery('DELETE FROM mycrm_reporttype WHERE reportid = ?', array($reportId));

			$result = $db->pquery('SELECT * FROM mycrm_homereportchart WHERE reportid = ?',array($reportId));
			$numOfRows = $db->num_rows($result);
			for ($i = 0; $i < $numOfRows; $i++) {
				$homePageChartIdsList[] = $adb->query_result($result, $i, 'stuffid');
			}
			if ($homePageChartIdsList) {
				$deleteQuery = 'DELETE FROM mycrm_homestuff WHERE stuffid IN (' . implode(",", $homePageChartIdsList) . ')';
	 ...
	 * @return <Array of Mycrm_Link_Model>
	 */
	...
	function getRecentRecords($limit = 10) {
		...
		$result = $db->pquery('SELECT * FROM mycrm_report ORDER BY reportid DESC LIMIT ?', array($limit));
