...
class Reports_Folder_Model extends Mycrm_Base_Model {
	...
	function save() {
		...
		$folderId = $this->getId();
		if(!empty($folderId)) {
			$db->pquery('UPDATE mycrm_reportfolder SET foldername = ?, description = ? WHERE folderid = ?',
					array($this->getName(), $this->getDescription(), $folderId));
		} else {
			$result = $db->pquery('SELECT MAX(folderid) AS folderid FROM mycrm_reportfolder', array());
			$folderId = (int) ($db->query_result($result, 0, 'folderid')) + 1;

			$db->pquery('INSERT INTO mycrm_reportfolder(folderid, foldername, description, state) VALUES(?, ?, ?, ?)', array($folderId, $this->getName(), $this->getDescription(), 'CUSTOMIZED'));
	...
	function delete() {
		$db = PearDatabase::getInstance();
		$db->pquery('DELETE FROM mycrm_reportfolder WHERE folderid = ?', array($this->getId()));
	}
	...
	function getReports($pagingModel) {
		...
		$reportClassInstance = Mycrm_Module_Model::getClassInstance('Reports');
		...
		$reportModuleModel = Mycrm_Module_Model::getInstance('Reports');
	...
	public static function getInstanceById($folderId) {
		$folderModel = Mycrm_Cache::get('reportsFolder',$folderId);
        if(!$folderModel){
            $db = PearDatabase::getInstance();
            $folderModel = Reports_Folder_Model::getInstance();

            $result = $db->pquery("SELECT * FROM mycrm_reportfolder WHERE folderid = ?", array($folderId));

            if ($db->num_rows($result) > 0) {
                $values = $db->query_result_rowdata($result, 0);
                $folderModel->setData($values);
            }
            Mycrm_Cache::set('reportsFolder',$folderId,$folderModel);
        }
        return $folderModel;
	}
    ...
    public static function getAll() {
        $db = PearDatabase::getInstance();
        $folders = Mycrm_Cache::get('reports', 'folders');
        if (!$folders) {
            $folders = array();
            $result = $db->pquery("SELECT * FROM mycrm_reportfolder ORDER BY foldername ASC", array());
            ...
            if ($noOfFolders > 0) {
                for ($i = 0; $i < $noOfFolders; $i++) {
                    ...
                    Mycrm_Cache::set('reportsFolder',$values['folderid'],$folderModel);
                }
            }
            Mycrm_Cache::set('reports','folders',$folders);
        }
        return $folders;
    }
	...
	function checkDuplicate() {
		...
		$query = 'SELECT 1 FROM mycrm_reportfolder WHERE foldername = ?';
	...
	function hasReports() {
		...
		$result = $db->pquery('SELECT 1 FROM mycrm_report WHERE folderid = ?', array($this->getId()));
	...
	public function getReportsCount() {
        ...
        // To get the report ids which are permitted for the user
            $query = "SELECT reportmodulesid, primarymodule from mycrm_reportmodules";
        ...
        //End
		$sql = "SELECT count(*) AS count FROM mycrm_report
				INNER JOIN mycrm_reportfolder ON mycrm_reportfolder.folderid = mycrm_report.folderid AND 
                mycrm_report.reportid in (".implode(',',$allowedReportIds).")";
		$fldrId = $this->getId();
		if($fldrId == 'All') {
			$fldrId = false;
		}
		...
		if (!$currentUserModel->isAdminUser()) {
			...
			if ($groupId) {
				$groupQuery = "(SELECT reportid from mycrm_reportsharing WHERE shareid IN ($groupId) AND setype = 'groups') OR ";
			}
			
			$sql .= " AND (mycrm_report.reportid IN (SELECT reportid from mycrm_reportsharing WHERE $groupQuery shareid = ? AND setype = 'users')
						OR mycrm_report.sharingtype = 'Public'
						OR mycrm_report.owner = ?
						OR mycrm_report.owner IN (SELECT mycrm_user2role.userid FROM mycrm_user2role
													...
													WHERE mycrm_role.parentrole LIKE ?))";
	 ...
	 * @param <Mycrm_Module_Model> - Reports Module Model
	...
	function getListViewQuery($folderId) {
        $sql = "select mycrm_report.*, mycrm_reportmodules.*, mycrm_reportfolder.folderid from mycrm_report 
                inner join mycrm_reportfolder on mycrm_reportfolder.folderid = mycrm_report.folderid 
                inner join mycrm_reportmodules on mycrm_reportmodules.reportmodulesid = mycrm_report.reportid ";
        
        if($folderId != "All") {
                $sql = $sql." where mycrm_reportfolder.folderid = ".$folderId;
        }        
        return $sql;
	}
}