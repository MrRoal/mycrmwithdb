...
$report_std_filter = new mycrmCRM_Smarty; 
...
function getVisibleCriteria($recordid='')
{
	...
	if($recordid!='')
	{
		$result = $adb->pquery("select sharingtype from mycrm_report where reportid=?",array($recordid));
		$selcriteria=$adb->query_result($result,0,"sharingtype");
	}
	if($selcriteria == ""){
		$selcriteria = 'Public';
	}
	$filter_result = $adb->query("select * from mycrm_reportfilters");
...
function getShareInfo($recordid=''){
	global $adb;
	$member_query = $adb->pquery("SELECT mycrm_reportsharing.setype,mycrm_users.id,mycrm_users.user_name FROM mycrm_reportsharing INNER JOIN mycrm_users on mycrm_users.id = mycrm_reportsharing.shareid WHERE mycrm_reportsharing.setype='users' AND mycrm_reportsharing.reportid = ?",array($recordid));
	...
	$member_query = $adb->pquery("SELECT mycrm_reportsharing.setype,mycrm_groups.groupid,mycrm_groups.groupname FROM mycrm_reportsharing INNER JOIN mycrm_groups on mycrm_groups.groupid = mycrm_reportsharing.shareid WHERE mycrm_reportsharing.setype='groups' AND mycrm_reportsharing.reportid = ?",array($recordid));
	$noofrows = $adb->num_rows($member_query);
	if($noofrows > 0){
		for($i=0;$i<$noofrows;$i++){
