...
class VTScheduledReport extends Reports {
	...
	public function getReportScheduleInfo() {
		...
		if(!empty($this->id)) {
			...
			if($cachedInfo == false) {
				$result = $adb->pquery('SELECT * FROM mycrm_scheduled_reports WHERE reportid=?', array($this->id));
	...
	public function sendEmail() {
		...
		$mycrmMailer->Body    = $contents;
		$mycrmMailer->ContentType = "text/html";
		...
		foreach($attachments as $attachmentName => $path) {
			$mycrmMailer->AddAttachment($path, $attachmentName);
		}

		$mycrmMailer->Send(true);
	...
	public function updateNextTriggerTime() {
		...
		$adb->pquery('UPDATE mycrm_scheduled_reports SET next_trigger_time=? WHERE reportid=?', array($nextTriggerTime, $this->id));
	}
	...
	public static function getScheduledReports($adb, $user) {
		...
		$currentTime = date('Y-m-d H:i:s');
		$result = $adb->pquery("SELECT * FROM mycrm_scheduled_reports
