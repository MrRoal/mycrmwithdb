 ...
 */
class Portal_ListView_Model extends Mycrm_ListView_Model {
    
	public function getListViewEntries($pagingModel) {
        $db = PearDatabase::getInstance();
        $moduleModel = Mycrm_Module_Model::getInstance('Portal');
        ...
        for($i = 0; $i < $db->num_rows($listResult); $i++) {
            ...
            $listViewEntries[$row['portalid']]['createdtime'] = Mycrm_Date_UIType::getDisplayDateValue($row['createdtime']);
        }
        $index = 0;
    ...
    public function getQuery() {
        $query = 'SELECT portalid, portalname, portalurl, createdtime FROM mycrm_portal';
