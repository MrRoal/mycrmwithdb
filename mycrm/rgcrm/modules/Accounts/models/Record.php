...
class Accounts_Record_Model extends Mycrm_Record_Model {
	...
	function getAccountHierarchy() {
		...
		foreach($hierarchy['entries'] as $accountId => $accountInfo) {
			...
			if($matches != null) {
				...
				$recordModel = Mycrm_Record_Model::getCleanInstance('Accounts');
	...
	public function checkDuplicate() {
		...
		$query = "SELECT 1 FROM mycrm_crmentity WHERE setype = ? AND label = ? AND deleted = 0";
