...
class Settings_ExtensionStore_Extension_Model extends Mycrm_Base_Model {
	...
	public function __construct() {
		parent::__construct();
		$this->EXTENSIONSTORE_LOOKUP_URL = 'https://marketplace.mycrm.com/api';
		global $site_URL;
		if (empty($site_URL)) {
    ...
    public static function getLanguageInstance($lang) {
        $sql = 'SELECT id,name,prefix FROM mycrm_language WHERE name = ?';
	...
	public function findListings($searchTerm=null, $searchType) {
         ...
         */
	public function logoutMarketPlace(Mycrm_Request $request) {
		    $sql = 'DELETE FROM mycrm_extnstore_users';
		    $db = PearDatabase::getInstance();
		    $db->pquery($sql, array());
	    }
	...
	public function getInstanceFromArray($listing) {
		...
		foreach ($listing as $key => $value) {
			...
			case 'minrange':   $key = 'mycrmVersion'; break;
			case 'maxrange':   $key = 'mycrmMaxVersion'; break;
    ...
    public static function getModuleFromExtnName($extnName) {
        $moduleModel = Mycrm_Module_Model::getInstance($extnName);
        if($moduleModel) {
            $moduleModel->set('extnType','module');
        }
        if(!$moduleModel) {
            if(self::getLanguageInstance($extnName)) {
               $moduleModel = new Mycrm_Module_Model();
       ...
       public function getCardDetails($cardId) {
        ...
        public function getLocationUrl($extensionId, $extensionName){
            ...
            if(is_admin($current_user)){
                ...
                return 'https://marketplace.mycrm.com/app/listings?id='.$extensionId;
            }
        }
}
