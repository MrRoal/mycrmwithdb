...
class Settings_ExtensionStore_Basic_Action extends Settings_Mycrm_IndexAjax_View {
    ...
    protected function postReview(Mycrm_Request $request) {
    ...
    protected function getDateString($date) {
        $dateString = Mycrm_Util_Helper::formatDateTimeIntoDayString($date);
        return $dateString;
    }


    protected function logoutMarketPlace(Mycrm_Request $request) { 
         $modelInstance = $this->getModelInstance(); 
         $modelInstance->logoutMarketPlace($request);
    }


    protected function uninstallExtension(Mycrm_Request $request) {
        ...
        if($extnType == 'module'){
            ...
            if($languageInstance) {
                ...
                Mycrm_Language::deregister($langPrefix);
                //remove files
    ...
    protected function registerAccount(Mycrm_Request $request) {
        $options = array();
    ...
    protected function updateTrialMode(Mycrm_Request $request) {
        $response = new Mycrm_Response();
        $importedModuleName = $request->get('extensionName');
        $trial = $request->get('trial');
        $db = PearDatabase::getInstance();
        $db->pquery('UPDATE mycrm_tab SET trial = ? WHERE name = ?', array($trial, $importedModuleName));
        $response->setResult(array('success' => true, 'message' => 'Extension Store Installed'));
        $response->emit();
    }
    
    protected function updateCardDetails(Mycrm_Request $request) {
