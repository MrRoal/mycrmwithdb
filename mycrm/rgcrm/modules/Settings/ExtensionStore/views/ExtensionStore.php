...
class Settings_ExtensionStore_ExtensionStore_View extends Settings_Mycrm_Index_View {
    ...
    public function process(Mycrm_Request $request) {
        ...
        $loaderInfo =  $loaderRequired ? $loaderInstance->mycrm_extensionloader_suggest() : null;
    ...
    protected function searchExtension(Mycrm_Request $request) {
    ...
    protected function detail(Mycrm_Request $request) {
        $viewer = $this->getViewer($request);
        $qualifiedModuleName = $request->getModule(false);
    ...
    protected function installationLog(Mycrm_Request $request) {
        $viewer = $this->getViewer($request);
        global $Mycrm_Utils_Log;
        $viewer->assign('MYCRM_UTILS_LOG', $Mycrm_Utils_Log);
            ...
            if ($package) {
                ...
                if (!$upgradeError) {
                    if (!$isLanguagePackage) {
                        $moduleModel = Mycrm_Module_Model::getInstance($importedModuleName);
        ...
        if ($extensionId && $extensionModel) {
            ...
            $viewer->assign('TARGET_MODULE_INSTANCE', Mycrm_Module_Model::getInstance($targetModuleName));
    ...
    protected function oneClickInstall(Mycrm_Request $request) {
        $viewer = $this->getViewer($request);
        global $Mycrm_Utils_Log;
        $viewer->assign('MYCRM_UTILS_LOG', $Mycrm_Utils_Log);
        $Mycrm_Utils_Log = true;
