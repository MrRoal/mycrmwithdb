...
class Settings_Picklist_Field_Model extends Mycrm_Field_Model {
    ...
    public function getPicklistValues($roleIdList, $groupMode='INTERSECTION') {
        ...
        $tableName = 'mycrm_'.$fieldName;
        $idColName = $fieldName.'id';
        $query = 'SELECT '.$fieldName;
        if($intersectionMode) {
            $query .= ',count(roleid) as rolecount ';
        }
        $query .= ' FROM  mycrm_role2picklist INNER JOIN '.$tableName.' ON mycrm_role2picklist.picklistvalueid = '.$tableName.'.picklist_valueid'.
	...
	public static function getEditablePicklistValues($fieldName){
        ...
        $query="SELECT $primaryKey ,$fieldName FROM mycrm_$fieldName WHERE presence=1 AND $fieldName <> '--None--'";
        $values = array();
	...
	public static function getNonEditablePicklistValues($fieldName){
        ...
        $query = "select $fieldName from mycrm_$fieldName where presence=0";
        $values = array();
        $result = $db->pquery($query, array());
        $num_rows = $db->num_rows($result);
        for($i=0; $i<$num_rows; $i++) {
