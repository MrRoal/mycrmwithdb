 ...
 */
class Settings_Profiles_Record_Model extends Settings_Mycrm_Record_Model {
	...
	public function hasModuleActionPermission($module, $action) {
		$actionId = false;
		if(is_object($action) && is_a($action, 'Mycrm_Action_Model')) {
			$actionId = $action->getId();
		} else {
			$action = Mycrm_Action_Model::getInstance($action);
			$actionId = $action->getId();
		}
		if(!$actionId) {
			return false;
	...
	public function getProfileTabModel($module) {
		$tabId = false;
		if(is_object($module) && is_a($module, 'Mycrm_Module_Model')) {
			$tabId = $module->getId();
		} else {
			$module = Mycrm_Module_Model::getInstance($module);
			$tabId = $module->getId();
		}
		if(!$tabId) {
			return false;
	...
	public function getProfileTabFieldModel($module, $field) {
		...
		$fieldId = false;
		if(is_object($field) && is_a($field, 'Mycrm_Field_Model')) {
			$fieldId = $field->getId();
		} else {
			$field = Mycrm_Field_Model::getInstance($field, $profileTabModel);
			$fieldId = $field->getId();
		}
		if(!$fieldId) {
			return false;
	...
	public function getProfileTabFieldPermissions($tabId) {
		...
		if(!$this->profile_tab_field_permissions[$tabId]) {
			...
			if($this->getId()) {
				$sql = 'SELECT * FROM mycrm_profile2field WHERE profileid=? AND tabid=?';
	...
	public function getProfileActionPermissions() {
		...
		if(!$this->profile_action_permissions) {
			...
			if($this->getId()) {
				$sql = 'SELECT * FROM mycrm_profile2standardpermissions WHERE profileid=?';
				$params = array($this->getId());
				$result = $db->pquery($sql, $params);
				$noOfRows = $db->num_rows($result);
				for($i=0; $i<$noOfRows; ++$i) {
	...
	public function getProfileUtilityPermissions() {
		...
		if(!$this->profile_utility_permissions) {
			...
			if($this->getId()) {
				$sql = 'SELECT * FROM mycrm_profile2utility WHERE profileid=?';
				$params = array($this->getId());
				$result = $db->pquery($sql, $params);
				$noOfRows = $db->num_rows($result);
				for($i=0; $i<$noOfRows; ++$i) {
	...
	public function getModulePermissions() {
		if(!$this->module_permissions) {
			$allModules = Mycrm_Module_Model::getAll(array(0), Settings_Profiles_Module_Model::getNonVisibleModulesList());
			$eventModule = Mycrm_Module_Model::getInstance('Events');
			...
			$allTabActions = Mycrm_Action_Model::getAll(true);
	...
	public function delete($transferToRecord) {
		...
		$db->pquery('DELETE FROM mycrm_profile2globalpermissions WHERE profileid=?', array($profileId));
		$db->pquery('DELETE FROM mycrm_profile2tab WHERE profileid=?', array($profileId));
		$db->pquery('DELETE FROM mycrm_profile2standardpermissions WHERE profileid=?', array($profileId));
		$db->pquery('DELETE FROM mycrm_profile2utility WHERE profileid=?', array($profileId));
		$db->pquery('DELETE FROM mycrm_profile2field WHERE profileid=?', array($profileId));

		$checkSql = 'SELECT roleid, count(profileid) AS profilecount FROM mycrm_role2profile
							WHERE roleid IN (select roleid FROM mycrm_role2profile WHERE profileid=?) GROUP BY roleid';
		...
		for($i=0; $i<$noOfRoles; ++$i) {
			...
			if($profileCount > 1) {
				$sql = 'DELETE FROM mycrm_role2profile WHERE roleid=? AND profileid=?';
				$params = array($roleId, $profileId);
			} else {
				$sql = 'UPDATE mycrm_role2profile SET profileid=? WHERE roleid=? AND profileid=?';
				$params = array($transferProfileId, $roleId, $profileId);
			}
			$db->pquery($sql, $params);
		}

		$db->pquery('DELETE FROM mycrm_profile WHERE profileid=?', array($profileId));
	}
	...
	public function save() {
		...
		$calendarModule = Mycrm_Module_Model::getInstance('Calendar');
		$eventModule = Mycrm_Module_Model::getInstance('Events');
		...
		$profileId = $this->getId();
		if(!$profileId) {
			$profileId = $db->getUniqueId('mycrm_profile');
			$this->setId($profileId);
			$sql = 'INSERT INTO mycrm_profile(profileid, profilename, description, directly_related_to_role) VALUES (?,?,?,?)';
			$params = array($profileId, $profileName, $description, $isProfileDirectlyRelatedToRole);
			$isNewProfile = true;
		} else {
			$sql = 'UPDATE mycrm_profile SET profilename=?, description=?, directly_related_to_role=? WHERE profileid=?';
		...
		$sql = 'INSERT INTO mycrm_profile2globalpermissions(profileid, globalactionid, globalactionpermission) VALUES (?,?,?)';
		...
		$allModuleModules = Mycrm_Module_Model::getAll(array(0), Settings_Profiles_Module_Model::getNonVisibleModulesList());
		$allModuleModules[$eventModule->getId()] = $eventModule;
		if(count($allModuleModules) > 0) {
			$actionModels = Mycrm_Action_Model::getAll(true);
	...
	protected function saveModulePermissions($moduleModel, $permissions) {
		...
		$db->pquery('DELETE FROM mycrm_profile2tab WHERE profileid=? AND tabid=?', array($profileId, $tabId));
		...
		if($moduleModel->isEntityModule() || $moduleModel->isUtilityActionEnabled()) {
			if($actionPermissions || $moduleModel->isUtilityActionEnabled()) {
				$actionIdsList = Mycrm_Action_Model::$standardActions;
				...
				foreach($actionPermissions as $actionId => $permission) {
					if(isset(Mycrm_Action_Model::$standardActions[$actionId])) {
				...
				if ($profileActionPermissions) {
					...
					$actionsUpdateQuery = 'UPDATE mycrm_profile2standardpermissions SET permissions = CASE ';
					...
					foreach (Mycrm_Action_Model::$utilityActions as $utilityActionId => $utilityActionName) {
						if(!isset($utilityIdsList[$utilityActionId])) {
							$utilityIdsList[$utilityActionId] = 'off';
						}
					}
					//Utility permissions
					$utilityUpdateQuery = 'UPDATE mycrm_profile2utility SET permission = CASE ';
					...
					$actionsInsertQuery .= 'INSERT INTO mycrm_profile2standardpermissions(profileid, tabid, operation, permissions) VALUES ';
					foreach ($actionsIdsList as $actionId => $permission) {
						$actionEnabled = true;
						$permissionValue = $this->tranformInputPermissionValue($permission);
					...
					$utilityInsertQuery .= 'INSERT INTO mycrm_profile2utility(profileid, tabid, activityid, permission) VALUES ';
					foreach($utilityIdsList as $actionId => $permission) {
						$permissionValue = $this->tranformInputPermissionValue($permission);
		...
		$sql = 'INSERT INTO mycrm_profile2tab(profileid, tabid, permissions) VALUES (?,?,?)';
		...
		if(is_array($fieldPermissions)) {
			foreach($fieldPermissions as $fieldId => $stateValue) {
				$db->pquery('DELETE FROM mycrm_profile2field WHERE profileid=? AND tabid=? AND fieldid=?',
				...
				$sql = 'INSERT INTO mycrm_profile2field(profileid, tabid, fieldid, visible, readonly) VALUES (?,?,?,?,?)';
	...
	public function getRecordLinks() {
		...
		$recordLinks = array(
			...
			array(
				...
				'linkurl' => "javascript:Settings_Mycrm_List_Js.triggerDelete(event,'".$this->getDeleteActionUrl()."')",
				'linkicon' => 'icon-trash'
			)
		);
	...
	public static function getAllByRole($roleId) {
		...
		$sql = 'SELECT mycrm_profile.*
					FROM mycrm_profile
					INNER JOIN
						mycrm_role2profile ON mycrm_profile.profileid = mycrm_role2profile.profileid
						AND
						mycrm_role2profile.roleid = ?';
		$params = array($roleId);
		$result = $db->pquery($sql, $params);
	...
	public static function getInstanceById($profileId) {
		...
		$sql = 'SELECT * FROM mycrm_profile WHERE profileid = ?';
    ...
    public static function getInstanceByName($profileName , $checkOnlyDirectlyRelated=false, $excludedRecordId = array()) {
        $db = PearDatabase::getInstance();
        $query = 'SELECT * FROM mycrm_profile WHERE profilename=?';
	...
	public function getUsersList($allUsers = false) {
		$db = PearDatabase::getInstance();
		$params = array(0);
		$query = 'SELECT id FROM mycrm_users
					INNER JOIN mycrm_user2role ON mycrm_user2role.userid = mycrm_users.id
					INNER JOIN mycrm_role2profile ON mycrm_role2profile.roleid = mycrm_user2role.roleid
					WHERE mycrm_users.deleted = ?';

		if (!$allUsers) {
			$query .= ' AND mycrm_role2profile.profileid = ?';
			$params[] = $this->getId();
		}
		$result = $db->pquery($query, $params);
	...
	/**
	 * Function to save user fields in mycrm_profile2field table
	...
	public function saveUserAccessbleFieldsIntoProfile2Field(){
		...
		if(!empty($profileId)){
			...
			//Added user fields into mycrm_profile2field and mycrm_def_org_field
			//We are using this field information in Email Templates.
			foreach ($userAccessbleFields as $fieldId => $fieldName) {
				$insertQuery = 'INSERT INTO mycrm_profile2field VALUES(?,?,?,?,?)';
			...
			foreach ($userAccessbleFields as $fieldId => $fieldName) {
				if(!in_array($fieldId, $def_org_fields)){
					$insertQuery = 'INSERT INTO mycrm_def_org_field VALUES(?,?,?,?)';
