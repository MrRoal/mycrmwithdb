...
class Settings_LayoutEditor_Field_Model extends Mycrm_Field_Model {
    ...
    public function delete() {
        ...
        $adb->pquery("delete from mycrm_cvcolumnlist where columnname = ? ", array($deletecolumnname));
        $adb->pquery("delete from mycrm_cvstdfilter where columnname = ?", array($column_cvstdfilter));
        $adb->pquery("delete from mycrm_cvadvfilter where columnname = ?", array($deletecolumnname));
        $adb->pquery("delete from mycrm_selectcolumn where columnname = ?", array($select_columnname));
        $adb->pquery("delete from mycrm_relcriteria where columnname = ?", array($select_columnname));
        $adb->pquery("delete from mycrm_reportsortcol where columnname = ?", array($select_columnname));
        $adb->pquery("delete from mycrm_reportdatefilter where datecolumnname = ?", array($column_cvstdfilter));
        $adb->pquery("delete from mycrm_reportsummary where columnname like ?", array('%'.$reportsummary_column.'%'));


        //Deleting from convert lead mapping mycrm_table- Jaguar
        if($fld_module=="Leads") {
            $deletequery = 'delete from mycrm_convertleadmapping where leadfid=?';
            ...
            $map_del_q = "update mycrm_convertleadmapping set ".$map_del_id[$fld_module]."=0 where ".$map_del_id[$fld_module]."=?";
        ...
        if($this->getFieldDataType() == 'picklist' || $this->getFieldDataType() == 'multipicklist') {
            $deltablequery = 'drop table mycrm_'.$adb->sql_escape_string($columnname);
            $adb->pquery($deltablequery, array());
            //To Delete Sequence Table 
            $deltableseqquery = 'drop table mycrm_'.$adb->sql_escape_string($columnname).'_seq'; 
            $adb->pquery($deltableseqquery, array()); 
            $adb->pquery("delete from  mycrm_picklist_dependency where sourcefield=? or targetfield=?", array($columnname,$columnname));
        }
    }
	...
	public function move($fieldNewDetails, $fieldOlderDetails) {
		...
		if ($olderBlockId == $newBlockId) {
			if ($newSequence > $olderSequence) {
				$updateQuery = 'UPDATE mycrm_field SET sequence = sequence-1 WHERE sequence > ? AND sequence <= ? AND block = ?';
			...
			$query = 'UPDATE mycrm_field SET sequence = ? WHERE fieldid = ?';
			$params = array($newSequence, $this->getId());
			$db->pquery($query, $params);
		} else {
			$updateOldBlockQuery = 'UPDATE mycrm_field SET sequence = sequence-1 WHERE sequence > ? AND block = ?';
			$params = array($olderSequence, $olderBlockId);
			$db->pquery($updateOldBlockQuery, $params);

			$updateNewBlockQuery = 'UPDATE mycrm_field SET sequence = sequence+1 WHERE sequence >= ? AND block = ?';
			$params = array($newSequence, $newBlockId);
			$db->pquery($updateNewBlockQuery, $params);

			$query = 'UPDATE mycrm_field SET sequence = ?, block = ? WHERE fieldid = ?';
    ...
    public static function makeFieldActive($fieldIdsList=array(), $blockId) {
        $db = PearDatabase::getInstance();
		$maxSequenceQuery = "SELECT MAX(sequence) AS maxsequence FROM mycrm_field WHERE block = ? AND presence IN (0,2) ";
		$res = $db->pquery($maxSequenceQuery,array($blockId));
		$maxSequence = $db->query_result($res,0,'maxsequence');

		$query = 'UPDATE mycrm_field SET presence = 2, sequence = CASE';
    ...
    public function isDefaultValueOptionDisabled() {
        if($this->isMandatoryOptionDisabled() || $this->getFieldDataType() == Mycrm_Field_Model::REFERENCE_TYPE || $this->get('uitype') == 69){
            return true;
        }
        return false;
    }
	...
	public static function getDetailsForMove($fieldIdsList = array()) {
		if ($fieldIdsList) {
			$db = PearDatabase::getInstance();
			$result = $db->pquery('SELECT fieldid, sequence, block, fieldlabel FROM mycrm_field WHERE fieldid IN ('. generateQuestionMarks($fieldIdsList) .')', $fieldIdsList);
	 ...
	 * @param <Mycrm_Module_Model> $moduleInstance
	...
	public static function getInstanceFromBlockIdList($blockId, $moduleInstance= false) {
		...
		$query = 'SELECT * FROM mycrm_field WHERE block IN('. generateQuestionMarks($blockId) .') AND mycrm_field.displaytype IN (1,2,4) ORDER BY sequence';
