...
class Settings_LayoutEditor_Module_Model extends Mycrm_Module_Model {
    ...
    public function addField($fieldType, $blockId, $params) {
        ...
        $max_fieldid = $db->getUniqueID("mycrm_field");
        ...
        if (isset($focus->customFieldTable)) {
            ...
            $tableName= 'mycrm_'.strtolower($moduleName).'cf';
        }
        ...
        $blockModel = Mycrm_Block_Model::getInstance($blockId, $this);
    ...
    public function getTypeDetailsForAddField($fieldType,$params) {
        switch ($fieldType) {
                ...
                Case 'Decimal' :
                               ...
                               // Fix for http://trac.mycrm.com/cgi-bin/trac.cgi/ticket/6363
                               $uichekdata='NN~O';
                               break;
               Case 'Percent' :
                               $uitype = 9;
    ...
    public function checkFIeldExists($fieldLabel) {
        ...
        $query = 'SELECT 1 FROM mycrm_field WHERE tabid IN ('.  generateQuestionMarks($tabId).') AND fieldlabel=?';
    ...
    public static function getInstanceByName($moduleName) {
        $moduleInstance = Mycrm_Module_Model::getInstance($moduleName);
	...
	public static function getEntityModulesList() {
		...
		$query = 'SELECT name FROM mycrm_tab WHERE
	...
	public function getRelations() {
		if($this->relations === null) {
			$this->relations = Mycrm_Relation_Model::getAllRelations($this, false);
		}
