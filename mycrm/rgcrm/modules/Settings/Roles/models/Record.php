 ...
 */
class Settings_Roles_Record_Model extends Settings_Mycrm_Record_Model {
	...
	public function getChildren() {
		...
		if(!$this->children) {
			...
			$sql = 'SELECT * FROM mycrm_role WHERE parentrole LIKE ? AND depth = ?';
	...
	public function getAllChildren() {
		...
		$sql = 'SELECT * FROM mycrm_role WHERE parentrole LIKE ?';
    ...
    public function getProfileIdList(){
        ...
        $db = PearDatabase::getInstance();
        $query = 'SELECT profileid FROM mycrm_role2profile WHERE roleid=?';
    ...
    public function getDirectlyRelatedProfileId() {
        ...
        $query = 'SELECT directly_related_to_role, mycrm_profile.profileid FROM mycrm_role2profile 
                  INNER JOIN mycrm_profile ON mycrm_profile.profileid = mycrm_role2profile.profileid 
                  WHERE mycrm_role2profile.roleid=?';
...
//	}
	...
	public function save() {
		...
		if(empty($roleId)) {
			$mode = '';
			$roleIdNumber = $db->getUniqueId('mycrm_role');
		...
		if($mode == 'edit') {
			$sql = 'UPDATE mycrm_role SET rolename=?, parentrole=?, depth=?, allowassignedrecordsto=? WHERE roleid=?';
			$params = array($this->getName(), $this->getParentRoleString(), $this->getDepth(), $this->get('allowassignedrecordsto'), $roleId);
			$db->pquery($sql, $params);
		} else {
			$sql = 'INSERT INTO mycrm_role(roleid, rolename, parentrole, depth, allowassignedrecordsto) VALUES (?,?,?,?,?)';
			$params = array($roleId, $this->getName(), $this->getParentRoleString(), $this->getDepth(), $this->get('allowassignedrecordsto'));
			$db->pquery($sql, $params);
			$picklist2RoleSQL = "INSERT INTO mycrm_role2picklist SELECT '".$roleId."',picklistvalueid,picklistid,sortid
					FROM mycrm_role2picklist WHERE roleid = ?";
		...
		if(!empty($profileIds)) {
			...
			if($noOfProfiles > 0) {
				$db->pquery('DELETE FROM mycrm_role2profile WHERE roleid=?', array($roleId));

				$sql = 'INSERT INTO mycrm_role2profile(roleid, profileid) VALUES (?,?)';
	...
	public function delete($transferToRole) {
		...
		$db->pquery('UPDATE mycrm_user2role SET roleid=? WHERE roleid=?', array($transferRoleId, $roleId));

		$db->pquery('DELETE FROM mycrm_role2profile WHERE roleid=?', array($roleId));
		$db->pquery('DELETE FROM mycrm_group2role WHERE roleid=?', array($roleId));
		$db->pquery('DELETE FROM mycrm_group2rs WHERE roleandsubid=?', array($roleId));

                $user_result=$db->pquery('select userid from mycrm_user2role WHERE roleid=?',array($roleId));
		...
		$db->pquery('DELETE FROM mycrm_role WHERE roleid=?', array($roleId));
	...
	public static function getInstanceById($roleId) {
		...
		$sql = 'SELECT * FROM mycrm_role WHERE roleid = ?';
		$params = array($roleId);
		$result = $db->pquery($sql, $params);
	...
	public static function getBaseRole() {
		...
		$sql = 'SELECT * FROM mycrm_role WHERE depth=0 LIMIT 1';
		$params = array();
		$result = $db->pquery($sql, $params);
		if($db->num_rows($result) > 0) {
			return self::getInstanceFromQResult($result, 0);
   ...
   public static function getInstanceByName($name, $excludedRecordId = array()) {
       $db = PearDatabase::getInstance();
       $sql = 'SELECT * FROM mycrm_role WHERE rolename=?';
       $params = array($name);
   ...
   public function getUsers() {
	   $db = PearDatabase::getInstance();
	   $result = $db->pquery('SELECT userid FROM mycrm_user2role WHERE roleid = ?', array($this->getId()));
