...
/**
 * Sharng Access Mycrm Module Model Class
 */
class Settings_SharingAccess_Rule_Model extends Mycrm_Base_Model {
	...
	static $dataShareTableColArr = array (
		...
		self::RULE_TYPE_ROLE => array (
			self::RULE_TYPE_GROUPS => array (
				'table' => 'mycrm_datashare_role2group',
				'source_id' => 'share_roleid',
				'target_id' => 'to_groupid'
			),
			self::RULE_TYPE_ROLE => array (
				'table' => 'mycrm_datashare_role2role',
				'source_id' => 'share_roleid',
				'target_id' => 'to_roleid'
			),
			self::RULE_TYPE_ROLE_AND_SUBORDINATES => array (
				'table' => 'mycrm_datashare_role2rs',
				'source_id' => 'share_roleid',
				'target_id' => 'to_roleandsubid'
			),
		),
		self::RULE_TYPE_ROLE_AND_SUBORDINATES => array (
			self::RULE_TYPE_GROUPS => array (
				'table' => 'mycrm_datashare_rs2grp',
				'source_id' => 'share_roleandsubid',
				'target_id' => 'to_groupid'
			),
			self::RULE_TYPE_ROLE => array (
				'table' => 'mycrm_datashare_rs2role',
				'source_id' => 'share_roleandsubid',
				'target_id' => 'to_roleid'
			),
			self::RULE_TYPE_ROLE_AND_SUBORDINATES => array (
				'table' => 'mycrm_datashare_rs2rs',
				'source_id' => 'share_roleandsubid',
				'target_id' => 'to_roleandsubid'
			),
		),
	...
	public function save() {
		...
		if(!$ruleId) {
			$ruleId = $db->getUniqueId('mycrm_datashare_module_rel');
			$this->set('shareid', $ruleId);

			$db->pquery("INSERT INTO mycrm_datashare_module_rel(shareid, tabid) VALUES(?,?)",
		...
		$sql = 'UPDATE mycrm_datashare_module_rel SET relationtype=? WHERE shareid=?';
	...
	public function delete() {
		...
		$db->pquery('DELETE FROM mycrm_datashare_module_rel WHERE shareid=?', array($ruleId));
        Settings_SharingAccess_Module_Model::recalculateSharingRules();
	}
	...
	public static function getInstance($moduleModel, $ruleId) {
		...
		$sql = 'SELECT * FROM mycrm_datashare_module_rel WHERE tabid = ? AND shareid = ?';
	...
	public static function getAllByModule($moduleModel) {
		...
		$sql = 'SELECT * FROM mycrm_datashare_module_rel WHERE tabid = ?';
