 ...
 */
class Settings_SharingAccess_Module_Model extends Mycrm_Module_Model {
	...
	public function save() {
		...
		$sql = 'UPDATE mycrm_def_org_share SET permission = ? WHERE tabid = ?';
	...
	public static function getInstance($value) {
		...
		if(Mycrm_Utils::isNumber($value)) {
			$query = 'SELECT * FROM mycrm_def_org_share INNER JOIN mycrm_tab ON mycrm_tab.tabid = mycrm_def_org_share.tabid WHERE mycrm_tab.tabid=?';
		} else {
			$query = 'SELECT * FROM mycrm_def_org_share INNER JOIN mycrm_tab ON mycrm_tab.tabid = mycrm_def_org_share.tabid WHERE name=?';
		}
	...
	/**
	 * Static Function to get the instance of Mycrm Module Model for all the modules
	 * @return <Array> - List of Mycrm Module Model or sub class instances
	 */
	public static function getAll($editable=false) {
		...
		$query = 'SELECT * FROM mycrm_def_org_share INNER JOIN mycrm_tab ON mycrm_tab.tabid = mycrm_def_org_share.tabid WHERE mycrm_tab.presence IN (0,2)';
		$params = array();
		if($editable) {
	...
	public static function recalculateSharingRules() {
		...
		$result = $db->pquery('SELECT id FROM mycrm_users WHERE deleted = ?', array(0));
