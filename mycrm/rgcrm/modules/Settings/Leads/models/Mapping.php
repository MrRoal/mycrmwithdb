...
class Settings_Leads_Mapping_Model extends Settings_Mycrm_Module_Model {
	 ...
	 * @return <Array> list of detail view link models <Mycrm_Link_Model>
	 */
	public function getDetailViewLinks() {
		return array(Mycrm_Link_Model::getInstanceFromValues(array(
				'linktype' => 'DETAILVIEW',
				'linklabel' => 'LBL_EDIT',
	 ...
	 * @return <Array> list of mapping link models <Mycrm_Link_Model>
	 */
	...
	public function getMapping($editable = false) {
		if (!$this->mapping) {
			...
			$query = 'SELECT * FROM mycrm_convertleadmapping';
			if ($editable) {
	...
	public function getFieldsInfo($fieldIdsList) {
		$leadModel = Mycrm_Module_Model::getInstance($this->getName());
		$leadId = $leadModel->getId();

		$db = PearDatabase::getInstance();
		$result = $db->pquery('SELECT fieldid, fieldlabel, uitype, typeofdata, fieldname, tablename, tabid FROM mycrm_field WHERE fieldid IN ('. generateQuestionMarks($fieldIdsList). ')', $fieldIdsList);
		$numOfRows = $db->num_rows($result);
	...
	public function save($mapping) {
		...
		if ($deleteMappingsList) {
			$db->pquery('DELETE FROM mycrm_convertleadmapping WHERE editable = 1 AND cfmid IN ('. generateQuestionMarks($deleteMappingsList) .')', $deleteMappingsList);
		}

		if ($createMappingsList) {
			$insertQuery = 'INSERT INTO mycrm_convertleadmapping(leadfid, accountfid, contactfid, potentialfid) VALUES ';
		...
		if ($updateMappingsList) {
			...
			$db->pquery("UPDATE mycrm_convertleadmapping $leadQuery, $accountQuery, $contactQuery, $potentialQuery WHERE editable = ?", array(1));
		}
	}
	...
	public static function getRestrictedFieldIdsList() {
		$db = PearDatabase::getInstance();
		$result = $db->pquery('SELECT * FROM mycrm_convertleadmapping WHERE editable = ?', array(0));
		$numOfRows = $db->num_rows($result);
	...
	public static function deleteMapping($mappingIdsList) {
		$db = PearDatabase::getInstance();
		$db->pquery('DELETE FROM mycrm_convertleadmapping WHERE cfmid IN ('. generateQuestionMarks($mappingIdsList). ')', $mappingIdsList);
	}
}