...
class Settings_Mycrm_TaxRecord_Model extends Mycrm_Base_Model{
	...
	public function getCreateTaxUrl() {
		return '?module=Mycrm&parent=Settings&view=TaxAjax';
	}
	
	public function getEditTaxUrl() {
		return '?module=Mycrm&parent=Settings&view=TaxAjax&type='.$this->getType().'&taxid='.$this->getId();
	}
    ...
    public function addTax() {
        ...
        if($this->isShippingTax()) {
            $taxname = "shtax".$taxid;
            $query = "ALTER TABLE mycrm_inventoryshippingrel ADD COLUMN $taxname decimal(7,3) DEFAULT NULL";
        } else {
            $taxname = "tax".$taxid;
            $query = "ALTER TABLE mycrm_inventoryproductrel ADD COLUMN $taxname decimal(7,3) DEFAULT NULL";
        }
