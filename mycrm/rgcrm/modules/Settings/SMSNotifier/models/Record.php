...
class Settings_SMSNotifier_Record_Model extends Settings_Mycrm_Record_Model {
	...
	public function save() {
		...
		if ($id) {
			$query = 'UPDATE mycrm_smsnotifier_servers SET providertype = ?, isactive = ?, username = ?, password = ?, parameters = ? WHERE id = ?';
			array_push($params, $id);
		} else {
			$query = 'INSERT INTO mycrm_smsnotifier_servers(providertype, isactive, username, password, parameters) VALUES(?, ?, ?, ?, ?)';
		}
	...
	static public function getInstanceById($recordId, $qualifiedModuleName) {
		$db = PearDatabase::getInstance();
		$result = $db->pquery('SELECT * FROM mycrm_smsnotifier_servers WHERE id = ?', array($recordId));
