...
class Settings_Currency_Record_Model extends Settings_Mycrm_Record_Model{
    ...
    public function isDisabledRestricted() {
        ...
        $query = 'SELECT 1 FROM mycrm_users WHERE currency_id = ?';
    ...
    public static function getAllNonMapped($includedIds = array()) {
        ...
        $query = 'SELECT mycrm_currencies.* FROM mycrm_currencies 
                    LEFT JOIN mycrm_currency_info ON mycrm_currency_info.currency_name = mycrm_currencies.currency_name
                    WHERE mycrm_currency_info.currency_name IS NULL or mycrm_currency_info.deleted=1';
        $params = array();
        if(!empty($includedIds)) {
            $params = $includedIds;
            $query .= ' or mycrm_currency_info.id IN('.generateQuestionMarks($includedIds).')';
        }
        $result = $db->pquery($query,$params);
