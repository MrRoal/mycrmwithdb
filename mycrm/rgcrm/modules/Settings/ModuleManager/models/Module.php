...
class Settings_ModuleManager_Module_Model extends Mycrm_Module_Model {

    public static function getNonVisibleModulesList() {
        return array('ModTracker','Webmails','Users','Mobile','Integration','WSAPP',
                     'ConfigEditor','FieldFormulas','MycrmBackup','CronTasks','Import' ,'Tooltip',
	 ...
	 * @return <Array of Mycrm_Module_Model>
	 */
	public static function getModulesSupportingSequenceNumbering() {
		$db = PearDatabase::getInstance();
		$sql="SELECT tabid, name FROM mycrm_tab WHERE isentitytype = 1 AND presence = 0 AND tabid IN
			(SELECT DISTINCT tabid FROM mycrm_field WHERE uitype = '4')";
