...
class Settings_MailConverter_Record_Model extends Settings_Mycrm_Record_Model {
	...
	public function hasRules() {
		...
		$result = $db->pquery('SELECT 1 FROM mycrm_mailscanner_rules WHERE scannerid = ?', array($this->getId()));
	...
	public function getRecordLinks() {
		...
		$links = array();
		if ($this->hasRules()) {
			$links[] = Mycrm_Link_Model::getInstanceFromValues(array(
					'linktype' => 'LISTVIEW',
	...
	public function delete() {
        ...
        $scanner = new Mycrm_MailScannerInfo(trim($this->getName()));
		$scanner->delete();
	}
	...
	public function save() {
        vimport('~~modules/Settings/MailConverter/handlers/MailScannerInfo.php');
		$scannerLatestInfo = new Mycrm_MailScannerInfo(false, false);
		...
		$isConnected = true;
		$scannerOldInfo = new Mycrm_MailScannerInfo($this->get('scannerOldName'));

		if(!$scannerOldInfo->compare($scannerLatestInfo)) {
            vimport('~~modules/Settings/MailConverter/handlers/MailBox.php');
			$mailBox = new Mycrm_MailBox($scannerLatestInfo);
	...
	public function scanNow() {
		...
		if ($isValid) {
			...
			$scannerInfo = new Mycrm_MailScannerInfo($this->getName());
			/** Start the scanning. */
	...
	public function saveFolders() {
		...
		$updateQuery = "UPDATE mycrm_mailscanner_folders SET enabled = CASE folderid ";
	...
	public function updateSequence($sequencesList) {
		...
		$updateQuery = "UPDATE mycrm_mailscanner_rules SET sequence = CASE";
	...
	public static function getCleanInstance() {
		...
		return $recordModel->setModule(Settings_Mycrm_Module_Model::getInstance('Settings:MailConverter'));
	}
	...
	public static function getInstanceById($recordId) {
		$db = PearDatabase::getInstance();
		$result = $db->pquery('SELECT * FROM mycrm_mailscanner WHERE scannerid = ', array($recordId));
		if ($db->num_rows($result)) {
	...
	public static function getAll() {
		$db = PearDatabase::getInstance();
		$moduleModel = Settings_Mycrm_Module_Model::getInstance('Settings:MailConverter');

		$result = $db->pquery('SELECT * FROM mycrm_mailscanner', array());
	...
	public static function getCount() {
		...
		$result = $db->pquery('SELECT 1 FROM mycrm_mailscanner', array());
		$numOfRows = $db->num_rows($result);
		return $numOfRows;
	}
