...
class Settings_MailConverter_RuleRecord_Model extends Settings_Mycrm_Record_Model {
	 ...
	 * @return <Array> List of actions <Mycrm_MailScannerAction>
	 */
	public function getActions() {
		$ruleId = $this->getId();
		if (!$this->actions && $ruleId) {
			$db = PearDatabase::getInstance();
			$result = $db->pquery("SELECT actionid FROM mycrm_mailscanner_ruleactions WHERE ruleid = ?", array($ruleId));
			$numOfRows = $db->num_rows($result);

			for($i=0; $i<$numOfRows; $i++) {
				$actionId = $db->query_result($result, $i, 'actionid');
				$this->actions[$actionId] = new Mycrm_MailScannerAction($actionId);
			}
		}
		return $this->actions;
	}
	...
	public function delete() {
		$rule = new Mycrm_MailScannerRule($this->getId());
		$rule->delete();
	}
	...
	public function save() {
		$recordId = $this->getId();
		$ruleModel = new Mycrm_MailScannerRule($recordId);
	...
	public static function getInstanceById($recordId) {
		$db = PearDatabase::getInstance();
		$result = $db->pquery('SELECT * FROM mycrm_mailscanner_rules WHERE ruleid = ', array($recordId));
	...
	public static function getAll($scannerId) {
		...
		$result = $db->pquery("SELECT * FROM mycrm_mailscanner_rules WHERE scannerid = ? ORDER BY sequence", array($scannerId));
		$numOfRows = $db->num_rows($result);
		for($i=0; $i<$numOfRows; $i++) {
	...
	public static function getRule($scannerId, $ruleId) {
		...
		$result = $db->pquery("SELECT * FROM mycrm_mailscanner_rules WHERE scannerid = ? AND ruleid = ?", array($scannerId, $ruleId));
		if ($db->num_rows($result)) {
	...
	public function getAssignedTo($scannerId, $ruleId) {
		$db = PearDatabase::getInstance();
		$result = $db->pquery("SELECT assigned_to FROM mycrm_mailscanner_rules WHERE scannerid = ? AND ruleid = ?", array($scannerId, $ruleId));
