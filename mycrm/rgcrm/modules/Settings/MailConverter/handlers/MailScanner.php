...
class Mycrm_MailScanner {
	...
	function getMailBox() {
		if(!$this->_mailbox) {
			$this->_mailbox = new Mycrm_MailBox($this->_scannerinfo);
	...
	function markMessageScanned($mailrecord, $crmid=false) {
		...
		$adb->pquery("INSERT INTO mycrm_mailscanner_ids(scannerid, messageid, crmid) VALUES(?,?,?)",
	...
	function isMessageScanned($mailrecord, $lookAtFolder) {
		global $adb;
		$messages = $adb->pquery("SELECT 1 FROM mycrm_mailscanner_ids WHERE scannerid=? AND messageid=?",
		...
		if($adb->num_rows($messages)) {
			...
			if($folderRescan && empty($relatedCRMId)) {
				$adb->pquery("DELETE FROM mycrm_mailscanner_ids WHERE scannerid=? AND messageid=?",
					Array($this->_scannerinfo->scannerid, $mailrecord->_uniqueid));
				$isScanned = false;
			}
		}
	...
	function LookupContact($email) {
		...
		$contactid = false;
		$contactres = $adb->pquery("SELECT contactid FROM mycrm_contactdetails INNER JOIN mycrm_crmentity ON crmid = contactid WHERE setype = ? AND email = ? AND deleted = ?", array('Contacts', $email, 0));
	...
	function LookupLead($email) {
		...
		$leadid = false;
		$leadres = $adb->pquery("SELECT leadid FROM mycrm_leaddetails INNER JOIN mycrm_crmentity ON crmid = leadid WHERE setype=? AND email = ? AND converted = ? AND deleted = ?", array('Leads', $email, 0, 0));
	...
	function LookupAccount($email) {
		...
		$accountid = false;
		$accountres = $adb->pquery("SELECT accountid FROM mycrm_account INNER JOIN mycrm_crmentity ON crmid = accountid WHERE setype=? AND (email1 = ? OR email2 = ?) AND deleted = ?", Array('Accounts', $email, $email, 0));
	...
	function LookupTicket($subjectOrId) {
		...
		if(!$checkTicketId) {
			$ticketres = $adb->pquery("SELECT ticketid FROM mycrm_troubletickets WHERE title = ? OR ticket_no = ?", Array($subjectOrId, $subjectOrId));
		...
		if(!$checkTicketId) {
			$ticketres = $adb->pquery("SELECT ticketid FROM mycrm_troubletickets WHERE ticket_no = ?", Array($subjectOrId));
			if($adb->num_rows($ticketres)) $checkTicketId = $adb->query_result($ticketres, 0, 'ticketid');
		}
		// Nothing found?
		...
		$ticketid = false;
		if($checkTicketId) {
			$crmres = $adb->pquery("SELECT setype, deleted FROM mycrm_crmentity WHERE crmid=?", Array($checkTicketId));
	...
	function getAccountId($contactId) {
		global $adb;
		$result = $adb->pquery("SELECT accountid FROM mycrm_contactdetails WHERE contactid=?", array($contactId));
    ...
    function disableMailScanner(){
		...
		$adb->pquery("UPDATE mycrm_mailscanner SET isvalid=? WHERE scannerid=?", array(0,$scannerId));
    }
