...
class Settings_Workflows_EditTask_View extends Settings_Mycrm_Index_View {
	...
	public function process(Mycrm_Request $request) {
		...
		if ($taskType === 'VTCreateEntityTask') {
			if ($taskObject->entity_type) {
				$relationModuleModel = Mycrm_Module_Model::getInstance($taskObject->entity_type);
        ...
        if($moduleModel->getName() == 'Invoice' || $moduleModel->getName() == 'Quotes') {
            $metaVariables['Portal Pdf Url'] = '(general : (__MycrmMeta__) portalpdfurl)';
        }
		...
		if(!empty ($repeat_date)){
		    $repeat_date = Mycrm_Date_UIType::getDisplayDateValue($repeat_date);
		}
		...
		$fromEmailFieldOptions .= '<option value="$(general : (__MycrmMeta__) supportName)<$(general : (__MycrmMeta__) supportEmailId)>"
		...
		$assignedToValues[vtranslate('LBL_USERS', 'Mycrm')] = $userList;
		$assignedToValues[vtranslate('LBL_GROUPS', 'Mycrm')] = $groupList;
