...
class Settings_Workflows_Edit_View extends Settings_Mycrm_Index_View {
	...
	public function step2(Mycrm_Request $request) {
		...
		if ($recordId) {
			...
			$selectedModule = Mycrm_Module_Model::getInstance($selectedModuleName);
		...
		// Added to show filters only when saved from mycrm6
	...
	function Step3(Mycrm_Request $request) {
		$viewer = $this->getViewer($request);
		$moduleName = $request->getModule();
		$qualifiedModuleName = $request->getModule(false);
