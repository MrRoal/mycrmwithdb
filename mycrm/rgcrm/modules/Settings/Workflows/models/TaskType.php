...
class Settings_Workflows_TaskType_Model extends Mycrm_Base_Model {
	...
	public static function getInstanceFromClassName($taskClass) {
		...
		$result = $db->pquery("SELECT * FROM com_mycrm_workflow_tasktypes where classname=?",array($taskClass));
	...
	public function getTaskBaseModule() {
		...
		switch($taskTypeName) {
			case 'VTCreateTodoTask' : return Mycrm_Module_Model::getInstance('Calendar');
			case 'VTCreateEventTask' : return Mycrm_Module_Model::getInstance('Events');
		}
	}
