 ...
 */
class Settings_Groups_Record_Model extends Settings_Mycrm_Record_Model {
	...
	public function save() {
		...
		if (empty($groupId)) {
			$mode = '';
			$groupId = $db->getUniqueId('mycrm_users');
			$this->setId($groupId);
		}

		if ($mode == 'edit') {
			$sql = 'UPDATE mycrm_groups SET groupname=?, description=? WHERE groupid=?';
			$params = array($this->getName(), $this->getDescription(), $groupId);
		} else {
			$sql = 'INSERT INTO mycrm_groups(groupid, groupname, description) VALUES (?,?,?)';
		...
		if (is_array($members)) {
			$db->pquery('DELETE FROM mycrm_users2group WHERE groupid=?', array($groupId));
			$db->pquery('DELETE FROM mycrm_group2grouprel WHERE groupid=?', array($groupId));
			$db->pquery('DELETE FROM mycrm_group2role WHERE groupid=?', array($groupId));
			$db->pquery('DELETE FROM mycrm_group2rs WHERE groupid=?', array($groupId));
			...
			for ($i = 0; $i < $noOfMembers; ++$i) {
				...
				if ($idComponents && count($idComponents) == 2) {
					...
					if ($memberType == Settings_Groups_Member_Model::MEMBER_TYPE_USERS) {
						$db->pquery('INSERT INTO mycrm_users2group(userid, groupid) VALUES (?,?)', array($memberId, $groupId));
					}
					if ($memberType == Settings_Groups_Member_Model::MEMBER_TYPE_GROUPS) {
						$db->pquery('INSERT INTO mycrm_group2grouprel(containsgroupid, groupid) VALUES (?,?)', array($memberId, $groupId));
					}
					if ($memberType == Settings_Groups_Member_Model::MEMBER_TYPE_ROLES) {
						$db->pquery('INSERT INTO mycrm_group2role(roleid, groupid) VALUES (?,?)', array($memberId, $groupId));
					}
					if ($memberType == Settings_Groups_Member_Model::MEMBER_TYPE_ROLE_AND_SUBORDINATES) {
						$db->pquery('INSERT INTO mycrm_group2rs(roleandsubid, groupid) VALUES (?,?)', array($memberId, $groupId));
					}
				}
			}
		}
	...
	protected function transferOwnership($transferToGroup) {
		...
		$query = 'UPDATE mycrm_crmentity SET smownerid=? WHERE smownerid=?';
	...
	public function delete($transferToGroup) {
		...
		$em->triggerEvent("mycrm.entity.beforegroupdelete", $entityData);
		...
		$db->pquery('DELETE FROM mycrm_group2rs WHERE groupid=?', array($groupId));
		$db->pquery('DELETE FROM mycrm_users2group WHERE groupid=?', array($groupId));
		$db->pquery("DELETE FROM mycrm_reportsharing WHERE shareid=? AND setype='groups'", array($groupId));

		$db->pquery('DELETE FROM mycrm_groups WHERE groupid=?', array($groupId));
	}
	...
	public static function getInstance($value) {
		...
		if (Mycrm_Utils::isNumber($value)) {
			$sql = 'SELECT * FROM mycrm_groups WHERE groupid = ?';
		} else {
			$sql = 'SELECT * FROM mycrm_groups WHERE groupname = ?';
		}
		$params = array($value);
		$result = $db->pquery($sql, $params);
