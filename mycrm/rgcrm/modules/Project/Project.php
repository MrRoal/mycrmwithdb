...
class Project extends CRMEntity {
     ...
     */
    var $customFieldTable = Array('mycrm_projectcf', 'projectid');

    /**
     * Mandatory for Saving, Include tables related to this module.
     */
    var $tab_name = Array('mycrm_crmentity', 'mycrm_project', 'mycrm_projectcf');
    ...
    var $tab_name_index = Array(
		...
		'mycrm_project'   => 'projectid',
	    'mycrm_projectcf' => 'projectid');
	...
	function getListQuery($module, $usewhere='') {
		...
		$query .= "	WHERE mycrm_crmentity.deleted = 0 ".$usewhere;
		return $query;
	}
	...
	function vtlib_handler($modulename, $event_type) {
		if($event_type == 'module.postinstall') {
			...
			$projectsResult = $adb->pquery('SELECT tabid FROM mycrm_tab WHERE name=?', array('Project'));
			...
			if(getTabid('CustomerPortal') && $projectTabid) {
				$checkAlreadyExists = $adb->pquery('SELECT 1 FROM mycrm_customerportal_tabs WHERE tabid=?', array($projectTabid));
				if($checkAlreadyExists && $adb->num_rows($checkAlreadyExists) < 1) {
					$maxSequenceQuery = $adb->query("SELECT max(sequence) as maxsequence FROM mycrm_customerportal_tabs");
					$maxSequence = $adb->query_result($maxSequenceQuery, 0, 'maxsequence');
					$nextSequence = $maxSequence+1;
					$adb->query("INSERT INTO mycrm_customerportal_tabs(tabid,visible,sequence) VALUES ($projectTabid,1,$nextSequence)");
					$adb->query("INSERT INTO mycrm_customerportal_prefs(tabid,prefkey,prefvalue) VALUES ($projectTabid,'showrelatedinfo',1)");
				}
			}

			// Add Gnatt chart to the related list of the module
			$relation_id = $adb->getUniqueID('mycrm_relatedlists');
			$max_sequence = 0;
			$result = $adb->query("SELECT max(sequence) as maxsequence FROM mycrm_relatedlists WHERE tabid=$projectTabid");
			if($adb->num_rows($result)) $max_sequence = $adb->query_result($result, 0, 'maxsequence');
			$sequence = $max_sequence+1;
			$adb->pquery("INSERT INTO mycrm_relatedlists(relation_id,tabid,related_tabid,name,sequence,label,presence) VALUES(?,?,?,?,?,?,?)",
			...
			$accountsModuleInstance = Mycrm_Module::getInstance('Accounts');
			$accountsModuleInstance->setRelatedList($moduleInstance, 'Projects', Array('ADD','SELECT'), 'get_dependents_list');

			// Add Project module to the related list of Accounts module
			$contactsModuleInstance = Mycrm_Module::getInstance('Contacts');
			...
			$helpDeskModuleInstance = Mycrm_Module::getInstance('HelpDesk');
			$helpDeskModuleInstance->setRelatedList($moduleInstance, 'Projects', Array('SELECT'), 'get_related_list');

			$modcommentsModuleInstance = Mycrm_Module::getInstance('ModComments');
			if($modcommentsModuleInstance && file_exists('modules/ModComments/ModComments.php')) {
				include_once 'modules/ModComments/ModComments.php';
				if(class_exists('ModComments')) ModComments::addWidgetTo(array('Project'));
			}

			$result = $adb->pquery("SELECT 1 FROM mycrm_modentity_num WHERE semodule = ? AND active = 1", array($modulename));
			if (!($adb->num_rows($result))) {
				//Initialize module sequence for the module
				$adb->pquery("INSERT INTO mycrm_modentity_num values(?,?,?,?,?,?)", array($adb->getUniqueId("mycrm_modentity_num"), $modulename, 'PROJ', 1, 1, 1));
			}
	...
	function get_gantt_chart($id, $cur_tab_id, $rel_tab_id, $actions=false){
		...
		$related_projecttasks = $adb->pquery("SELECT pt.* FROM mycrm_projecttask AS pt
												INNER JOIN mycrm_crmentity AS crment ON pt.projecttaskid=crment.crmid
		...
		$related_projectmilestones = $adb->pquery("SELECT pm.* FROM mycrm_projectmilestone AS pm
													INNER JOIN mycrm_crmentity AS crment on pm.projectmilestoneid=crment.crmid
	...
	function transferRelatedRecords($module, $transferEntityIds, $entityId) {
		...
		$rel_table_arr = Array("ProjectTask"=>"mycrm_projecttask",'ProjectMilestone'=>'mycrm_projectmilestone',
                                "Documents"=>"mycrm_senotesrel","Attachments"=>"mycrm_seattachmentsrel");

		$tbl_field_arr = Array("mycrm_projecttask"=>"projecttaskid",'mycrm_projectmilestone'=>'projectmilestoneid',
                                "mycrm_senotesrel"=>"notesid","mycrm_seattachmentsrel"=>"attachmentsid");

		$entity_tbl_field_arr = Array("mycrm_projecttask"=>"projectid",'mycrm_projectmilestone'=>'projectid',
                                    "mycrm_senotesrel"=>"crmid","mycrm_seattachmentsrel"=>"crmid");
