...
class Import_Queue_Action extends Mycrm_Action_Controller {
	...
	public static function add($request, $user) {
		...
		if (!Mycrm_Utils::CheckTable('mycrm_import_queue')) {
		...
		$db->pquery('INSERT INTO mycrm_import_queue VALUES(?,?,?,?,?,?,?,?)',
				array($db->getUniqueID('mycrm_import_queue'),
						$user->id,
	...
	public static function remove($importId) {
		$db = PearDatabase::getInstance();
		if(Mycrm_Utils::CheckTable('mycrm_import_queue')) {
			$db->pquery('DELETE FROM mycrm_import_queue WHERE importid=?', array($importId));
		}
	}
	...
	public static function removeForUser($user) {
		$db = PearDatabase::getInstance();
		if(Mycrm_Utils::CheckTable('mycrm_import_queue')) {
			$db->pquery('DELETE FROM mycrm_import_queue WHERE userid=?', array($user->id));
		}
	}
	...
	public static function getUserCurrentImportInfo($user) {
		...
		if(Mycrm_Utils::CheckTable('mycrm_import_queue')) {
			$queueResult = $db->pquery('SELECT * FROM mycrm_import_queue WHERE userid=? LIMIT 1', array($user->id));
	...
	public static function getImportInfo($module, $user) {
		...
		if(Mycrm_Utils::CheckTable('mycrm_import_queue')) {
			$queueResult = $db->pquery('SELECT * FROM mycrm_import_queue WHERE tabid=? AND userid=?',
	...
	public static function getImportInfoById($importId) {
		...
		if(Mycrm_Utils::CheckTable('mycrm_import_queue')) {
			$queueResult = $db->pquery('SELECT * FROM mycrm_import_queue WHERE importid=?', array($importId));
	...
	public static function getAll($status=false) {
		...
		$query = 'SELECT * FROM mycrm_import_queue';
		$params = array();
		if($status !== false) {
			$query .= ' WHERE status = ?';
	...
	static function updateStatus($importId, $status) {
		$db = PearDatabase::getInstance();
		$db->pquery('UPDATE mycrm_import_queue SET status=? WHERE importid=?', array($status, $importId));
	}
