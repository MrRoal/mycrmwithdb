...
class Mycrm_DependencyPicklist {

	static function getDependentPicklistFields($module='') {
		global $adb;

		if(empty($module)) {
			$result = $adb->pquery('SELECT DISTINCT sourcefield, targetfield, tabid FROM mycrm_picklist_dependency', array());
		} else {
			$tabId = getTabid($module);
			$result = $adb->pquery('SELECT DISTINCT sourcefield, targetfield, tabid FROM mycrm_picklist_dependency WHERE tabid=?', array($tabId));
		}
		...
		if($noofrows > 0) {
			...
			for($i=0; $i<$noofrows; ++$i) {
				...
				$fieldResult = $adb->pquery('SELECT fieldlabel FROM mycrm_field WHERE fieldname = ?', array($sourceField));
				$sourceFieldLabel = $adb->query_result($fieldResult,0,'fieldlabel');

				$fieldResult = $adb->pquery('SELECT fieldlabel FROM mycrm_field WHERE fieldname = ?', array($targetField));
	...
	static function getAvailablePicklists($module) {
		...
		$query="select mycrm_field.fieldlabel,mycrm_field.fieldname" .
				" FROM mycrm_field inner join mycrm_picklist on mycrm_field.fieldname = mycrm_picklist.name" .
				" where displaytype=1 and mycrm_field.tabid=? and mycrm_field.uitype in ('15','16') " .
				" and mycrm_field.presence in (0,2) ORDER BY mycrm_picklist.picklistid ASC";
	...
	static function savePickListDependencies($module, $dependencyMap) {
		...
		for($i=0; $i<count($valueMapping); ++$i) {
			...
			$checkForExistenceResult = $adb->pquery("SELECT id FROM mycrm_picklist_dependency WHERE tabid=? AND sourcefield=? AND targetfield=? AND sourcevalue=CAST(? AS CHAR CHARACTER SET utf8) COLLATE utf8_bin",
			...
			if($adb->num_rows($checkForExistenceResult) > 0) {
				...
				$adb->pquery("UPDATE mycrm_picklist_dependency SET targetvalues=?, criteria=? WHERE id=?",
						array($serializedTargetValues, $serializedCriteria, $dependencyId));
			} else {
				$adb->pquery("INSERT INTO mycrm_picklist_dependency (id, tabid, sourcefield, targetfield, sourcevalue, targetvalues, criteria)
								VALUES (?,?,?,?,?,?,?)",
						array($adb->getUniqueID('mycrm_picklist_dependency'), $tabId, $sourceField, $targetField, $sourceValue,
	...
	static function deletePickListDependencies($module, $sourceField, $targetField) {
		...
		$adb->pquery("DELETE FROM mycrm_picklist_dependency WHERE tabid=? AND sourcefield=? AND targetfield=?",
	...
	static function getPickListDependency($module, $sourceField, $targetField) {
		...
		$result = $adb->pquery('SELECT * FROM mycrm_picklist_dependency WHERE tabid=? AND sourcefield=? AND targetfield=?',
				array($tabId,$sourceField,$targetField));
	...
	static function getPicklistDependencyDatasource($module) {
		...
		$result = $adb->pquery('SELECT * FROM mycrm_picklist_dependency WHERE tabid=?', array($tabId));
		$noofrows = $adb->num_rows($result);
	...
	static function getJSPicklistDependencyDatasource($module) {
		$picklistDependencyDatasource = Mycrm_DependencyPicklist::getPicklistDependencyDatasource($module);
	...
	static function checkCyclicDependency($module, $sourceField, $targetField) {
		...
		$result = $adb->pquery('SELECT 1 FROM mycrm_picklist_dependency
	...
	static function getDependentPickListModules() {
		...
		$query = 'SELECT distinct mycrm_field.tabid, mycrm_tab.tablabel, mycrm_tab.name as tabname FROM mycrm_field
						...
						INNER JOIN mycrm_picklist ON mycrm_picklist.name = mycrm_field.fieldname
