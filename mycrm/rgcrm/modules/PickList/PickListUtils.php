...
function getUserFldArray($fld_module,$roleid){
	...
	$query="select mycrm_field.fieldlabel,mycrm_field.columnname,mycrm_field.fieldname, mycrm_field.uitype" .
			" FROM mycrm_field inner join mycrm_picklist on mycrm_field.fieldname = mycrm_picklist.name" .
			" where (displaytype=1 and mycrm_field.tabid=? and mycrm_field.uitype in ('15','55','33','16') " .
			" or (mycrm_field.tabid=? and fieldname='salutationtype' and fieldname !='vendortype')) " .
...
function getPickListModules(){
	...
	$query = 'select distinct mycrm_field.fieldname,mycrm_field.tabid,mycrm_tab.tablabel, mycrm_tab.name as tabname,uitype from mycrm_field inner join mycrm_tab on mycrm_tab.tabid=mycrm_field.tabid where uitype IN (15,33) and mycrm_field.tabid != 29 and mycrm_tab.presence != 1 and mycrm_field.presence in (0,2) order by mycrm_field.tabid ASC';
	// END
...
function getrole2picklist(){
	global $adb;
	$query = "select rolename,roleid from mycrm_role where roleid not in('H1') order by roleid";
	$result = $adb->pquery($query, array());
	while($row = $adb->fetch_array($result)){
...
function getAllPickListValues($fieldName,$lang = Array() ){
	global $adb;
	$sql = 'SELECT * FROM mycrm_'.$adb->sql_escape_string($fieldName);
...
function getEditablePicklistValues($fieldName, $lang= array(), $adb){
	...
	$sql="select $fieldName from mycrm_$fieldName where presence=1 and $fieldName <> '--None--'";
	$res = $adb->query($sql);
...
function getNonEditablePicklistValues($fieldName, $lang=array(), $adb){
	$values = array();
	$fieldName = $adb->sql_escape_string($fieldName);
	$sql = "select $fieldName from mycrm_$fieldName where presence=0";
	$result = $adb->query($sql);
	$count = $adb->num_rows($result);
	for($i=0;$i<$count;$i++){
...
function getAssignedPicklistValues($tableName, $roleid, $adb, $lang=array()){
	...
	if($adb->num_rows($result)){
		...
		$sql = "SELECT distinct ".$adb->sql_escape_string($tableName)." FROM ". $adb->sql_escape_string("mycrm_$tableName")
				. " inner join mycrm_role2picklist on ".$adb->sql_escape_string("mycrm_$tableName").".picklist_valueid=mycrm_role2picklist.picklistvalueid"
