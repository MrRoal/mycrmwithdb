...
require_once 'vtlib/Mycrm/Net/Client.php'; 
...
Class Google_Contacts_Connector extends WSAPP_TargetConnector {
    ...
    /**
     * Tarsform Google Records to Mycrm Records
    ...
    protected function fireRequest($url,$headers,$params=array(),$method='POST') { 
        $httpClient = new Mycrm_Net_Client($url); 
    ...
    public function getContacts($SyncState) {
        ...
        if (count($feed['entry']) > 0) { 
            ...
            $maxModifiedTime = date('Y-m-d H:i:s', strtotime(Google_Contacts_Model::mycrmFormat($lastEntry['updated']['$t'])) + 1); 
            if ($this->totalRecords > $this->maxResults) {
                if (!Google_Utils_Helper::getSyncTime('Contacts', $user)) {
                    $query['updated-min'] = $this->googleFormat(date('Y-m-d H:i:s', strtotime(Google_Contacts_Model::mycrmFormat($lastEntry['updated']['$t'])))); 
...
/**
    ...
    /**
     * Push the mycrm records to google
     * @param <array> $records mycrm records to be pushed to google
     * @return <array> pushed records
     */
    ...
    /**
     * Tarsform  Mycrm Records to Google Records
     * @param <array> $vtContacts 
     * @return <array> tranformed mycrm Records
     */
