...
class RecycleBin_Module_Model extends Mycrm_Module_Model {
	...
	public function getListViewMassActions() {
		...
		foreach($massActionLinks as $massActionLink) {
			$links[] = Mycrm_Link_Model::getInstanceFromValues($massActionLink);
		}
	...
	public function emptyRecycleBin(){
		...
		$getIdsQuery='SELECT crmid from mycrm_crmentity WHERE deleted=?';
		...
		$db->query('DELETE FROM mycrm_crmentity WHERE deleted = 1');
		$db->query('DELETE FROM mycrm_relatedlists_rb');
	...
	public function deleteRecords($recordIds){
	        $db = PearDatabase::getInstance(); 
		//Delete the records in mycrm crmentity and relatedlists.
		$query = 'DELETE FROM mycrm_crmentity WHERE deleted = ? and crmid in('.generateQuestionMarks($recordIds).')';
		$db->pquery($query, array(1, $recordIds));
		
		$query = 'DELETE FROM mycrm_relatedlists_rb WHERE entityid in('.generateQuestionMarks($recordIds).')';
		$db->pquery($query, array($recordIds));

		// Delete entries of attachments from mycrm_attachments and mycrm_seattachmentsrel
		$this->deleteFiles($recordIds);
	...
	public function deleteFiles($recordIds){
		$db = PearDatabase::getInstance(); 
		$getAttachmentsIdQuery='SELECT * FROM mycrm_seattachmentsrel WHERE crmid in('.generateQuestionMarks($recordIds).')';
		...
		if(!empty($attachmentsIds)){
                        $deleteRelQuery='DELETE FROM mycrm_seattachmentsrel WHERE crmid in('.generateQuestionMarks($recordIds).')';
                        $db->pquery($deleteRelQuery,array($recordIds));
                        $attachmentsLocation=array();
                        $getPathQuery='SELECT * FROM mycrm_attachments WHERE attachmentsid in ('.generateQuestionMarks($attachmentsIds).')';
                        ...
                        $deleteAttachmentQuery='DELETE FROM mycrm_attachments WHERE attachmentsid in ('.generateQuestionMarks($attachmentsIds).')';
	...
	public function restore($sourceModule, $recordIds){
		...
		for($i=0;$i<count($recordIds);$i++) {
          ...
          public function getDeletedRecordsTotalCount() {  
                $db = PearDatabase::getInstance();  
                $totalCount = $db->pquery('select count(*) as count from mycrm_crmentity where deleted=1',array());  
