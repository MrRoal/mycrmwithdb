...
class Migration_Index_View extends Mycrm_Basic_View {
	...
	protected function step1(Mycrm_Request $request) {
	...
	protected function step2(Mycrm_Request $request){
		$moduleName = $request->getModule();
		$viewer = $this->getViewer($request);
	...
	public function applyDBChanges(){
		...
		//update mycrm version in db
		$migrationModuleModel->updateMycrmVersion();
	...
	public static function insertAdvFilter($queryid, $filters) {
		if ($queryid != "") {
			...
			self::ExecuteQuery('INSERT INTO mycrm_relcriteria_grouping VALUES(?,?,?,?)', array(1, $queryid, '', $conditionExpression));
		}
	}
