...
class Migration_Module_Model extends Mycrm_Module_Model {
	
	public function getDBVersion(){
		$db = PearDatabase::getInstance();
		
		$result = $db->pquery('SELECT current_version FROM mycrm_version', array());
	...
	public function getLatestSourceVersion(){
		return vglobal('mycrm_current_version');
	}
	
	/**
	 * Function to update the latest mycrm version in db
	 * @return type
	 */
	public function updateMycrmVersion(){
		$db = PearDatabase::getInstance();
		$db->pquery('UPDATE mycrm_version SET current_version=?,old_version=?', array($this->getLatestSourceVersion(), $this->getDBVersion()));
		return true;
	}
