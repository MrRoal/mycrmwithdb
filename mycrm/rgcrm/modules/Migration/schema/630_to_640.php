...
$query = 'UPDATE mycrm_currencies_seq SET id = (SELECT currencyid FROM mycrm_currencies ORDER BY currencyid DESC LIMIT 1)';
$adb->pquery($query, array());

$uniqId = $adb->getUniqueID('mycrm_currencies'); 
$result = $adb->pquery('SELECT 1 FROM mycrm_currencies WHERE currency_name = ?',array('CFP Franc')); 
 
if($adb->num_rows($result)<=0){ 
Migration_Index_View::ExecuteQuery('INSERT INTO mycrm_currencies VALUES (?,?,?,?)', array($uniqId, 'CFP Franc', 'XPF', 'F')); 
} 

//Adding new timezone (GMT+11:00) New Caledonia 
    $sortOrderResult = $adb->pquery("SELECT sortorderid FROM mycrm_time_zone WHERE time_zone = ?", array('Asia/Yakutsk'));
    if ($adb->num_rows($sortOrderResult)) {
        $sortOrderId = $adb->query_result($sortOrderResult, 0, 'sortorderid');
        $adb->pquery("UPDATE mycrm_time_zone SET sortorderid = (sortorderid + 1) WHERE sortorderid > ?", array($sortOrderId));
        Migration_Index_View::ExecuteQuery('INSERT INTO mycrm_time_zone (time_zone, sortorderid, presence) VALUES (?, ?, ?)', array('Etc/GMT-11', ($sortOrderId + 1), 1));
