 ...
 */
class Users_Field_Model extends Mycrm_Field_Model {
	...
	public function getPicklistValues() {
		if($this->get('uitype') == 32) {
			return Mycrm_Language_Handler::getAllLanguages();
		}
        else if ($this->get('uitype') == '115') {
            $db = PearDatabase::getInstance();
            
            $query = 'SELECT '.$this->getFieldName().' FROM mycrm_'.$this->getFieldName();
    ...
    public function getDisplayValue($value, $recordId = false, $recordInstance = false) {
		 ...
		 if($this->get('uitype') == 32){
			return Mycrm_Language_Handler::getLanguageLabel($value);
		 }
