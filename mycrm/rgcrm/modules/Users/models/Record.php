...
class Users_Record_Model extends Mycrm_Record_Model {
	...
	public static function getAll($onlyActive=true) {
		...
		$sql = 'SELECT id FROM mycrm_users';
		$params = array();
		if($onlyActive) {
			$sql .= ' WHERE status = ?';
			$params[] = 'Active';
	...
	public function getImageDetails() {
		...
		if ($recordId) {
			$query = "SELECT mycrm_attachments.* FROM mycrm_attachments
            LEFT JOIN mycrm_salesmanattachmentsrel ON mycrm_salesmanattachmentsrel.attachmentsid = mycrm_attachments.attachmentsid
            WHERE mycrm_salesmanattachmentsrel.smid=?";
	...
	public function getAccessibleUsers($private="",$module = false) {
		...
		$accessibleUser = Mycrm_Cache::get('mycrm-'.$this->getRole().'-'.$currentUserRoleModel->get('allowassignedrecordsto'), 'accessibleusers');
        if(empty($accessibleUser)) {
			...
			Mycrm_Cache::set('mycrm-'.$this->getRole().'-'.$currentUserRoleModel->get('allowassignedrecordsto'), 'accessibleusers',$accessibleUser);
		}
	...
	public function getAllUsersOnRoles($roles) {
		...
		$sql = 'SELECT userid FROM mycrm_user2role WHERE roleid IN ('.  generateQuestionMarks($roleIds).')';
		...
		if($noOfUsers > 0) {
			...
			$query = 'SELECT id, first_name, last_name FROM mycrm_users WHERE status = ? AND id IN ('.  generateQuestionMarks($userIds).')';
	...
	public function getAccessibleGroups($private="",$module = false) {
        ...
        $accessibleGroups = Mycrm_Cache::get('mycrm-'.$private, 'accessiblegroups');
        if(!$accessibleGroups){
            $accessibleGroups = get_group_array(false, "ACTIVE", "", $private,$module);
            Mycrm_Cache::set('mycrm-'.$private, 'accessiblegroups',$accessibleGroups);
        }
	...
	public function deleteImage($imageId) {
		...
		$checkResult = $db->pquery('SELECT smid FROM mycrm_salesmanattachmentsrel WHERE attachmentsid = ?', array($imageId));
		$smId = $db->query_result($checkResult, 0, 'smid');

		if ($this->getId() === $smId) {
			$db->pquery('DELETE FROM mycrm_attachments WHERE attachmentsid = ?', array($imageId));
			$db->pquery('DELETE FROM mycrm_salesmanattachmentsrel WHERE attachmentsid = ?', array($imageId));
			return true;
		}
		return false;
	}
	...
	function saveTagCloud() {
		$db = PearDatabase::getInstance();
		$db->pquery("UPDATE mycrm_homestuff SET visible = ? WHERE userid=? AND stufftype='Tag Cloud'",
	...
	public static function getUserGroups($userId){
		$db = PearDatabase::getInstance();
		$groupIds = array();
		$query = "SELECT groupid FROM mycrm_users2group WHERE userid=?";
    ...
    public static function getCount($onlyActive = false) {
        $db = PearDatabase::getInstance();
        $query = 'SELECT 1 FROM mycrm_users ';
        $params = array();
	...
	public static function getInstanceByName($userName) {
		$db = PearDatabase::getInstance();
		$result = $db->pquery('SELECT id FROM mycrm_users WHERE user_name = ?', array($userName));
	...
	public function delete() {
		...
		$db = PearDatabase::getInstance();
		$query = 'SELECT is_owner FROM mycrm_users WHERE id = ?';
	...
	public function getActiveAdminUsers() {
		...
		$sql = 'SELECT id FROM mycrm_users WHERE status=? AND is_admin=?';
	...
	public function getUserHash() {
		$db = PearDatabase::getInstance();
		$query = 'SELECT user_hash FROM mycrm_users WHERE id = ?';
        ...
        public function deleteUserPermanently($userId, $newOwnerId) {
                ...
                $sql = "UPDATE mycrm_crmentity SET smcreatorid=?,smownerid=? WHERE smcreatorid=? AND setype=?";
                $db->pquery($sql, array($newOwnerId, $newOwnerId, $userId,'ModComments'));
                
                //update history details in mycrm_modtracker_basic 
                $sql ="update mycrm_modtracker_basic set whodid=? where whodid=?"; 
                $db->pquery($sql, array($newOwnerId, $userId)); 

                //update comments details in mycrm_modcomments 
                $sql ="update mycrm_modcomments set userid=? where userid=?"; 
                $db->pquery($sql, array($newOwnerId, $userId));

                $sql = "DELETE FROM mycrm_users WHERE id=?";
