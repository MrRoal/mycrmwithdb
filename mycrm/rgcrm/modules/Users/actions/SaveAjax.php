...
class Users_SaveAjax_Action extends Mycrm_SaveAjax_Action {
	...
	public function process(Mycrm_Request $request) {
		...
		foreach ($fieldModelList as $fieldName => $fieldModel) {
			$fieldValue = $displayValue = Mycrm_Util_Helper::toSafeHTML($recordModel->get($fieldName));
			if ($fieldModel->getFieldDataType() !== 'currency') {
				$displayValue = $fieldModel->getDisplayValue($fieldValue, $recordModel->getId());
			}
			if($fieldName == 'language') {
				$displayValue =  Mycrm_Language_Handler::getLanguageLabel($fieldValue);
			}
	...
	public function userExists(Mycrm_Request $request){
	...
	public function savePassword(Mycrm_Request $request) {
		$module = $request->getModule();
        ...
        /*
         * To restore a user
         * @param Mycrm_Request Object
         */
        public function restoreUser(Mycrm_Request $request) {
                ...
                $db->pquery("UPDATE mycrm_users SET deleted=? WHERE id=?", array(0,$record));
