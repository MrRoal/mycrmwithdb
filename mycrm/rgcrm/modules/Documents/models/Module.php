...
class Documents_Module_Model extends Mycrm_Module_Model {
	...
	public static function getAllFolders() {
		...
		$result = $db->pquery('SELECT * FROM mycrm_attachmentsfolder ORDER BY sequence', array());
	...
	public function getQueryByModuleField($sourceModule, $field, $record, $listQuery) {
		if($sourceModule === 'Emails' && $field === 'composeEmail') {
			$condition = ' (( mycrm_notes.filelocationtype LIKE "%I%")) AND mycrm_notes.filename != "" AND mycrm_notes.filestatus = 1';
		} else {
			$condition = " mycrm_notes.notesid NOT IN (SELECT notesid FROM mycrm_senotesrel WHERE crmid = '$record') AND mycrm_notes.filestatus = 1";
		}
