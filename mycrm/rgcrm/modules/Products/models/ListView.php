...
class Products_ListView_Model extends Mycrm_ListView_Model {
	...
	public function addSubProductsQuery($listQuery){
		...
		$query = " LEFT JOIN mycrm_seproductsrel ON mycrm_seproductsrel.crmid = mycrm_products.productid AND mycrm_seproductsrel.setype='Products'";
		$splitQuery[0] .= $query;
		$productId = $this->get('productId');
		$query1 = " AND mycrm_seproductsrel.productid = $productId";
		$splitQuery[1] .= $query1;
	...
	public function getSubProducts($subProductId){
		...
		if(!empty($subProductId)){
            $db = PearDatabase::getInstance();
			$result = $db->pquery("SELECT mycrm_seproductsrel.crmid from mycrm_seproductsrel INNER JOIN
                mycrm_crmentity ON mycrm_seproductsrel.crmid = mycrm_crmentity.crmid 
					AND mycrm_crmentity.deleted = 0 AND mycrm_seproductsrel.setype=? 
				WHERE mycrm_seproductsrel.productid=?", array($this->getModule()->get('name'), $subProductId ));
