...
class Products_Record_Model extends Mycrm_Record_Model {
	...
	function getTaxes() {
		...
		$result = $db->pquery('SELECT * FROM mycrm_producttaxrel
					INNER JOIN mycrm_inventorytaxinfo ON mycrm_inventorytaxinfo.taxid = mycrm_producttaxrel.taxid
					INNER JOIN mycrm_crmentity ON mycrm_producttaxrel.productid = mycrm_crmentity.crmid AND mycrm_crmentity.deleted = 0
					WHERE mycrm_producttaxrel.productid = ? AND mycrm_inventorytaxinfo.deleted = 0', array($this->getId()));
		$taxes = array();
	...
	static function getListPriceValues($id) {
    	$db = PearDatabase::getInstance();            
        $listPrice = $db->pquery('SELECT * FROM mycrm_productcurrencyrel WHERE productid	= ?', array($id)); 
	...
	function getSubProducts() {
		...
		$result = $db->pquery("SELECT mycrm_products.productid FROM mycrm_products
			INNER JOIN mycrm_crmentity ON mycrm_crmentity.crmid = mycrm_products.productid
			LEFT JOIN mycrm_seproductsrel ON mycrm_seproductsrel.crmid = mycrm_products.productid AND mycrm_products.discontinued = 1 
                        AND mycrm_seproductsrel.setype='Products'
			...
			WHERE mycrm_crmentity.deleted = 0 AND mycrm_seproductsrel.productid = ? ", array($this->getId()));

		$subProductList = array();
		for($i=0; $i<$db->num_rows($result); $i++) {
			$subProductId = $db->query_result($result, $i, 'productid');
			$subProductList[] = Mycrm_Record_Model::getInstanceById($subProductId, 'Products');
		}
	...
	public function getImageDetails() {
		...
		if ($recordId) {
			$sql = "SELECT mycrm_attachments.*, mycrm_crmentity.setype FROM mycrm_attachments
						...
						WHERE mycrm_crmentity.setype = 'Products Image' AND mycrm_seattachmentsrel.crmid = ?";
	...
	public static function getSearchResult($searchKey, $module=false) {
		...
		$query = 'SELECT label, crmid, setype, createdtime FROM mycrm_crmentity WHERE label LIKE ? AND mycrm_crmentity.deleted = 0';
		$params = array("%$searchKey%");

		if($module !== false) {
			$query .= ' AND setype = ?';
			if($module == 'Products'){
				$query = 'SELECT label, crmid, setype, createdtime FROM mycrm_crmentity INNER JOIN mycrm_products ON 
							mycrm_products.productid = mycrm_crmentity.crmid WHERE label LIKE ? AND mycrm_crmentity.deleted = 0 
							AND mycrm_products.discontinued = 1 AND setype = ?';
			}else if($module == 'Services'){
				$query = 'SELECT label, crmid, setype, createdtime FROM mycrm_crmentity INNER JOIN mycrm_service ON 
							mycrm_service.serviceid = mycrm_crmentity.crmid WHERE label LIKE ? AND mycrm_crmentity.deleted = 0 
							AND mycrm_service.discontinued = 1 AND setype = ?';
			}
			$params[] = $module;
		}
	...
	public function getActiveStatusOfRecord(){
		...
		$recordId = $this->getId();
		$db = PearDatabase::getInstance();
		$result = $db->pquery('SELECT discontinued FROM mycrm_products WHERE productid = ?',array($recordId));
