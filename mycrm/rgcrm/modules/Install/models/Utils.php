...
class Install_Utils_Model {
	...
	/**
	 * Function returns the php.ini file settings required for installing mycrmCRM
	 * @return <Array>
	 */
	...
	/**
	 * Variable has the recommended php settings for smooth running of mycrmCRM
	 * @var <Array>
	 */
	...
	/**
	 * Returns the recommended php settings for mycrmCRM
	 * @return type
	 */
	...
	/**
	 * Function checks for mycrmCRM installation prerequisites
	 * @return <Array>
	 */
		...
		if($db_type) {
			...
			if(@$conn->Connect($db_hostname,$db_username,$db_password)) {
				...
				if(@$conn->Connect($db_hostname, $db_username, $db_password, $db_name)) {
					...
					if(!$db_utf8_support) {
						$db_utf8_support = Mycrm_Util_Helper::checkDbUTF8Support($conn);
					}
				}
				$conn->Close();
			}
	...
	public static function installModules() {
		...
		$moduleFolders = array('packages/mycrm/mandatory', 'packages/mycrm/optional');
