...
class Install_Index_view extends Mycrm_View_Controller {
	...
	public function preProcess(Mycrm_Request $request, $display = true) {
		...
		if(is_file($configFileName) && filesize($configFileName) > 0) {
			...
			$defaultModuleInstance = Mycrm_Module_Model::getInstance($defaultModule);
	...
	public function Step1(Mycrm_Request $request) {
		$viewer = $this->getViewer($request);
		$moduleName = $request->getModule();
	...
	public function Step2(Mycrm_Request $request) {
		$viewer = $this->getViewer($request);
		$moduleName = $request->getModule();
		$viewer->view('Step2.tpl', $moduleName);
	}

	public function Step3(Mycrm_Request $request) {
		$viewer = $this->getViewer($request);
		$moduleName = $request->getModule();
	...
	public function Step4(Mycrm_Request $request) {
		$viewer = $this->getViewer($request);
		$moduleName = $request->getModule();
	...
	public function Step5(Mycrm_Request $request) {
	...
	public function Step6(Mycrm_Request $request) {
		$moduleName = $request->getModule();
		$viewer = $this->getViewer($request);
	...
	public function Step7(Mycrm_Request $request) {
		...
		$moduleName = $request->getModule();
                ...
                if(!$isInstalled){
                    ...
                    $viewer->assign('CURRENT_VERSION', $_SESSION['mycrm_version']);
