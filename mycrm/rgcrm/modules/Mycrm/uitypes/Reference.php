...
class Mycrm_Reference_UIType extends Mycrm_Base_UIType {
	...
	public function getReferenceModule($value) {
		...
		if(in_array($referenceEntityType, $referenceModuleList)) {
			return Mycrm_Module_Model::getInstance($referenceEntityType);
		} elseif (in_array('Users', $referenceModuleList)) {
			return Mycrm_Module_Model::getInstance('Users');
		}
		return null;
	}
	...
	public function getDisplayValue($value) {
		...
		if($referenceModule && !empty($value)) {
			...
			if($referenceModuleName == 'Users') {
				...
				$nameResult = $db->pquery('SELECT first_name, last_name FROM mycrm_users WHERE id = ?', array($value));
