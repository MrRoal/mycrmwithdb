...
class Mycrm_ModuleMeta_Model extends Mycrm_Base_Model {
	...
	/**
	 * creates an instance of Mycrm_ModuleMeta_Model
	 * @param <String> $name - module name
	 * @param <Object> $user - Users Object
	 * @return Mycrm_ModuleMeta_Model
	 */
	public static function getInstance($name, $user) {
		$self = new Mycrm_ModuleMeta_Model();
		...
		if(method_exists($focus, 'getImportableFields')) {
			...
			foreach($moduleFields as $fieldName => $fieldInstance) {
				if(($this->isEditableField($fieldInstance)
							&& ($fieldInstance->getTableName() != 'mycrm_crmentity' || $fieldInstance->getColumnName() != 'modifiedby')
