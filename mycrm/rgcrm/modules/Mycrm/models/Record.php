...
/**
 * Mycrm Entity Record Model Class
 */
class Mycrm_Record_Model extends Mycrm_Base_Model {
	...
	public function getName() {
		...
		return Mycrm_Util_Helper::toSafeHTML(decode_html($displayName));
	}
	 ...
	 * @return Mycrm_Record_Model instance
	 */
	...
	/**
	 * Function returns the Mycrm_Field_Model
	...
	/**
	 * Static Function to get the instance of a clean Mycrm Record Model for the given module name
	 * @param <String> $moduleName
	 * @return Mycrm_Record_Model or Module Specific Record Model instance
	 */
	...
	/**
	 * Static Function to get the instance of the Mycrm Record Model given the recordid and the module name
	...
	public static function getInstanceById($recordId, $module=null) {
		...
		if(is_object($module) && is_a($module, 'Mycrm_Module_Model')) {
			...
			$module = Mycrm_Module_Model::getInstance($moduleName);
		}
	...
	public function deleteImage($imageId) {
		...
		$checkResult = $db->pquery('SELECT crmid FROM mycrm_seattachmentsrel WHERE attachmentsid = ?', array($imageId));
		$crmId = $db->query_result($checkResult, 0, 'crmid');

		if ($this->getId() === $crmId) {
			$db->pquery('DELETE FROM mycrm_attachments WHERE attachmentsid = ?', array($imageId));
			$db->pquery('DELETE FROM mycrm_seattachmentsrel WHERE attachmentsid = ?', array($imageId));
			return true;
		}
		return false;
	}
	...
	public function getDescriptionValue() {
		...
		if(empty($description)) {
			$db = PearDatabase::getInstance();
			$result = $db->pquery("SELECT description FROM mycrm_crmentity WHERE crmid = ?", array($this->getId()));
