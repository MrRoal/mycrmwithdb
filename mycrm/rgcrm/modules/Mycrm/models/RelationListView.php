...
class Mycrm_RelationListView_Model extends Mycrm_Base_Model {
	...
	public function getSelectRelationLinks() {
		...
		foreach($selectLinkList as $selectLink) {
			$selectLinkModel[] = Mycrm_Link_Model::getInstanceFromValues($selectLink);
		}
		return $selectLinkModel;
	}
	...
	public function getAddRelationLinks() {
		...
		foreach($addLinkList as $addLink) {
			$addLinkModel[] = Mycrm_Link_Model::getInstanceFromValues($addLink);
		}
		return $addLinkModel;
	}
	...
	public function getEntries($pagingModel) {
		...
		if($orderBy) {
            ...
            if($orderByFieldModuleModel && $orderByFieldModuleModel->isReferenceField()) {
                ...
                $qualifiedOrderBy = 'mycrm_crmentity'.$orderByFieldModuleModel->get('column');
                $selectAndFromClause .= ' LEFT JOIN mycrm_crmentity AS '.$qualifiedOrderBy.' ON '.
                ...
                $query .= ' ORDER BY '.$qualifiedOrderBy.'.label '.$sortOrder;
				 ...
				 $query .= ' ORDER BY COALESCE(CONCAT(mycrm_users.first_name,mycrm_users.last_name),mycrm_groups.groupname) '.$sortOrder;
			} else{
		...
		for($i=0; $i< $db->num_rows($result); $i++ ) {
			...
			$record = Mycrm_Record_Model::getCleanInstance($relationModule->get('name'));
	...
	public function getRelationQuery() {
		...
		$query = $queryComponents[0].' ,mycrm_crmentity.crmid FROM '.$queryComponents[1];
	...
	public static function getInstance($parentRecordModel, $relationModuleName, $label=false) {
		...
		$className = Mycrm_Loader::getComponentClassName('Model', 'RelationListView', $parentModuleName);
		$instance = new $className();

		$parentModuleModel = $parentRecordModel->getModule();
		$relatedModuleModel = Mycrm_Module_Model::getInstance($relationModuleName);
		$instance->setRelatedModuleModel($relatedModuleModel);
		
		$relationModel = Mycrm_Relation_Model::getInstance($parentModuleModel, $relatedModuleModel, $label);
		...
		if(!$relationModel){
			...
			foreach ($referenceFieldOfParentModule as $fieldName=>$fieldModel) {
				...
				if(in_array($relatedModuleName, $refredModulesOfReferenceField)){
					$relationModelClassName = Mycrm_Loader::getComponentClassName('Model', 'Relation', $parentModuleModel->getName());
	...
	public function getRelatedEntriesCount() {
		...
		if ($position) {
			...
			$relationQuery = 'SELECT COUNT(DISTINCT mycrm_crmentity.crmid) AS count'; 
    ...
    public function getCurrencySymbol($recordId, $fieldModel) {
        ...
        if(($fieldName == 'currency_id') && ($moduleName == 'Products' || $moduleName == 'Services')) {
            $query = "SELECT currency_symbol FROM mycrm_currency_info WHERE id = ("; 
            if($moduleName == 'Products') 
                $query .= "SELECT currency_id FROM mycrm_products WHERE productid = ?)"; 
            else if($moduleName == 'Services')
                $query .= "SELECT currency_id FROM mycrm_service WHERE serviceid = ?)"; 

            $result = $db->pquery($query, array($recordId)); 
            return $db->query_result($result, 0, 'currency_symbol');    
        } else if(($tableName == 'mycrm_invoice' || $tableName == 'mycrm_quotes' || $tableName == 'mycrm_purchaseorder' || $tableName == 'mycrm_salesorder') &&
            ...
            $query = "SELECT currency_symbol FROM mycrm_currency_info WHERE id = ( SELECT currency_id FROM ".$tableName." WHERE ".$focus->table_index." = ? )";
            $result = $db->pquery($query, array($recordId)); 
            return $db->query_result($result, 0, 'currency_symbol');
        } else {
