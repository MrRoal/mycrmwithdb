...
class Mycrm_Util_Helper {
	 ...
	 * @return <String> mycrm6 displayable data
	 */
	public static function toMycrm6SafeHTML($input) {
		$allowableTags = '<a><br>';
    ...
    public static function checkRecordExistance($recordId){
        global $adb;
        $query = 'Select deleted from mycrm_crmentity where crmid=?';
	...
	public static function formatDateIntoStrings($date, $time = false) {
		...
		$dateTimeInUserFormat = Mycrm_Datetime_UIType::getDisplayDateTimeValue($date . ' ' . $time);
		...
		if ($currentUser->get('hour_format') === '12') {
			$displayTime = Mycrm_Time_UIType::getTimeValueInAMorPM($displayTime);
		}

		$today = Mycrm_Date_UIType::getDisplayDateValue(date('Y-m-d H:i:s'));
		$tomorrow = Mycrm_Date_UIType::getDisplayDateValue(date('Y-m-d H:i:s', strtotime('tomorrow')));
	...
	public static function getRecordName ($recordId, $checkDelete=false) {
        ...
        $query = 'SELECT label from mycrm_crmentity where crmid=?';
        if($checkDelete) {
            $query.= ' AND deleted=0';
        }
        $result = $adb->pquery($query,array($recordId));
	...
	public static function formatDateTimeIntoDayString($dateTime) {
		$currentUser = Users_Record_Model::getCurrentUserModel();
		$dateTimeInUserFormat = Mycrm_Datetime_UIType::getDisplayDateTimeValue($dateTime);
    ...
    public static function getPickListValues($fieldName) {
		...
		$primaryKey = Mycrm_Util_Helper::getPickListId($fieldName);
        $query = 'SELECT '.$primaryKey.', '.$fieldName.' FROM mycrm_'.$fieldName.' order by sortorderid';
        $values = array();
	...
	public static function getBaseCurrency() {
		$db = PearDatabase::getInstance();
		$result = $db->pquery('SELECT * FROM mycrm_currency_info WHERE defaultid < 0', array());
    ...
    public static function getRoleBasedPicklistValues($fieldName, $roleId) {
        ...
        $query = "SELECT $fieldName
                  FROM mycrm_$fieldName
                      INNER JOIN mycrm_role2picklist on mycrm_role2picklist.picklistvalueid = mycrm_$fieldName.picklist_valueid
                  WHERE roleid=? and picklistid in (select picklistid from mycrm_picklist) order by sortorderid";
...
/**
	...
	public static function getCreator($recordId) {
		...
		$db = PearDatabase::getInstance();
		$result = $db->pquery('SELECT smcreatorid FROM mycrm_crmentity WHERE crmid = ?', array($recordId));
	 ...
	 * @param <DateTime> $dateTime
	 * @param <Mycrm_Users_Model> $userObject
	...
	public static function convertTimeIntoUsersDisplayFormat($time, $userObject = null) {
		...
		if($userModel->get('hour_format') == '12') {
			$time = Mycrm_Time_UIType::getTimeValueInAMorPM($time);
		}
    ...
    */
	...
	public static function getCurrentInfoOfUser() {
		$db = PearDatabase::getInstance();
		$currentUser = Users_Record_Model::getCurrentUserModel();
	    $result = $db->pquery('SELECT * FROM mycrm_currency_info WHERE id = ?', array($currentUser->get('currency_id')));
		if($db->num_rows($result)) return $db->query_result_rowdata($result, 0);
	}
    ...
    */
	public static function setCalendarDefaultActivityTypesForUser($userId) {
		$db = PearDatabase::getInstance();
		$userEntries = $db->pquery('SELECT 1 FROM mycrm_calendar_user_activitytypes WHERE userid=?', array($userId));
                $activityIds = array();
		if($db->num_rows($userEntries) <= 0) {
			$queryResult = $db->pquery('SELECT id, defaultcolor FROM mycrm_calendar_default_activitytypes', array());
			...
			foreach($activityIds as $activityId=>$color) {
				$db->pquery('INSERT INTO mycrm_calendar_user_activitytypes (id, defaultid, userid, color) VALUES (?,?,?,?)', array($db->getUniqueID('mycrm_calendar_user_activitytypes'), $activityId, $userId, $color));
			}
		}
    ...
    public static function isUserDeleted($userid) {
        $db = PearDatabase::getInstance();
        $result = $db->pquery('SELECT deleted FROM mycrm_users WHERE id = ? AND (status=? OR deleted=?)', array($userid, 'Inactive', 1));
