...
class Mycrm_Popup_View extends Mycrm_Footer_View {
	 ...
	 * @return <String>
	 */
	function getModule(Mycrm_request $request) {
		$moduleName = $request->getModule();
		return $moduleName;
	}
	...
	function getHeaderScripts(Mycrm_Request $request) {
		...
		$jsFileNames = array(
			...
			'modules.Mycrm.resources.BaseList',
	...
	public function initializeListViewContents(Mycrm_Request $request, Mycrm_Viewer $viewer) {
        ...
        $isRecordExists = Mycrm_Util_Helper::checkRecordExistance($relatedParentId);
		...
		if(!empty($relatedParentModule) && !empty($relatedParentId)) {
			$parentRecordModel = Mycrm_Record_Model::getInstanceById($relatedParentId, $relatedParentModule);
			$listViewModel = Mycrm_RelationListView_Model::getInstance($parentRecordModel, $moduleName, $label);
		}else{
