...
class Mycrm_FindDuplicates_View extends Mycrm_List_View {
	...
	function getHeaderScripts(Mycrm_Request $request) {
		...
		$jsFileNames = array(
			...
			'modules.Mycrm.resources.FindDuplicates',
		);
	...
	public function initializeListViewContents(Mycrm_Request $request, Mycrm_Viewer $viewer) {
		...
		$massActionLink = array(
			...
			'linkurl' => 'Javascript:Mycrm_FindDuplicates_Js.massDeleteRecords("index.php?module='.$module.'&action=MassDelete");',
			'linkicon' => ''
		);
		$massActionLinks[] = Mycrm_Link_Model::getInstanceFromValues($massActionLink);
		...
		$dataModelInstance = Mycrm_FindDuplicate_Model::getInstance($module);
	...
	function getRecordsCount(Mycrm_Request $request) {
		$moduleName = $request->getModule();
		$duplicateSearchFields = $request->get('fields');
		$dataModelInstance = Mycrm_FindDuplicate_Model::getInstance($moduleName);
