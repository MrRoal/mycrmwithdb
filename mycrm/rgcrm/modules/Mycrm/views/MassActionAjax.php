...
class Mycrm_MassActionAjax_View extends Mycrm_IndexAjax_View {
	 ...
	 */
	function showMassEditForm (Mycrm_Request $request){
		...
		$recordStructureInstance = Mycrm_RecordStructure_Model::getInstanceForModule($moduleModel, Mycrm_RecordStructure_Model::RECORD_STRUCTURE_MODE_MASSEDIT);
		$fieldInfo = array();
	 ...
	 * @param Mycrm_Request $request
	 */
	function showAddCommentForm(Mycrm_Request $request){
	 ...
	 * @param Mycrm_Request $request
	 */
	function showComposeEmailForm(Mycrm_Request $request) {
		$moduleName = 'Emails';
		$sourceModule = $request->getModule();
		$cvId = $request->get('viewname');
		$selectedIds = $request->get('selected_ids');
		...
		$moduleModel = Mycrm_Module_Model::getInstance($sourceModule);
        ...
        if($emailFieldCount > 1) {
            ...
            $queryWithFromClause = 'SELECT '. implode(',',$emailColumnNames). ' FROM mycrm_crmentity ';
            foreach($emailFields as $emailFieldModel) {
                ...
                $queryWithFromClause .= ' INNER JOIN '.$fieldTableName .
                            ' ON '.$fieldTableName.'.'.$tabNameIndexList[$fieldTableName].'= mycrm_crmentity.crmid';
            }
            $query =  $queryWithFromClause . ' WHERE mycrm_crmentity.deleted = 0 AND crmid IN ('.  generateQuestionMarks($recordIds).') AND (';
	 ...
	 * @param Mycrm_Request $request
	 */
	function showSendSMSForm(Mycrm_Request $request) {
		...
		if(count($selectedIds) == 1){
			...
			$selectedRecordModel = Mycrm_Record_Model::getInstanceById($recordId, $sourceModule);
	 ...
	 * @return integer
	 */
	function getRecordsListFromRequest(Mycrm_Request $request, $module = false) {
		$cvId = $request->get('viewname');
		$selectedIds = $request->get('selected_ids');
		$excludedIds = $request->get('excluded_ids');
        if(empty($module)) {
	 ...
	 * @param Mycrm_Request $request
	 */
	function showMailMergeTemplates(Mycrm_Request $request) {
		$selectedIds = $request->get('selected_ids');
		$excludedIds = $request->get('excluded_ids');
		$cvId = $request->get('viewname');
		$module = $request->getModule();
	 ...
	 * @param Mycrm_Request $request
	 */
	function showDuplicatesSearchForm(Mycrm_Request $request) {
		$module = $request->getModule();
	...
	function transferOwnership(Mycrm_Request $request){
		$module = $request->getModule();
		$moduleModel = Mycrm_Module_Model::getInstance($module);
