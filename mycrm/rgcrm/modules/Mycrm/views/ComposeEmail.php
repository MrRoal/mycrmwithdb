...
class Mycrm_ComposeEmail_View extends Mycrm_Footer_View {
	...
	public function composeMailData($request){
		...
		if ($documentIds) {
			...
			foreach ($documentIds as $documentId) {
				$documentRecordModel = Mycrm_Record_Model::getInstanceById($documentId, $sourceModule);
		...
		if ($sourceRecordId) {
			$sourceRecordModel = Mycrm_Record_Model::getInstanceById($sourceRecordId);
		...
		foreach($selectIds as $id) {
			if ($id) {
				...
				if (count($parentIdComponents) > 1) {
					...
					$recordModel = Mycrm_Record_Model::getInstanceById($id, $fieldModule);
				} else {
					$recordModel = Mycrm_Record_Model::getInstanceById($id);
				}
				if($selectedFields){
		...
		$documentsModel = Mycrm_Module_Model::getInstance('Documents');
		$documentsURL = $documentsModel->getInternalDocumentsURL();

		$emailTemplateModuleModel = Settings_Mycrm_Module_Model::getInstance('Settings:EmailTemplates');
		...
		$viewer->assign('MODULE_IS_ACTIVE', $userPrevilegesModel->hasModulePermission(Mycrm_Module_Model::getInstance('EmailTemplates')->getId()));
		//
	...
	function emailEdit($request){
        ...
        foreach($parentIds as $index=>$parentFieldId) {
			...
			if ($parentIdComponents[1] === '-1') {
				...
				$parentRecordModel = Mycrm_Record_Model::getInstanceById($parentId);
			}
