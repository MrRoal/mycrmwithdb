...
class Mycrm_BasicAjax_View extends Mycrm_Basic_View {
	 ...
	 */
	function showAdvancedSearch(Mycrm_Request $request) {
		//Modules for which search is excluded
		$excludedModuleForSearch = array('Mycrm', 'Reports');
	 ...
	 * @param Mycrm_Request $request
	 */
	function showSearchResults(Mycrm_Request $request) {
		...
		if(is_array($advFilterList) && count($advFilterList) > 0) {
			...
			for($i=0; $i<$rows; ++$i) {
				...
				$recordInstance = Mycrm_Record_Model::getInstanceById($row[0]);
			...
			$matchingRecords =  Mycrm_Record_Model::getSearchResult($searchKey, $searchModule);
		}
