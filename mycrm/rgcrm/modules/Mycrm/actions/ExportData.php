...
class Mycrm_ExportData_Action extends Mycrm_Mass_Action {
	 ...
	 * @param Mycrm_Request $request
	 */
	function ExportData(Mycrm_Request $request) {
		$db = PearDatabase::getInstance();
		$moduleName = $request->get('source_module');

		$this->moduleInstance = Mycrm_Module_Model::getInstance($moduleName);
	 ...
	 */
	function getExportQuery(Mycrm_Request $request) {
		...
		switch($mode) {
			...
			case 'ExportCurrentPage' :	$pagingModel = new Mycrm_Paging_Model();
	 ...
	 * @return <String>
	 */
	function getExportContentType(Mycrm_Request $request) {
	...
	function sanitizeValues($arr){
		...
		if(empty ($this->fieldArray)){
			...
			foreach($this->fieldArray as $fieldName => $fieldObj){
				...
				if($fieldObj->get('table') == 'mycrm_inventoryproductrel' && ($fieldName == 'discount_amount' || $fieldName == 'discount_percent')){
		...
		foreach($arr as $fieldName=>&$value){
			...
			if($fieldname != 'hdnTaxType' && ($uitype == 15 || $uitype == 16 || $uitype == 33)){
				...
				$value = Mycrm_Util_Helper::getOwnerName($value);
