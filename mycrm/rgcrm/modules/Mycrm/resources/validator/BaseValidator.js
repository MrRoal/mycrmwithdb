 ...
 * All Rights Reserved.
 *************************************************************************************/
jQuery.Class("Mycrm_Base_Validator_Js",{
	...
	invokeValidation: function(field, rules, i, options){
		...
		var listOfValidators = Mycrm_Base_Validator_Js.getValidator(field);
	...
	getValidator: function(field){
		...
		if(Mycrm_Base_Validator_Js.moduleName == false && moduleEle.length > 0) {
			Mycrm_Base_Validator_Js.moduleName = moduleEle.val();
		}
		
		var fieldInstance = Mycrm_Field_Js.getInstance(fieldInfo);
		var validatorsOfType = Mycrm_Base_Validator_Js.getValidatorsFromFieldType(fieldInstance);
		...
		if(fieldData.hasOwnProperty(dataValidator)){
			...
			for(var key in specialValidators){
				...
				var validatorOfNames = Mycrm_Base_Validator_Js.getValidatorClassName(specialValidator.name);
				if(validatorOfNames != ""){
	...
	getValidatorsFromFieldType: function(fieldInstance){
		...
		validatorsOfType.push(Mycrm_Base_Validator_Js.getValidatorClassName(fieldType));
        return validatorsOfType;
	},
	
	getValidatorClassName: function(validatorName){
		var validatorsOfType = '';
		var className = Mycrm_Base_Validator_Js.getClassName(validatorName);
		var fallBackClassName = Mycrm_Base_Validator_Js.getFallBackClassName(validatorName);
	...
	getClassName: function(validatorName){
		if(Mycrm_Base_Validator_Js.moduleName != false) {
			var moduleName = Mycrm_Base_Validator_Js.moduleName;
		} else {
	...
	getFallBackClassName: function(validatorName){
		return "Mycrm_"+validatorName+"_Validator_Js";
	}
},{
	field: "",
	error: "",
