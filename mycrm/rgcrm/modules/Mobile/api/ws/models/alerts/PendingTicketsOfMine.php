...
class Mobile_WS_AlertModel_PendingTicketsOfMine extends Mobile_WS_AlertModel {
	...
	function query() {
		$sql = "SELECT crmid FROM mycrm_troubletickets INNER JOIN 
				mycrm_crmentity ON mycrm_crmentity.crmid=mycrm_troubletickets.ticketid 
				WHERE mycrm_crmentity.deleted=0 AND mycrm_crmentity.smownerid=? AND 
				mycrm_troubletickets.status <> 'Closed'";
		return $sql;
	}
