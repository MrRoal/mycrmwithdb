...
class Mobile {
	...
	static function alert_lookup($handlerPath, $handlerClass) {
		global $adb;
		$check = $adb->pquery("SELECT id FROM mycrm_mobile_alerts WHERE handler_path=? and handler_class=?", array($handlerPath, $handlerClass));
	...
	static function alert_register($handlerPath, $handlerClass) {
		...
		if (self::alert_lookup($handlerPath, $handlerClass) === false) {
			Mycrm_Utils::Log("Registered alert {$handlerClass} [$handlerPath]");
			$adb->pquery("INSERT INTO mycrm_mobile_alerts (handler_path, handler_class, deleted) VALUES(?,?,?)", array($handlerPath, $handlerClass, 0));
		}
	}
	static function alert_deregister($handlerPath, $handlerClass) {
		global $adb;
		Mycrm_Utils::Log("De-registered alert {$handlerClass} [$handlerPath]");
		$adb->pquery("DELETE FROM mycrm_mobile_alerts WHERE handler_path=? AND handler_class=?", array($handlerPath, $handlerClass));
	}
	static function alert_markdeleted($handlerPath, $handlerClass, $flag) {
		global $adb;
		$adb->pquery("UPDATE mycrm_mobile_alerts SET deleted=? WHERE handler_path=? AND handler_class=?", array($flag, $handlerPath, $handlerClass));
	}
...
class Mobile_WS_Operation {
	...
	function register() {
		global $adb;
		$checkresult = $adb->pquery("SELECT 1 FROM mycrm_ws_operation WHERE name = ?", array($this->opName));
		if($adb->num_rows($checkresult)) {
			return;
		}
		
		Mycrm_Utils::Log("Enabling webservice operation {$this->opName}", true);
