...
class Mobile_ICAL {
	...
	function generate() {
		...
		$properties = array();
		$properties['prodid'] = '-//mycrm/Mobile/NONSGML 1.0//EN';
		...
		$query = "SELECT " . implode(',', $fieldnames) . " FROM mycrm_activity
			INNER JOIN mycrm_crmentity ON
			(mycrm_activity.activityid=mycrm_crmentity.crmid 	AND mycrm_crmentity.deleted = 0 AND mycrm_crmentity.smownerid = ?)
			LEFT JOIN mycrm_activity_reminder ON mycrm_activity_reminder.activity_id=mycrm_activity.activityid
