...
class Rss_Record_Model extends Mycrm_Record_Model {
	...
	public function save($url) {
        ...
        $sql = "INSERT INTO mycrm_rss (rssid,rssurl,rsstitle) values (?,?,?)";
    ...
    public function delete() {
        ...
        $sql = 'DELETE FROM mycrm_rss where rssid = ?';
    ...
    public function makeDefault() {
        ...
        $sql = 'UPDATE mycrm_rss set starred = 0';
        $db->pquery($sql,array());
        
        $sql = 'UPDATE mycrm_rss set starred = 1 where rssid = ?';
        $db->pquery($sql,array($recordId));
    }
	...
	static public function getInstanceById($recordId, $qualifiedModuleName) {
		$db = PearDatabase::getInstance();
		$result = $db->pquery('SELECT * FROM mycrm_rss WHERE rssid = ?', array($recordId));
    ...
    public function getDefaultRss() {
        ...
        $result = $db->pquery('SELECT rssid FROM mycrm_rss where starred = 1', array());
        $recordId = $db->query_result($result,'0','rssid');
        if($recordId) {
            $this->setId($recordId);
        } else {
            $result = $db->pquery('SELECT rssid FROM mycrm_rss', array());
            $recordId = $db->query_result($result,'0','rssid');
            $this->setId($recordId);
        }
    }
