...
class EmailTemplates_Module_Model extends Mycrm_Module_Model {
	...
	public function saveRecord($recordModel) {
		...
		if(empty($templateid)){
			$templateid = $db->getUniqueID('mycrm_emailtemplates');
			$sql = "INSERT INTO mycrm_emailtemplates(templatename, subject, description, body, deleted, templateid) VALUES (?,?,?,?,?,?)";
		}else{
			$sql = "UPDATE mycrm_emailtemplates SET templatename=?, subject=?, description=?, body=?, deleted=? WHERE templateid = ?";
		}
	...
	public function deleteRecord($recordModel) {
		...
		$db = PearDatabase::getInstance();
		$db->pquery('DELETE FROM mycrm_emailtemplates WHERE templateid = ? ', array($recordId));
	}
	...
	public function deleteAllRecords() {
		$db = PearDatabase::getInstance();
		$db->pquery('DELETE FROM mycrm_emailtemplates', array());
	}
	...
	public function getAllModuleList(){
		...
		$query = 'SELECT DISTINCT(name) AS modulename FROM mycrm_tab 
				  LEFT JOIN mycrm_field ON mycrm_field.tabid = mycrm_tab.tabid
				  WHERE (mycrm_field.uitype = ? AND mycrm_tab.presence = ?)';
	...
	public function getRecordIds($skipRecords){
		...
		$query = 'SELECT templateid FROM mycrm_emailtemplates WHERE templateid NOT IN ('.generateQuestionMarks($skipRecords).')';
