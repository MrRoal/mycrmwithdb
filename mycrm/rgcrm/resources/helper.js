...
jQuery.Class("Mycrm_Helper_Js",{
	...
	requestToShowComposeEmailForm : function(selectedId,fieldname,fieldmodule){
		...
		var emailsMassEditInstance = Mycrm_Helper_Js.getEmailMassEditInstance();
	...
	getInternalMailer  : function(selectedId,fieldname,fieldmodule){
		var module = 'Emails';
		var cacheResponse = Mycrm_Helper_Js.checkServerConfigResponseCache;
		var  checkServerConfigPostOperations = function (data) {
			if(data == true){
				Mycrm_Helper_Js.requestToShowComposeEmailForm(selectedId,fieldname,fieldmodule);
			} else {
				alert(app.vtranslate('JS_EMAIL_SERVER_CONFIGURATION'));
			}
		}
		if(cacheResponse === ''){
			var checkServerConfig = Mycrm_Helper_Js.checkServerConfig(module);
			checkServerConfig.then(function(data){
				Mycrm_Helper_Js.checkServerConfigResponseCache = data;
				checkServerConfigPostOperations(Mycrm_Helper_Js.checkServerConfigResponseCache);
			});
		} else {
			checkServerConfigPostOperations(Mycrm_Helper_Js.checkServerConfigResponseCache);
		}
