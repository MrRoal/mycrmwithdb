	...
	registerRecordPreSaveEvent : function(form){
		...
		form.on(Mycrm_Edit_Js.recordPreSave, function(e, data) {
			...
			if(groupingSeparatorValue == decimalSeparatorValue){
				Mycrm_Helper_Js.showPnotify(app.vtranslate('JS_DECIMAL_SEPARATOR_AND_GROUPING_SEPARATOR_CANT_BE_SAME'));
				e.preventDefault();
			}
			...
			if(record == ''){
				if(newPassword != confirmPassword){
					Mycrm_Helper_Js.showPnotify(app.vtranslate('JS_REENTER_PASSWORDS'));
					e.preventDefault();
				}
					...
					thisInstance.checkDuplicateUser(userName).then(
						function(data){
							if(data.result) {
								...
								Mycrm_Helper_Js.showPnotify(app.vtranslate('JS_USER_EXISTS'));
							}
						}, 
						function (data, error){
							thisInstance.duplicateCheckCache[userName] = data.result;
