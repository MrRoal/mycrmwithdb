...
var Mycrm_CustomView_Js = {
	...
	loadFilterView : function(url) {
		...
		AppConnector.request(url).then(
			function(data){
				...
				Mycrm_CustomView_Js.registerEvents();
				Mycrm_CustomView_Js.advanceFilterInstance = Mycrm_AdvanceFilter_Js.getInstance(jQuery('.filterContainer',contents));
			},
			function(error,err){
	...
	getContentsContainer : function() {
		if(Mycrm_CustomView_Js.contentsCotainer == false) {
			Mycrm_CustomView_Js.contentsCotainer = jQuery('div.contentsDiv');
		}
		return Mycrm_CustomView_Js.contentsCotainer;
	},

	getColumnListSelect2Element : function() {
		return Mycrm_CustomView_Js.columnListSelect2Element;
	},
	...
	getColumnSelectElement : function() {
		if(Mycrm_CustomView_Js.columnSelectElement == false) {
			Mycrm_CustomView_Js.columnSelectElement = jQuery('#viewColumnsSelect');
		}
		return Mycrm_CustomView_Js.columnSelectElement;
	},
	...
	getSelectedColumnsList : function() {
		if(Mycrm_CustomView_Js.selectedColumnsList == false) {
			Mycrm_CustomView_Js.selectedColumnsList = jQuery('#selectedColumnsList');
		}
		return Mycrm_CustomView_Js.selectedColumnsList;
	},
	...
	makeColumnListSortable : function() {
		var select2Element = Mycrm_CustomView_Js.getColumnListSelect2Element();
		...
		chozenChoiceElement.sortable({
                ...
                start: function() { Mycrm_CustomView_Js.getSelectedColumnsList().select2("onSortStart"); },
                update: function() { Mycrm_CustomView_Js.getSelectedColumnsList().select2("onSortEnd"); }
            });
	},
	...
	getSelectedColumns : function() {
		var columnListSelectElement = Mycrm_CustomView_Js.getColumnSelectElement();
		var select2Element = Mycrm_CustomView_Js.getColumnListSelect2Element();
	...
	arrangeSelectChoicesInOrder : function() {
		var contentsContainer = Mycrm_CustomView_Js.getContentsContainer();
		var chosenElement = Mycrm_CustomView_Js.getColumnListSelect2Element();
		var choicesContainer = chosenElement.find('ul.select2-choices');
		var choicesList = choicesContainer.find('li.select2-search-choice');
		var coulmnListSelectElement = Mycrm_CustomView_Js.getColumnSelectElement();
	...
	saveAndViewFilter : function(){
		Mycrm_CustomView_Js.saveFilter().then(
			function(response){
				if (response.success) {
					...
					Mycrm_Helper_Js.showPnotify(params);
				}
			},
			function(error) {
	...
	registerSelect2ElementForColumnsSelection : function() {
		var selectElement = Mycrm_CustomView_Js.getColumnSelectElement();
		app.changeSelectElementView(selectElement, 'select2', {maximumSelectionSize: 12,dropdownCss : {'z-index' : 0}});
	},

	registerEvents: function(){
		Mycrm_CustomView_Js.registerSelect2ElementForColumnsSelection();
		var contentsContainer = Mycrm_CustomView_Js.getContentsContainer();
		jQuery('.stndrdFilterDateSelect').datepicker();
		jQuery('.chzn-select').chosen();

		var select2Element = app.getSelect2ElementFromSelect(Mycrm_CustomView_Js.getColumnSelectElement());
		Mycrm_CustomView_Js.columnListSelect2Element = select2Element;

		//To arrange the chosen choices in the order that is selected
		Mycrm_CustomView_Js.arrangeSelectChoicesInOrder();

		jQuery("#standardDateFilter").change(function(){
			Mycrm_CustomView_Js.loadDateFilterValues();
		});

		Mycrm_CustomView_Js.makeColumnListSortable();

		jQuery("#CustomView").submit(function(e) {
			var selectElement = Mycrm_CustomView_Js.getColumnSelectElement();
			var select2Element = app.getSelect2ElementFromSelect(selectElement);
			var result = Mycrm_MultiSelect_Validator_Js.invokeValidation(selectElement);
			if(result != true){
			...
			if(result == true){
				...
				var advfilterlist = Mycrm_CustomView_Js.advanceFilterInstance.getValues();
				jQuery('#advfilterlist').val(JSON.stringify(advfilterlist));
				jQuery('input[name="columnslist"]', contentsContainer).val(JSON.stringify(Mycrm_CustomView_Js.getSelectedColumns()));
				Mycrm_CustomView_Js.saveAndViewFilter();
				return false;
			} else {
