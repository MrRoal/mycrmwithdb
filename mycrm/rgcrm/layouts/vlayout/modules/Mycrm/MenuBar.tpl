						...
						<span class="dropdown span settingIcons">
							...
							<ul class="dropdown-menu themeMenuContainer">
								<div id="themeContainer">
									{assign var=COUNTER value=0}
									{assign var=THEMES_LIST value=Mycrm_Theme::getAllSkins()}
										...
										{foreach key=index item=obj from=$childLinks}
											{if $obj->getLabel() eq NULL}
												...
												{else}
													...
													{/if}
												<li>
														<a target="{$obj->target}" id="menubar_item_right_{Mycrm_Util_Helper::replaceSpaceWithUnderScores($label)}" {if $label=='Switch to old look'}switchLook{/if} href="{$href}" {$onclick}>{vtranslate($label,$MODULE)}</a>
												</li>
											{/if}
										{/foreach}
									</ul>
