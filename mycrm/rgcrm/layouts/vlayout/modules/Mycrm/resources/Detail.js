...
jQuery.Class("Mycrm_Detail_Js",{

    detailInstance : false,

	getInstance: function(){
        if( Mycrm_Detail_Js.detailInstance == false ){
            var module = app.getModuleName();
            var view = app.getViewName();
            var moduleClassName = module+"_"+view+"_Js";
            var fallbackClassName = Mycrm_Detail_Js;
            if(typeof window[moduleClassName] != 'undefined'){
                var instance = new window[moduleClassName]();
            }else{
                var instance = new fallbackClassName();
            }
            Mycrm_Detail_Js.detailInstance = instance;
        }
        return Mycrm_Detail_Js.detailInstance;
	},
	...
	triggerSendEmail : function(detailActionUrl, module){
        Mycrm_Helper_Js.checkServerConfig(module).then(function(data){
			if(data == true){
                var currentInstance = Mycrm_Detail_Js.getInstance();
                ...
                Mycrm_Index_Js.showComposeEmailPopup(params);
			} else {
    ...
    triggerDetailViewAction : function(detailActionUrl, callBackFunction){
		var detailInstance = Mycrm_Detail_Js.getInstance();
    ...
    triggerSendSms : function(detailActionUrl, module) {
        Mycrm_Helper_Js.checkServerConfig(module).then(function(data){
			if(data == true){
                Mycrm_Detail_Js.triggerDetailViewAction(detailActionUrl);
			} else {
	...
	fieldUpdatedEvent : 'Mycrm.Field.Updated',
	widgetPostLoad : 'Mycrm.Widget.PostLoad',
	...
	fieldPreSave : 'Mycrm.Field.PreSave',
	...
	loadRelatedList : function(pageNumber){
		var relatedListInstance = new Mycrm_RelatedList_Js(this.getRecordId(), app.getModuleName(), this.getSelectedTab(), this.getRelatedModuleName());
	...
	registerEventForRelatedListPagination : function(){
		...
		detailContentsHolder.on('click','#relatedListNextPageButton',function(e){
			...
			var relatedController = new Mycrm_RelatedList_Js(thisInstance.getRecordId(), app.getModuleName(), selectedTabElement, relatedModuleName);
	...
	registerEventForRelatedList : function(){
		...
		detailContentsHolder.on('click', 'a.relationDelete', function(e){
			e.stopImmediatePropagation();
			var element = jQuery(e.currentTarget);
			var instance = Mycrm_Detail_Js.getInstance();
	...
	ajaxEditHandling : function(currentTdElement) {
			...
			var saveHandler = function(e) {
                ...
                var fieldInfo = Mycrm_Field_Js.getInstance(fieldElement.data('fieldinfo'));
                ...
                if((""+previousValue) == (""+ajaxEditNewValue)) { // Normalize(99!="099") Fix http://code.mycrm.com/mycrm/mycrmcrm/issues/16 
	...
	registerEventForActivityWidget : function(){
		...
		jQuery('.createActivity').on('click', function(e){
			...
			if(quickCreateNode.length <= 0) {
				Mycrm_Helper_Js.showPnotify(app.vtranslate('JS_NO_CREATE_OR_NOT_QUICK_CREATE_ENABLED'))
			}
			...
			var callbackFunction = function() {
				...
				if(previousValue == ajaxEditNewValue) {
					...
					currentTarget.show();
                                        ...
                                        //If validation fails  
                                        if(errorExists) {  
                                                Mycrm_Helper_Js.addClickOutSideEvent(currentDiv, callbackFunction);   
                                                                return;   
                                        }
	...
	addRelationBetweenRecords : function(relatedModule, relatedModuleRecordId){
		...
		var thisInstance = this;
		var selectedTabElement = thisInstance.getSelectedTab();
		var relatedController = new Mycrm_RelatedList_Js(thisInstance.getRecordId(), app.getModuleName(), selectedTabElement, relatedModule);
	...
	registerEventForEmailsRelatedRecord : function(){
		...
		var popupInstance = Mycrm_Popup_Js.getInstance();
		...
		detailContentsHolder.on('click','[name="emailsEditView"]',function(e){
			...
			Mycrm_Helper_Js.checkServerConfig(module).then(function(data){
				if(data == true){
					...
					Mycrm_Helper_Js.showPnotify(app.vtranslate('JS_EMAIL_SERVER_CONFIGURATION'));
				}
			})
		})
	},
	...
	registerPostTagCloudWidgetLoad : function() {
		var thisInstance = this;
		app.getContentsContainer().on('Mycrm.Widget.Load.LBL_TAG_CLOUD',function(e,data){
	...
	registerEvents : function(){
		...
		detailContentsHolder.on('click', '.relatedPopup', function(e){
			var editViewObj = new Mycrm_Edit_Js();
