...
var Mycrm_Index_Js = {
	...
	registerWidgetsEvents : function() {
		...
		widgets.on({
				shown: function(e) {
					...
					Mycrm_Index_Js.loadWidgets(widgetContainer);
	...
	loadWidgets : function(widgetContainer, open) {
		...
		AppConnector.request(listViewWidgetParams).then(
			...
			if(typeof open == 'undefined') open = true;
				...
				jQuery('.bodyContents').trigger('Mycrm.Widget.Load.'+label,jQuery(widgetContainer));
			}
		);
	},
	...
	showComposeEmailPopup : function(params, cb){
		...
		Mycrm_Helper_Js.checkServerConfig(currentModule).then(function(data){
			if(data == true){
	...
	registerActivityReminder : function() {
		...
		if(activityReminder != '') {
			...
			if((currentTime + activityReminder) > nextActivityReminderCheck) {
				Mycrm_Index_Js.requestReminder();
				setTimeout('Mycrm_Index_Js.requestReminder()', activityReminder);
		...
		AppConnector.request(url).then(function(data){
			if(data.success && data.result) {
				for(i=0; i< data.result.length; i++) {
					...
					Mycrm_Index_Js.showReminderPopup(record);
				}
			}
		});
	},
	...
	showReminderPopup : function(record) {
		...
		var notify = Mycrm_Helper_Js.showPnotify(params);
	...
	registerResizeEvent: function(){
		...
		$(window).bind('resizeEnd', function() {
			Mycrm_Index_Js.adjustTopMenuBarItems();
		});
	},
	/**
	...
	registerTooltipEvents: function() {
		...
		function registerToolTipDestroy() {
			jQuery('button[name="vtTooltipClose"]').on('click', function(e){
				...
				lastPopover.popover('hide');
				// Fix suggested http://code.mycrm.com/mycrm/mycrmcrm/issues/43
	...
	registerEvents : function(){
		Mycrm_Index_Js.registerWidgetsEvents();
		Mycrm_Index_Js.loadWidgetsOnLoad();
		Mycrm_Index_Js.registerActivityReminder();
		Mycrm_Index_Js.adjustTopMenuBarItems();
		Mycrm_Index_Js.registerPostAjaxEvents();
		Mycrm_Index_Js.changeSkin();
		Mycrm_Index_Js.registerShowHideLeftPanelEvent();
		Mycrm_Index_Js.registerResizeEvent();
	},

	registerPostAjaxEvents: function() {
		Mycrm_Index_Js.registerTooltipEvents();
	}
}
