...
jQuery.Class("Mycrm_List_Js",{

	listInstance : false,

	getRelatedModulesContainer : false,

	massEditPreSave : 'Mycrm.MassEdit.PreSave',

   getInstance: function(){
		if(Mycrm_List_Js.listInstance == false){
			...
			if(parentModule == 'Settings'){
				...
				var fallbackClassName = parentModule+"_Mycrm_List_Js";
				if(typeof window[fallbackClassName] == 'undefined') {
					fallbackClassName = "Mycrm_List_Js";
				}
			} else {
				moduleClassName = module+"_List_Js";
				fallbackClassName = "Mycrm_List_Js";
			...
			Mycrm_List_Js.listInstance = instance;
			return instance;
		}
		return Mycrm_List_Js.listInstance;
	},
		/*
	...
	triggerSendEmail : function(massActionUrl, module, params){
		var listInstance = Mycrm_List_Js.getInstance();
		...
		if(validationResult != true){
			...
			Mycrm_Index_Js.showComposeEmailPopup(actionParams);
		} else {
	...
	triggerSendSms : function(massActionUrl, module){
		...
		if(validationResult != true){
			...
			Mycrm_Helper_Js.checkServerConfig(module).then(function(data){
				...
				if(data == true){
					Mycrm_List_Js.triggerMassAction(massActionUrl);
				} else {
		...
		if(validationResult != true){
			...
			Mycrm_Helper_Js.showConfirmationBox({'message' : message}).then(
				function(e) {
                    ...
                    var listViewInstance = Mycrm_List_Js.getInstance();
	...
	triggerMassEdit : function(massEditUrl) {
		...
		Mycrm_List_Js.triggerMassAction(massEditUrl, function(container){
			...
			var editInstance = Mycrm_Edit_Js.getInstance();
	...
	showDuplicateSearchForm : function(url) {
		...
		app.showModalWindow("", url, function() {
			progressIndicatorElement.progressIndicator({'mode' : 'hide'});
			Mycrm_List_Js.registerDuplicateSearchButtonEvent();
		});
	},
	...
	massActionSave : function(form, isMassEdit){
		...
		if(isMassEdit) {
			...
			if(fieldsChanged == false){
				Mycrm_Helper_Js.showPnotify(app.vtranslate('NONE_OF_THE_FIELD_VALUES_ARE_CHANGED_IN_MASS_EDIT'));
				form.find('[name="saveButton"]').removeAttr('disabled');
				aDeferred.reject();
				return aDeferred.promise();
			}
			//on submit form trigger the massEditPreSave event
			var massEditPreSaveEvent = jQuery.Event(Mycrm_List_Js.massEditPreSave);
	...
	registerRecordAccessCheckEvent : function(form) {
		...
		form.on(Mycrm_List_Js.massEditPreSave, function(e) {
	...
	registerPageNavigationEvents : function(){
		...
		jQuery('#listViewPageJump').on('click',function(e){
            if(typeof Mycrm_WholeNumberGreaterThanZero_Validator_Js.invokeValidation(jQuery('#pageToJump'))!= 'undefined') {
		...
		jQuery('#listViewPageJumpDropDown').on('click','li',function(e){
			...
			if(e.which == 13){
				...
				var response = Mycrm_WholeNumberGreaterThanZero_Validator_Js.invokeValidation(element);
	...
	*/
	...
	registerCreateFilterClickEvent : function(){
		...
		jQuery('#createFilter').on('click',function(event){
			...
			Mycrm_CustomView_Js.loadFilterView(createUrl);
		});
	},
	...
	registerEditFilterClickEvent : function(){
		...
		if(listViewFilterBlock != false){
			listViewFilterBlock.on('mouseup','li i.editFilter',function(event){
				...
				Mycrm_CustomView_Js.loadFilterView(editUrl);
	...
	registerDeleteRecordClickEvent: function(){
		...
		listViewContentDiv.on('click','.deleteRecordButton',function(e){
			...
			Mycrm_List_Js.deleteRecord(recordId);
			e.stopPropagation();
		});
	},
	/*
	...
	registerReferenceFieldsForValidation : function(form){
		...
		form.find('.sourceField').on(Mycrm_Edit_Js.referenceSelectionEvent,function(e,params){
			var element = jQuery(e.currentTarget);
		...
		form.find('.clearReferenceSelection').on(Mycrm_Edit_Js.referenceDeSelectionEvent,function(e){
