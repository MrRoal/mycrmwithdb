 ...
 * All Rights Reserved.
 ************************************************************************************/
Mycrm_List_Js('Mycrm_FindDuplicates_Js',{

	massDeleteRecords : function(url) {
		var listInstance = new Mycrm_FindDuplicates_Js();
		...
		Mycrm_List_Js.massDeleteRecords(url,listInstance);
	}
},{
	...
	registerMergeRecordEvent : function(cb) {
		...
		jQuery('input[name="merge"]').on('click', function(e) {
			...
			if(mergeRecordsCheckBoxes.length < 2) {
				Mycrm_Helper_Js.showMessage({text: app.vtranslate('JS_SELECT_ATLEAST_TWO_RECORD_FOR_MERGING')});
				return false;
			} else {
				var count = 0;
				var records = [];
				...
				mergeRecordsCheckBoxes.each(function(key, obj) {
					...
					if(ele.data('group') != groupName) {
						Mycrm_Helper_Js.showMessage({text: app.vtranslate('JS_SELECT_RECORDS_TO_MERGE_FROM_SAME_GROUP')});
						stop = true;
						return false;
					}
				...
				if(count > 3) {
					Mycrm_Helper_Js.showMessage({text: app.vtranslate('JS_ALLOWED_TO_SELECT_MAX_OF_THREE_RECORDS')});
					return false;
				}
