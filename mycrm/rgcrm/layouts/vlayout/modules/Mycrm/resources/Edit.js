...
jQuery.Class("Mycrm_Edit_Js",{

	//Event that will triggered when reference field is selected
	referenceSelectionEvent : 'Mycrm.Reference.Selection',

	//Event that will triggered when reference field is selected
	referenceDeSelectionEvent : 'Mycrm.Reference.DeSelection',

	//Event that will triggered before saving the record
	recordPreSave : 'Mycrm.Record.PreSave',

	refrenceMultiSelectionEvent : 'Mycrm.MultiReference.Selection',

	preReferencePopUpOpenEvent : 'Mycrm.Referece.Popup.Pre',

	editInstance : false,

    postReferenceSelectionEvent: 'Mycrm.PostReference.Selection',
	...
	getInstanceByModuleName : function(moduleName){
		...
		if(parentModule == 'Settings'){
			...
			var fallbackClassName = parentModule+"_Mycrm_Edit_Js";
			if(typeof window[fallbackClassName] == 'undefined') {
				fallbackClassName = "Mycrm_Edit_Js";
			}
		} else {
			moduleClassName = moduleName+"_Edit_Js";
			fallbackClassName = "Mycrm_Edit_Js";
	...
	getInstance: function(){
		if(Mycrm_Edit_Js.editInstance == false){
			var instance = Mycrm_Edit_Js.getInstanceByModuleName();
			Mycrm_Edit_Js.editInstance = instance;
			return instance;
		}
		return Mycrm_Edit_Js.editInstance;
	}
	...
	openPopUp : function(e){
		...
		var prePopupOpenEvent = jQuery.Event(Mycrm_Edit_Js.preReferencePopUpOpenEvent);
		...
		popupInstance.show(params,function(data){
			...
			if(isMultiple) {
                    sourceFieldElement.trigger(Mycrm_Edit_Js.refrenceMultiSelectionEvent,{'data':dataList});
			}
                sourceFieldElement.trigger(Mycrm_Edit_Js.postReferenceSelectionEvent,{'data':responseData});
		});
	},
	...
	setReferenceFieldValue : function(container, params) {
		...
		fieldElement.trigger(Mycrm_Edit_Js.referenceSelectionEvent, {'source_module' : popupReferenceModule, 'record' : id, 'selectedName' : selectedName});
	...
	registerAutoCompleteFields : function(container) {
		...
		container.find('input.autoComplete').autocomplete({
			...
			'select' : function(event, ui ){
                ...
                fieldElement.trigger(Mycrm_Edit_Js.postReferenceSelectionEvent,{'data':selectedItemData});
			},
	...
	registerClearReferenceSelectionEvent : function(container) {
		container.find('.clearReferenceSelection').on('click', function(e){
			...
			element.trigger(Mycrm_Edit_Js.referenceDeSelectionEvent);
			e.preventDefault();
		})
	},
	...
	registerEventForImageDelete : function(){
		...
		formElement.find('.imageDelete').on('click',function(e){
			...
			if(exisitingImages.length < 1 && mandatoryStatus){
				...
				imageUploadElement.attr('data-validation-engine','validate[required,funcCall[Mycrm_Base_Validator_Js.invokeValidation]]');
		...
		editViewForm.submit(function(e){
			...
			if(typeof editViewForm.data('submit') != "undefined") {
				...
				if(editViewForm.validationEngine('validate')) {
					...
					var recordPreSaveEvent = jQuery.Event(Mycrm_Edit_Js.recordPreSave);
	...
	registerRecordAccessCheckEvent : function(form) {
		...
		form.on(Mycrm_Edit_Js.recordPreSave, function(e, data) {
