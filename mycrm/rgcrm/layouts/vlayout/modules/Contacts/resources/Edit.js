 ...
 * All Rights Reserved.
 *************************************************************************************/
Mycrm_Edit_Js("Contacts_Edit_Js",{},{
	...
	registerReferenceSelectionEvent : function(container) {
		...
		jQuery('input[name="account_id"]', container).on(Mycrm_Edit_Js.referenceSelectionEvent, function(e, data){
	...
	checkForPortalUser : function(form){
		...
		if(response){
			if(primaryEmailField.length == 0){
				Mycrm_Helper_Js.showPnotify(app.vtranslate('JS_PRIMARY_EMAIL_FIELD_DOES_NOT_EXISTS'));
				return false;
			}
			if(primaryEmailValue == ""){
				Mycrm_Helper_Js.showPnotify(app.vtranslate('JS_PLEASE_ENTER_PRIMARY_EMAIL_VALUE_TO_ENABLE_PORTAL_USER'));
				return false;
			}
		}
		return true;
