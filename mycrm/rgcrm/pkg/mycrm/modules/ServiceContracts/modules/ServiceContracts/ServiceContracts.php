...
class ServiceContracts extends CRMEntity {
	...
	var $table_name = 'mycrm_servicecontracts';
	 ...
	 */
	var $customFieldTable = Array('mycrm_servicecontractscf', 'servicecontractsid');

	/**
	 * Mandatory for Saving, Include tables related to this module.
	 */
	var $tab_name = Array('mycrm_crmentity', 'mycrm_servicecontracts', 'mycrm_servicecontractscf');
	...
	var $tab_name_index = Array(
		...
		'mycrm_servicecontracts' => 'servicecontractsid',
		'mycrm_servicecontractscf'=>'servicecontractsid');
	...
	var $search_fields = Array(
		...
		'Subject' => Array('servicecontracts', 'subject'),
		'Contract No' => Array('servicecontracts', 'contract_no'),
		'Assigned To' => Array('mycrm_crmentity','assigned_user_id'),
		'Used Units' => Array('servicecontracts','used_units'),
		'Total Units' => Array('servicecontracts','total_units')
	);
	...
	function generateReportsSecQuery($module,$secmodule,$queryplanner) {
        ...
        $matrix->setDependency('mycrm_crmentityServiceContracts',array('mycrm_groupsServiceContracts','mycrm_usersServiceContracts'));
		$matrix->setDependency('mycrm_servicecontracts',array('mycrm_servicecontractscf','mycrm_crmentityServiceContracts'));
        if (!$queryplanner->requireTable('mycrm_servicecontracts', $matrix)) {
			return '';
		}

		$query = $this->getRelationQuery($module,$secmodule,"mycrm_servicecontracts","servicecontractsid", $queryplanner);

        if ($queryplanner->requireTable("mycrm_crmentityServiceContracts",$matrix)){
			$query .= " left join mycrm_crmentity as mycrm_crmentityServiceContracts on mycrm_crmentityServiceContracts.crmid = mycrm_servicecontracts.servicecontractsid  and mycrm_crmentityServiceContracts.deleted=0";
		}
        if ($queryplanner->requireTable("mycrm_servicecontractscf")){
		    $query .= " left join mycrm_servicecontractscf on mycrm_servicecontractscf.servicecontractsid = mycrm_servicecontracts.servicecontractsid";
		}
        if ($queryplanner->requireTable("mycrm_groupsServiceContracts")){
			$query .= " left join mycrm_groups as mycrm_groupsServiceContracts on mycrm_groupsServiceContracts.groupid = mycrm_crmentityServiceContracts.smownerid";
		}
		if ($queryplanner->requireTable("mycrm_usersServiceContracts")){
			$query .= " left join mycrm_users as mycrm_usersServiceContracts on mycrm_usersServiceContracts.id = mycrm_crmentityServiceContracts.smownerid";
		}
        if($queryplanner->requireTable("mycrm_contactdetailsRelServiceContracts")){
            $query .= " left join mycrm_contactdetails as mycrm_contactdetailsRelServiceContracts on mycrm_contactdetailsRelServiceContracts.contactid = mycrm_servicecontracts.sc_related_to";
        }
        if($queryplanner->requireTable("mycrm_accountRelServiceContracts")){
            $query .= " left join mycrm_account as mycrm_accountRelServiceContracts on mycrm_accountRelServiceContracts.accountid = mycrm_servicecontracts.sc_related_to";
        }
         if ($queryplanner->requireTable("mycrm_lastModifiedByServiceContracts")){
			$query .= " left join mycrm_users as mycrm_lastModifiedByServiceContracts on mycrm_lastModifiedByServiceContracts.id = mycrm_crmentityServiceContracts.modifiedby ";
		}
        if ($queryplanner->requireTable("mycrm_createdbyServiceContracts")){
			$query .= " left join mycrm_users as mycrm_createdbyServiceContracts on mycrm_createdbyServiceContracts.id = mycrm_crmentityServiceContracts.smcreatorid ";
		}
        return $query;
	}
	/**
	...
	*/
	function vtlib_handler($moduleName, $eventType) {
		...
		global $adb;
 		...
 		if($eventType == 'module.postinstall') {
			...
			$helpDeskInstance = Mycrm_Module::getInstance("HelpDesk");
			$helpDeskInstance->setRelatedList($moduleInstance,"Service Contracts",Array('ADD','SELECT'));

			// Initialize module sequence for the module
			$adb->pquery("INSERT into mycrm_modentity_num values(?,?,?,?,?,?)",array($adb->getUniqueId("mycrm_modentity_num"),$moduleName,'SERCON',1,1,1));

			// Make the picklist value 'Complete' for status as non-editable
			$adb->query("UPDATE mycrm_contract_status SET presence=0 WHERE contract_status='Complete'");
	...
	function save_related_module($module, $crmid, $with_module, $with_crmids) {
	 ...
	 function updateHelpDeskRelatedTo($focusId, $entityIds) {
		...
		$selectTicketsQuery = "SELECT ticketid FROM mycrm_troubletickets
		...
		for($i=0; $i < $noOfTickets; ++$i) {
			...
			$serviceContractsRelateToTypeResult = $this->db->pquery('SELECT setype FROM mycrm_crmentity WHERE crmid =
				(SELECT sc_related_to FROM mycrm_servicecontracts WHERE servicecontractsid = ?)', array($focusId));
			...
			if($serviceContractsRelateToType == 'Accounts') {
				$updateQuery = "UPDATE mycrm_troubletickets, mycrm_servicecontracts SET parent_id=mycrm_servicecontracts.sc_related_to" .
					" WHERE mycrm_servicecontracts.sc_related_to IS NOT NULL AND mycrm_servicecontracts.sc_related_to != 0" .
					" AND mycrm_servicecontracts.servicecontractsid = ? AND mycrm_troubletickets.ticketid = ?";
				$this->db->pquery($updateQuery, array($focusId, $ticketId));
			} elseif($serviceContractsRelateToType == 'Contacts') {
				$updateQuery = "UPDATE mycrm_troubletickets, mycrm_servicecontracts SET contact_id=mycrm_servicecontracts.sc_related_to" .
					" WHERE mycrm_servicecontracts.sc_related_to IS NOT NULL AND mycrm_servicecontracts.sc_related_to != 0" .
					" AND mycrm_servicecontracts.servicecontractsid = ? AND mycrm_troubletickets.ticketid = ?";
				$this->db->pquery($updateQuery, array($focusId, $ticketId));
			}
	...
	function updateServiceContractState($focusId) {
		...
		$contractTicketsResult = $this->db->pquery("SELECT relcrmid FROM mycrm_crmentityrel
														WHERE module = 'ServiceContracts'
														AND relmodule = 'HelpDesk' AND crmid = ?
													UNION
														SELECT crmid FROM mycrm_crmentityrel
	...
	function updateUsedUnits($usedUnits) {
		...
		$updateQuery = "UPDATE mycrm_servicecontracts SET used_units = $usedUnits WHERE servicecontractsid = ?";
	...
	function calculateProgress() {
		...
		if($contractStatus == 'Complete' || (!empty($usedUnits) && !empty($totalUnits) && $usedUnits >= $totalUnits)) {
			if(empty($endDate)) {
				$endDate = date('Y-m-d');
				$this->db->pquery('UPDATE mycrm_servicecontracts SET end_date=? WHERE servicecontractsid = ?', array(date('Y-m-d'), $this->id));
			}
		} else {
			$endDate = null;
			$this->db->pquery('UPDATE mycrm_servicecontracts SET end_date=? WHERE servicecontractsid = ?', array(null, $this->id));
		}
		...
		if(count($updateCols) > 0) {
			$updateQuery = 'UPDATE mycrm_servicecontracts SET '. implode(",", $updateCols) .' WHERE servicecontractsid = ?';
	...
	function unlinkRelationship($id, $return_module, $return_id) {
		...
		$sql = 'SELECT tabid, tablename, columnname FROM mycrm_field WHERE fieldid IN (SELECT fieldid FROM mycrm_fieldmodulerel WHERE module=? AND relmodule IN ('.$return_modules.'))';
	...
	function transferRelatedRecords($module, $transferEntityIds, $entityId) {
		...
		$rel_table_arr = Array("Documents"=>"mycrm_senotesrel","Attachments"=>"mycrm_seattachmentsrel");

		$tbl_field_arr = Array("mycrm_senotesrel"=>"notesid","mycrm_seattachmentsrel"=>"attachmentsid");

		$entity_tbl_field_arr = Array("mycrm_senotesrel"=>"crmid","mycrm_seattachmentsrel"=>"crmid");
