...
require_once 'modules/Mycrm/CRMEntity.php';
require_once 'include/QueryGenerator/QueryGenerator.php';
require_once 'vtlib/Mycrm/Mailer.php';
require_once 'include/events/include.inc';


class Import_Data_Action extends Mycrm_Action_Controller {
	...
	public function getDefaultFieldValues($moduleMeta) {
		...
		if ($className != 'MycrmLineItemMeta') {
	...
	public function importData() {
		...
		$moduleModel = Mycrm_Module_Model::getInstance($this->module);
        // pre fetch the fields and premmisions of module
        Mycrm_Field_Model::getAllForModule($moduleModel);
        if($this->user->is_admin == 'off'){
            Mycrm_Field_Model::preFetchModuleFieldPermission($moduleModel->getId());
        }
	...
	public function createRecords() {
		...
		for ($i = 0; $i < $numberOfRecords; ++$i) {
            ...
            if($createRecord || $mergeType == Import_Utils_Helper::$AUTO_MERGE_MERGEFIELDS || $mergeType == Import_Utils_Helper::$AUTO_MERGE_OVERWRITE){
                ...
                $label = trim($label);
                $adb->pquery('UPDATE mycrm_crmentity SET label=? WHERE crmid=?', array($label, $recordId));
            }
        ...
        if($this->entityData) {
            ...
            $entity->triggerEvent('mycrm.batchevent.save',$this->entityData);
        }
	...
	public function transformForImport($fieldData, $moduleMeta, $fillDefault=true, $checkMandatoryFieldValues=true) {
		...
		foreach ($fieldData as $fieldName => $fieldValue) {
			...
			if ($fieldInstance->getFieldDataType() == 'owner') {
				...
				$olderCacheEnable = Mycrm_Cache::$cacheEnable;
				Mycrm_Cache::$cacheEnable = false;
				...
				if (!in_array($picklistValueInLowerCase, $allPicklistValuesInLowerCase)) {
					$moduleObject = Mycrm_Module::getInstance($moduleMeta->getEntityName());
					$fieldObject = Mycrm_Field::getInstance($fieldName, $moduleObject);
				...
				Mycrm_Cache::$cacheEnable = $olderCacheEnable;
	...
	public static function runScheduledImport() {
		...
		foreach ($scheduledImports as $scheduledId => $importDataController) {
			...
			$emailSubject = 'mycrm CRM - Scheduled Import Report for '.$importDataController->module;
			...
			$emailData = 'mycrm CRM has just completed your import process. <br/><br/>' .
							$importResult . '<br/><br/>'.
			...
			$mycrmMailer->to = array( array($userEmail, $userName));
			$mycrmMailer->Subject = $emailSubject;
			$mycrmMailer->Body    = $emailData;
			$mycrmMailer->Send();

			$importDataController->finishImport();
		}
		Mycrm_Mailer::dispatchQueue(null);
	}
