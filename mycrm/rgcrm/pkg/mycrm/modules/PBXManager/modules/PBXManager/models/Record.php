...
class PBXManager_Record_Model extends Mycrm_Record_Model{
    
    const moduletableName = 'mycrm_pbxmanager';
    const lookuptableName = 'mycrm_pbxmanager_phonelookup';
    const entitytableName = 'mycrm_crmentity';
    ...
    public function saveRecordWithArrray($params){
        $moduleModel = Mycrm_Module_Model::getInstance('PBXManager');
        $recordModel = Mycrm_Record_Model::getCleanInstance('PBXManager');
    ...
    public static function getUserNumbers(){
        $numbers = null;
        $db = PearDatabase::getInstance();
        $query = 'SELECT id, phone_crm_extension FROM mycrm_users';
        $result = $db->pquery($query, array());
