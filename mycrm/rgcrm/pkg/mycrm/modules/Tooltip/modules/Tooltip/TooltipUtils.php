...
function getFieldList($module_name, $field_name = "") {
	...
	$query = "select * from mycrm_field where tabid = ?";
	$params = array($tabid);
	if (!empty ($field_name)) {
		$query .= " and fieldname not like ?";
		$params = array($tabid,$field_name);
	}
	$query.= " and columnname not like 'imagename' and uitype not in (61, 122) and mycrm_field.presence in (0,2)";
...
function moduleList() {
	global $adb;
	$sql = "select distinct mycrm_field.tabid,name from mycrm_field inner join mycrm_tab on mycrm_field.tabid=mycrm_tab.tabid where name not in ('Calendar','Events','Users')";
...
function tooltip_exists($fieldid, $related_fieldid) {
	global $adb;
	$query = "select * from mycrm_quickview where fieldid=? and related_fieldid=?";
...
function getToolTipText($view,$fieldname,$module,$value){
	...
	$quickview = 'select fieldname,fieldlabel from mycrm_quickview inner join mycrm_field on mycrm_quickview.related_fieldid=mycrm_field.fieldid where mycrm_quickview.fieldid = ? and currentview= ? and mycrm_field.presence in (0,2) order by mycrm_quickview.sequence';
...
function ToolTipExists($fieldname,$tabid){
	if(empty($fieldname) || empty($tabid)){
		...
		global $adb;
		$sql = "select fieldid from mycrm_field where tabid = ? and fieldname = ? and mycrm_field.presence in (0,2)";
		$result = $adb->pquery($sql,array($tabid,$fieldname));
		$count = $adb->num_rows($result);
		if($count > 0){
			$fieldid = $adb->query_result($result,0,'fieldid');
		
			$sql = "select * from mycrm_quickview where fieldid = ?";
...
function QuickViewFieldList($module){
	...
	$query = "select * from mycrm_field where tabid = ? and columnname not like 'imagename' and uitype not in (61, 122) and mycrm_field.presence in (0,2)";
