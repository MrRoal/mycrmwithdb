...
function SaveTooltipInformation($fieldid, $sequence, $view=1){
	...
	if(empty($fieldid)){
		...
		if(!empty($_REQUEST['checkedFields'])){
			...
			//add to mycrm_quickview table
			foreach($checkedFields as $checkedField){
				$query = "insert into mycrm_quickview (fieldid,related_fieldid,sequence,currentview) values (?,?,?,?)";
...
function deleteOldInfo($fieldid, $view=1){
	...
	$query = "delete from mycrm_quickview where fieldid = ? and currentview = ?";
...
function getFieldLabels($checkedFields){
	...
	$sql = "select * from mycrm_field where fieldid in (".generateQuestionMarks($checkedFields).") and mycrm_field.presence in (0,2)";
