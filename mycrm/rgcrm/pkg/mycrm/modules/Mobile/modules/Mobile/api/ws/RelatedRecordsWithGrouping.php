...
class Mobile_WS_RelatedRecordsWithGrouping extends Mobile_WS_QueryWithGrouping {
	...
	function process(Mobile_API_Request $request) {
		...
		if ($functionHandler) {
			...
			$querySEtype = "mycrm_crmentity.setype as setype";
			if ($relatedmodule == 'Calendar') {
				$querySEtype = "mycrm_activity.activitytype as setype";
			}
			
			$query = sprintf("SELECT mycrm_crmentity.crmid, $querySEtype %s", substr($query, stripos($query, 'FROM')));
