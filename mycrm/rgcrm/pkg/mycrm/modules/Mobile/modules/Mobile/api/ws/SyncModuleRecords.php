		...
		if (!empty($lastSyncTime) && $currentPage === 0) {
			...
			if ($hasAssignedToField) {
				if ($isPrivateMode) {
					...
					$andsmowneridequal = " AND mycrm_crmentity.smownerid=?";
				} else {
			...
			if ($module == 'Calendar') {
				$queryDeleted = $adb->pquery("SELECT activityid as crmid, activitytype as setype FROM mycrm_activity
					INNER JOIN mycrm_crmentity ON mycrm_activity.activityid=mycrm_crmentity.crmid
					AND mycrm_crmentity.deleted=1 AND mycrm_crmentity.setype=? AND mycrm_crmentity.modifiedtime > ?
					LEFT JOIN mycrm_users ON mycrm_users.id = mycrm_crmentity.smownerid
					LEFT JOIN mycrm_groups ON mycrm_groups.groupid = mycrm_crmentity.smownerid WHERE 1=1	$andsmowneridequal ",
				$queryDeletedParameters);
			} else if ($module == 'Leads') {
				$queryDeleted = $adb->pquery("SELECT crmid, modifiedtime, setype FROM mycrm_crmentity
				INNER JOIN mycrm_leaddetails ON mycrm_leaddetails.leadid=mycrm_crmentity.crmid
				LEFT JOIN mycrm_users ON mycrm_users.id = mycrm_crmentity.smownerid
				LEFT JOIN mycrm_groups ON mycrm_groups.groupid = mycrm_crmentity.smownerid
				WHERE (mycrm_crmentity.deleted=1 OR (mycrm_crmentity.deleted=0 AND mycrm_leaddetails.converted=1)) AND mycrm_crmentity.setype=? AND mycrm_crmentity.modifiedtime > ? $andsmowneridequal", $queryDeletedParameters);
			} else {
				$queryDeleted = $adb->pquery("SELECT crmid, modifiedtime, setype FROM mycrm_crmentity
				LEFT JOIN mycrm_users ON mycrm_users.id = mycrm_crmentity.smownerid
				LEFT JOIN mycrm_groups ON mycrm_groups.groupid = mycrm_crmentity.smownerid
				WHERE mycrm_crmentity.deleted=1 AND mycrm_crmentity.setype=? AND mycrm_crmentity.modifiedtime > ? $andsmowneridequal", $queryDeletedParameters);
			}
