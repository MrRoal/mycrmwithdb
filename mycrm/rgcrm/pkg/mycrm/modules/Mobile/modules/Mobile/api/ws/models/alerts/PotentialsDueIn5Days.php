...
class Mobile_WS_AlertModel_PotentialsDueIn5Days extends Mobile_WS_AlertModel {
	...
	function query() {
		$sql = Mobile_WS_Utils::getModuleListQuery('Potentials', 
					"mycrm_potential.sales_stage not like 'Closed%' AND 
					DATEDIFF(mycrm_potential.closingdate, CURDATE()) <= 5"
				);
		return preg_replace("/^SELECT count\(\*\) as count(.*)/i", "SELECT crmid $1", Mycrm_Functions::mkCountQuery($sql));
	}
