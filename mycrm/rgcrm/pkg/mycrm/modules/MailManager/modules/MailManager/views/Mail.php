		...
		if ('open' == $this->getOperationArg($request)) {
			...
			if (!empty($to_string)) {
				...
				foreach($toArray as $to) {
					$relatedtos = MailManager::lookupMailInMycrm($to, $currentUserModel);
					...
					if ($status === true) {
						...
						if ($realid[1] == -1) {
							$db->pquery('DELETE FROM mycrm_salesmanactivityrel WHERE smid=? AND activityid=?',$params);
							$db->pquery('INSERT INTO mycrm_salesmanactivityrel VALUES (?,?)', $params);
						} else {
							$db->pquery('DELETE FROM mycrm_seactivityrel WHERE crmid=? AND activityid=?', $params);
							$db->pquery('INSERT INTO mycrm_seactivityrel VALUES (?,?)', $params);
						}
					}
				}
			}
