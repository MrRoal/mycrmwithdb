...
class OutlookSyncServer extends SyncServer{
    ...
    function getDestinationHandleDetails(){
        return array('handlerclass' => 'OutlookMycrmCRMHandler',
            'handlerpath' => 'modules/WSAPP/Handlers/OutlookMycrmCRMHandler.php');
	}
     ...
     function put($key, $element, $user) {
        ...
        foreach ($records as $record) {
            ...
            if (!empty($record['crmid'])) {
                ...
                $lookupResult = $db->pquery("SELECT crmid,modifiedtime FROM mycrm_crmentity WHERE crmid=?", array($crmid[1]));
