...
class WSAPP {
	...
	function registerCustomWebservices( $operations ) {
		...
		foreach($operations as $operation_name => $operation_info) {	
			$checkres = $adb->pquery("SELECT operationid FROM mycrm_ws_operation WHERE name=?", array($operation_name));
			if($checkres && $adb->num_rows($checkres) < 1) {
				$operation_id = $adb->getUniqueId('mycrm_ws_operation');
			
				$operation_res = $adb->pquery(
					"INSERT INTO mycrm_ws_operation (operationid, name, handler_path, handler_method, type, prelogin) 
					VALUES (?,?,?,?,?,?)",
				...
				foreach($operation_parameters as $parameter_name => $parameter_type) {
					$adb->pquery(
						"INSERT INTO mycrm_ws_operation_parameters (operationid, name, type, sequence) 
						VALUES(?,?,?,?)", array($operation_id, $parameter_name, $parameter_type, ($parameter_index+1))
					);
					++$parameter_index;
				}
				Mycrm_Utils::Log("Opearation $operation_name enabled successfully.");
			} else {
				Mycrm_Utils::Log("Operation $operation_name already exists.");
			}
		}
	}
    ...
    function registerHandlers(){
        ...
        $appTypehandler = array();
        $appTypehandler['type'] = "mycrmCRM";
        $appTypehandler['handlerclass'] = "mycrmCRMHandler";
        $appTypehandler['handlerpath'] = "modules/WSAPP/Handlers/mycrmCRMHandler.php";
        $handlerDetails[] = $appTypehandler;

        foreach($handlerDetails as $appHandlerDetails)
             $adb->pquery("INSERT INTO mycrm_wsapp_handlerdetails VALUES(?,?,?)",array($appHandlerDetails['type'],$appHandlerDetails['handlerclass'],$appHandlerDetails['handlerpath']));
    }
    
    function registerMycrmCRMApp(){
       $db = PearDatabase::getInstance();
        $appName = "mycrmCRM";
        $type  ="user";
        $uid = uniqid();
        $db->pquery("INSERT INTO mycrm_wsapp (name, appkey,type) VALUES(?,?,?)", array($appName, $uid,$type));
    }
	...
	function registerWsappWorkflowhandler(){
		...
		$em->registerHandler('mycrm.entity.aftersave', 'modules/WSAPP/WorkFlowHandlers/WSAPPAssignToTracker.php', 'WSAPPAssignToTracker','',$dependentEventHandlersJson);
	}

	function registerSynclibEventHandler(){
		$className='WSAPP_MycrmSyncEventHandler';
		$path = 'modules/WSAPP/synclib/handlers/MycrmSyncEventHandler.php';
		$type = 'mycrmSyncLib';
