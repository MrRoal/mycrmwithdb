 ...
 */
class ModComments_Record_Model extends Mycrm_Record_Model {
	...
	public function getImagePath() {
		...
		if($commentor) {
			...
			if (!empty($customer) && $isMailConverterType != 1) {
                $recordModel = Mycrm_Record_Model::getInstanceById($customer);
	...
	public static function getInstanceById($record) {
		...
		$result = $db->pquery('SELECT mycrm_modcomments.*, mycrm_crmentity.smownerid,
					mycrm_crmentity.createdtime, mycrm_crmentity.modifiedtime FROM mycrm_modcomments
	...
	public function getParentRecordModel() {
		...
		return Mycrm_Record_Model::getInstanceById($parentRecordId);
	...
	public function getCommentedByModel() {
		$customer = $this->get('customer');
		if(!empty($customer)) {
			return Mycrm_Record_Model::getInstanceById($customer, 'Contacts');
		} else {
			$commentedBy = $this->get('smownerid');
			if($commentedBy) {
			    $commentedByModel = Mycrm_Record_Model::getInstanceById($commentedBy, 'Users');
			    if(empty($commentedByModel->entity->column_fields['user_name'])) {
				$activeAdmin = Users::getActiveAdminUser();
				$commentedByModel = Mycrm_Record_Model::getInstanceById($activeAdmin->id, 'Users');
			    }
	 ...
	 * @param <Mycrm_Paging_Model> - paging model
	...
	public static function getRecentComments($parentRecordId, $pagingModel){
		...
		$listView = Mycrm_ListView_Model::getInstance('ModComments');
		...
		$query = $query ." AND related_to = ? ORDER BY mycrm_crmentity.createdtime DESC
	...
	public static function getAllParentComments($parentId) {
		...
		$query = $query ." AND related_to = ? AND parent_comments = '' ORDER BY mycrm_crmentity.createdtime DESC";
	...
	public function getChildCommentsCount() {
		...
		$query = 'SELECT 1 FROM mycrm_modcomments WHERE parent_comments = ? AND related_to = ?';
