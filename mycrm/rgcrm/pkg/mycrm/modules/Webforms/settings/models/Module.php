...
class Settings_Webforms_Module_Model extends Settings_Mycrm_Module_Model {

	var $baseTable = 'mycrm_webforms';
	var $baseIndex = 'id';
	...
	public function deleteRecord($recordModel) {
		...
		$db->pquery("DELETE from mycrm_webforms_field WHERE webformid = ?", array($recordId));
		$db->pquery("DELETE from mycrm_webforms WHERE id = ?", array($recordId));
		return true;
	}
}
