...
class Settings_Webforms_Record_Model extends Settings_Mycrm_Record_Model {
	 ...
	 * @return <Array> list of link models <Mycrm_Link_Model>
	 */
	public function getRecordLinks() {
		...
		$recordLinks = array(
				...
				array(
						...
						'linkurl' => "javascript:Settings_Mycrm_List_Js.triggerDelete(event,'".$this->getDeleteUrl()."');",
						'linkicon' => 'icon-trash'
				)
		);
	...
	public function getDetailViewLinks() {
		...
		foreach ($detailViewLinks as $detailViewLink) {
			$linkModelList['DETAILVIEWBASIC'][] = Mycrm_Link_Model::getInstanceFromValues($detailViewLink);
		}
	...
	public function getSelectedFieldsList($mode='') {
		if (!$this->selectedFields) {
			...
			if ($targetModule) {
				...
				$targetModuleModel = Mycrm_Module_Model::getInstance($targetModule);
				$allFields = $targetModuleModel->getFields();

				$result = $db->pquery("SELECT * FROM mycrm_webforms_field WHERE webformid = ? ORDER BY sequence", array($this->getId()));
	...
	public function save() {
		...
		if ($mode) {
			$updateQuery = "UPDATE mycrm_webforms SET description = ?, returnurl = ?, ownerid = ?, enabled = ?, captcha = ? , roundrobin = ?, roundrobin_userid = ?, roundrobin_logic = ? ,targetmodule = ? WHERE id = ?";
			...
			$insertQuery = "INSERT INTO mycrm_webforms(name, targetmodule, publicid, enabled, description, ownerid, returnurl, captcha, roundrobin, roundrobin_userid, roundrobin_logic) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
		...
		$db->pquery("DELETE FROM mycrm_webforms_field WHERE webformid = ?", array($this->getId()));

		//Saving data of source module fields info for this webform
		$selectedFieldsData = $this->get('selectedFieldsData');
		$sourceModuleModel = Mycrm_Module_Model::getInstance($this->get('targetmodule'));

		$fieldInsertQuery = "INSERT INTO mycrm_webforms_field(webformid, fieldname, neutralizedfield, defaultvalue, required, sequence, hidden) VALUES(?, ?, ?, ?, ?, ?, ?)";
		foreach ($selectedFieldsData as $fieldName => $fieldDetails) {
			...
			$fieldModel = Mycrm_Field_Model::getInstance($fieldName, $sourceModuleModel);
			...
			if ($dataType === 'date') {
				$fieldDefaultValue = Mycrm_Date_UIType::getDBInsertedValue($fieldDefaultValue);
			}
	
			if ($dataType === 'time') {
				$fieldDefaultValue = Mycrm_Time_UIType::getTimeValueWithSeconds($fieldDefaultValue);
			}
			...
			if ($dataType === 'reference') {
				...
				$referenceObject = MycrmWebserviceObject::fromName($db,$referenceModule);
	...
	public function checkDuplicate() {
		...
		$query = "SELECT 1 FROM mycrm_webforms WHERE name = ?";
	...
	static public function getInstanceById($recordId, $qualifiedModuleName) {
		...
		$result = $db->pquery("SELECT * FROM mycrm_webforms WHERE id = ?", array($recordId));
		if ($db->num_rows($result)) {
			$recordModelClass = Mycrm_Loader::getComponentClassName('Model', 'Record', $qualifiedModuleName);
	 ...
	 * @return <Settings_Mycrm_Module_Model>
	 */
	static public function getCleanInstance($moduleName) {
		$recordModel = new self();
		$moduleModel = Settings_Mycrm_Module_Model::getInstance($moduleName);
