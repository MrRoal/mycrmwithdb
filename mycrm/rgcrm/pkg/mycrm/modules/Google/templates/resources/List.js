...
jQuery.Class("Contact",{
                        ...
                        AppConnector.request(url).then(
                           function(data) {
                               ...
                               if(jQuery('#norefresh').length == 0){
                                listInstance  =  Mycrm_List_Js.getInstance();
    ...
    packFieldmappingsForSubmit : function(container) {
        ...
        jQuery.each(rows,function(index,row) {
            var tr = jQuery(row);
            var mycrm_field_name = tr.find('.mycrm_field_name').not('.select2-container').val();
            ...
            var map = {};
            map['mycrm_field_name'] = mycrm_field_name;
    ...
    validateFieldMappings : function(container) {
        ...
        var customMapElements = jQuery('select.mycrm_field_name');
        ...
        container.find('button#save_syncsetting').on('click',function() {
            thisInstance.validateFieldMappings(container).then(function() {
                ...
                AppConnector.request(serializedFormData).then(function(data) {
                    ...
                    app.hideModalWindow();
                    Mycrm_Helper_Js.showMessage({text: app.vtranslate('JS_SAVED_SUCCESSFULLY')});
                });
            }, function(inputs) {
    ...
    registerAddCustomFieldMappingEvent : function(container) {
        ...
        jQuery('.addCustomFieldMapping',container).on('click',function(e) {
            ...
            var mycrmFields = currentSelectionElement.data('mycrmfields');
            
            var mycrmFieldSelectElement = '<select class="mycrm_field_name" style="width:200px" data-category="'+selectionType+'">';
            if(!Object.keys(mycrmFields).length) {
                alert(app.vtranslate('JS_SUITABLE_MYCRM_FIELD_NOT_AVAILABLE_FOR_MAPPING'));
                return;
            }
            
            var customMapElements = jQuery('select.mycrm_field_name[data-category="'+selectionType+'"]');
            var mappedCustomFields = [];
            jQuery.each(customMapElements,function(i,elem) {
                    mappedCustomFields.push(jQuery(elem).val());
            });
            var numberOfOptions = 0;
            jQuery.each(mycrmFields,function(fieldname,fieldLabel) {
                if(jQuery.inArray(fieldname,mappedCustomFields) === -1) {
                    numberOfOptions++;
                    var option = '<option value="'+fieldname+'">'+fieldLabel+'</option>';
                    mycrmFieldSelectElement += option;
                }
            });
            ...
            mycrmFieldSelectElement += '</select>';
            ...
            if(selectionType != 'custom') {
                ...
                googleTypeSelectElement += '</select>\n\
                                 &nbsp;&nbsp;<input type="text" class="google-custom-label" style="visibility:hidden;width:190px;" data-validation-engine="validate[required,funcCall[Mycrm_Base_Validator_Js.invokeValidation]]" />';
            } else {
                googleTypeSelectElement = '<input type="hidden" class="google_field_name" value="'+ googleFields[selectionType]['name'] +'" />';
                googleTypeSelectElement += '<input type="hidden" class="google-type" value="'+selectionType+'" />';
                googleTypeSelectElement += '<input type="text" class="google-custom-label" style="width:190px;" data-validation-engine="validate[required,funcCall[Mycrm_Base_Validator_Js.invokeValidation]]" />';
            }
            var tabRow = '<tr>\n\
                            <td>' + mycrmFieldSelectElement + '</td>\n\
            ...
            thisInstance.registerMycrmFieldSelectOnChangeEvent(container,lastRow.find('select.mycrm_field_name'));
            thisInstance.registerGoogleTypeChangeEvent(container,lastRow.find('select.google-type'));
            lastRow.find('select.mycrm_field_name').trigger('change');
    ...
    registerDeleteCustomFieldMappingEvent : function(container) {
        jQuery('.deleteCustomMapping',container).on('click',function() {
            ...
            var currentCategory = currentRow.find('select.mycrm_field_name').data('category');
            currentRow.remove();
            jQuery('select.mycrm_field_name[data-category="'+currentCategory+'"]').trigger('change');
    ...
    removeOptionFromSelectList : function(selectElement,optionValue,category) {
        ...
        var categorisedSelectElements = jQuery('select.mycrm_field_name[data-category="'+category+'"]');
    ...
    registerMycrmFieldSelectOnChangeEvent : function(container,selectElement) {
        var thisInstance = this;
        if(typeof selectElement === 'undefined') {
            selectElement = jQuery('select.mycrm_field_name',container);   
        }
        selectElement.on('change', function(e){
            var element = jQuery(e.currentTarget);
            var category = element.data('category');
            
            var allCategorizedSelects = jQuery('select.mycrm_field_name[data-category="'+category+'"]');
            var selectedValues = [];
            ...
            jQuery.each(allCategorizedSelects, function(i, selectElement){
                if(e.currentTarget !== selectElement || allCategorizedSelects.length == 1) {
                    var allCategoryFieldLabelValues = jQuery('li.addCustomFieldMapping[data-type="'+category+'"]').data('mycrmfields');
    ...
    registerPostSettingRenderEvents : function(container) {
        ...
        this.registerMycrmFieldSelectOnChangeEvent(container);
        this.registerGoogleTypeChangeEvent(container);

        jQuery('select.mycrm_field_name',container).trigger('change');
