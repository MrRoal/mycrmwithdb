...
<div id = 'sync_message' class='row-fluid' >
    <div class='padding10 span12'>     
        ...
        {if $STATE eq 'home'}
            {if $SYNCTIME}
            <p class="muted" id='synctime'><small title="{Mycrm_Util_Helper::formatDateTimeIntoDayString($SYNCTIME)}">{vtranslate('LBL_SYNCRONIZED',$MODULE_NAME)} : {Mycrm_Util_Helper::formatDateDiffInStrings($SYNCTIME)}</small></p>
            {else}
