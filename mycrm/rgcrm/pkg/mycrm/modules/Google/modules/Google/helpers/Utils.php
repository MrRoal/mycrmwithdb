...
class Google_Utils_Helper {
    ...
    const settings_table_name = 'mycrm_google_sync_settings'; 
    const fieldmapping_table_name = 'mycrm_google_sync_fieldmapping'; 
    ...
    public static function updateSyncTime($sourceModule, $modifiedTime = false) {
        ...
        if (!self::getSyncTime($sourceModule)) {
            if ($modifiedTime) {
                $db->pquery('INSERT INTO mycrm_google_sync (googlemodule,user,synctime,lastsynctime) VALUES (?,?,?,?)', array($sourceModule, $user->id, $modifiedTime, date('Y-m-d H:i:s')));
            }
        } else {
            $db->pquery('UPDATE mycrm_google_sync SET synctime = ?,lastsynctime = ? WHERE user=? AND googlemodule=?', array($modifiedTime, date('Y-m-d H:i:s'), $user->id, $sourceModule));
        }
    }
    ...
    private static function intialiseUpdateSchema() {
        if (!Mycrm_Utils::CheckTable('mycrm_google_sync')) {
            Mycrm_Utils::CreateTable('mycrm_google_sync', '(googlemodule varchar(50),user int(10), synctime datetime,lastsynctime datetime)', true);
        }
    }
    ...
    public static function getSyncTime($sourceModule) {
        ...
        self::intialiseUpdateSchema();
        $user = Users_Record_Model::getCurrentUserModel();
        $result = $db->pquery('SELECT synctime FROM mycrm_google_sync WHERE user=? AND googlemodule=?', array($user->id, $sourceModule));
    ...
    public static function getLastSyncTime($sourceModule) {
        ...
        self::intialiseUpdateSchema();
        $user = Users_Record_Model::getCurrentUserModel();
        $result = $db->pquery('SELECT lastsynctime FROM mycrm_google_sync WHERE user=? AND googlemodule=?', array($user->id, $sourceModule));
        if ($result && $db->num_rows($result) > 0) {
            $row = $db->fetch_array($result);
    ...
    static function saveFieldMappings($sourceModule, $fieldMappings) {
        ...
        $sql = 'INSERT INTO ' . self::fieldmapping_table_name . ' (mycrm_field,google_field,google_field_type,google_custom_label,user) VALUES ';
    ...
    static function getFieldMappingForUser($user = false) {
        ...
        $sql = 'SELECT mycrm_field,google_field,google_field_type,google_custom_label FROM ' . self::fieldmapping_table_name . ' WHERE user = ?';
        $result = $db->pquery($sql,array($user->getId()));
        for($i=0;$i<$db->num_rows($result);$i++) {  
            $row = $db->fetch_row($result);
            $fieldmapping[$row['mycrm_field']] = array(
