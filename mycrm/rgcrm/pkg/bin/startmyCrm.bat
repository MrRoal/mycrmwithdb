...
:initiate
rem if true means , mycrm crm mysql is being used
...
:StartApacheService
echo ""
echo "making an attempt to kill any existing mycrmcrm service"
echo ""
bin\apache -k stop -n mycrmcrm504
...
echo ""
echo ""
echo "Installing  mycrmcrm504 apache service after sleeping for 10 seconds"
echo ""
%SLEEP_STR% -n 10 127.0.0.1>nul
bin\apache -k install -n mycrmcrm504 -f conf\httpd.conf
echo ""
echo "Starting  mycrmcrm504 apache service"
echo ""
bin\apache -n mycrmcrm504 -k start
IF ERRORLEVEL 1 goto stopservice
goto checkmysql
...
echo ""
echo ""
echo "Apache in the location %apache_dir% is not running. Start Apache and then start mycrm crm"
echo ""
echo ""
...
:checkdatabase
echo ""
echo "check to see if mycrmcrm504 database already exists"
echo ""
mysql --port=%mysql_port% --user=%mysql_username% --password=%mysql_password% -e "show databases like 'mycrmcrm504'" | "%WINDIR%\system32\find.exe" "mycrmcrm504" > NUL
IF ERRORLEVEL 1 goto dbnotexists
echo ""
ECHO  "mycrmcrm504 database exists"
echo ""
goto end
...
:dbnotexists
echo ""
ECHO "mycrmcrm504 database does not exist"
echo ""
echo %cd%
echo ""
echo "Proceeding to create database mycrmcrm504 and populate the same"
echo ""
mysql --user=%mysql_username% --password=%mysql_password% --port=%mysql_port% -e "create database if not exists mycrmcrm504"
echo ""
echo "mycrmcrm504 database created"
echo ""
goto end
...
echo ""
echo ""
echo "********* Change the apache port in startmyCrm.bat and stopmyCrm.bat too and then access the product from the browser in the following manner http://localhost:apacheport******* "
echo ""
echo ""
echo "Thank You"
echo ""
