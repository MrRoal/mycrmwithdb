...
require_once 'modules/Mycrm/helpers/ShortURL.php';
global $adb;
$adb = PearDatabase::getInstance();

if (isset($_REQUEST['user_name']) && isset($_REQUEST['emailId'])) {
    $username = vtlib_purify($_REQUEST['user_name']);
    $result = $adb->pquery('select email1 from mycrm_users where user_name= ? ', array($username));
    ...
    if (vtlib_purify($_REQUEST['emailId']) == $email) {
        ...
        $content = 'Dear Customer,<br><br> 
                            You recently requested a password reset for your MycrmCRM Open source Account.<br> 
		            ...
		            Regards,<br> 
		            MycrmCRM Open source Support Team.<br>' ;
        $mail = new PHPMailer();
        $query = "select from_email_field,server_username from mycrm_systems where server_type=?";
        $params = array('email');
        $result = $adb->pquery($query,$params);
        $from = $adb->query_result($result,0,'from_email_field');
        if($from == '') {$from =$adb->query_result($result,0,'server_username'); }
        $subject='Request : ForgotPassword - mycrmcrm';
